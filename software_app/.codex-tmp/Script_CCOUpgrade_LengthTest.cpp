...
//        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发�?-命令，等�?-回复");
...
void Script_CCOUpgrade_LengthTest::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
    ...
    {
#ifdef SENCE2
        if(!p_BuildNetwork_GW->startBuildNetFlag)//场景2 关注p_BuildNetwork_GW->startBuildNetFlag标志,开始组网标�?
            p_BuildNetwork_GW->processMsg(dvcType,id,data,datalen);
#elif defined(SENCE3)||defined(SENCE4)
        if(!p_BuildNetwork_GW->buildNetworkResultFlag)//场景3�? 关注p_BuildNetwork_GW->buildNetworkResultFlag标志，组网完成标�?
            p_BuildNetwork_GW->processMsg(dvcType,id,data,datalen);
#elif defined SENCE1
        if(true){}
#endif
...
void Script_CCOUpgrade_LengthTest::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("收到控制命令回复，参数个�?%1").arg(QString::number(params.size())));
    ...
    while(haveCompleteMsg)
    {
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_EraseOldData_15F1_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x15&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp&&uchar(p_Frame3762Base->info_field_.info_field_up.msg_seq)==msgSeq)
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("%1，清除下装回�?2")
                    ...
                    {
                        QString state=QString("%1，清除下装回复异常，实际回复%2，期望回�?x00000000")
                                .arg(metaEnum.valueToKey(emScriptRunState))
                                .arg("0x"+QString::number(ptr->current_identify_,16).rightJustified(8,'0'));
                    ...
                    else
                    {
                        ...
                        {
                            QString state=QString("%1，读取文件内容为�?).arg(metaEnum.valueToKey(emScriptRunState));
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing,state);
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed,state+testCase);
                            return;
                        }
                        ...
                        currentLen=0;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("发送文件传输，文件长度�?0，等待回�?).arg(QString::number(currentLen)));
            ...
            case Wait_FileTransfer_Len0_Finish:
        {
            if(p_Frame3762Base->afn_ == 0x15&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp&&uchar(p_Frame3762Base->info_field_.info_field_up.msg_seq)==msgSeq)
            {
                p_timer->stop();
                shared_ptr<Afn15F1> ptr=dynamic_pointer_cast<Afn15F1>(p_Frame3762Base);
                p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("%1，文件传输，文件长度�?2回复%3")
                                                     .arg(metaEnum.valueToKey(emScriptRunState))
                                                     .arg(QString::number(currentLen))
                                                     .arg("0x"+QString::number(ptr->current_identify_,16).rightJustified(8,'0')));
                if(ptr->current_identify_!=0x00000000)
                {
                    QString state=QString("%1，文件传输，文件长度�?2回复异常，实际回�?3，期望回�?x00000000")
                            .arg(metaEnum.valueToKey(emScriptRunState))
                            .arg(QString::number(currentLen))
                            .arg("0x"+QString::number(ptr->current_identify_,16).rightJustified(8,'0'));
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing,state);
...
//            case Wait_FileTransfer_Len65535_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x15&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp&&uchar(p_Frame3762Base->info_field_.info_field_up.msg_seq)==msgSeq)
                {
                    ...
                    {
                        QString state=QString("%1，文件传输，文件长度�?2回复异常，实际回�?3，期望回�?xFFFFFFFF")
                                .arg(metaEnum.valueToKey(emScriptRunState))
                                .arg(QString::number(currentLen))
                                .arg("0x"+QString::number(actualId,16).rightJustified(8,'0'));
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing,state);
    ...
    switch(emScriptRunState)
    {
        case Wait_QueryVersion_Before_Finish:
        {
            if(isCCO)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString(metaEnum.valueToKey(emScriptRunState))+"升级前外部版�?+outVersionBefore+"，内部版�?+innerVersionBefore);
        ...
        case Wait_FileTransfer_LenMax_Finish:
        {
            ...
            p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString(metaEnum.valueToKey(emScriptRunState))+"升级前外部版�?+outVersionBefore+"，内部版�?+innerVersionBefore
                                                 +"\n升级后外部版�?+outVersion+"，内部版�?+innerVersion
                                                 +"\n升级后目标外部版�?+outVersionAim+"，目标内部版�?+innerVersionAim);
            if((outVersion!=outVersionBefore||innerVersion!=innerVersionBefore)&&(outVersion==outVersionAim&&innerVersion==innerVersionAim))//与旧版本不一致，与目标版本相�?
            {
                p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString(metaEnum.valueToKey(emScriptRunState))+"文件传输升级成功，版本符合要求，外部版本"+outVersion+"，内部版�?+innerVersion+testCase);
                p_AbstractScriptHost->updateProgress(ProcessState_Success,QString("脚本执行成功，外部版�?+outVersion+"，内部版�?+innerVersion+testCase));
            }
            else
            {
                p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString(metaEnum.valueToKey(emScriptRunState))+"文件传输升级失败，版本不符合要求，外部版�?+outVersion+"，内部版�?+innerVersion+testCase);
                p_AbstractScriptHost->updateProgress(ProcessState_Failed,QString(metaEnum.valueToKey(emScriptRunState))+"文件传输升级失败，版本不符合要求，外部版�?+outVersion+"，内部版�?+innerVersion+testCase);
            }
            break;
        }
        default:
...
void Script_CCOUpgrade_LengthTest::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    if(frame==p_Afn15F1)
    {
        if(emScriptRunState==Wait_EraseOldData_15F1_Finish)
        {
            ...
            p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("》》清除下装文�?5F1�?1\n").arg(QString(sendMsgOct.toHex())));
        }
        else
        {
            ...
            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("总段�?(%1); 该段文件长度=(%2); 该段文件索引=(%3);\n%4")
...
void Script_CCOUpgrade_LengthTest::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    ...
    QStringList dvcList;
    dvcList<<"单�?<<"三�?<<"国网路由"<<"南网路由"<<"采集�?;
    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("测试脚本==>>>>上位机【设备类�?%1】的报文=%2").arg(dvcList.at(int(dvcType))).arg(QString(msg.toHex())));
}
...
void Script_CCOUpgrade_LengthTest::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        ...
        case Wait_EraseOldData_15F1_Finish:
        {
            ...
            if(++tryTimes>=3)
            {
                QString state=QString("%1，清除下装回复超�?)
                        .arg(metaEnum.valueToKey(emScriptRunState));
                p_AbstractScriptHost->updateProgress(ProcessState_Processing,state);
...
//        case Wait_FileTransfer_Len65535_Finish:
        case Wait_FileTransfer_LenMin_Finish:
        {
            ...
            if(++tryTimes>=3)
            {
                QString state=QString("%1，文件传输回复超�?)
                        .arg(metaEnum.valueToKey(emScriptRunState));
                p_AbstractScriptHost->updateProgress(ProcessState_Processing,state);
                p_AbstractScriptHost->updateProgress(ProcessState_Failed,state);
            }
...
QString Script_CCOUpgrade_LengthTest::selectUpgradeFile()
{
    ...
    QStringList paths;
    QString error;
    paths<<fileDir+"路由程序(�?�?"<<fileDir+"路由程序(�?�?";
    for(auto path:paths)
    {
        ...
        if(files.count() != 1)
        {
            error="获取到的CCO升级文件数不�?�?+QString::number(files.count());
...
//        }
        ...
        if(verList.size() < 3)  // 需要至�?个元素：时间戳、外部版本、内部版�?
        {
