#include "Script_VersionHead_CcoNormal.h"

Script_VersionHead_CcoNormal::Script_VersionHead_CcoNormal(QObject *parent) : QObject(parent)
{
    ...
    p_SetCcoAddr_05F1 = make_shared<Afn05F1>();
...
Script_VersionHead_CcoNormal::~Script_VersionHead_CcoNormal()
{
...
void Script_VersionHead_CcoNormal::execute()
{
...
void Script_VersionHead_CcoNormal::stop()
{
...
void Script_VersionHead_CcoNormal::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_VersionHead_CcoNormal::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_VersionHead_CcoNormal::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
        ...
        if(paraDic->keys().contains("TestHead2"))
        {
            this->TestHead2 = (*paraDic)["TestHead2"];
        }
        result = true;
    }
    return result;
}
void Script_VersionHead_CcoNormal::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_VersionHead_CcoNormal::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_VersionHead_CcoNormal::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        case ReadInfoBefore:
        {
            ...
            if(p_Frame3762Base->afn_==char(0xf0) && dtValue3762==7 && p_Frame3762Base->ctrl_field_.dir==kDirUp)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "路由初始版本头："+p_QueryVersionHeadF0F7_Up->data_unit_up_.parameter_content_.toHex());
            ...
            else if(p_Frame3762Base->afn_==0x03 && dtValue3762==1 && p_Frame3762Base->ctrl_field_.dir==kDirUp)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "03F1路由初始厂商芯片代码"+(p_ChkCcoOutVrsn_03F1_Up->vendor_code_+p_ChkCcoOutVrsn_03F1_Up->chip_code_).toLatin1().toHex()
                                                     +"  外部版本"+p_ChkCcoOutVrsn_03F1_Up->version_time_+p_ChkCcoOutVrsn_03F1_Up->version_);
            ...
            else if(p_Frame3762Base->afn_==0x10 && dtValue3762==104 && p_Frame3762Base->ctrl_field_.dir==kDirUp)
            {
                ...
                if(memcmp(p_CtrInfoList->at(0)->ccoAddr,p_ChkCcoOutVrsnInfo_10F104_Up->network_node_info_unit_.node_info_group_list_[0].node_address_.addr,6)==0)
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "10F104路由初始厂商芯片代码"+(rptVrsnMnfcCode+rptVrsnChipCode).toHex()
                                +"  外部版本"+(rptVrsnDate+rptVrsn).toHex());

                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "F0F15查询路由内部版本");
                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_ChkCcoInVrsn_F0F15_Down);
                    p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                }
                else
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("10F104回复主节点地址有误：")+QByteArray(p_ChkCcoOutVrsnInfo_10F104_Up->network_node_info_unit_.node_info_group_list_[0].node_address_.addr,6).toHex());
                }
            }
            else if(p_Frame3762Base->afn_==char(0xF0) && dtValue3762==15 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                ccoInVrsn_init = p_ChkCcoInVrsn_F0F15_Up->version_time_+p_ChkCcoInVrsn_F0F15_Up->version_;
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "F0F15厂商芯片代码"+p_ChkCcoInVrsn_F0F15_Up->vendor_code_+p_ChkCcoInVrsn_F0F15_Up->chip_code_
                            +"  路由初始内部版本"+ccoInVrsn_init);

                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "开始F0F8设置路由待测省份版本头");
            ...
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到设置路由版本头确认，2s后路由应复位...");
                p_delayTimer->start(10*1000);
            }
            else if(p_Frame3762Base->afn_==0x00 && dtValue3762==2 && p_Frame3762Base->ctrl_field_.dir==kDirUp)
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "设置版本头路由回复否认，不符合要求");
            }
            else if(p_Frame3762Base->afn_==0x03 && dtValue3762==10 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_delayTimer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "路由复位，收到03F10运行模式信息上报，查询路由版本头");
                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryVersionHeadF0F7_Down);
                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                emScriptRunState = ReadInfoAfterReset;
            }
            else
            {
...
//            {
//                p_timer->stop();
//                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到设置路由版本头确认，查询路由版本头");
//                p_delayTimer->start(3*60*1000);
//                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryVersionHeadF0F7_Down);
//                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
//            }
...
//                {
//                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "查询路由版本头与设置一致，开始03F1读路由外部版本");
//                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_ChkCcoOutVrsn_03F1_Down);
//                    p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
//                }
//                else
//                {
//                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, "查询路由版本头与设置不一致："+p_QueryVersionHeadF0F7_Up->data_unit_up_.parameter_content_.toHex());
//                }
//            }
//            else if(p_Frame3762Base->afn_==0x03 && dtValue3762==1 && p_Frame3762Base->ctrl_field_.dir==kDirUp)
//            {
...
//                if(code_03F1==code && vrsn_03F1==year+month+day+vrsn1+vrsn2)
//                {
//                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "03F1路由外部版本和版本头一致，开始10F104查询外部版本");
//                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_ChkCcoOutVrsnInfo_10F104_Down);
//                    p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
//                }
//                else
//                {
//                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, "03F1路由外部版本和版本头不一致，版本头厂商芯片代码"+code+"版本"+year+month+day+vrsn1+vrsn2);
//                }
//            }
//            else if(p_Frame3762Base->afn_==0x10 && dtValue3762==104 && p_Frame3762Base->ctrl_field_.dir==kDirUp)
//            {
...
//                    QString vrsn1 = QString("%1").arg(TestHead.mid(16,2).toInt(nullptr,16),2,10,QChar('0'));
//                    QString vrsn2 = QString("%1").arg(TestHead.mid(18,2).toInt(nullptr,16),2,10,QChar('0'));
//                    if(code_10F104==code && vrsn_10F104==year+month+day+vrsn1+vrsn2)
//                    {
//                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "10F104路由外部版本和版本头一致，开始F0F15查询内部版本");
//                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_ChkCcoInVrsn_F0F15_Down);
//                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
//                    }
//                    else
//                    {
//                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "10F104路由外部版本和版本头不一致，版本头厂商芯片代码"+code+"版本"+year+month+day+vrsn1+vrsn2);
//                    }
//                }
//                else
//                {
...
//                p_timer->stop();
//                shared_ptr<AfnF0F15> p_ChkCcoInVrsn_F0F15_Up=std::dynamic_pointer_cast<AfnF0F15>(p_Frame3762Base);
//                QString ccoInVrsn = p_ChkCcoInVrsn_F0F15_Up->version_time_+p_ChkCcoInVrsn_F0F15_Up->version_;
//                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "F0F15厂商芯片代码"+p_ChkCcoInVrsn_F0F15_Up->vendor_code_+p_ChkCcoInVrsn_F0F15_Up->chip_code_
//                            +"  路由内部版本"+ccoInVrsn);
//                if(ccoInVrsn == ccoInVrsn_init)
//                {
//                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "设置版本头后路由内部版本未改变，等待2min路由复位...");
//                }
//                else
//                {
//                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, "设置版本头后路由内部版本发生改变，初始版本"+ccoInVrsn_init);
//                }
//            }
//            else if(p_Frame3762Base->afn_==0x03 && dtValue3762==10 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
//            {
//                p_delayTimer->stop();
//                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "路由复位，收到03F10运行模式信息上报，再03F1读外部版本");
//                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_ChkCcoOutVrsn_03F1_Down);
//                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
//                emScriptRunState = ReadInfoAfterReset;
//            }
...
//        }
        case ReadInfoAfterReset:
        {
            ...
            else if(p_Frame3762Base->afn_==0x10 && dtValue3762==104 && p_Frame3762Base->ctrl_field_.dir==kDirUp)
            {
                p_timer->stop();
                shared_ptr<Afn10F104> p_ChkCcoOutVrsnInfo_10F104_Up=std::dynamic_pointer_cast<Afn10F104>(p_Frame3762Base);
                uchar addr[6] = {0xff,0xff,0xff,0xff,0xff,0xff};
                if(memcmp(addr,p_ChkCcoOutVrsnInfo_10F104_Up->network_node_info_unit_.node_info_group_list_[0].node_address_.addr,6)==0)
                {
                    QByteArray rptVrsnDate;
                    rptVrsnDate.append(p_ChkCcoOutVrsnInfo_10F104_Up->network_node_info_unit_.node_info_group_list_[0].node_info_.ver_year_)
                            .append(p_ChkCcoOutVrsnInfo_10F104_Up->network_node_info_unit_.node_info_group_list_[0].node_info_.ver_month_)
                ...
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("10F104回复主节点地址非全F：")+QByteArray(p_ChkCcoOutVrsnInfo_10F104_Up->network_node_info_unit_.node_info_group_list_[0].node_address_.addr,6).toHex());
                }
            }
            else if(p_Frame3762Base->afn_==char(0xF0) && dtValue3762==15 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(ccoInVrsn == ccoInVrsn_init)
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "路由内部版本未改变，重新设置主节点地址");
                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->dvcId,INSIGNIFICANCE,p_SetCcoAddr_05F1);
                    p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    emScriptRunState = SetCcoAddr;
                }
                else
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, "路由内部版本发生改变，初始版本"+ccoInVrsn_init);
                }
            }
            else
            {
        ...
        case SetCcoAddr:
        {
            ...
            {
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到03F10，路由复位");
            }
            else
            {
                QByteArray tmpSendMsg=prcsOther3762Msg(uchar(p_Frame3762Base->afn_),p_Frame3762Base->dt1_,p_Frame3762Base->dt2_,uchar(p_Frame3762Base->info_field_.info_field_up.msg_seq));
        ...
        case SetAndReadInfo_2:
        {
            dtValue3762 = get3762Dt(p_Frame3762Base->dt1_,p_Frame3762Base->dt2_);
            if(p_Frame3762Base->afn_==0x00 && dtValue3762==1 && p_Frame3762Base->ctrl_field_.dir==kDirUp)
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到设置相同省份版本头确认，等待10s看是否复位...");
                p_delayTimer->start(10*1000);
            }
            else if(p_Frame3762Base->afn_==0x00 && dtValue3762==2 && p_Frame3762Base->ctrl_field_.dir==kDirUp)
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "设置相同省份版本头路由回复否认，不符合要求");
            }
            else if(p_Frame3762Base->afn_==0x03 && dtValue3762==10 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_delayTimer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "设置相同省份版本头后收到03F10运行模式信息上报，路由出现复位");
            }
            else if(p_Frame3762Base->afn_==char(0xf0) && dtValue3762==7 && p_Frame3762Base->ctrl_field_.dir==kDirUp)
            {
                p_timer->stop();
                shared_ptr<AfnF0F7> p_QueryVersionHeadF0F7_Up=dynamic_pointer_cast<AfnF0F7>(p_Frame3762Base);
                if(p_QueryVersionHeadF0F7_Up->data_unit_up_.parameter_content_.toHex() == TestHead2)
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "查询路由版本头与设置一致，开始03F1读路由外部版本");
                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_ChkCcoOutVrsn_03F1_Down);
                    p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
            ...
            else if(p_Frame3762Base->afn_==0x03 && dtValue3762==1 && p_Frame3762Base->ctrl_field_.dir==kDirUp)
            {
                ...
                QString code = TestHead2.mid(8,8);
                QString year = QString::number((TestHead2.mid(26,2)+TestHead2.mid(24,2)).toInt(nullptr,16)).right(2);
                QString month = QString("%1").arg(TestHead2.mid(28,2).toInt(nullptr,16),2,10,QChar('0'));
                QString day = QString("%1").arg(TestHead2.mid(30,2).toInt(nullptr,16),2,10,QChar('0'));
                QString vrsn1 = QString("%1").arg(TestHead2.mid(16,2).toInt(nullptr,16),2,10,QChar('0'));
                QString vrsn2 = QString("%1").arg(TestHead2.mid(18,2).toInt(nullptr,16),2,10,QChar('0'));
                if(code_03F1==code && vrsn_03F1==year+month+day+vrsn1+vrsn2)
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "03F1路由外部版本和版本头一致，开始10F104查询外部版本");
                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_ChkCcoOutVrsnInfo_10F104_Down);
            ...
            else if(p_Frame3762Base->afn_==char(0xF0) && dtValue3762==15 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(ccoInVrsn == ccoInVrsn_init)
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Success, "设置相同省份版本头后路由内部版本未改变");
                }
                else
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, "设置相同省份版本头后路由内部版本发生改变，初始版本"+ccoInVrsn_init);
                }
            }
            else
            {
...
void Script_VersionHead_CcoNormal::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_VersionHead_CcoNormal::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_VersionHead_CcoNormal::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_SetVersionHeadF0F8_Down)
    {
        ...
        {
            p_SetVersionHeadF0F8_Down->data_unit_down_.parameter_length_=ushort(TestHead2.length()/2);
            p_SetVersionHeadF0F8_Down->data_unit_down_.parameter_content_=QByteArray::fromHex(TestHead2.toLatin1());
        }
...
void Script_VersionHead_CcoNormal::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_VersionHead_CcoNormal::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
...
//    }
    default:
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "超时未回复");
        break;
    }
    }
}
void Script_VersionHead_CcoNormal::maxAllowTimer_timeoutProc()
{
...
void Script_VersionHead_CcoNormal::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    if(emScriptRunState == ReadInfoBefore)
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "设置版本头后等待10s未收到路由复位后03F10运行模式信息上报");
    }
    else if(emScriptRunState == SetCcoAddr)
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "开始设置相同省份版本头");
        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,2,p_SetVersionHeadF0F8_Down);
        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
        emScriptRunState = SetAndReadInfo_2;
    }
    else if(emScriptRunState == SetAndReadInfo_2)
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "设置相同省份版本头后10s路由未出现复位，查询路由版本头");
        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryVersionHeadF0F7_Down);
        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
    }
    else
...
void Script_VersionHead_CcoNormal::calPowerOnReportRate()
{
