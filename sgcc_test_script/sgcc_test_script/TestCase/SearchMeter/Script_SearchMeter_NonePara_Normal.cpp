#include "Script_SearchMeter_NonePara_Normal.h"

Script_SearchMeter_NonePara_Normal::Script_SearchMeter_NonePara_Normal(QObject *parent) : QObject(parent)
{
...
Script_SearchMeter_NonePara_Normal::~Script_SearchMeter_NonePara_Normal()
{
...
void Script_SearchMeter_NonePara_Normal::execute()
{
...
void Script_SearchMeter_NonePara_Normal::stop()
{
...
void Script_SearchMeter_NonePara_Normal::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_SearchMeter_NonePara_Normal::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_SearchMeter_NonePara_Normal::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_SearchMeter_NonePara_Normal::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_SearchMeter_NonePara_Normal::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_SearchMeter_NonePara_Normal::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_SearchMeter_11F5_Finish:
            {
                ...
                {
                    p_timer->stop();
                    if(paraInitFlag==true)
                    {
                        paraInitFlag=false;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到确认，发送--激活主动注册（11F5），等待--确认");
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_ActiveRegister_11F5);
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
//                        p_delayTimer->start((activeTime+2)*60*1000);
                        p_delayTimer->start(18*60*1000); // 开始搜表后18分钟，10F6查询
                    }
                    else
                    {
                        activeFlag=true;
                ...
                else if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x08&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    else
                    {
                        if(uchar(p_QueryRouterState_10F4_Up->router_operate_state_unit_.work_switch_.register_allow_flag_)==0x01)
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "注册允许标志为【允许】，符合要求，查询主动注册从节点（10F6）");
//                            index = 1;
...
//                            query10F6List.clear();
              ...
              //      {
                    if(p_ReportRouterState_Up->router_work_task_change_==0x02)
                    {
                        ...
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "搜表未上报");
                        }
                        else
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "统计上报数"+QString::number(nodeInfoList.size()));
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "5s后查状态");
...
//                    }
                ...
                else if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x20&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(p_QueryRegisterNode_10F6_Up->register_node_info_unit_.node_total_num_>=nodeInfoList.size())
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("查询主动注册从节点，符合要求，11F5搜表上报数%1，10F6回复节点总数%2")
                                                             .arg(nodeInfoList.size()).arg(p_QueryRegisterNode_10F6_Up->register_node_info_unit_.node_total_num_));
//                        nodeInfoList.clear();
...
//                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "查询主动注册从节点，不符合要求，11F5搜表上报数："+QString::number(nodeInfoList.size())+"，10F6回复节点总数："+QString::number(p_QueryRegisterNode_10F6_Up->register_node_info_unit_.node_total_num_));
//                        for(int i=0;i<p_QueryRegisterNode_10F6_Up->register_node_info_unit_.this_node_num_;i++)
//                        {
//                            NodeInfo_Struct node;
//                            memcpy(node.nodeAddress.addr,p_QueryRegisterNode_10F6_Up->register_node_info_unit_.node_info_group_list_.at(i).node_address_.addr,6);
//                            node.protocol=1;
//                            node.deviceType=1;
//                            query10F6List.append(node);
//                        }
//                        if(query10F6List.size()<p_QueryRegisterNode_10F6_Up->register_node_info_unit_.node_total_num_)
//                        {
//                            index += 5;
//                            sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryRegisterNode_10F6);
//                            p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
...
//                            for(int i=0;i<nodeInfoList.size();i++)
//                            {
//                                for(int j=0;j<query10F6List.size();j++)
//                                {
//                                    if(nodeInfoList.at(i).nodeAddress == query10F6List.at(j).nodeAddress)
//                                        query10F6List[j].isExist = true;
//                                }
//                            }
//                            QString restAddrs;
//                            for(int i=0;i<query10F6List.size();i++)
//                            {
//                                if(query10F6List.at(i).isExist == false)
//                                {
//                                    restAddrs.append(QByteArray(query10F6List.at(i).nodeAddress.addr,6).toHex()).append('\n');
//                                }
//                            }
//                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "06F4未上报但10F6中存在的表号：\n" + restAddrs);
//                            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "查询主动注册从节点，不符合要求，搜表上报数："+QString::number(nodeInfoList.size())+"，10F6回复节点总数："+QString::number(p_QueryRegisterNode_10F6_Up->register_node_info_unit_.node_total_num_));
//                        }
                    }
                }
                else
            ...
            case Wait_SearchMeter_11F225_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    {
                        paraInitFlag=false;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到确认，发送--激活主动注册(新增)11F225，等待--确认");
                ...
                else if(p_Frame3762Base->afn_ == 0x06&&p_Frame3762Base->dt1_==0x04&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_delayTimer->stop();
                    shared_ptr<Afn06F3> p_ReportRouterState_Up=dynamic_pointer_cast<Afn06F3>(p_Frame3762Base);
            //        if(judgeExistState()==None)//只上报不在档案内的电表
            //        {
                    if(p_ReportRouterState_Up->router_work_task_change_==0x02)
                    {
                        activeFlag=false;
                        ...
                        if(nodeInfoList.size()==0)
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "11F225搜表未上报");
                        }
                        else
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "11F225统计上报数"+QString::number(nodeInfoList.size()));
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "5s后查状态");
                            p_delayTimer->start(5*1000);
                        }
                    }
...
//                    }
                ...
                else if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x20&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(p_QueryRegisterNode_10F6_Up->register_node_info_unit_.node_total_num_>=nodeInfoList.size())
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("查询主动注册从节点，符合要求，11F225搜表上报数%1，10F6回复节点总数%2")
                                                             .arg(nodeInfoList.size()).arg(p_QueryRegisterNode_10F6_Up->register_node_info_unit_.node_total_num_));
                        emScriptRunState=ScriptSuccess;
                        resultFlag=true;
                        p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("搜表测试成功"));
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "查询主动注册从节点，不符合要求，11F225搜表上报数："+QString::number(nodeInfoList.size())+"，10F6回复节点总数："+QString::number(p_QueryRegisterNode_10F6_Up->register_node_info_unit_.node_total_num_));
                    }
                }
                else
                {
...
void Script_SearchMeter_NonePara_Normal::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_SearchMeter_NonePara_Normal::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_SearchMeter_NonePara_Normal::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_SearchMeter_NonePara_Normal::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_SearchMeter_NonePara_Normal::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
...
void Script_SearchMeter_NonePara_Normal::maxAllowTimer_timeoutProc()
{
...
void Script_SearchMeter_NonePara_Normal::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
   // p_maxAllowTimer->stop();
    if(activeFlag == true)
    {
        ...
        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryRegisterNode_10F6);
        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
//        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "路由搜表时间到后未上报工况变动");
    }
    else
    {
...
void Script_SearchMeter_NonePara_Normal::statisticsNode()
{
    notInRouterNoteList.clear();
    inRouterNoteList.clear();
    //判断是否存在于档案中
...
Script_SearchMeter_NonePara_Normal::ParaExistState Script_SearchMeter_NonePara_Normal::judgeExistState()
{
    statisticsNode();
