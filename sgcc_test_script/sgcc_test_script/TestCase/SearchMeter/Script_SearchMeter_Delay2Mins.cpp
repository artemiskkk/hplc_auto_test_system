#include "Script_SearchMeter_Delay2Mins.h"

Script_SearchMeter_Delay2Mins::Script_SearchMeter_Delay2Mins(QObject *parent) : QObject(parent)
{
...
Script_SearchMeter_Delay2Mins::~Script_SearchMeter_Delay2Mins()
{
...
void Script_SearchMeter_Delay2Mins::execute()
{
...
//        p_delayTimer->start(delayTime*65*1000);//2min10sec
        p_delayTimer->start(delayTime*60*1000);//2min
    }
    p_maxAllowTimer->start((timerForReachThresld+timerAfterReachThresld)*1000);
}

void Script_SearchMeter_Delay2Mins::stop()
{
...
void Script_SearchMeter_Delay2Mins::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_SearchMeter_Delay2Mins::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}

bool Script_SearchMeter_Delay2Mins::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_SearchMeter_Delay2Mins::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_SearchMeter_Delay2Mins::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_SearchMeter_Delay2Mins::processMsgFromCCO(DvcType dvcType, int dvcId)
{
...
//                    searchFlag=true;
                ...
                else if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x08&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(searchFlag==true&&reachDelayFlag==true)
                    {
...
//                            reachDelayFlag=false;
                        }
                        else
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "注册允许标志或当前状态，不符合要求");
                        }
                    }
                    else if(searchFlag==false&&reachDelayFlag==false)
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "222222222");
                    ...
                    else if(searchFlag==true&&reachDelayFlag==false)
                    {
                        if(uchar(p_QueryRouterState_10F4_Up->router_operate_state_unit_.work_switch_.register_allow_flag_)==0x01&&uchar(p_QueryRouterState_10F4_Up->router_operate_state_unit_.work_switch_.current_state_)==0x01)
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "注册允许标志为【允许】，当前状态为 搜表 ，符合要求");
...
//                            p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                            ...
                            resultFlag=true;
                            p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("延时2min搜表测试成功"));
                        }
                        else
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "注册允许标志或当前状态，不符合要求");
                ...
                else if(p_Frame3762Base->afn_ == 0x06&&p_Frame3762Base->dt1_==0x08&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    if(searchFlag == false)
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "在非搜表状态收到06F4上报注册从节点");
                        return;
                    }
//                    if(p_delayTimer->isActive() && p_delayTimer->remainingTime()>20*1000)
//                    {
//                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "延时未到2min收到06F4上报注册从节点");
//                        return;
//                    }
...
//                    {
                    if(p_ReportRouterState_Up->router_work_task_change_==0x02)
                    {
                        ...
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "回复确认，5s后查状态");
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_Confirm_00F1);
...
//                    else
//                    {
//                        p_AbstractScriptHost->updateProgress(ProcessState_Failed,"上报注册从节点与档案不一致，上报数："+QString::number(nodeInfoList.size())+"，档案数："+QString::number(p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().size()));
//                    }
                }
...
//                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--11F225激活主动注册(新增)（延时2min），等待--确认");
...
//                            reachDelayFlag=false;
                ...
                else if(p_Frame3762Base->afn_ == 0x06&&p_Frame3762Base->dt1_==0x08&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    for(int j=0;j<p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().size();j++)
                    {
                        if(memcmp(p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(j)->mtrAddr,p_ReportRegisterNode_Up->report_node_info_unit_.report_node_address_.addr,6)==0)
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("11F225有档案内的表上报，不符合要求，表号%1").arg(QString(QByteArray(p_ReportRegisterNode_Up->report_node_info_unit_.report_node_address_.addr,6).toHex())));
                            return;
                        }
                    }
                    num_reportOutOfRecord++;
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("11F225有档案外的表上报，符合要求，表号%1").arg(QString(QByteArray(p_ReportRegisterNode_Up->report_node_info_unit_.report_node_address_.addr,6).toHex())));
                    bool reportTwiceFlag = false;
                    for(int i=0;i<nodeInfoList.size();i++)
                    {
                        if(memcmp(nodeInfoList.at(i).nodeAddress.addr,p_ReportRegisterNode_Up->report_node_info_unit_.report_node_address_.addr,6)==0)
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "回复确认，当前上报数"+QString::number(num_reportOutOfRecord));
                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_Confirm_00F1);
                }
                else if(p_Frame3762Base->afn_ == 0x06&&p_Frame3762Base->dt1_==0x04&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
           //         p_delayTimer->stop();
                    ...
                    {
                        searchFlag=false;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "路由上报工况变动，搜表结束");
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "回复确认，5s后查状态");
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_Confirm_00F1);
                        p_delayTimer->start(5*1000);
                    }
                ...
                else if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x20&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "查询主动注册从节点，不符合要求，10F6回复节点总数"+QString::number(p_QueryRegisterNode_10F6_Up->register_node_info_unit_.node_total_num_)+"，11F225搜表上报档案外的表数"+QString::number(num_reportOutOfRecord));
                    }
                }
                else
                {
...
void Script_SearchMeter_Delay2Mins::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_SearchMeter_Delay2Mins::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_SearchMeter_Delay2Mins::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    if(frame==p_ActiveRegister_11F5)
    {
        ...
        QByteArray startTime=QByteArray::fromHex(QDateTime::currentDateTime().addSecs(delayTime*60).toString("ssmmhhddMMyy").toLatin1());
...
void Script_SearchMeter_Delay2Mins::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_SearchMeter_Delay2Mins::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
...
void Script_SearchMeter_Delay2Mins::maxAllowTimer_timeoutProc()
{
...
void Script_SearchMeter_Delay2Mins::delayTimer_timeoutProc()
{
//    p_delayTimer->stop();
//    if(searchFlag == true)
//    {
...
//    //p_AbstractScriptHost->updateProgress(ProcessState_Error, "State machine run error!!!==p_delayTimer");
    ...
    if((searchFlag == false) && (reachDelayFlag == false))
    {
        ...
        reachDelayFlag= true;
        p_delayTimer->start(240*1000); // 搜表开始后4分10秒，搜表结束前查询10F6
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--查询路由运行状态（10F4），等待--回复");
        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryRouterState_10F4);
        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
        return;
    }else if((searchFlag == true) && (reachDelayFlag == true)) {
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "搜表结束前发送--查询路由运行状态（10F6），等待--回复");
        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryRegisterNode_10F6);
        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
    }
...
bool Script_SearchMeter_Delay2Mins::isAllExist()
{
    bool result=true;
    for(int i=0;i<nodeInfoList.size();i++)
    {
