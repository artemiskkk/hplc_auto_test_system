#include "Script_SearchMeter_PartialPara_Offline.h"

Script_SearchMeter_PartialPara_Offline::Script_SearchMeter_PartialPara_Offline(QObject *parent) : QObject(parent)
{
...
Script_SearchMeter_PartialPara_Offline::~Script_SearchMeter_PartialPara_Offline()
{
...
void Script_SearchMeter_PartialPara_Offline::execute()
{
...
void Script_SearchMeter_PartialPara_Offline::stop()
{
...
void Script_SearchMeter_PartialPara_Offline::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_SearchMeter_PartialPara_Offline::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_SearchMeter_PartialPara_Offline::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_SearchMeter_PartialPara_Offline::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_SearchMeter_PartialPara_Offline::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_SearchMeter_PartialPara_Offline::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
            ...
            case Wait_SearchMeter_11F5_Finish:
            {
                ...
                else if(p_Frame3762Base->afn_ == 0x06&&p_Frame3762Base->dt1_==0x04&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(p_ReportRouterState_Up->router_work_task_change_==0x02)
                    {
                        ...
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("查询网络拓扑10F21"));
                ...
                else if(p_Frame3762Base->afn_==0x10&&p_Frame3762Base->dt1_==0x10&&p_Frame3762Base->dt2_==0x02&&p_Frame3762Base->ctrl_field_.dir==kDirUp)
                {
                    ...
                    ushort nodeNumInline = p_ResTopoInfo_Afn10F21->network_typelogy_info_unit_.node_total_num_;
                    meterNum = p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().size();
                    if(nodeNumInline-1>meterNum)
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("查询拓扑入网数%1>档案数%2+1，无问题").arg(nodeNumInline).arg(meterNum));
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("查询拓扑入网数%1<=档案数%2+1").arg(nodeNumInline).arg(meterNum));
                    }
                }
                else if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x20&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "10F6查询主动注册从节点总数量："+QString::number(p_QueryRegisterNode_10F6_Up->register_node_info_unit_.node_total_num_));
                    }
                    else
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "10F6查询主动注册从节点总数量为0");
                    }
                }
                else
                {
        ...
        case AfterOffline:
        {
            if(p_Frame3762Base->afn_==0x10&&p_Frame3762Base->dt1_==0x10&&p_Frame3762Base->dt2_==0x02&&p_Frame3762Base->ctrl_field_.dir==kDirUp)
            {
                ...
                if(nodeNumInline-1==meterNum)
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("搜表结束3min后查询拓扑入网数%1=档案数%2+1，黑名单节点已离线").arg(nodeNumInline).arg(meterNum));
                }
                else
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("搜表结束3min后查询拓扑入网数%1!=档案数%2+1").arg(nodeNumInline).arg(meterNum));
                }
            }
            else
            {
...
void Script_SearchMeter_PartialPara_Offline::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_SearchMeter_PartialPara_Offline::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_SearchMeter_PartialPara_Offline::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_QueryNetTopoInfo_10F21)
    {
        ...
        p_QueryNetTopoInfo_10F21->node_start_no_=1;
        p_QueryNetTopoInfo_10F21->node_num_=5;
...
void Script_SearchMeter_PartialPara_Offline::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_SearchMeter_PartialPara_Offline::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
...
void Script_SearchMeter_PartialPara_Offline::maxAllowTimer_timeoutProc()
{
...
void Script_SearchMeter_PartialPara_Offline::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    if(activeFlag==true)
    {
...
void Script_SearchMeter_PartialPara_Offline::statisticsNode()
{
...
Script_SearchMeter_PartialPara_Offline::ParaExistState Script_SearchMeter_PartialPara_Offline::judgeExistState()
{
    statisticsNode();
