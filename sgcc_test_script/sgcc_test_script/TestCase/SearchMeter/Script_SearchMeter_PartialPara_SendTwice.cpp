#include "Script_SearchMeter_PartialPara_SendTwice.h"

Script_SearchMeter_PartialPara_SendTwice::Script_SearchMeter_PartialPara_SendTwice(QObject *parent) : QObject(parent)
{
...
Script_SearchMeter_PartialPara_SendTwice::~Script_SearchMeter_PartialPara_SendTwice()
{
...
void Script_SearchMeter_PartialPara_SendTwice::execute()
{
    ...
    else
    {
        ...
        emScriptRunState=Wait_SearchMeter1_11F5_Finish;
...
void Script_SearchMeter_PartialPara_SendTwice::stop()
{
...
void Script_SearchMeter_PartialPara_SendTwice::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_SearchMeter_PartialPara_SendTwice::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_SearchMeter_PartialPara_SendTwice::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_SearchMeter_PartialPara_SendTwice::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_SearchMeter_PartialPara_SendTwice::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_SearchMeter_PartialPara_SendTwice::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        case Wait_SearchMeter1_11F5_Finish:
        {
            if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_timer->stop();
                //delay(1000);
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "激活主动注册(2min)回复确认");

                emScriptRunState=Wait_SearchMeter2_11F5_Finish;
                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_ActiveRegister_11F5);
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--激活主动注册11F5(5min)，等待--确认");
                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
            }
            else
            {
        ...
        case Wait_SearchMeter2_11F5_Finish:
        {
            if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "激活主动注册(5min)回复确认，不符合要求");
            }
            ...
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "激活主动注册(5min)回复否认，符合要求，1min后查10F4");
            ...
            else if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x08&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(uchar(p_QueryRouterState_10F4_Up->router_operate_state_unit_.work_switch_.current_state_)==0x01)
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "10F4当前状态：搜表");

                    emScriptRunState=Wait_SearchMeterEnd;
                    p_delayTimer->start(2*60*1000);
                }
                else
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, "10F4当前状态有误："+QString::number(uchar(p_QueryRouterState_10F4_Up->router_operate_state_unit_.work_switch_.current_state_)));
                }
            }
            else
            {
        ...
        case Wait_SearchMeterEnd:
        {
            if(p_Frame3762Base->afn_ == 0x06&&p_Frame3762Base->dt1_==0x04&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(p_ReportRouterState_Up->router_work_task_change_==0x02)
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "回复确认，等5s后查状态");
                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_Confirm_00F1);
        ...
        case AfterSearchMeterEnd:
        {
            if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x08&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_timer->stop();
                shared_ptr<Afn10F4> p_QueryRouterState_10F4_Up=dynamic_pointer_cast<Afn10F4>(p_Frame3762Base);
                if(uchar(p_QueryRouterState_10F4_Up->router_operate_state_unit_.work_switch_.current_state_)==0x03)
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Success, "10F4当前状态：其他");
...
void Script_SearchMeter_PartialPara_SendTwice::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_SearchMeter_PartialPara_SendTwice::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_SearchMeter_PartialPara_SendTwice::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    if(frame==p_ActiveRegister_11F5)
    {
        ...
        if(emScriptRunState==Wait_SearchMeter1_11F5_Finish)
        {
            p_ActiveRegister_11F5->last_time_=2;
        }
        else if(emScriptRunState==Wait_SearchMeter2_11F5_Finish)
        {
            p_ActiveRegister_11F5->last_time_=5;
        }
...
void Script_SearchMeter_PartialPara_SendTwice::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_SearchMeter_PartialPara_SendTwice::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
...
void Script_SearchMeter_PartialPara_SendTwice::maxAllowTimer_timeoutProc()
{
...
void Script_SearchMeter_PartialPara_SendTwice::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    if(emScriptRunState == Wait_SearchMeter2_11F5_Finish)
    {
        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryRouterState_10F4);
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--查询路由运行状态10F4，等待--回复");
        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
    }
    else if(emScriptRunState == Wait_SearchMeterEnd)
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "开始搜表后等待3min未上报06F3搜表结束");
    }
...
void Script_SearchMeter_PartialPara_SendTwice::statisticsNode()
{
...
Script_SearchMeter_PartialPara_SendTwice::ParaExistState Script_SearchMeter_PartialPara_SendTwice::judgeExistState()
{
    statisticsNode();
