#include "Script_SearchMeter_PartialPara_Normal.h"

Script_SearchMeter_PartialPara_Normal::Script_SearchMeter_PartialPara_Normal(QObject *parent) : QObject(parent)
{
...
Script_SearchMeter_PartialPara_Normal::~Script_SearchMeter_PartialPara_Normal()
{
...
void Script_SearchMeter_PartialPara_Normal::execute()
{
    ...
    else
    {
        ...
        p_delayTimer->start(18*60*1000); //20分钟搜表时间结束前（18分钟时）查询10F6
    }
    p_maxAllowTimer->start((timerForReachThresld+timerAfterReachThresld)*1000);

}
void Script_SearchMeter_PartialPara_Normal::stop()
{
...
void Script_SearchMeter_PartialPara_Normal::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_SearchMeter_PartialPara_Normal::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_SearchMeter_PartialPara_Normal::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_SearchMeter_PartialPara_Normal::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_SearchMeter_PartialPara_Normal::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_SearchMeter_PartialPara_Normal::processMsgFromCCO(DvcType dvcType, int dvcId)
{
...
//                            p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                ...
                else if(p_Frame3762Base->afn_ == 0x06&&p_Frame3762Base->dt1_==0x04&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    else
                    {
                        ...
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "111发送--查询路由运行状态（10F4），等待--回复");
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryRouterState_10F4);
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    }
                }
...
void Script_SearchMeter_PartialPara_Normal::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_SearchMeter_PartialPara_Normal::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_SearchMeter_PartialPara_Normal::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_SearchMeter_PartialPara_Normal::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_SearchMeter_PartialPara_Normal::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
...
void Script_SearchMeter_PartialPara_Normal::maxAllowTimer_timeoutProc()
{
...
void Script_SearchMeter_PartialPara_Normal::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
   // p_maxAllowTimer->stop();
    if(activeFlag == true)
...
void Script_SearchMeter_PartialPara_Normal::statisticsNode()
{
    notInRouterNoteList.clear();
    inRouterNoteList.clear();
    for(int j=0;j<p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().size();j++)
...
Script_SearchMeter_PartialPara_Normal::ParaExistState Script_SearchMeter_PartialPara_Normal::judgeExistState()
{
    statisticsNode();
