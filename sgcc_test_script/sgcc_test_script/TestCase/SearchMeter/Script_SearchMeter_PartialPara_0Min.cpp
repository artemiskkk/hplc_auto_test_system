#include "Script_SearchMeter_PartialPara_0Min.h"

Script_SearchMeter_PartialPara_0Min::Script_SearchMeter_PartialPara_0Min(QObject *parent) : QObject(parent)
{
...
Script_SearchMeter_PartialPara_0Min::~Script_SearchMeter_PartialPara_0Min()
{
...
void Script_SearchMeter_PartialPara_0Min::execute()
{
...
void Script_SearchMeter_PartialPara_0Min::stop()
{
...
void Script_SearchMeter_PartialPara_0Min::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_SearchMeter_PartialPara_0Min::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_SearchMeter_PartialPara_0Min::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_SearchMeter_PartialPara_0Min::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
    ...
    if(emScriptRunState==Wait_BuildNetFinish_Whole)
    {
        ...
        else
        {
         ...
         //   p_timer->stop();
            emScriptRunState=Wait_SearchMeter_11F5_Finish;
            sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_ActiveRegister_11F5);
            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--激活主动注册11F5(0min)，等待--确认");
            p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
        }
    }
    else
...
void Script_SearchMeter_PartialPara_0Min::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_SearchMeter_PartialPara_0Min::processMsgFromCCO(DvcType dvcType, int dvcId)
{
        ...
        case Wait_SearchMeter_11F5_Finish:
        {
            ...
            else if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x08&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(uchar(p_QueryRouterState_10F4_Up->router_operate_state_unit_.work_switch_.register_allow_flag_)==0x01
                        && uchar(p_QueryRouterState_10F4_Up->router_operate_state_unit_.work_switch_.current_state_)==0x01)
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "当前状态为【搜表】，注册允许状态为【允许】，符合要求");
                    activeFlag = true;                    
                    p_timer->start(62*60*1000);//现在是默认1小时逻辑
                }
                else
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("路由状态有误：当前状态【%1】，注册允许状态【%2】")
                                                         .arg(QString::number(p_QueryRouterState_10F4_Up->router_operate_state_unit_.work_switch_.current_state_))
                                                         .arg(QString::number(p_QueryRouterState_10F4_Up->router_operate_state_unit_.work_switch_.register_allow_flag_)));
                }
            }
...
//                    p_delayTimer->start(11*60*1000);
            ...
            else if(p_Frame3762Base->afn_ == 0x06&&p_Frame3762Base->dt1_==0x10&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("收到06F5事件上报，从节点设备类型%1，通信协议类型%2，内容%3").arg(QString::number(p_EventReport_06F5_Up->report_event_unit_.node_device_type_))
                                                     .arg(QString::number(p_EventReport_06F5_Up->report_event_unit_.node_protocol_type_)).arg(QString(p_EventReport_06F5_Up->report_event_unit_.frame_content_.toHex())));
            ...
            else if(p_Frame3762Base->afn_ == 0x06&&p_Frame3762Base->dt1_==0x04&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                {
                    p_delayTimer->stop();
                    p_timer->stop();//现在是默认1小时逻辑
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "06F3路由上报工况变动：搜表任务结束，上报表数"+QString::number(nodeInfoList.size()));
                    emScriptRunState = AfterSearchMeterEnd;

                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "回复确认，等待5s查状态");
                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_Confirm_00F1);
                    p_delayTimer->start(5*1000);
//                    p_AbstractScriptHost->updateProgress(ProcessState_Success, "收到06F3上报搜表结束");
//                    emScriptRunState=ScriptSuccess;
        ...
        case AfterSearchMeterEnd:
        {
            if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x08&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                shared_ptr<Afn10F4> p_QueryRouterState_10F4_Up=dynamic_pointer_cast<Afn10F4>(p_Frame3762Base);
                if(uchar(p_QueryRouterState_10F4_Up->router_operate_state_unit_.work_switch_.register_allow_flag_)==0x01
                        && uchar(p_QueryRouterState_10F4_Up->router_operate_state_unit_.work_switch_.current_state_)==0x03)
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Success, "当前状态为【其他】，注册允许状态为【允许】，符合要求");
                    emScriptRunState=ScriptSuccess;
                }
                else
                {
...
void Script_SearchMeter_PartialPara_0Min::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_SearchMeter_PartialPara_0Min::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_SearchMeter_PartialPara_0Min::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    if(frame==p_ActiveRegister_11F5)
    {
        ...
        p_ActiveRegister_11F5->last_time_=0;
...
void Script_SearchMeter_PartialPara_0Min::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_SearchMeter_PartialPara_0Min::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
        ...
        case Wait_SearchMeter_11F5_Finish:
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "启动持续时间为0搜表之后，1小时后未收到路由上报06F3工况变动");
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "启动持续时间为0搜表之后，1小时后未收到路由上报06F3工况变动");
            break;
        }
        default:
        {
...
void Script_SearchMeter_PartialPara_0Min::maxAllowTimer_timeoutProc()
{
...
void Script_SearchMeter_PartialPara_0Min::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    if(emScriptRunState == Wait_SearchMeter_11F5_Finish)
    {
        if(activeFlag == false)
        {
...
//            if(++tryTimes>2)
//            {
//                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "11min内未收到06F4搜表上报或06F3搜表结束");
//                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "路由在搜表期间，电表入网，路由会去查询电表的信息，路由没有查询成功时，就不认为从节点搜成功，不会上报06F4。\n"
//                                                                              "路由仍处于搜表状态，但脚本因为已经超过11min未上报06F4，会判断失败。\n"
//                                                                              "对照路由日志确定路由是否真正搜表完成，如果未搜表完成，可复测几次。");
//                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "11min内未收到06F4搜表上报或06F3搜表结束，原因请查看运行日志");
//            }
//            else
//            {
//                p_delayTimer->start(11*60*1000);
//                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("开始第%1轮等待").arg(QString::number(tryTimes)));
//            }
        }
    }
    else if(emScriptRunState == AfterSearchMeterEnd)
    {
    ...
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Error, "emScriptRunState error:delayTimer");
    }
}

void Script_SearchMeter_PartialPara_0Min::statisticsNode()
{
...
Script_SearchMeter_PartialPara_0Min::ParaExistState Script_SearchMeter_PartialPara_0Min::judgeExistState()
{
    statisticsNode();
