#include "Script_SearchMeter_PartialPara_Stop.h"

Script_SearchMeter_PartialPara_Stop::Script_SearchMeter_PartialPara_Stop(QObject *parent) : QObject(parent)
{
...
Script_SearchMeter_PartialPara_Stop::~Script_SearchMeter_PartialPara_Stop()
{
...
void Script_SearchMeter_PartialPara_Stop::execute()
{
...
void Script_SearchMeter_PartialPara_Stop::stop()
{
...
void Script_SearchMeter_PartialPara_Stop::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_SearchMeter_PartialPara_Stop::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_SearchMeter_PartialPara_Stop::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_SearchMeter_PartialPara_Stop::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
    ...
    if(emScriptRunState==Wait_BuildNetFinish_Whole)
    {
        ...
        else
        {
           ...
           // p_timer->stop();
            emScriptRunState=Wait_SearchMeter_11F5_Finish;
            delay(100*1000);  // 解决搜网时间过长导致11F5下发有收不到的问题
            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--激活主动注册（11F5），等待--确认");
            sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_ActiveRegister_11F5);
            p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
             //   p_delayTimer->start(activeTime*60*1000);//2min;
...
void Script_SearchMeter_PartialPara_Stop::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_SearchMeter_PartialPara_Stop::processMsgFromCCO(DvcType dvcType, int dvcId)
{
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_SearchMeter_11F5_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    {
                        activeFlag=true;
                        p_delayTimer->start(activeTime*30*1000);//一半
                    ...
                    else
                    {
                        activeFlag=false;
                    //    p_delayTimer->stop();
                        if(judgeExistState()!=Partial)
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("档案配置数%0，上报数%1，在档案数%2，不在档案数%3").arg(p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().size())
                                                                 .arg(nodeInfoList.size()).arg(inRouterNoteList.size()).arg(notInRouterNoteList.size()));
                            if(inRouterNoteList.size()<p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().size())
                            {
                                ...
                                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "有档案中的表没有上报");
                            }
                            else
                            {
                                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "没有上报档案外的表");
                            }
                        }
                        else
                        {
                            stopFlag=false;
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到确认，5s后查状态");
...
//                            p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                        ...
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "注册允许标志不符合要求");
                        }
                    }
                }
...
//                    }
                ...
                {
//                    p_delayTimer->stop();
//                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, "路由不应06F3上报工况变动");
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "06F3路由上报工况变动：搜表任务结束，上报表数"+QString::number(nodeInfoList.size()));
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "回复确认");
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_Confirm_00F1);
                    }
                    else
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "06F3路由上报工况变动，路由工作任务变动类型有误："+QString::number(p_ReportRouterState_Up->router_work_task_change_));
                    }
                }
...
//p_AbstractScriptHost->updateProgress(ProcessState_Processing, "判断1,10F6数:"+QString::number(p_QueryRegisterNode_10F6_Up->register_node_info_unit_.node_total_num_)+" 不在档案数:"+QString::number(notInRouterNoteList.size()));
                    if(p_QueryRegisterNode_10F6_Up->register_node_info_unit_.node_total_num_>=notInRouterNoteList.size())
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("查询主动注册从节点，符合要求，11F5搜表上报节点不在档案数%1，10F6回复节点总数%2")
                                                             .arg(notInRouterNoteList.size()).arg(p_QueryRegisterNode_10F6_Up->register_node_info_unit_.node_total_num_));
//                        nodeInfoList.clear();
//                        inRouterNoteList.clear();
//                        notInRouterNoteList.clear();
//                        activeFlag=false;
//                        stopFlag=false;
...
//                   //     p_delayTimer->start(activeTime*60*1000);//2min
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("档案配置数%0，11F5搜表上报数%1，在档案数%2，不在档案数%3，10F6回复节点总数%4").arg(p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().size())
                                                             .arg(nodeInfoList.size()).arg(inRouterNoteList.size()).arg(notInRouterNoteList.size()).arg(p_QueryRegisterNode_10F6_Up->register_node_info_unit_.node_total_num_));
            ...
            case Wait_SearchMeter_11F225_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    {
                        activeFlag=true;
                        p_delayTimer->start(activeTime*30*1000);//一半时间
                     //   startFlag=true;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到确认，发送--查询路由运行状态（10F4），等待--回复");
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryRouterState_10F4);
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    ...
                    else
                    {
                        activeFlag=false;
                   //     p_delayTimer->stop();
                        if(judgeExistState()!=None)
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("档案配置数%0，搜表上报数%1，在档案数%2，不在档案数%3").arg(p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().size())
                                                                 .arg(nodeInfoList.size()).arg(inRouterNoteList.size()).arg(notInRouterNoteList.size()));
                            if(inRouterNoteList.size()!=0)
                            {
                                QString meters_inRouterNote;
                                for(int k=0;k<inRouterNoteList.size();k++)
                                {
                                    meters_inRouterNote.append(QByteArray(inRouterNoteList.at(k).nodeAddress.addr,6).toHex()).append("\n");
                                }
                                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "11F225上报了档案中的表：\n"+meters_inRouterNote);
                                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "11F225上报了档案中的表");
                            }
                            else
                            {
                                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "11F225没有上报档案外的表");
                            }
                        }
                        else
                        {
...
//                        activeFlag=false;
//                    }
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "回复确认，11F225当前上报数:"+QString::number(nodeInfoList.size()));
                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_Confirm_00F1);
                }
                else if(p_Frame3762Base->afn_ == 0x06&&p_Frame3762Base->dt1_==0x04&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
...
//                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, "路由不应06F3上报工况变动");
                ...
                else if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x20&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();
                    shared_ptr<Afn10F6> p_QueryRegisterNode_10F6_Up=dynamic_pointer_cast<Afn10F6>(p_Frame3762Base);
//p_AbstractScriptHost->updateProgress(ProcessState_Processing, "判断2,10F6数:"+QString::number(p_QueryRegisterNode_10F6_Up->register_node_info_unit_.node_total_num_)+" 不在档案数:"+QString::number(notInRouterNoteList.size()));
                    if(p_QueryRegisterNode_10F6_Up->register_node_info_unit_.node_total_num_>=notInRouterNoteList.size())
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("查询主动注册从节点，符合要求，11F225搜表上报节点不在档案数%1，10F6回复节点总数%2")
                                                             .arg(notInRouterNoteList.size()).arg(p_QueryRegisterNode_10F6_Up->register_node_info_unit_.node_total_num_));
                        emScriptRunState=ScriptSuccess;
                        resultFlag=true;
                        p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("搜表测试成功"));
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("档案配置数%0，11F225搜表上报数%1，在档案数%2，不在档案数%3，10F6回复节点总数%4").arg(p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().size())
                                                             .arg(nodeInfoList.size()).arg(inRouterNoteList.size()).arg(notInRouterNoteList.size()).arg(p_QueryRegisterNode_10F6_Up->register_node_info_unit_.node_total_num_));
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "查询主动注册从节点，不符合要求");
                    }
                }
...
void Script_SearchMeter_PartialPara_Stop::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_SearchMeter_PartialPara_Stop::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_SearchMeter_PartialPara_Stop::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_SearchMeter_PartialPara_Stop::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_SearchMeter_PartialPara_Stop::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
...
void Script_SearchMeter_PartialPara_Stop::maxAllowTimer_timeoutProc()
{
...
void Script_SearchMeter_PartialPara_Stop::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
   // p_maxAllowTimer->stop();
...
bool Script_SearchMeter_PartialPara_Stop::isAllExist()
{
    bool result=true;
    for(int i=0;i<nodeInfoList.size();i++)
    {
...
void Script_SearchMeter_PartialPara_Stop::statisticsNode()
{
    notInRouterNoteList.clear();
    inRouterNoteList.clear();
    //判断是否存在于档案中
    ...
    for(int i=0;i<nodeInfoList.size();i++)
    {
        if(nodeInfoList.at(i).isExist==false)
        {
            notInRouterNoteList.append(nodeInfoList.at(i));
        }
        else
        {
            inRouterNoteList.append(nodeInfoList.at(i));
        }
    }
}
Script_SearchMeter_PartialPara_Stop::ParaExistState Script_SearchMeter_PartialPara_Stop::judgeExistState()
{
    statisticsNode();
    if(inRouterNoteList.size()==p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().size()&&notInRouterNoteList.size()==0)
    {
        return Whole;
    }
    else if(inRouterNoteList.size()==p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().size()&&notInRouterNoteList.size()>0)
    {
        return Partial;
    }
    else if((inRouterNoteList.size()==0&&notInRouterNoteList.size()>0))
    {
        return None;
    }
    else
