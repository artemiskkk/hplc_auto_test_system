#include "Script_SearchMeter_WholePara_Stop.h"

Script_SearchMeter_WholePara_Stop::Script_SearchMeter_WholePara_Stop(QObject *parent) : QObject(parent)
{
...
Script_SearchMeter_WholePara_Stop::~Script_SearchMeter_WholePara_Stop()
{
...
void Script_SearchMeter_WholePara_Stop::execute()
{
...
void Script_SearchMeter_WholePara_Stop::stop()
{
...
void Script_SearchMeter_WholePara_Stop::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_SearchMeter_WholePara_Stop::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_SearchMeter_WholePara_Stop::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_SearchMeter_WholePara_Stop::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_SearchMeter_WholePara_Stop::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_SearchMeter_WholePara_Stop::processMsgFromCCO(DvcType dvcType, int dvcId)
{
...
void Script_SearchMeter_WholePara_Stop::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_SearchMeter_WholePara_Stop::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_SearchMeter_WholePara_Stop::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_SearchMeter_WholePara_Stop::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_SearchMeter_WholePara_Stop::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
...
void Script_SearchMeter_WholePara_Stop::maxAllowTimer_timeoutProc()
{
...
void Script_SearchMeter_WholePara_Stop::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    if(activeFlag == true)
    {
...
bool Script_SearchMeter_WholePara_Stop::isAllExist()
{
    for(int j=0;j<p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().size();j++)
    {
