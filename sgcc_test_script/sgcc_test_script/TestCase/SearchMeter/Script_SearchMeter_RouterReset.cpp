#include "Script_SearchMeter_RouterReset.h"

Script_SearchMeter_RouterReset::Script_SearchMeter_RouterReset(QObject *parent) : QObject(parent)
{
...
Script_SearchMeter_RouterReset::~Script_SearchMeter_RouterReset()
{
...
void Script_SearchMeter_RouterReset::execute()
{
...
void Script_SearchMeter_RouterReset::stop()
{
...
void Script_SearchMeter_RouterReset::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_SearchMeter_RouterReset::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_SearchMeter_RouterReset::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_SearchMeter_RouterReset::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_SearchMeter_RouterReset::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_SearchMeter_RouterReset::processMsgFromCCO(DvcType dvcType, int dvcId)
{
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_SearchMeter_11F5_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "路由硬件初始化回复确认，等待10后查询10F4");
                        activeFlag=false;
                ...
                else if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x08&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    else
                    {
                        ...
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "注册允许标志为【允许】，符合要求");
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "主动注册11F5，符合要求");
                            ////////
                ...
                else if(p_Frame3762Base->afn_ == 0x06&&p_Frame3762Base->dt1_==0x08&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(initFlag==false)
                    {
                        ...
                        emScriptRunState=Wait_SearchMeter_11F5_Finish;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送-路由硬件初始化（01F10）");
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                        initFlag=true;
                    }
                }
            ...
            case Wait_SearchMeter_11F225_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(activeFlag==false)
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "激活主动注册(新增)11F225，回复确认");
                        activeFlag=true;
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryRouterState_10F4);
                        emScriptRunState=Wait_SearchMeter_11F225_Finish;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--查询路由运行状态（10F4），等待--回复");
                ...
                {
                    p_timer->stop();
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "激活主动注册(新增)11F225，回复否认，暂不支持");
                    activeFlag=true;
                    resultFlag=true;
                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_HardReset_01F1);
                    emScriptRunState=Wait_SearchMeter_11F225_Finish;
                ...
                else if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x08&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    else
                    {
                        ...
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "注册允许标志为【允许】，符合要求");
//                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "主动注册11F225，符合要求");
                            emScriptRunState=ScriptSuccess;
                            resultFlag=true;
                            p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("硬件初始化-搜表测试成功;"));
                        }
                        else
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "注册允许标志为【允许】，不符合要求");
                ...
                else if(p_Frame3762Base->afn_ == 0x06&&p_Frame3762Base->dt1_==0x08&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    for(int j=0;j<p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().size();j++)
                    {
                        ...
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("11F225有档案中的表上报，不符合要求，表号%1；").arg(QString(QByteArray(p_ReportRegisterNode_Up->report_node_info_unit_.report_node_address_.addr,6).toHex())));
                        }
                    }
                  //  num_reportOutOfRecord++;
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("11F225有档案外的表上报，表号%1；").arg(QString(QByteArray(p_ReportRegisterNode_Up->report_node_info_unit_.report_node_address_.addr,6).toHex())));
                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_Confirm_00F1);
...
void Script_SearchMeter_RouterReset::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_SearchMeter_RouterReset::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_SearchMeter_RouterReset::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_SearchMeter_RouterReset::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_SearchMeter_RouterReset::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
...
void Script_SearchMeter_RouterReset::maxAllowTimer_timeoutProc()
{
...
void Script_SearchMeter_RouterReset::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
   // p_maxAllowTimer->stop();
...
bool Script_SearchMeter_RouterReset::isAllExist()
{
    bool result=true;
