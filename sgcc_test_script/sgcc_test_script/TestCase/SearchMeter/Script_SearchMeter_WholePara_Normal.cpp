#include "Script_SearchMeter_WholePara_Normal.h"

Script_SearchMeter_WholePara_Normal::Script_SearchMeter_WholePara_Normal(QObject *parent) : QObject(parent)
{
...
Script_SearchMeter_WholePara_Normal::~Script_SearchMeter_WholePara_Normal()
{
...
void Script_SearchMeter_WholePara_Normal::execute()
{
...
void Script_SearchMeter_WholePara_Normal::stop()
{
...
void Script_SearchMeter_WholePara_Normal::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_SearchMeter_WholePara_Normal::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_SearchMeter_WholePara_Normal::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_SearchMeter_WholePara_Normal::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_SearchMeter_WholePara_Normal::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_SearchMeter_WholePara_Normal::processMsgFromCCO(DvcType dvcType, int dvcId)
{
...
//                        p_delayTimer->start((activeTime+2)*60*1000);//2min
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("查询主动注册从节点，不符合要求，10F6回复节点总数%0，11F5搜表上报档案外表数%1").arg(p_QueryRegisterNode_10F6_Up->register_node_info_unit_.node_total_num_).arg(num_reportOutOfRecord));
                    }
                }
                else
                {
            ...
            case Wait_SearchMeter_11F225_Finish:
            {
                ...
                else if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x20&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("查询主动注册从节点，不符合要求，10F6回复节点总数%0，11F225搜表上报档案外表数%1").arg(p_QueryRegisterNode_10F6_Up->register_node_info_unit_.node_total_num_).arg(num_reportOutOfRecord));
                    }
                }
                else
                {
...
void Script_SearchMeter_WholePara_Normal::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_SearchMeter_WholePara_Normal::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_SearchMeter_WholePara_Normal::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_SearchMeter_WholePara_Normal::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_SearchMeter_WholePara_Normal::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
...
void Script_SearchMeter_WholePara_Normal::maxAllowTimer_timeoutProc()
{
...
void Script_SearchMeter_WholePara_Normal::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
   // p_maxAllowTimer->stop();
    if(activeFlag == true)
...
bool Script_SearchMeter_WholePara_Normal::isAllExist()
{
  //  bool result=true;
    for(int j=0;j<p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().size();j++)
    {
...
//            if(nodeInfoList.at(i).isExist==false)
//            {
//                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("上报节点地址在档案中不存在或协议不一致：表号%1，协议0%2").arg(QString(QByteArray(nodeInfoList.at(i).nodeAddress.addr,6).toHex())).arg(int(nodeInfoList.at(i).protocol)));
//                result=false;
//                break;
//            }
//        }
