#include "Script_CcoSearchMeter_RouteWorking.h"


Script_CcoSearchMeter_RouteWorking:: Script_CcoSearchMeter_RouteWorking(QObject *parent) : QObject(parent)
{
...
Script_CcoSearchMeter_RouteWorking::~ Script_CcoSearchMeter_RouteWorking()
{
...
void  Script_CcoSearchMeter_RouteWorking::execute()
{
    p_AbstractScriptHost->updateProgress(ProcessState_Start, "Test start!");
    p_AbstractScriptHost->updateProgress(ProcessState_Start, "******************************************************");

    userCaseIds=QString("[GW-CCO-F012-0014-V01],");

    QString script_ID,script_Desc,script_CheckKeyPoint;
    script_ID=QString("用例ID：GW-CCO-F012-0013-V01 搜表-不暂停路由 \n\n");
...
void  Script_CcoSearchMeter_RouteWorking::stop()
{
...
void  Script_CcoSearchMeter_RouteWorking::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void  Script_CcoSearchMeter_RouteWorking::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}

bool  Script_CcoSearchMeter_RouteWorking::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void  Script_CcoSearchMeter_RouteWorking::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void  Script_CcoSearchMeter_RouteWorking::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void  Script_CcoSearchMeter_RouteWorking::processMsgFromCCO(DvcType dvcType, int dvcId)
{
...
//                is11F225=false;
                ...
                p_delayTimer->start((activeTime-1)*60*1000); // 搜表时间6分钟结束前（5分钟时），查询10F6
        ...
        case Wait_QueryRouteRunState_10F4_Finish_1:
        {
            ...
            else if(p_Frame3762Base->afn_ == 0x06&&p_Frame3762Base->dt1_==0x08&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "Wait_QueryRouteRunState_10F4_Finish_1 期望[上报表号不重复],实际[上报表号重复]"+userCaseIds);
...
//                    p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                ...
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_QueryRouteRunState_10F4_Finish_2 期望[注册允许标志为【禁止】，当前状态为 其他],实际[注册允许标志或当前状态，不符合要求]"+userCaseIds);
                }
...
bool Script_CcoSearchMeter_RouteWorking::extractAndProcess3762Frame(QByteArray& buf,QByteArray& completeFrame)
{
    completeFrame.clear();
...
void  Script_CcoSearchMeter_RouteWorking::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
...
void  Script_CcoSearchMeter_RouteWorking::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void  Script_CcoSearchMeter_RouteWorking::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
//        memcpy(&p_ActiveRegister_11F5->start_time_,startTime,6);
    ...
    else if(frame==p_QueryRegisterNode_10F6)
    {
        ...
        p_QueryRegisterNode_10F6->node_start_no_=0x01;
...
void  Script_CcoSearchMeter_RouteWorking::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void  Script_CcoSearchMeter_RouteWorking::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
...
void  Script_CcoSearchMeter_RouteWorking::maxAllowTimer_timeoutProc()
{
...
void  Script_CcoSearchMeter_RouteWorking::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
...
bool  Script_CcoSearchMeter_RouteWorking::isAllExist()
{
    bool result=true;
    for(int i=0;i<nodeInfoList.size();i++)
    {
