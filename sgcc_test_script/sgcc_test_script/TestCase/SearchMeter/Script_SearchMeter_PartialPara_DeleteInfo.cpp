#include "Script_SearchMeter_PartialPara_DeleteInfo.h"

Script_SearchMeter_PartialPara_DeleteInfo::Script_SearchMeter_PartialPara_DeleteInfo(QObject *parent) : QObject(parent)
{
...
Script_SearchMeter_PartialPara_DeleteInfo::~Script_SearchMeter_PartialPara_DeleteInfo()
{
...
void Script_SearchMeter_PartialPara_DeleteInfo::execute()
{
...
void Script_SearchMeter_PartialPara_DeleteInfo::stop()
{
...
void Script_SearchMeter_PartialPara_DeleteInfo::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_SearchMeter_PartialPara_DeleteInfo::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_SearchMeter_PartialPara_DeleteInfo::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_SearchMeter_PartialPara_DeleteInfo::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_SearchMeter_PartialPara_DeleteInfo::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_SearchMeter_PartialPara_DeleteInfo::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_SearchMeter_11F5_Finish:
            {
                ...
                else if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x20&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "10F6查询主动注册从节点总数量："+QString::number(p_QueryRegisterNode_10F6_Up->register_node_info_unit_.node_total_num_)+"，等3s");
        ...
        case AfterSearchMeter:
        {
            if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "终止主动注册回复确认，5s后查10F6");
                    activeFlag=false;
            ...
            else if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x20&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(p_QueryRegisterNode_10F6_Up->register_node_info_unit_.node_total_num_==0)
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Success, "10F6查询主动注册从节点总数量为0，历史搜表信息已清除");
                    emScriptRunState=ScriptSuccess;
                }
                else
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, "10F6查询主动注册从节点总数量"+QString::number(p_QueryRegisterNode_10F6_Up->register_node_info_unit_.node_total_num_)+"，未删除历史搜表信息");
                }
            }
            else
            {
...
void Script_SearchMeter_PartialPara_DeleteInfo::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_SearchMeter_PartialPara_DeleteInfo::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_SearchMeter_PartialPara_DeleteInfo::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_SearchMeter_PartialPara_DeleteInfo::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_SearchMeter_PartialPara_DeleteInfo::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
...
void Script_SearchMeter_PartialPara_DeleteInfo::maxAllowTimer_timeoutProc()
{
...
void Script_SearchMeter_PartialPara_DeleteInfo::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    if(emScriptRunState == Wait_SearchMeter_11F5_Finish)
    {
        ...
        else
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--激活主动注册（11F5），等待--确认");
            sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_ActiveRegister_11F5);
            emScriptRunState=AfterSearchMeter;
            p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
        }
    }
    else if(emScriptRunState == AfterSearchMeter)
    {
//        if(activeFlag == true)
//        {
//            sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_StopActiveRegister_11F6);
...
void Script_SearchMeter_PartialPara_DeleteInfo::statisticsNode()
{
...
Script_SearchMeter_PartialPara_DeleteInfo::ParaExistState Script_SearchMeter_PartialPara_DeleteInfo::judgeExistState()
{
    statisticsNode();
