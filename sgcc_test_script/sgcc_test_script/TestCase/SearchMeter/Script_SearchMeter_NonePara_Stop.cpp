#include "Script_SearchMeter_NonePara_Stop.h"

Script_SearchMeter_NonePara_Stop::Script_SearchMeter_NonePara_Stop(QObject *parent) : QObject(parent)
{
...
Script_SearchMeter_NonePara_Stop::~Script_SearchMeter_NonePara_Stop()
{
...
void Script_SearchMeter_NonePara_Stop::execute()
{
...
void Script_SearchMeter_NonePara_Stop::stop()
{
...
void Script_SearchMeter_NonePara_Stop::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_SearchMeter_NonePara_Stop::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_SearchMeter_NonePara_Stop::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_SearchMeter_NonePara_Stop::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_SearchMeter_NonePara_Stop::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_SearchMeter_NonePara_Stop::processMsgFromCCO(DvcType dvcType, int dvcId)
{
                    ...
                    else
                    {
                       ...
                       //      p_delayTimer->stop();
                            ...
                            else
                            {
                                ...
                                stopFlag=false;
                                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到确认，统计上报数"+QString::number(nodeInfoList.size()));
                ...
                else if(p_Frame3762Base->afn_ == 0x06&&p_Frame3762Base->dt1_==0x08&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                  //  p_timer->stop();
                    if(activeFlag == false)
                    {
                        if(stopFlag == true)
                        {
                           p_AbstractScriptHost->updateProgress(ProcessState_Processing, "停止搜表后收到06F4上报，忽略");
                        }
                        else
                        {
                    ...
                    //     if(judgeExistState()==None)///没办法达到条件
//                    {
//                        stopFlag=true;
//                        activeFlag=false;
//                    }
                ...
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                  ...
                  //      p_delayTimer->start(activeTime*60*1000);//2min
                    ...
                    //        p_delayTimer->stop();
                            ...
                            {
                                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到确认，11F225统计上报数"+QString::number(nodeInfoList.size()));
                                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "5s后查状态");
                                stopFlag=false;
                                p_delayTimer->start(5*1000);
                            }
...
bool Script_SearchMeter_NonePara_Stop::extractAndProcess3762Frame(QByteArray& buf,QByteArray& completeFrame)
{
    completeFrame.clear();
...
void Script_SearchMeter_NonePara_Stop::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_SearchMeter_NonePara_Stop::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_SearchMeter_NonePara_Stop::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_ParameterInit_01F2)
    {
        ...
        p_ParameterInit_01F2->ctrl_field_.prm=kPassive;
...
void Script_SearchMeter_NonePara_Stop::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_SearchMeter_NonePara_Stop::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
...
void Script_SearchMeter_NonePara_Stop::maxAllowTimer_timeoutProc()
{
...
void Script_SearchMeter_NonePara_Stop::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
   // p_maxAllowTimer->stop();
...
bool Script_SearchMeter_NonePara_Stop::isAllExist()
{
    bool result=true;
    for(int i=0;i<nodeInfoList.size();i++)
    {
...
void Script_SearchMeter_NonePara_Stop::statisticsNode()
{
    notInRouterNoteList.clear();
    inRouterNoteList.clear();
    //判断是否存在于档案中
...
Script_SearchMeter_NonePara_Stop::ParaExistState Script_SearchMeter_NonePara_Stop::judgeExistState()
{
    statisticsNode();
    ...
    else if((inRouterNoteList.size()==0&&notInRouterNoteList.size()>p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().size()))
    {
        return None;
    }
    else
