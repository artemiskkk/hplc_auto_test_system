#include "Script_AllowReportingFromNodeStatusChanges_Shanxi.h"

Script_AllowReportingFromNodeStatusChanges_Shanxi::Script_AllowReportingFromNodeStatusChanges_Shanxi(QObject *parent) : QObject(parent)
{
    ...
    p_AllowReporting_05F201=make_shared<Afn05F201>();
...
Script_AllowReportingFromNodeStatusChanges_Shanxi::~Script_AllowReportingFromNodeStatusChanges_Shanxi()
{
...
void Script_AllowReportingFromNodeStatusChanges_Shanxi::execute()
{
...
void Script_AllowReportingFromNodeStatusChanges_Shanxi::stop()
{
...
void Script_AllowReportingFromNodeStatusChanges_Shanxi::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_AllowReportingFromNodeStatusChanges_Shanxi::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_AllowReportingFromNodeStatusChanges_Shanxi::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_AllowReportingFromNodeStatusChanges_Shanxi::processMsg(DvcType dvcType,int id,uchar* data,int datalen)
{
...
void Script_AllowReportingFromNodeStatusChanges_Shanxi::processCtrlDvcRes(DvcType dvcType,QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
    ...
    switch(emScriptRunState)
    {
        ...
        case Wait_AllowReporting_05F201_Finish:
        {
            break;
        }
...
void Script_AllowReportingFromNodeStatusChanges_Shanxi::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_AllowReportingFromNodeStatusChanges_Shanxi::processMsgFromCCO(DvcType dvcType, int dvcId)
{
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_ParaInit_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_AllowReporting_05F201);
                    emScriptRunState=Wait_AllowReporting_05F201_Finish;
            ...
            case Wait_QueryNetScale_2_10F9_Finish:
            {
                ...
                else if(p_Frame3762Base->afn_ == 0x06&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "上报的06F10从节点状态变化事件个数成功");
                    ...
                    //只是测试允许上报，只要有一个上报说明就是允许上报的，就没有回复确认，正常操作过程中上报之后，要回复确认！！！！！
...
void Script_AllowReportingFromNodeStatusChanges_Shanxi::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_AllowReportingFromNodeStatusChanges_Shanxi::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_AllowReportingFromNodeStatusChanges_Shanxi::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    sendMsgOct.clear();
    ...
    else if(frame==p_AllowReporting_05F201)
    {
        p_AllowReporting_05F201->ctrl_field_.dir=kDirDown;
        p_AllowReporting_05F201->ctrl_field_.prm=kActive;
        p_AllowReporting_05F201->ctrl_field_.comn_type=kHplc;

        p_AllowReporting_05F201->info_field_.info_field_down.msg_seq=char(msgSeq++);
        p_AllowReporting_05F201->info_field_.info_field_down.comu_rate=0;
        p_AllowReporting_05F201->info_field_.info_field_down.comu_module_ident=0;
        switch (emSetSwitchState)
        {
            case Open_05F201:
            {
                p_AllowReporting_05F201->sta_attest_flag_ = 1 ;
                break;
            }

            case Close_05F201:
            {
                p_AllowReporting_05F201->sta_attest_flag_ = 0 ;
                break;
            }
        }

        sendMsgOct=p_AllowReporting_05F201->EncodeFrame();
...
void Script_AllowReportingFromNodeStatusChanges_Shanxi::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
...
void Script_AllowReportingFromNodeStatusChanges_Shanxi::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
...
void Script_AllowReportingFromNodeStatusChanges_Shanxi::maxAllowTimer_timeoutProc()
{
