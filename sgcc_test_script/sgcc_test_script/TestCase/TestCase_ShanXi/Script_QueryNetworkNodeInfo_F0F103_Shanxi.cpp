#include "Script_QueryNetworkNodeInfo_F0F103_Shanxi.h"

Script_QueryNetworkNodeInfo_F0F103_Shanxi::Script_QueryNetworkNodeInfo_F0F103_Shanxi(QObject *parent) : QObject(parent)
{
    ...
    p_QueryNetworkNodeInfo_F0F103=make_shared<AfnF0F103>();
...
Script_QueryNetworkNodeInfo_F0F103_Shanxi::~Script_QueryNetworkNodeInfo_F0F103_Shanxi()
{
...
void Script_QueryNetworkNodeInfo_F0F103_Shanxi::execute()
{
...
void Script_QueryNetworkNodeInfo_F0F103_Shanxi::stop()
{
...
void Script_QueryNetworkNodeInfo_F0F103_Shanxi::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_QueryNetworkNodeInfo_F0F103_Shanxi::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_QueryNetworkNodeInfo_F0F103_Shanxi::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_QueryNetworkNodeInfo_F0F103_Shanxi::processMsg(DvcType dvcType,int id,uchar* data,int datalen)
{
...
void Script_QueryNetworkNodeInfo_F0F103_Shanxi::processCtrlDvcRes(DvcType dvcType,QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
    ...
    switch(emScriptRunState)
    {
        ...
        case Wait_QueryNetworkNodeInfo_F0F103_Finish:
        {
            break;
        }
        case Wait_QueryNetworkNodeInfo_F0F103_20_Finish:
        {
            break;
        }
        case Wait_QueryNetworkNodeInfo_F0F103_40_Finish:
        {
            break;
        }
        case Wait_QueryNetworkNodeInfo_F0F103_64_Finish:
        {
            break;
        }
        case ScriptSuccess:
...
void Script_QueryNetworkNodeInfo_F0F103_Shanxi::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_QueryNetworkNodeInfo_F0F103_Shanxi::processMsgFromCCO(DvcType dvcType, int dvcId)
{
...
//                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNetScale_10F9);
//                        emScriptRunState=Wait_QueryNetScale_2_10F9_Finish;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "延迟60s发送--查询网络规模（10F9），等待--回复");
                        //延时查询时间，根据网络规模，可以进行配置
                        p_delayTimer->start(60*1000);
                    }
                    else if(p_QueryNetScale_10F9_Up->network_scale_>=netScale)
                    {
                        if(p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().size()>=everytime_query_network_node_info_num)
                        {
                            everytime_query_network_node_info_num = 10;
                            sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNetworkNodeInfo_F0F103);
                            emScriptRunState=Wait_QueryNetworkNodeInfo_F0F103_Finish;
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--查询网络节点信息（F0F103）10个，等待--确认");
                            p_timer->start(10*1000);
                        }
                        else
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("现场表档案不足，小于%1个").arg(everytime_query_network_node_info_num));
                        }
                    }
                    else
                    {
            ...
            case Wait_QueryNetworkNodeInfo_F0F103_Finish:
            {
                if(p_Frame3762Base->afn_ == char(0xf0)&&p_Frame3762Base->dt1_==0x40&&p_Frame3762Base->dt2_==0x0C&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();
                    shared_ptr<AfnF0F103> p_QueryNetworkNodeInfo_F0F103_Up=dynamic_pointer_cast<AfnF0F103>(p_Frame3762Base);
                    if(everytime_query_network_node_info_num==p_QueryNetworkNodeInfo_F0F103_Up->network_node_info_unit_.this_node_num_)
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "查询指定节点信息10个，回复正确");
                        everytime_query_network_node_info_num = 20;
                        if(p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().size()>=everytime_query_network_node_info_num)
                        {
                            sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNetworkNodeInfo_F0F103);
                            emScriptRunState=Wait_QueryNetworkNodeInfo_F0F103_20_Finish;
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--查询网络节点信息（F0F103）20个，等待--确认");
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("查询指定节点信息错误，上报%1个").arg(QString(int(p_QueryNetworkNodeInfo_F0F103_Up->network_node_info_unit_.this_node_num_))));
                    }
                }
                else
                {
            ...
            case Wait_QueryNetworkNodeInfo_F0F103_20_Finish:
            {
                ...
                {
                    p_timer->stop();
                    shared_ptr<AfnF0F103> p_QueryNetworkNodeInfo_F0F103_20_Up=dynamic_pointer_cast<AfnF0F103>(p_Frame3762Base);
                    if(everytime_query_network_node_info_num==p_QueryNetworkNodeInfo_F0F103_20_Up->network_node_info_unit_.this_node_num_)
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "查询指定节点信息20个，回复正确");
                        everytime_query_network_node_info_num = 40;
                        if(p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().size()>=everytime_query_network_node_info_num)
                        {
                            sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNetworkNodeInfo_F0F103);
                            emScriptRunState=Wait_QueryNetworkNodeInfo_F0F103_40_Finish;
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--查询网络节点信息（F0F103）40个，等待--确认");
                            p_timer->start(20*1000);
                        }
                        else
                        {
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("查询指定节点信息错误，上报%1个").arg(QString(int(p_QueryNetworkNodeInfo_F0F103_20_Up->network_node_info_unit_.this_node_num_))));
                    }
                }
                else
                {
            ...
            case Wait_QueryNetworkNodeInfo_F0F103_40_Finish:
            {
                ...
                {
                    p_timer->stop();
                    shared_ptr<AfnF0F103> p_QueryNetworkNodeInfo_F0F103_40_Up=dynamic_pointer_cast<AfnF0F103>(p_Frame3762Base);
                    if(everytime_query_network_node_info_num==p_QueryNetworkNodeInfo_F0F103_40_Up->network_node_info_unit_.this_node_num_)
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "查询指定节点信息40个，回复正确");
                        everytime_query_network_node_info_num = 64;
                        //回复数据过大，如果64个节点数据可以正常回复，这段可以注释掉
                        //----------------------------------------
                        emScriptRunState=ScriptSuccess;
                        resultFlag=true;
                        p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("查询指定节点信息测试成功！"));
                        //----------------------------------------
                        if(p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().size()>=everytime_query_network_node_info_num)
                        {
                            sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNetworkNodeInfo_F0F103);
                            emScriptRunState=Wait_QueryNetworkNodeInfo_F0F103_64_Finish;
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--查询网络节点信息（F0F103）64个，等待--确认");
                            p_timer->start(20*1000);
                        }
                        else
                        {
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("查询指定节点信息错误，上报%1个").arg(QString(int(p_QueryNetworkNodeInfo_F0F103_40_Up->network_node_info_unit_.this_node_num_))));
                    }
                }
                else
                {
            ...
            case Wait_QueryNetworkNodeInfo_F0F103_64_Finish:
            {
                ...
                {
                    p_timer->stop();
                    shared_ptr<AfnF0F103> p_QueryNetworkNodeInfo_F0F103_64_Up=dynamic_pointer_cast<AfnF0F103>(p_Frame3762Base);
                    if(everytime_query_network_node_info_num==p_QueryNetworkNodeInfo_F0F103_64_Up->network_node_info_unit_.this_node_num_)
                    {
                        emScriptRunState=ScriptSuccess;
                        resultFlag=true;
                        p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("查询指定节点信息测试成功！"));
...
void Script_QueryNetworkNodeInfo_F0F103_Shanxi::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_QueryNetworkNodeInfo_F0F103_Shanxi::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_QueryNetworkNodeInfo_F0F103_Shanxi::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    sendMsgOct.clear();
    ...
    else if(frame==p_QueryNetworkNodeInfo_F0F103)
    {
        p_QueryNetworkNodeInfo_F0F103->ctrl_field_.dir=kDirDown;
        p_QueryNetworkNodeInfo_F0F103->ctrl_field_.prm=kActive;
        p_QueryNetworkNodeInfo_F0F103->ctrl_field_.comn_type=kHplc;

        p_QueryNetworkNodeInfo_F0F103->info_field_.info_field_down.msg_seq=char(msgSeq++);
        p_QueryNetworkNodeInfo_F0F103->info_field_.info_field_down.comu_rate=0;
        p_QueryNetworkNodeInfo_F0F103->info_field_.info_field_down.comu_module_ident=0;

        p_QueryNetworkNodeInfo_F0F103->node_num_ = everytime_query_network_node_info_num;

        p_QueryNetworkNodeInfo_F0F103->node_address_list_.clear();
        ...
        if(everytime_query_network_node_info_num<=64)
        {
            for(int i=0;i<everytime_query_network_node_info_num;i++)
            {
                MeterStruct meter;
                memcpy(meter.meterNo.addr,p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(i)->mtrAddr,6);
                p_QueryNetworkNodeInfo_F0F103->node_address_list_.append(meter.meterNo);
            }
        }
        else
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "查询节点数量最大支持64个，超过64个，上报失败！！！！");
        }

        sendMsgOct=p_QueryNetworkNodeInfo_F0F103->EncodeFrame();
        sendMsgLog=QString("》》查询网络节点信息（F0F103）：%1\n").arg(QString(sendMsgOct.toHex().toUpper()));
    }
...
void Script_QueryNetworkNodeInfo_F0F103_Shanxi::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
        ...
        case Wait_QueryNetScale_2_10F9_Finish:
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_QueryNetScale_2_10F9_Finish timeout!!!");
            break;
        }
        case Wait_QueryNetworkNodeInfo_F0F103_Finish:
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_QueryNetworkNodeInfo_F0F103_Finish timeout!!!");
            break;
        }
        case Wait_QueryNetworkNodeInfo_F0F103_20_Finish:
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_QueryNetworkNodeInfo_F0F103_20_Finish timeout!!!");
            break;
        }
        case Wait_QueryNetworkNodeInfo_F0F103_40_Finish:
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_QueryNetworkNodeInfo_F0F103_40_Finish timeout!!!");
            break;
        }
        case Wait_QueryNetworkNodeInfo_F0F103_64_Finish:
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_QueryNetworkNodeInfo_F0F103_64_Finish timeout!!!");
            break;
        }
        default:
        {
...
void Script_QueryNetworkNodeInfo_F0F103_Shanxi::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
...
void Script_QueryNetworkNodeInfo_F0F103_Shanxi::maxAllowTimer_timeoutProc()
{
    ...
    switch(emScriptRunState)
    {
        ...
        case Wait_QueryNetworkNodeInfo_F0F103_Finish:
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_QueryNetworkNodeInfo_F0F103_Finish maxAllow timeout!!!");
            break;
        }
        case Wait_QueryNetworkNodeInfo_F0F103_20_Finish:
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_QueryNetworkNodeInfo_F0F103_20_Finish maxAllow timeout!!!");
            break;
        }
        case Wait_QueryNetworkNodeInfo_F0F103_40_Finish:
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_QueryNetworkNodeInfo_F0F103_40_Finish maxAllow timeout!!!");
            break;
        }
        case Wait_QueryNetworkNodeInfo_F0F103_64_Finish:
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_QueryNetworkNodeInfo_F0F103_64_Finish maxAllow timeout!!!");
            break;
        }
        default:
        {
