#include "Script_ProhibitReportingFromNodeStatusChanges_Shanxi.h"

Script_ProhibitReportingFromNodeStatusChanges_Shanxi::Script_ProhibitReportingFromNodeStatusChanges_Shanxi(QObject *parent) : QObject(parent)
{
    ...
    p_ProhibitReporting_05F201 = make_shared<Afn05F201>();
...
Script_ProhibitReportingFromNodeStatusChanges_Shanxi::~Script_ProhibitReportingFromNodeStatusChanges_Shanxi()
{
...
void Script_ProhibitReportingFromNodeStatusChanges_Shanxi::execute()
{
...
void Script_ProhibitReportingFromNodeStatusChanges_Shanxi::stop()
{
...
void Script_ProhibitReportingFromNodeStatusChanges_Shanxi::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ProhibitReportingFromNodeStatusChanges_Shanxi::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ProhibitReportingFromNodeStatusChanges_Shanxi::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ProhibitReportingFromNodeStatusChanges_Shanxi::processMsg(DvcType dvcType,int id,uchar* data,int datalen)
{
...
void Script_ProhibitReportingFromNodeStatusChanges_Shanxi::processCtrlDvcRes(DvcType dvcType,QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
    ...
    switch(emScriptRunState)
    {
        ...
        case Wait_ProhibitReporting_05F201_Finish:
        {
            break;
        }
...
void Script_ProhibitReportingFromNodeStatusChanges_Shanxi::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ProhibitReportingFromNodeStatusChanges_Shanxi::processMsgFromCCO(DvcType dvcType, int dvcId)
{
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_ParaInit_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_ProhibitReporting_05F201);
                    emScriptRunState=Wait_ProhibitReporting_05F201_Finish;
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--设置禁止上报（05F201），等待--确认");
                    p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                }
                else
                {
            ...
            case Wait_ProhibitReporting_05F201_Finish:
            {
                ...
                {
                    p_timer->stop();//接收到一条完整报文
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing,"设置禁止从节点状态变化上报成功");
            ...
            case Wait_QueryNetScale_2_10F9_Finish:
            {
                ...
                {
                    p_timer->stop();
                    shared_ptr<Afn06F10> p_NumberReportSlaveNodeStateChangeEvents_Up=dynamic_pointer_cast<Afn06F10>(p_Frame3762Base);
                    if(p_NumberReportSlaveNodeStateChangeEvents_Up->report_node_state_unit_.this_report_node_num_<=netScale)
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("上报的06F10从节点状态变化事件个数为：%1").arg(p_NumberReportSlaveNodeStateChangeEvents_Up->report_node_state_unit_.this_report_node_num_));
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "禁止上报06F10从节点状态变化事件测试失败");
//                        emScriptRunState = ScriptSuccess;
//                        resultFlag=true;
//                        p_AbstractScriptHost->updateProgress(ProcessState_Success, "允许从节点状态变化上报事件成功");
                    }
                    else
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "上报的06F10从节点状态变化事件个数失败");
                    }
...
void Script_ProhibitReportingFromNodeStatusChanges_Shanxi::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ProhibitReportingFromNodeStatusChanges_Shanxi::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ProhibitReportingFromNodeStatusChanges_Shanxi::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    sendMsgOct.clear();
    ...
    else if(frame==p_ProhibitReporting_05F201)
    {
        p_ProhibitReporting_05F201->ctrl_field_.dir=kDirDown;
        p_ProhibitReporting_05F201->ctrl_field_.prm=kActive;
        p_ProhibitReporting_05F201->ctrl_field_.comn_type=kHplc;

        p_ProhibitReporting_05F201->info_field_.info_field_down.msg_seq=char(msgSeq++);
        p_ProhibitReporting_05F201->info_field_.info_field_down.comu_rate=0;
        p_ProhibitReporting_05F201->info_field_.info_field_down.comu_module_ident=0;
        switch (emSetSwitchState)
        {
            case Open_05F201:
            {
                p_ProhibitReporting_05F201->sta_attest_flag_ = 1 ;
                break;
            }

            case Close_05F201:
            {
                p_ProhibitReporting_05F201->sta_attest_flag_ = 0 ;
                break;
            }
        }

        sendMsgOct=p_ProhibitReporting_05F201->EncodeFrame();
...
void Script_ProhibitReportingFromNodeStatusChanges_Shanxi::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
        ...
        case Wait_ProhibitReporting_05F201_Finish:
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_ProhibitReporting_05F201_Finish timeout!!!");
            break;
        }
        case Wait_QueryNetScale_10F9_Finish:
        {
...
void Script_ProhibitReportingFromNodeStatusChanges_Shanxi::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    ...
    case Wait_QueryNetScale_2_10F9_Finish:
    {
        emScriptRunState = ScriptSuccess;
        resultFlag=true;
        p_AbstractScriptHost->updateProgress(ProcessState_Success, "禁止上报06F10从节点状态变化事件个数测试成功");
        break;
    }
    default:
    {
...
void Script_ProhibitReportingFromNodeStatusChanges_Shanxi::maxAllowTimer_timeoutProc()
{
    ...
    switch(emScriptRunState)
    {
        ...
        case Wait_ProhibitReporting_05F201_Finish:
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_ProhibitReporting_05F201_Finish maxAllow timeout!!!");
            break;
        }
        case Wait_QueryNetScale_10F9_Finish:
        {
