#include "Script_BuildNetwork_GW_HardReset.h"

Script_BuildNetwork_GW_HardReset::Script_BuildNetwork_GW_HardReset(QObject *parent) : QObject(parent)
{
...
Script_BuildNetwork_GW_HardReset::~Script_BuildNetwork_GW_HardReset()
{
...
void Script_BuildNetwork_GW_HardReset::execute()
{
...
void Script_BuildNetwork_GW_HardReset::stop()
{
...
void Script_BuildNetwork_GW_HardReset::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_BuildNetwork_GW_HardReset::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_BuildNetwork_GW_HardReset::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_BuildNetwork_GW_HardReset::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_BuildNetwork_GW_HardReset::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_BuildNetwork_GW_HardReset::processMsgFromCCO(DvcType dvcType, int dvcId)
{
...
//                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("当前网络规模为%1").arg(p_QueryNetScale_10F9_Up->network_scale_));
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("组网-硬件初始化测试成功！"));
...
void Script_BuildNetwork_GW_HardReset::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_BuildNetwork_GW_HardReset::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_BuildNetwork_GW_HardReset::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_BuildNetwork_GW_HardReset::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_BuildNetwork_GW_HardReset::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_BuildNetwork_GW_HardReset::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        default:
...
void Script_BuildNetwork_GW_HardReset::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNetScale_10F9);
    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--查询网络规模（10F9），等待--回复");
