#include "Script_BuildNetwork_GW_AddNode_V3.h"

Script_BuildNetwork_GW_AddNode_V3::Script_BuildNetwork_GW_AddNode_V3(QObject *parent) : QObject(parent)
{
...
Script_BuildNetwork_GW_AddNode_V3::~Script_BuildNetwork_GW_AddNode_V3()
{
...
void Script_BuildNetwork_GW_AddNode_V3::execute()
{
    ...
    else
    {
        ...
        emScriptRunState=Wait_AddNode_Repeat_Finish;
        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_AddSlaveNode_11F1);
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "组网完成，发送--添加重复从节点（11F1），等待--确认");
        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
    }
    p_maxAllowTimer->start((timerForReachThresld+timerAfterReachThresld)*1000);
}
void Script_BuildNetwork_GW_AddNode_V3::stop()
{
...
void Script_BuildNetwork_GW_AddNode_V3::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_BuildNetwork_GW_AddNode_V3::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_BuildNetwork_GW_AddNode_V3::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_BuildNetwork_GW_AddNode_V3::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
    ...
    if(emScriptRunState==Wait_BuildNetFinish_Whole)
    {
        ...
        else
        {
            if(p_BuildNetwork_GW->emScriptRunState==BuildNetFinish&&p_BuildNetwork_GW->buildNetworkResultFlag==true)
            {
                ...
                emScriptRunState=Wait_AddNode_Repeat_Finish;
                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_AddSlaveNode_11F1);
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "组网完成，发送--添加重复从节点（11F1），等待--否认");
                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
            }
        }
    }
...
void Script_BuildNetwork_GW_AddNode_V3::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_BuildNetwork_GW_AddNode_V3::processMsgFromCCO(DvcType dvcType, int dvcId)
{
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_AddNode_Repeat_Finish:
            {
                ...
                else if(p_Frame3762Base->afn_==0x00&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "添加从节点回复否认，错误状态字为6(表号重复)，符合要求");

                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "组网完成，发送--查询网络规模（10F9），等待--回复");
                        emScriptRunState=Wait_QueryNetScale_Repeat_Finish;
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNetScale_10F9);
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    }
                    else
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "收到否认，错误状态字不是6(表号重复)："+QString::number(p_Deny_Up->error_code_));
                    }
                }
                else
                {
            ...
            case Wait_QueryNetScale_Repeat_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("当前网络规模为%1，组网完成，发送--删除从节点（11F2），等待--确认").arg(netScale));
            ...
            case Wait_DeleteNode_Finish:
            {
                ...
                {
                    p_timer->stop();
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "删除从节点回复确认！发送--查询网络规模（10F9），等待--回复");
                    emScriptRunState=Wait_QueryNetScale_Delete_Finish;
//                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNetScale_10F9);
//                    p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    p_delayTimer->start(30*1000);//30s后查
                }
                else
                {
                    QByteArray tmpSendMsg=prcsOther3762Msg(uchar(p_Frame3762Base->afn_),p_Frame3762Base->dt1_,p_Frame3762Base->dt2_,uchar(p_Frame3762Base->info_field_.info_field_up.msg_seq));
            ...
            case Wait_QueryNetScale_Delete_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("当前网络规模为%1，符合要求，发送--查询从节点数量（10F1），等待--回复").arg(netScale-1));
                ...
                else if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    {
                        //开始添加
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("当前从节点数量为%1，组网完成，发送--添加从节点（11F1），等待--确认").arg(netScale-2));
                        emScriptRunState=Wait_AddNode_New_Finish;
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_AddSlaveNode_11F1);
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    }
                    else
            ...
            case Wait_AddNode_New_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "添加新从节点收到确认！发送--查询网络规模（10F9），等待--回复");
                    emScriptRunState=Wait_QueryNetScale_New_Finish;
//                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNetScale_10F9);
//                    p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    p_delayTimer->start(30*1000);//30s后查
                }
            ...
            case Wait_QueryNetScale_New_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();//接收到一条完整报文
            ...
            case Wait_ParaInit_Building_Finish:
            {
                ...
                {
                    p_timer->stop();
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "参数初始化回复确认！发送--添加从节点（11F1），等待--确认");
            ...
            case Wait_AddNode_Building_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    else
                    {
                        ...
                        emScriptRunState=Wait_QueryNetScale_Building_Finish;
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNetScale_10F9);
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    }
                }
            ...
            case Wait_QueryNetScale_Building_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();//接收到一条完整报文
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "组网中，发送--添加从节点（11F1），等待--确认");
                        emScriptRunState=Wait_AddNode_Repeat_Building_Finish;
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_AddSlaveNode_11F1);
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    }
                    else
            ...
            case Wait_AddNode_Repeat_Building_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();
                ...
                else if(p_Frame3762Base->afn_==0x00&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(p_Deny_Up->error_code_==6)
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "添加从节点回复否认，错误状态字为6(表号重复)，符合要求，发送--查询网络规模");
                        emScriptRunState=Wait_QueryNetScale_Repeat_Building_Finish;
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNetScale_10F9);
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    }
                    else
            ...
            case Wait_QueryNetScale_Repeat_Building_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();
                    ...
                    if(p_QueryNetScale_10F9_Up->network_scale_==netScale)
                    {
                        ...
                        resultFlag=true;
                        p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("组网-添加从节点测试成功！"));
                    }
                    else
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing,"等待5s后查询网络规模");
...
void Script_BuildNetwork_GW_AddNode_V3::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_BuildNetwork_GW_AddNode_V3::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_BuildNetwork_GW_AddNode_V3::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_BuildNetwork_GW_AddNode_V3::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_BuildNetwork_GW_AddNode_V3::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_BuildNetwork_GW_AddNode_V3::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        default:
...
void Script_BuildNetwork_GW_AddNode_V3::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--查询网络规模（10F9），等待--回复");
    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNetScale_10F9);
...
bool Script_BuildNetwork_GW_AddNode_V3::extractAndProcess3762Frame(QByteArray& buf,QByteArray& completeFrame)
{
    completeFrame.clear();
