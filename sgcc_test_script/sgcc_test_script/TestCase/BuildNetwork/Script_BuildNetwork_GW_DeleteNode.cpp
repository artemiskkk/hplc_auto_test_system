#include "Script_BuildNetwork_GW_DeleteNode.h"

Script_BuildNetwork_GW_DeleteNode::Script_BuildNetwork_GW_DeleteNode(QObject *parent) : QObject(parent)
{
...
Script_BuildNetwork_GW_DeleteNode::~Script_BuildNetwork_GW_DeleteNode()
{
    // 1. 首先停止所有定时器并断开连接
    if(p_timer != nullptr) {
        p_timer->stop();
...
void Script_BuildNetwork_GW_DeleteNode::execute()
{
    ...
    else
    {
        ...
        emScriptRunState=Wait_DeleteNode_Finish;
        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_DeleteSlaveNode_11F2);
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "组网完成，发送--删除从节点（11F2），等待--确认");
        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
    }
    p_maxAllowTimer->start((timerForReachThresld+timerAfterReachThresld)*1000);
}
void Script_BuildNetwork_GW_DeleteNode::stop()
{
    // 首先停止所有定时器并断开连接
    if(p_timer != nullptr) {
        p_timer->stop();
...
void Script_BuildNetwork_GW_DeleteNode::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_BuildNetwork_GW_DeleteNode::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_BuildNetwork_GW_DeleteNode::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_BuildNetwork_GW_DeleteNode::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_BuildNetwork_GW_DeleteNode::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_BuildNetwork_GW_DeleteNode::processMsgFromCCO(DvcType dvcType, int dvcId)
{
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_DeleteNode_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    emScriptRunState=Wait_QueryNetScale_Delete_Finish;
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing,"等待3分钟后查询网络规模");
            ...
            case Wait_QueryNetScale_Delete_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(p_QueryNetScale_10F9_Up->network_scale_==netScale-1)
                    {
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("当前网络规模为%1，不符合要求，继续等待查询").arg(p_QueryNetScale_10F9_Up->network_scale_));
                        p_delayTimer->start(3*60*1000);
                    }
                }
                else if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(p_QueryNodeNum_10F1_Up->node_total_num_==netScale-2)
                    {
                        //开始添加
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_ParaInit_01F2);
                        emScriptRunState=Wait_ParaInit_Building_Finish;
            ...
            case Wait_ParaInit_Building_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();//接收到一条完整报文
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "参数初始化回复确认！");
                    emScriptRunState=Wait_AddNode_Building_Finish;
            ...
            case Wait_AddNode_Building_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();//接收到一条完整报文
                    ...
                    else
                    {
                        ...
                        emScriptRunState=Wait_QueryNetScale_Add_Building_Finish;
            ...
            case Wait_QueryNetScale_Add_Building_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();//接收到一条完整报文
                    ...
                    {
                        emScriptRunState=Wait_DeleteNode_Building_Finish;
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_DeleteSlaveNode_11F2);
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "组网中，发送--删除从节点（11F2），等待--确认");
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    }
                    else
                    {
            ...
            case Wait_DeleteNode_Building_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();//接收到一条完整报文
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("删除从节点收到确认！"));
                    emScriptRunState=Wait_QueryNetScale_Delete_Building_Finish;
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing,"等待180s后查询网络规模");
            ...
            case Wait_QueryNetScale_Delete_Building_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();//接收到一条完整报文
                    ...
                    if(p_QueryNetScale_10F9_Up->network_scale_==netScale-1)
                    {
                        ...
                        emScriptRunState=Wait_QueryTopoInfo_Building_Finish;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--查询网络拓扑信息（10F21），等待--回复");
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    }
                    else if(p_QueryNetScale_10F9_Up->network_scale_>netScale-1)
                    {
                ...
                else if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(p_QueryNodeNum_10F1_Up->node_total_num_==netScale-2)
                    {
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNetScale_10F9);
                        emScriptRunState=Wait_QueryNetScale_Delete_Building_Finish;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "组网中，发送--查询网络规模（10F9），等待--回复");
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    }
                    else
                    {
            ...
            case Wait_QueryTopoInfo_Building_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x10&&p_Frame3762Base->dt2_==0x02&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    topoList.append(p_QueryTopoInfo_10F21_Up->network_typelogy_info_unit_.network_typelogy_info_List_);
                    index+=num;
                    ...
                    {
                        if(isTopoAllRight()==true)
                        {
                            emScriptRunState=ScriptSuccess;
                            resultFlag=true;
                            p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("组网-删除从节点测试成功！"));
                        }
                        else
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("网络拓扑信息，不符合要求"));
                        }
                    }
                }
                else
...
void Script_BuildNetwork_GW_DeleteNode::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_BuildNetwork_GW_DeleteNode::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_BuildNetwork_GW_DeleteNode::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_BuildNetwork_GW_DeleteNode::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_BuildNetwork_GW_DeleteNode::timer_timeoutProc()
{
    p_timer->stop();
    tryTimes++;
    ...
    switch(emScriptRunState)
    {
        ...
        case Wait_DeleteNode_Finish:
        {
            if(tryTimes < 3) {
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, 
                    QString("删除从节点超时，第%1次重试").arg(tryTimes));
                sendMsg(CCO_GW, p_CtrInfoList->at(0)->ctrlID, INSIGNIFICANCE, p_DeleteSlaveNode_11F2);
                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
            } else {
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "删除从节点超时，已重试3次！");
            }
            break;
        }
        case Wait_QueryNetScale_Delete_Finish:
        ...
        case Wait_QueryNetScale_Delete_Building_Finish:
        {
            if(tryTimes < 3) {
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, 
                    QString("查询网络规模超时，第%1次重试").arg(tryTimes));
                sendMsg(CCO_GW, p_CtrInfoList->at(0)->ctrlID, INSIGNIFICANCE, p_QueryNetScale_10F9);
                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
            } else {
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "查询网络规模超时，已重试3次！");
            }
            break;
        }
        case Wait_ParaInit_Building_Finish:
        {
            if(tryTimes < 3) {
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, 
                    QString("参数初始化超时，第%1次重试").arg(tryTimes));
                sendMsg(CCO_GW, p_CtrInfoList->at(0)->ctrlID, INSIGNIFICANCE, p_ParaInit_01F2);
                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
            } else {
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "参数初始化超时，已重试3次！");
            }
            break;
        }
        case Wait_AddNode_Building_Finish:
        {
            if(tryTimes < 3) {
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, 
                    QString("添加从节点超时，第%1次重试").arg(tryTimes));
                sendMsg(CCO_GW, p_CtrInfoList->at(0)->ctrlID, INSIGNIFICANCE, p_AddSlaveNode_11F1);
                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
            } else {
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "添加从节点超时，已重试3次！");
            }
            break;
        }
        case Wait_DeleteNode_Building_Finish:
        ...
        case Wait_QueryTopoInfo_Building_Finish:
        {
            if(tryTimes < 3) {
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, 
                    QString("查询网络拓扑超时，第%1次重试").arg(tryTimes));
                sendMsg(CCO_GW, p_CtrInfoList->at(0)->ctrlID, INSIGNIFICANCE, p_QueryTopoInfo_10F21);
                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
            } else {
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "查询网络拓扑超时，已重试3次！");
            }
            break;
        }
        case ScriptSuccess:
        ...
        default:
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Error, 
                QString("状态机超时错误，当前状态=%1").arg(emScriptRunState));
            break;
        }
    }
}
void Script_BuildNetwork_GW_DeleteNode::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        default:
...
void Script_BuildNetwork_GW_DeleteNode::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNetScale_10F9);
    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--查询网络规模（10F9），等待--回复");
...
bool Script_BuildNetwork_GW_DeleteNode::isTopoAllRight()
{
    for(int i=0;i<topoList.size();i++)
    {
        if(memcmp(p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(0)->mtrAddr,topoList.at(i).node_address_.addr,6)==0)
        {
            return false;
        }
    }
