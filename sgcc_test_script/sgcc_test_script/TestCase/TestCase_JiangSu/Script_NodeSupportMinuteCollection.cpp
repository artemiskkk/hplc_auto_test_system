#include "Script_NodeSupportMinuteCollection.h"

Script_NodeSupportMinuteCollection::Script_NodeSupportMinuteCollection(QObject *parent) : QObject(parent)
{
...
Script_NodeSupportMinuteCollection::~Script_NodeSupportMinuteCollection()
{
...
void Script_NodeSupportMinuteCollection::execute()
{
...
void Script_NodeSupportMinuteCollection::stop()
{
...
void Script_NodeSupportMinuteCollection::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_NodeSupportMinuteCollection::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}

bool Script_NodeSupportMinuteCollection::config(const QMap<QString, QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_NodeSupportMinuteCollection::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_NodeSupportMinuteCollection::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_NodeSupportMinuteCollection::processMsgFromCCO(DvcType dvcType, int dvcId)
{
            ...
            case Wait_10F7_QuerySlaveNodeIdInfo_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x40&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(index<p_QuerySlaveNodeIdInfo_10F7->node_id_info_unit_.node_total_num_)
                    {
                        ...
                        {
                            if(0x01==p_QuerySlaveNodeIdInfo_10F7->node_id_info_unit_.node_id_info_list_.at(i).node_type_.minute_collection_)
                            {
                        ...
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("应答从节点数为%1，从节点总数为%2，等待6秒后重新查询").arg(node_num).arg(p_QuerySlaveNodeIdInfo_10F7->node_id_info_unit_.node_total_num_));
                        p_delayTimer->start(6*1000);
                    }
                    else
                    {
...
void Script_NodeSupportMinuteCollection::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_NodeSupportMinuteCollection::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_NodeSupportMinuteCollection::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_NodeSupportMinuteCollection::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_NodeSupportMinuteCollection::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_NodeSupportMinuteCollection::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        default:
...
void Script_NodeSupportMinuteCollection::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
