#include "Script_MinuteCollect_NoCalibrationTime.h"

Script_MinuteCollect_NoCalibrationTime::Script_MinuteCollect_NoCalibrationTime(QObject *parent) : QObject(parent)
{
...
Script_MinuteCollect_NoCalibrationTime::~Script_MinuteCollect_NoCalibrationTime()
{
...
void Script_MinuteCollect_NoCalibrationTime::execute()
{
    ...
    else
    {
        ...
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "等待20min后开始点名上报");
        p_delayTimer->start(waitTime*60*1000);
    }
    p_maxAllowTimer->start(timerMaxReachThresld*60*1000);
}

void Script_MinuteCollect_NoCalibrationTime::stop()
{
...
void Script_MinuteCollect_NoCalibrationTime::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_MinuteCollect_NoCalibrationTime::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}

bool Script_MinuteCollect_NoCalibrationTime::config(const QMap<QString, QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_MinuteCollect_NoCalibrationTime::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_MinuteCollect_NoCalibrationTime::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_MinuteCollect_NoCalibrationTime::processMsgFromCCO(DvcType dvcType, int dvcId)
{
                    ...
                    if(p_ParallelReadMeter_F1F1_Up->unit_up_.frame_length_!=0)
                    {
                        ...
                        if(temp_88==msgBuf[14]&&temp_02==msgBuf[15])
                        {
                              ...
                              if(temp_60==msgBuf[18]&&temp_12==msgBuf[19]&&temp_03==msgBuf[20]&&temp_00==msgBuf[21])
                              {
                                  ...
                                  {
                                      p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("收到分钟冻结-复合数据上报-不符合要求！"));
                                      //sendMsg(CCO_GW,dvcId,meterIndex,p_ParallelReadMeter_F1F1);
                                      //p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("发送--集中器响应上报报文--F1F1"));
                                      //p_timer->start((maxMonitorTime*2)*1000);
                                  }
                                  else if(temp_c8 == msgBuf[msgBuf.size()-6]&&dayFlay==false&&monFlay==false)
                                  {
                                      p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("收到--采集器正常上报后无后续数据！测试成功！"));
...
//                                     p_timer->start((maxMonitorTime*2)*1000);
                               ...
                               else if(temp_06==msgBuf[msgBuf.size()-6]&&minFlag==true&&dayFlay==false&&monFlay==false)
                               {
...
//                                   p_delayTimer->start(5*60*1000);
                        ...
                        else if(temp_87==msgBuf[14]&&temp_01==msgBuf[15])
                        {
                            ...
                            {
                                if(temp_06==msgBuf[21]&&minFlag==true&&dayFlay==false&&monFlay==false)
                                {
                                    p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("收到--采集器正常上报否认报文！测试成功！"));
                                    emScriptRunState=ScriptSuccess;
                                }
...
//                                        }
//                                        //**收到日冻结-复合数据上报
//                                        else if(MsgBase_OOP_ptr->service_type_==REPORT_NOTIFICATION_SERVER && MsgBase_OOP_ptr->service_sub_type_==uchar(ReportNotificationType::kReportNotificationRecordList))
//                                        {
...
//                                                        && std::dynamic_pointer_cast<CsdROad>(p_ReportNotificationRecordList->list_result_record_[1].rcsd_ptr->list_csd_[1])->road_==road)
//                                                {
//                                                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("收到日冻结-复合数据上报"));
//                                                    emSendOopProtype=pro_reportResponseRecordList;
//                                                    sendMsg(CCO_GW,dvcId,meterIndex,p_ParallelReadMeter_F1F1);
...
//                                                        && std::dynamic_pointer_cast<CsdROad>(p_ReportNotificationRecordList->list_result_record_[1].rcsd_ptr->list_csd_[1])->road_==road_1
//                                                        && std::dynamic_pointer_cast<RecordResponseDAR>(p_ReportNotificationRecordList->list_result_record_[2].response_data_ptr)->dar_==0xc8)
//                                                {
...
//                                                        && std::dynamic_pointer_cast<CsdROad>(p_ReportNotificationRecordList->list_result_record_[1].rcsd_ptr->list_csd_[1])->road_==road_2)
//                                                {
//                                                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("收到--采集器正常上报分钟冻结数据！不符合要求！"));
//                                                }
//                                                //**收到分钟冻结采集后续上报报文
...
void Script_MinuteCollect_NoCalibrationTime::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_MinuteCollect_NoCalibrationTime::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_MinuteCollect_NoCalibrationTime::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_MinuteCollect_NoCalibrationTime::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_MinuteCollect_NoCalibrationTime::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_MinuteCollect_NoCalibrationTime::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_Finish_F1F1:
...
void Script_MinuteCollect_NoCalibrationTime::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
...
void Script_MinuteCollect_NoCalibrationTime::readInfoInit()
{
...
bool Script_MinuteCollect_NoCalibrationTime::isMeterExist(Address address)
{
...
int Script_MinuteCollect_NoCalibrationTime::getReadInfo(Address address)
{
    for(int i=0;i<readInfoList.size();i++)
    {
        if(address==readInfoList.at(i).meterNo)
...
double Script_MinuteCollect_NoCalibrationTime::calSuccessRate()
{
...
QString Script_MinuteCollect_NoCalibrationTime::calCostTime()
{
...
QString Script_MinuteCollect_NoCalibrationTime::getFailMeterNo()
{
    QString failMeterNo;
    int count=0;
    for(int i=0;i<readInfoList.size();i++)
...
bool Script_MinuteCollect_NoCalibrationTime::isAllRead(int meterID)
{
    bool sus=true;
...
bool Script_MinuteCollect_NoCalibrationTime::isAllMeterReadSuccess()
{
    bool sus=true;
    for(int i=0;i<readInfoList.size();i++)
    {
...
void Script_MinuteCollect_NoCalibrationTime::CalcAvrgConsumeTimeLen(uchar rdFlag)
{
