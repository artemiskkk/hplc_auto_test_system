#include "Script_ReadMeter_F1F1_DelayTime20s.h"

Script_ReadMeter_F1F1_DelayTime20s::Script_ReadMeter_F1F1_DelayTime20s(QObject *parent) : QObject(parent)
{
...
Script_ReadMeter_F1F1_DelayTime20s::~Script_ReadMeter_F1F1_DelayTime20s()
{
...
void Script_ReadMeter_F1F1_DelayTime20s::execute()
{
...
void Script_ReadMeter_F1F1_DelayTime20s::stop()
{
...
void Script_ReadMeter_F1F1_DelayTime20s::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ReadMeter_F1F1_DelayTime20s::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}

bool Script_ReadMeter_F1F1_DelayTime20s::config(const QMap<QString, QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ReadMeter_F1F1_DelayTime20s::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeter_F1F1_DelayTime20s::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ReadMeter_F1F1_DelayTime20s::processMsgFromCCO(DvcType dvcType, int dvcId)
{
        ...
        switch(emScriptRunState)
        {
                        ...
                        while(parallelCount<maxParallelNum)
                        {
                            ...
                            if(readInfoList.at(meterIndex).readFlag!=ReadSuccess)
                            {
                                ...
                                if(meterIndex==0)
                              {
                                ...
                                QString sTime = stTime.toString("hh:mm:ss");
                                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("【StartTime】当前时间为：%1").arg(sTime));
                              }
            ...
            case Wait_Finish_F1F1:
            {
                if(p_Frame3762Base->afn_==char(0xF1)&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    //**判断空报文与普通报文
                    //p_ParallelReadMeter_F1F1_Up->unit_up_.frame_content_==nullptr

                    if(0==p_ParallelReadMeter_F1F1_Up->unit_up_.frame_length_)
                    {
                        //**判断收到空报文时间间隔是否是20s
                        edTime=QTime::currentTime();
                        QString eTime = edTime.toString("hh:mm:ss");
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("【EndTime】当前时间为：%1").arg(eTime));
                        int elapsed=stTime.secsTo(edTime);
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("elapsed=【%1】").arg(elapsed));
                        if(15<=elapsed&&elapsed<=25)
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("并发抄表空报文延时上报时间是20s，符合要求！测试成功"));
                        ...
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("空报文延时上报时间不是20s，不符合要求！测试失败！"));
                        }
                    }
                    else
                    {
...
void Script_ReadMeter_F1F1_DelayTime20s::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
        ...
        switch(emScriptRunState)
        {
            ...
            {
                //**指定表不应答
                int datalen=sizeof(MsgBase_645_ptr->addr_);
                QByteArray temp=QByteArray::fromRawData(reinterpret_cast<char*>(MsgBase_645_ptr->addr_), datalen);
                QString meAdderss645 = QString(temp.toHex());
                ...
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("报文中addr=%1").arg(meAdderss645));
                    if(unreFlag==false)
                    {
                        if(meAdderss645==meAdder)
                        {
...
void Script_ReadMeter_F1F1_DelayTime20s::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_F1F1_DelayTime20s::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_ReadMeter_F1F1_DelayTime20s::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_F1F1_DelayTime20s::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_ReadMeter_F1F1_DelayTime20s::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_Finish_F1F1:
...
void Script_ReadMeter_F1F1_DelayTime20s::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
...
void Script_ReadMeter_F1F1_DelayTime20s::readInfoInit()
{
...
bool Script_ReadMeter_F1F1_DelayTime20s::isMeterExist(Address address)
{
...
int Script_ReadMeter_F1F1_DelayTime20s::getReadInfo(Address address)
{
    for(int i=0;i<readInfoList.size();i++)
    {
        if(address==readInfoList.at(i).meterNo)
...
double Script_ReadMeter_F1F1_DelayTime20s::calSuccessRate()
{
...
QString Script_ReadMeter_F1F1_DelayTime20s::calCostTime()
{
...
QString Script_ReadMeter_F1F1_DelayTime20s::getFailMeterNo()
{
    QString failMeterNo;
    int count=0;
    for(int i=0;i<readInfoList.size();i++)
...
bool Script_ReadMeter_F1F1_DelayTime20s::isAllRead(int meterID)
{
    bool sus=true;
    for(int i=0;i<readInfoList.at(meterID).dataUnitList.size();i++)
    {
...
bool Script_ReadMeter_F1F1_DelayTime20s::isAllMeterReadSuccess()
{
    bool sus=true;
    for(int i=0;i<readInfoList.size();i++)
    {
...
void Script_ReadMeter_F1F1_DelayTime20s::CalcAvrgConsumeTimeLen(uchar rdFlag)
{
