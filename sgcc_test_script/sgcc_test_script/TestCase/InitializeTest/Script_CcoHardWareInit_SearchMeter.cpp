#include "Script_CcoHardWareInit_SearchMeter.h"

Script_CcoHardWareInit_SearchMeter::Script_CcoHardWareInit_SearchMeter(QObject *parent) : QObject(parent)
{
...
Script_CcoHardWareInit_SearchMeter::~Script_CcoHardWareInit_SearchMeter()
{
...
void Script_CcoHardWareInit_SearchMeter::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
void Script_CcoHardWareInit_SearchMeter::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
bool Script_CcoHardWareInit_SearchMeter::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_CcoHardWareInit_SearchMeter::execute()
{
    p_AbstractScriptHost->updateProgress(ProcessState_Start,"脚本执行流程描述打印：-----whc(2024年1月20)\r\n"
                                                             ...
                                                             "2.03F4查询主节点地址，10F1查询从节点数量，10F21网络拓扑信息，10F9网络规模，10F40查询路由模块ID，10F40查询路由芯片ID，F0F41查询SN，03F16查询频段，10F4查询路由状态，Wait_QueryInitialState_Finish；\r\n"
                                                             "3.11F5启动从节点主动注册，搜表时长1min，10F4查询路由运行状态，Wait_QueryHardWareInit_Finish；\r\n"
...
void Script_CcoHardWareInit_SearchMeter::stop()
{
    p_timer->stop();
    p_delayTimer->stop();
    p_maxAllowTimer->stop();
...
void Script_CcoHardWareInit_SearchMeter::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_CcoHardWareInit_SearchMeter::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_CcoHardWareInit_SearchMeter::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        case Wait_QueryInitialState_Finish:
        {
            ...
            else  if(p_Frame3762Base->afn_ == 0x10 && dtValue3762==21 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                net_info_10F21_ = int(p_QueryNetInfo_10F21_Up->network_typelogy_info_unit_.node_total_num_);
                p_AbstractScriptHost->updateProgress(ProcessState_Processing,  QString(">>>收到节点总数量为：%1").arg(net_info_10F21_));

                if(net_info_10F21_ == node_num_10F1_+1)
                {
            ...
            else  if(p_Frame3762Base->afn_ == 0x10 && dtValue3762==9 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                net_size_10F9_ = int(p_QueryNetSize_10F9->network_scale_);
                p_AbstractScriptHost->updateProgress(ProcessState_Processing,  QString(">>>收到网络规模为：%1").arg(net_size_10F9_));
        ...
        case Wait_QueryHardWareInit_Finish:
        {
            ...
            else if(p_Frame3762Base->afn_ == 0x10 && dtValue3762==4 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if (router_complete_flag ==1 && current_state ==1)
                {
        ...
        case Wait_QueryCheckState_Finish:
        {
            ...
            else  if(p_Frame3762Base->afn_ == 0x10 && dtValue3762==1 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(node_num_10F1_ == target_data)
                {
                    ...
                    tryTimes =0;
                    index =0;
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, ">>>>发送--查询10F21网络拓扑信息，等待--回复");
                    delay(15*1000); // 等待路由入网完成，15秒后查询
                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNetInfo_10F21);
                    p_timer->start(CCO_CMD_TIMEOUT_TIME*1000);
                }
                else
            ...
            else  if(p_Frame3762Base->afn_ == 0x10 && dtValue3762==21 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(target_data == 1 )
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed,  QString(">>>Wait_QueryCheckState_Finish，Wait_QueryNetInfo_10F21，期望路由回复节点总数量和接收路由回复节点总数量不一致测试失败，期望节点总数量：%1，接收节点总数量：%2").arg(net_info_10F21_).arg(target_data)+test_name_);
            }
            else  if(p_Frame3762Base->afn_ == 0x10 && dtValue3762==9 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_timer->stop();
...
void Script_CcoHardWareInit_SearchMeter::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_CcoHardWareInit_SearchMeter::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_CcoHardWareInit_SearchMeter::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_CcoHardWareInit_SearchMeter::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_CcoHardWareInit_SearchMeter::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
    case Wait_BuildNetFinish_Whole:
    ...
    case Wait_QueryInitialState_Finish:
    {
        ...
        case Wait_QueryNetInfo_10F21:
        {
            if(++tryTimes<3)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_QueryInitialState_Finish,Wait_QueryNetInfo_10F21，查询从节信息（10F2）3次超时失败")+test_name_);
            break;
        }
    ...
    case Wait_QueryCheckState_Finish:
    {
        ...
        case Wait_QueryNetInfo_10F21:
        {
            if(++tryTimes<3)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_QueryCheckState_Finish,Wait_QueryNetInfo_10F21，查询从节信息（10F2）3次超时失败")+test_name_);
            break;
        }
        case Wait_QueryNetSize_10F9:
        {
...
void Script_CcoHardWareInit_SearchMeter::maxAllowTimer_timeoutProc()
{
    p_delayTimer->stop();
    switch(emScriptRunState)
    {
...
void Script_CcoHardWareInit_SearchMeter::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    switch(emScriptRunState)
    {
    ...
    case Wait_QueryHardWareInit_Finish:
    {
        tryTimes =0;
        index =0;
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString(">>>>延时时间到，发送10F4查询路由运行状态"));
        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryWorkSwitch_10F4);
        p_timer->start(CCO_CMD_TIMEOUT_TIME*1000);
        break;
    }
...
bool Script_CcoHardWareInit_SearchMeter::meterIsExist(Address meterAddr)
{
