#include "Script_CcoParaInit_SearchMeter.h"
Script_CcoParaInit_SearchMeter::Script_CcoParaInit_SearchMeter(QObject *parent) : QObject(parent)
{
...
Script_CcoParaInit_SearchMeter::~Script_CcoParaInit_SearchMeter()
{
...
void Script_CcoParaInit_SearchMeter::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
void Script_CcoParaInit_SearchMeter::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
bool Script_CcoParaInit_SearchMeter::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_CcoParaInit_SearchMeter::execute()
{
    p_AbstractScriptHost->updateProgress(ProcessState_Start,"脚本执行流程描述打印：-----whc(2024年1月20)\r\n"
                                                             ...
                                                             "2.05F1设置主节点地址，03F4查询主节点地址，10F1查询从节点数量，10F21查询网络拓扑信息，10F40查询路由模块ID信息，10F40查询路由芯片ID信息，F0F41查询路由生产SN，03F16查询频段，Wait_QueryInitialState_Finish；\r\n"
                                                             "3.05F2设置事件上报开关：禁止，Wait_SetEventReport_05F2；\r\n"
                                                             "4.05F6设置台区识别开关：开启，10F4查询事件上报开关，10F4查询台区识别开，Wait_SetAreaIdentification_05F6；\r\n"
                                                             "5.11F5启动从节点主动注册，搜表时长1min，等待5S，10F4查询路由状态，Wait_StartSearchMeter_11F5；\r\n"
                                                             "6.01F2设置参数初始化，10F4查询路由运行状态，03F4查询主节点地址，10F1查询从节点数量，10F21网络拓扑信息，10F40查询路由ID信息，10F40查询路由芯片ID信息，F0F41查询路由SN，03F16查询路由当前频段，Wait_QueryCheckState_Finish；\r\n");
...
void Script_CcoParaInit_SearchMeter::stop()
{
    p_timer->stop();
    p_delayTimer->stop();
    p_maxAllowTimer->stop();
...
void Script_CcoParaInit_SearchMeter::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_CcoParaInit_SearchMeter::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_CcoParaInit_SearchMeter::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        case Wait_QueryInitialState_Finish:
        {
            ...
            else if(p_Frame3762Base->afn_ == 0x03 && dtValue3762==16 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(freq_03F16_== freq_03F16)
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed,  QString("Wait_QueryInitialState_Finish,Wait_QueryFreq_03F16,查询路由频段与设置路由频段不一致测试失败，期望路由频段：%1，查询路由频段：%2，").arg(freq_03F16_).arg(freq_03F16)+test_name_);
        ...
        case Wait_SetAreaIdentification_05F6:
        {
            ...
            else if(p_Frame3762Base->afn_ == 0x10 && dtValue3762==4 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Processing,  QString(">>>收到事件上报开关为：%1，台区区分开关为：%2").arg(event_report_flag_10F4).arg(area_difference_flag_10F4));

                if (event_report_flag_10F4 == event_report_flag_10F4_  &&  area_difference_flag_10F4==area_difference_flag_10F4_)
                {
                    emScriptRunState = Wait_StartSearchMeter_11F5;
                    tryTimes =0;
                    index =0;
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, ">>>>发送--11F5启动从节点主动注册，搜表时长1min，等待--回复");
                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_NodeActiveRegister_11F5);
                    p_timer->start(CCO_CMD_TIMEOUT_TIME*1000);
                }
                else
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed,  QString(">>>Wait_SetAreaIdentification_05F6，10F4查询事件上报开关/台区区分开关与设置不一致测试失败，收到事件上报开关为：%1，台区区分开关为：%2").arg(event_report_flag_10F4).arg(area_difference_flag_10F4)+test_name_);
            }
            else
            {
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到其他376.2报文，等待--回复");
        ...
        case Wait_StartSearchMeter_11F5:
        {
            if(p_Frame3762Base->afn_== char(0x00) && dtValue3762==1 && p_Frame3762Base->ctrl_field_.dir==kDirUp)
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, ">>>>收到确认，11F5启动从节点主动注册成功。");
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, ">>>>延时5s,然后进行10F4查询路由状态。");
            ...
            else if(p_Frame3762Base->afn_ == 0x10 && dtValue3762==4 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Processing,  QString(">>>收到路由完成标志为：%1，当前状态为：%2,期望值路由完成标志为：1(路由完成标志: 路由学习完成)，当前状态为：1(当前状态：01-搜表)").arg(router_complete_flag).arg(current_state));
                ...
                if (current_state == 0x01)
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed,  QString(">>>Wait_StartSearchMeter_11F5，路由回复当前状态与期望不一致测试失败，路由当前状态为：%1，期望为：0,%3。").arg(current_state).arg(test_name_));
            }
            else
            {
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到其他376.2报文，等待--回复");
        ...
        case Wait_QueryCheckState_Finish:
        {
            ...
            else if(p_Frame3762Base->afn_ == 0x10 && dtValue3762==4 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString(">>>>收到10F40查询路由运行状态报文，事件上报开关为：%1，台区区分开关为：%2，当前状态为：%3").arg(event_report_flag_10F4).arg(area_difference_flag_10F4).arg(current_state));
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString(">>>>期望10F40查询回来路由运行状态，事件上报开关为：%1，台区区分开关为：%2").arg(event_report_flag_10F4_).arg(area_difference_flag_10F4_));
                if (event_report_flag_10F4 == event_report_flag_10F4_ && area_difference_flag_10F4==area_difference_flag_10F4_) // 主动注册过程，参数初始化，查到的状态为搜表---这里再次需要讨论逻辑合理性，建议参数初始化判断如果在搜表中，停止搜表
                {
                    tryTimes=0;
                    index=0;
                    emQueryState=Wait_QueryMasterAddr_03F4;
...
void Script_CcoParaInit_SearchMeter::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_CcoParaInit_SearchMeter::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_CcoParaInit_SearchMeter::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_SetFreq_05F16)
    {
        ...
        p_SetFreq_05F16->carrier_frequence_range_=2;
...
void Script_CcoParaInit_SearchMeter::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_CcoParaInit_SearchMeter::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
    case Wait_BuildNetFinish_Whole:
    ...
    case Wait_SetAreaIdentification_05F6:
    {
        ...
        case Wait_QueryFreq_03F16:
        {
            ...
            if(++tryTimes<3)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_StartSearchMeter_11F5，Wait_QueryFreq_03F16，设置路由路由台区区分识别开关状态（05F6）3次超时失败")+test_name_);
            break;
        }
        case Wait_QueryRouteState_10F4:
        {
            ...
            if(++tryTimes<3)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_StartSearchMeter_11F5，Wait_QueryRouteState_10F4，查询路由运行状态（10F4）3次超时失败")+test_name_);
    ...
    case Wait_StartSearchMeter_11F5:
    {
        ...
        case Wait_QueryFreq_03F16:
        {
            ...
            if(++tryTimes<3)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_StartSearchMeter_11F5，参数初始化命令（01F2）3次超时失败")+test_name_);
            break;
        }
        case Wait_QueryRouteState_10F4:
        {
...
void Script_CcoParaInit_SearchMeter::maxAllowTimer_timeoutProc()
{
    p_delayTimer->stop();
    switch(emScriptRunState)
    {
...
void Script_CcoParaInit_SearchMeter::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    switch(emScriptRunState)
    {
...
bool Script_CcoParaInit_SearchMeter::meterIsExist(Address meterAddr)
{
