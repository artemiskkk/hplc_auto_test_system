#include "Script_InitializeTest_ReadMeter.h"

Script_InitializeTest_ReadMeter::Script_InitializeTest_ReadMeter(QObject *parent) : QObject(parent)
{
...
Script_InitializeTest_ReadMeter::~Script_InitializeTest_ReadMeter()
{
...
void Script_InitializeTest_ReadMeter::execute()
{
...
void Script_InitializeTest_ReadMeter::stop()
{
...
void Script_InitializeTest_ReadMeter::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_InitializeTest_ReadMeter::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_InitializeTest_ReadMeter::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_InitializeTest_ReadMeter::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_InitializeTest_ReadMeter::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_InitializeTest_ReadMeter::processMsgFromCCO(DvcType dvcType, int dvcId)
{
...
//                            p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("路由ID为%1，符合要求").arg(QString(p_QueryRouterID_10F40_Up->read_chip_id_unit_up_.id_content_.toHex())));
                ...
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "接收--监控从节点（13F1）回复；");
                }
                else
                {
                    QByteArray tmpSendMsg=prcsOther3762Msg(uchar(p_Frame3762Base->afn_),p_Frame3762Base->dt1_,p_Frame3762Base->dt2_,uchar(p_Frame3762Base->info_field_.info_field_up.msg_seq));
...
void Script_InitializeTest_ReadMeter::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_InitializeTest_ReadMeter::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_InitializeTest_ReadMeter::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_InitializeTest_ReadMeter::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_InitializeTest_ReadMeter::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_InitializeTest_ReadMeter::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_HardResetTest_Finish:
...
void Script_InitializeTest_ReadMeter::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    index=0;
...
bool Script_InitializeTest_ReadMeter::meterIsExist(Address meterAddr)
{
