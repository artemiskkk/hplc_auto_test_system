#include "Script_CcoHardWareInit_FrequentReset_1.h"
Script_CcoHardWareInit_FrequentReset_1::Script_CcoHardWareInit_FrequentReset_1(QObject *parent) : QObject(parent)
{
...
Script_CcoHardWareInit_FrequentReset_1::~Script_CcoHardWareInit_FrequentReset_1()
{
...
void Script_CcoHardWareInit_FrequentReset_1::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
void Script_CcoHardWareInit_FrequentReset_1::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
bool Script_CcoHardWareInit_FrequentReset_1::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_CcoHardWareInit_FrequentReset_1::execute()
{
...
void Script_CcoHardWareInit_FrequentReset_1::stop()
{
    p_timer->stop();
    p_delayTimer->stop();
...
void Script_CcoHardWareInit_FrequentReset_1::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_CcoHardWareInit_FrequentReset_1::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_CcoHardWareInit_FrequentReset_1::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        case Wait_QueryCheckState_Finish:
        {
            ...
            else  if(p_Frame3762Base->afn_ == 0x10 && dtValue3762==7 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(query_id_10F7 == query_id_10F7_)
                {
                    emQueryState = Wait_QueryMasterAddr_03F4;
                    tryTimes =0;
                    index =0;
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed,  QString(">>>Wait_QueryCheckState_Finish，Wait_QueryModuleInfor_10F7，收到从节点信息应答的信息与第一次查询回来的信息不一致，测试失败。1%").arg(test_name_));
            }
...
void Script_CcoHardWareInit_FrequentReset_1::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_CcoHardWareInit_FrequentReset_1::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_CcoHardWareInit_FrequentReset_1::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    if(frame==p_confirm_00F1)
    {
        p_confirm_00F1->ctrl_field_={kHplc,kActive,kDirDown};
        p_confirm_00F1->info_field_.info_field_down.msg_seq=char(++msgSeq);
        uchar data[6]={0xFF,0xFF,0xFF,0xFF,0x00,0x00};
        memcpy(p_confirm_00F1->data_info_,data,6);
...
void Script_CcoHardWareInit_FrequentReset_1::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_CcoHardWareInit_FrequentReset_1::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
    case Wait_BuildNetFinish_Whole:
...
void Script_CcoHardWareInit_FrequentReset_1::maxAllowTimer_timeoutProc()
{
    p_delayTimer->stop();
    switch(emScriptRunState)
    {
...
void Script_CcoHardWareInit_FrequentReset_1::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    switch(emScriptRunState)
    {
...
bool Script_CcoHardWareInit_FrequentReset_1::meterIsExist(Address meterAddr)
{
