#ifndef SCRIPT_CCOPARAINIT_ROUTEUPGRADE_H
#define SCRIPT_CCOPARAINIT_ROUTEUPGRADE_H

#include <QObject>
#include "../BuildNetwork_GW.h"
#include "../Upgrade/FileTransfer.h"
#include "../Upgrade/QueryVersion.h"
#define SENCE2

class Script_CcoParaInit_RouteUpgrade : public QObject, public AbstractScript,public DynamicCreate<Script_CcoParaInit_RouteUpgrade>
{
    Q_OBJECT
public:
    explicit Script_CcoParaInit_RouteUpgrade(QObject *parent = nullptr);
    ~Script_CcoParaInit_RouteUpgrade();
    enum Script_CcoParaInit_RouteUpgrade_RunState
    {
        ScriptInit,
        Wait_BuildNetFinish_Whole,
        Wait_QueryVersion_Before_Finish,
        Wait_FileTransfer_15F1_Finish,
        Wait_ParameterInit_01F2_Finish,
        Wait_QueryVersion_Finish,
        ScriptSuccess
    };

private:
    Q_ENUM(Script_CcoParaInit_RouteUpgrade_RunState)
    Script_CcoParaInit_RouteUpgrade_RunState emScriptRunState=ScriptInit;
    QMetaEnum metaEnum=QMetaEnum::fromType<Script_CcoParaInit_RouteUpgrade_RunState>();
    BuildNetwork_GW *p_BuildNetwork_GW;//组网通用脚本
    FileTransfer *p_FileTransfer=nullptr;//文件传输流程
    QueryVersion *p_QueryVersion=nullptr;//查询版本
    ...
    //定义帧格式处理
    shared_ptr<Afn01F2> p_ParameterInit_01F2 =nullptr;
    ...
    QString testCase="【GW-CCO-F014-0012-V01】参数初始化-路由升级";
    ST_FileTransferParameter parameter;
    QString outVersionBefore;
    QString innerVersionBefore;
private:
    ...
    //
    QString selectUpgradeFile();
private slots:
    void timer_timeoutProc();//命令发送超时定时器槽函数
    void maxAllowTimer_timeoutProc();//脚本最大运行时间定时器槽函数
    void delayTimer_timeoutProc();//命令延迟定时器槽函数
    void slotNoticeFileTransState(ProcessState,const QString& state);
    void slotNoticeQueryVersionState(ProcessState,const QString& state);
    void slotNoticeQueryVersionResult(bool isCCO,QMap<QString,QString> versionInfo);
};

#endif // SCRIPT_CCOPARAINIT_ROUTEUPGRADE_H
