#include "Script_CcoHardWareInit_FrequentReset_2.h"
Script_CcoHardWareInit_FrequentReset_2::Script_CcoHardWareInit_FrequentReset_2(QObject *parent) : QObject(parent)
{
...
Script_CcoHardWareInit_FrequentReset_2::~Script_CcoHardWareInit_FrequentReset_2()
{
...
void Script_CcoHardWareInit_FrequentReset_2::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
void Script_CcoHardWareInit_FrequentReset_2::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
bool Script_CcoHardWareInit_FrequentReset_2::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_CcoHardWareInit_FrequentReset_2::execute()
{
    p_AbstractScriptHost->updateProgress(ProcessState_Start,"脚本执行流程描述打印：-----whc(2024年04月13)\r\n"
                                                             "1.通用组网流程，组网场景3(需要组网完成)，Wait_BuildNetFinish_Whole；\r\n"
                                                             "2.03F4查询主节点地址，10F1查询从节点数量，10F21查询网络拓扑信息，10F7查询模块ID信息，F0F38查询组网方式，F0F37设置组网方式，10F2查询节点信息，F0F40查询路由模块ID，F0F40查询路由芯片ID，F0F41查询SN，Wait_QueryInitialState_Finish；\r\n"
                                                             "3.05F16设置频段，03F16查询频段，Wait_SetFreq_05F16；\r\n"
                                                             "3.05F2设置路由事件上报开关状态，Wait_SetEventReport_05F2；\r\n"
                                                             "3.05F6设置路由台区区分上报开关状态，10F4查询路由运行状态，Wait_SetAreaIdentification_05F6；\r\n"
                                                             "3.01F1硬件初始化100次，03F10上报路由工况，Wait_QueryHardWareInit_Finish；\r\n"
...
void Script_CcoHardWareInit_FrequentReset_2::stop()
{
    p_timer->stop();
    p_delayTimer->stop();
...
void Script_CcoHardWareInit_FrequentReset_2::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_CcoHardWareInit_FrequentReset_2::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_CcoHardWareInit_FrequentReset_2::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        case Wait_QueryInitialState_Finish:
        {
            ...
            else  if(p_Frame3762Base->afn_ == 0x10 && dtValue3762==21 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(net_info_10F21 == p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->size()+1)
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed,  QString(">>>Wait_QueryInitialState_Finish，Wait_QueryNetInfo_10F21，接收到的网络拓扑信息节点总数量等于从节点数量+1测试失败，%1").arg(test_name_));
            }
            else if(p_Frame3762Base->afn_ == 0x10 && dtValue3762==7 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                emQueryState = Wait_SetBulidChanle_F0F37;
                tryTimes =0;
                index =0;
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, ">>>>发送--设置组网方式F0F37，等待--回复");
                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_SetChannel_F0F37);
                p_timer->start(CCO_CMD_TIMEOUT_TIME*1000);
            }
            else if(p_Frame3762Base->afn_== char(0x00) && dtValue3762==1 && p_Frame3762Base->ctrl_field_.dir==kDirUp)//设置组网方式
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, ">>>>收到确认，F0F37设置组网方式成功。");
            ...
            else if(p_Frame3762Base->afn_ == char(0xf0) && dtValue3762==38 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(p_QueryBulidChanle->freq_ == channel_type_)
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed,  QString(">>>Wait_QueryInitialState_Finish，Wait_QueryBulidChanle_F0F38，查询回来的组网方式与期望组网方式不一致测试失败，查询回来组网方式：%1，期望组网方式：%2， %3").arg(p_QueryBulidChanle->freq_).arg(0x00).arg(test_name_));
            }
            ...
            else if(p_Frame3762Base->afn_ == char(0xF0) && dtValue3762==41 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                    ...
                    &&p_QueryRouterSN_F0F41_Up->device_type_==0x02)
                {
                    ...
                    emScriptRunState = Wait_SetFreq_05F16;
                    emQueryState =Wait_SetFreq_05F16_1;
                    tryTimes =0;
                    index =0;
        ...
        case Wait_SetFreq_05F16:
        {
            if(p_Frame3762Base->afn_== char(0x00) && dtValue3762==1 && p_Frame3762Base->ctrl_field_.dir==kDirUp)
            {
                ...
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, ">>>>收到确认，05F16设置频段成功,等待120秒后查询频段。");
            ...
            else if(p_Frame3762Base->afn_ == 0x03 && dtValue3762==16 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(freq_03F16 ==set_freq_)
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed,  QString(">>>Wait_SetFreq_05F16，Wait_QueryFreq_03F16，查询回来的频段与期望频段不一致测试失败，查询回来频段：%1，期望频段：%2， %3").arg(freq_03F16).arg(set_freq_).arg(test_name_));
            }
            else
            {
        ...
        case Wait_SetEventReport_05F2:
        {
            if(p_Frame3762Base->afn_== char(0x00) && dtValue3762==1 && p_Frame3762Base->ctrl_field_.dir==kDirUp)
            {
                ...
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, ">>>>收到确认，05F2设置事件上报成功。");
        ...
        case Wait_SetAreaIdentification_05F6:
        {
            if(p_Frame3762Base->afn_== char(0x00) && dtValue3762==1 && p_Frame3762Base->ctrl_field_.dir==kDirUp)
            {
                ...
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, ">>>>收到确认，05F6设置台区区分开关成功。");
            ...
            else if(p_Frame3762Base->afn_ == 0x10 && dtValue3762==4 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Processing,  QString(">>>收到事件上报开关为：%1，期望值为：关闭。台区区分开关为：%2，期望值为：开启。").arg(event_report_flag_10F4_).arg(area_difference_flag_10F4_));
                p_AbstractScriptHost->updateProgress(ProcessState_Processing,  QString(">>>路由完成标志为：%1，期望值为：0x0未完成").arg(router_complete_flag));

                if(event_report_flag_10F4_==0x00 && area_difference_flag_10F4_==0x01 && router_complete_flag == 0x01)//路由学习完成
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed,  QString(">>>Wait_SetAreaIdentification_05F6，Wait_QueryWorkSwitch_10F4，查询回来的时间上报开关状态/台区区分状态/路由完成标志三个量中至少有一个不符合期望测试失败%1").arg(test_name_));
            }
            else
            {
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到其他376.2报文，等待--回复");
        ...
        case Wait_QueryHardWareInit_Finish:
        {
            ...
            {
                p_timer->stop();
                reset_index_ = reset_index_+1;
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, ">>>>收到确认，01F1硬件初始化成功,复位次数为："+QString::number(reset_index_));
                if (reset_index_ < 100)
                {
                    tryTimes =0;
                    index =0;
        ...
        case Wait_QueryCheckState_Finish:
        {
            ...
            if(p_Frame3762Base->afn_ == 0x10 && dtValue3762==21 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                else if (query_10F21_index <=5 )
                {
                    p_delayTimer->start(60*1000);
                }
                else
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed,  QString(">>>Wait_QueryCheckState_Finish，Wait_QueryNetInfo_10F21，接收到的网络拓扑信息节点总数量不是档案数量+1，测试失败(如果硬件复位后模块入网太快，此处预估有概率性失败)，%1").arg(test_name_));
            }
            else  if(p_Frame3762Base->afn_ == 0x10 && dtValue3762==7 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_timer->stop();
            ...
            else  if(p_Frame3762Base->afn_ == char(0xf0) && dtValue3762==38 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Processing,  QString(">>>查询双模组网方式F0F38为：%1").arg(p_QueryBulidChanle->freq_));
                if (channel_type_ == p_QueryBulidChanle->freq_)
                {
                    emQueryState = Wait_QueryNodeNum_10F1;
                    tryTimes =0;
                    index =0;
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed,  QString(">>>Wait_QueryCheckState_Finish，Wait_QueryBulidChanle_F0F38，期望组网方式跟实际不一致测试失败，期望组网方式：%1，回复组网方式：%2,%3").arg(channel_type_).arg(p_QueryBulidChanle->freq_).arg(test_name_));
            }
            else  if(p_Frame3762Base->afn_ == 0x10 && dtValue3762==1 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_timer->stop();
            ...
            else  if(p_Frame3762Base->afn_ == 0x03 && dtValue3762==16 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(set_freq_ == target_data)
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed,  QString(">>>Wait_QueryCheckState_Finish，Wait_QueryFreq_03F16，期望路由频段和接收路由频段不一致测试失败，期望路由频段：%1，接收路由频N：%2").arg(set_freq_).arg(target_data)+test_name_);
...
void Script_CcoHardWareInit_FrequentReset_2::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_CcoHardWareInit_FrequentReset_2::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_CcoHardWareInit_FrequentReset_2::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_SetChannel_F0F37)
    {
        ...
        p_SetChannel_F0F37->freq_=channel_type_;
    ...
    else if(frame==p_SetFreq_05F16)
    {
        ...
        p_SetFreq_05F16->carrier_frequence_range_=char(set_freq_);
...
void Script_CcoHardWareInit_FrequentReset_2::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_CcoHardWareInit_FrequentReset_2::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
    case Wait_BuildNetFinish_Whole:
    ...
    case Wait_QueryInitialState_Finish:
    {
        ...
        case Wait_SetBulidChanle_F0F37:
        {
            if(++tryTimes<3)
            {
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, ">>>>发送--设置组网方式F0F37，等待--回复");
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_QueryInitialState_Finish,Wait_SetBulidChanle_F0F37，F0F37设置组网方式3次超时失败")+test_name_);
            break;
        }
    ...
    case Wait_SetFreq_05F16:
    {
        ...
        case Wait_SetFreq_05F16_1:
        {
            ...
            if(++tryTimes<3)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_SetFreq_05F16，Wait_SetFreq_05F16_1，设置路由路由台区区分识别开关状态（05F6）3次超时失败")+test_name_);
            break;
        }
        case Wait_QueryFreq_03F16:
        {
            ...
            if(++tryTimes<3)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_SetFreq_05F16，Wait_QueryFreq_03F16，设置路由路由台区区分识别开关状态（05F6）3次超时失败")+test_name_);
            break;
        }
        default:
            break;
    ...
    case Wait_SetAreaIdentification_05F6:
    {
        ...
        case Wait_QueryFreq_03F16:
        {
            if(++tryTimes<3)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_SetAreaIdentification_05F6，设置路由台区区分开关状态（05F6）3次超时失败")+test_name_);
            break;
        }
        case Wait_QueryWorkSwitch_10F4:
        {
            if(++tryTimes<3)
            {
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, ">>>>发送-10F4查询路由运行状态，等待--回复");
                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryWorkSwitch_10F4);
                p_timer->start(CCO_CMD_TIMEOUT_TIME*1000);
            }
            else
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_SetAreaIdentification_05F6，Wait_QueryWorkSwitch_10F4,查询路由运行状态（10F4）3次超时失败")+test_name_);
            break;
        }
        default:
            break;
...
void Script_CcoHardWareInit_FrequentReset_2::maxAllowTimer_timeoutProc()
{
    p_delayTimer->stop();
    switch(emScriptRunState)
    {
...
void Script_CcoHardWareInit_FrequentReset_2::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    switch(emScriptRunState)
    {
    ...
    case Wait_QueryCheckState_Finish:
    {
        tryTimes =0;
        index =0;
        query_10F21_index = query_10F21_index+1;
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, ">>>>发送--查询网络拓扑信息（10F21），等待--回复");
        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNetNum_10F21);
        p_timer->start(CCO_CMD_TIMEOUT_TIME*1000);
        break;
...
bool Script_CcoHardWareInit_FrequentReset_2::meterIsExist(Address meterAddr)
{
