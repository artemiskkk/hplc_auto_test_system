#include "Script_InitializeTest.h"

Script_InitializeTest::Script_InitializeTest(QObject *parent) : QObject(parent)
{
...
Script_InitializeTest::~Script_InitializeTest()
{
...
void Script_InitializeTest::execute()
{
...
void Script_InitializeTest::stop()
{
...
void Script_InitializeTest::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_InitializeTest::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_InitializeTest::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_InitializeTest::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_InitializeTest::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_InitializeTest::processMsgFromCCO(DvcType dvcType, int dvcId)
{
...
//                    router_complete_flag_ = uchar(p_QueryWorkSwitch_10F4_Up->router_operate_state_unit_.operate_state_word_.router_complete_flag_);
            ...
            case Wait_HardResetTest_Finish:
            {
                ...
                else if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==char(0x80)&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(uchar(p_QueryFreq_03F16_Up->carrier_frequence_range_)==runFreq)
                    {
                        ...
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--查询路由ID（10F40），等待--回复");
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    }
                    else
                    {
...
void Script_InitializeTest::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_InitializeTest::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_InitializeTest::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_InitializeTest::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_InitializeTest::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
    ...
    case Wait_ParaInitTest_Finish:
    {
        ...
        else if(emQueryState==Wait_QueryFreq_03F16)
        {
            ...
            else
            {
                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryRouterID_10F40);
                emQueryState=Wait_QueryRouterID_10F40;
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "第二次回复超时，发下一条--查询路由ID（10F40），等待--回复");
                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                resendFlag=false;
            }
        }
        else if(emQueryState==Wait_QueryRouterID_10F40)
        {
            if(resendFlag==false)
            {
                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryRouterID_10F40);
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "第一次回复超时，重发--查询路由ID（10F40），等待--回复");
                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                resendFlag = true;
                resendNum++;
            }
            else
            {
                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryRouterChipID_10F40);
                emQueryState=Wait_QueryChipID_10F40;
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "第二次回复超时，发下一条--查询芯片ID（10F40），等待--回复");
                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                resendFlag=false;
            }
        }
        else if(emQueryState==Wait_QueryChipID_10F40)
        {
            ...
            {
                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryRouterChipID_10F40);
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "第一次回复超时，重发--查询芯片ID（10F40），等待--回复");
                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                resendFlag = true;
                resendNum++;
            }
...
void Script_InitializeTest::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_HardResetTest_Finish:
...
void Script_InitializeTest::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
}

bool Script_InitializeTest::meterIsExist(Address meterAddr)
{
