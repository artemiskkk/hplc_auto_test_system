#include "Script_CcoParaInit_NetworkingFinish.h"

Script_CcoParaInit_NetworkingFinish::Script_CcoParaInit_NetworkingFinish(QObject *parent) : QObject(parent)
{
    ...
    p_QueryNetInfo_10F21=make_shared<Afn10F21>();
    ...
    p_QueryRouteState_10F4=make_shared<Afn10F4>();
    p_ParameterInit_01F2=make_shared<Afn01F2>();
    p_RouterRequestRead_14F1=make_shared<Afn14F1>();
    p_NodeActiveRegister_11F5 = make_shared<Afn11F5>();
    p_QueryModuleInfor_10F7 = make_shared<Afn10F7>();

    p_SetEventReport_05F2=make_shared<Afn05F2>();
    p_SetAreaIdentification_05F6=make_shared<Afn05F6>();
...
Script_CcoParaInit_NetworkingFinish::~Script_CcoParaInit_NetworkingFinish()
{
...
void Script_CcoParaInit_NetworkingFinish::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
void Script_CcoParaInit_NetworkingFinish::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
bool Script_CcoParaInit_NetworkingFinish::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_CcoParaInit_NetworkingFinish::execute()
{
    p_AbstractScriptHost->updateProgress(ProcessState_Start,"脚本执行流程描述打印：-----whc(2024年1月20)\r\n"
                                                             ...
                                                             "2.05F1设置主节点地址，03F4查询主节点地址，10F1查询从节点数量，10F2查询从节点信息，10F21查询网络拓扑信息，10F40查询路由模块ID信息，10F40查询路由芯片ID信息，F0F41查询路由生产SN，05F16设置路由当前频段0，03F16查询当前频段，Wait_QueryInitialState_Finish；\r\n"
                                                             "3.05F2设置事件上报开关：禁止，Wait_SetEventReport_05F2；\r\n"
                                                             "4.05F6设置台区识别开关：开启，10F4查询路由运行状态，Wait_SetAreaIdentification_05F6；\r\n"
                                                             "5.01F2设置参数初始化，10F4查询事件上报开关/台区识别开关/路由完成标志，03F4查询主节点地址，10F1查询从节点数量，10F2查询从节点信息，10F21查询网络拓扑信息，10F40查询路由ID信息，10F40查询路由芯片ID信息，F0F41查询路由SN，03F16查询路由当前频段，10F7查询模块ID信息，Wait_QueryCheckState_Finish；\r\n");
...
void Script_CcoParaInit_NetworkingFinish::stop()
{
    p_timer->stop();
    p_delayTimer->stop();
    p_maxAllowTimer->stop();
...
void Script_CcoParaInit_NetworkingFinish::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_CcoParaInit_NetworkingFinish::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_CcoParaInit_NetworkingFinish::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        case Wait_QueryInitialState_Finish:
        {
            ...
            else  if(p_Frame3762Base->afn_ == 0x10 && dtValue3762==2 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if (p_QueryNodeInfo_10F2_Up->node_info_data_unit_.node_info_group_list_.size()>=2 )
                {
                    QString node_info_10F2 = QString(QByteArray(Address(p_QueryNodeInfo_10F2_Up->node_info_data_unit_.node_info_group_list_.at(0).node_address_).addr,6).toHex());
                    node_info_10F2 = node_info_10F2+ "、"+ QString(QByteArray(Address(p_QueryNodeInfo_10F2_Up->node_info_data_unit_.node_info_group_list_.at(1).node_address_).addr,6).toHex());
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing,  QString(">>>收到节点信息为：%1").arg(node_info_10F2));
                    if(meterIsExist(p_QueryNodeInfo_10F2_Up->node_info_data_unit_.node_info_group_list_.at(0).node_address_)==true)
                    {
                        emQueryState = Wait_QueryNetInfo_10F21;
                        tryTimes =0;
                        index =0;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, ">>>>发送--10F21网络拓扑信息，等待--回复");
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNetInfo_10F21);
                        p_timer->start(CCO_CMD_TIMEOUT_TIME*1000);
                    }
                    else
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed,  QString(">>>Wait_QueryInitialState_Finish，Wait_QueryNodeInfo_10F2，接收到的节点信息的数量不为期望节点数量(期望为2)测试失败，%1").arg(test_name_));
            ...
            else  if(p_Frame3762Base->afn_ == 0x10 && dtValue3762==21 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(net_info_10F21 == node_num_10F1_+1)
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed,  QString(">>>Wait_QueryInitialState_Finish，Wait_QueryNetInfo_10F21，接收到的网络拓扑信息节点总数量不为从节点数量+1测试失败，%1").arg(test_name_));
            }
            else  if(p_Frame3762Base->afn_ == char(0x10) && dtValue3762==40 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                else if (emQueryState==Wait_QueryChipID_10F40)
                {
                        ...
                        &&p_QueryRouterID_10F40_Up->read_chip_id_unit_up_.device_type_==0x02)
                    {
                        ...
                        emQueryState = Wait_SetFreq_05F16;
                        tryTimes =0;
                        index =0;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, ">>>>发送-设置路由频段（05F16），等待--回复");
            ...
            else  if(p_Frame3762Base->afn_== char(0x00) && dtValue3762==1 && p_Frame3762Base->ctrl_field_.dir==kDirUp)
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, ">>>>收到确认，05F16设置路由频段成功。");
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString(">>>>开始90s延时，延时结束后查询路由频段。"));
            ...
            else if(p_Frame3762Base->afn_ == 0x03 && dtValue3762==16 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                uchar freq_03F16 = uchar(p_QueryFreq_03F16_Up->carrier_frequence_range_);

                p_AbstractScriptHost->updateProgress(ProcessState_Processing,  QString(">>>收到路由频段为：%1").arg(freq_03F16));
                if(freq_03F16_== freq_03F16)
                {
                    emScriptRunState =Wait_SetEventReport_05F2;
                    emQueryState = Wait_QueryFreq_03F16;
                    tryTimes =0;
                    index =0;
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, ">>>>发送-设置路由路由事件上报开关状态（05F2），等待--回复");
                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_SetEventReport_05F2);
                    p_timer->start(CCO_CMD_TIMEOUT_TIME*1000);
                }
                else
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed,  QString("Wait_QueryInitialState_Finish,Wait_QueryFreq_03F16,查询路由频段与设置路由频段不一致测试失败，设置路由频段：%1，查询路由频段：%2，").arg(freq_03F16_).arg(freq_03F16)+test_name_);
        ...
        case Wait_SetEventReport_05F2:
        {
            if(p_Frame3762Base->afn_== char(0x00) && dtValue3762==1 && p_Frame3762Base->ctrl_field_.dir==kDirUp)
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, ">>>>收到确认，05F2设置路由事件上报开关成功。");

                emScriptRunState =Wait_SetAreaIdentification_05F6;
                emQueryState = Wait_QueryFreq_03F16;
                tryTimes =0;
                index =0;
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, ">>>>发送-设置路由路由台区区分识别开关状态（05F6），等待--回复");
                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_SetAreaIdentification_05F6);
                p_timer->start(CCO_CMD_TIMEOUT_TIME*1000);
            }
            else
            {
        ...
        case Wait_SetAreaIdentification_05F6:
        {
            if(p_Frame3762Base->afn_== char(0x00) && dtValue3762==1 && p_Frame3762Base->ctrl_field_.dir==kDirUp)
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, ">>>>收到确认，05F6设置路由台区区分开关状态成功。");
                emQueryState = Wait_QueryRouteState_10F4;
                tryTimes =0;
                index =0;
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, ">>>>发送-查询路由运行状态（10F4），等待--回复");
                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryRouteState_10F4);
                p_timer->start(CCO_CMD_TIMEOUT_TIME*1000);
            }
            else if(p_Frame3762Base->afn_ == 0x10 && dtValue3762==4 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_timer->stop();
                shared_ptr<Afn10F4> p_QueryRouteState_10F4_Up=dynamic_pointer_cast<Afn10F4>(p_Frame3762Base);
                uchar event_report_flag_10F4 = uchar(p_QueryRouteState_10F4_Up->router_operate_state_unit_.work_switch_.event_report_flag_);
                uchar area_difference_flag_10F4 = uchar(p_QueryRouteState_10F4_Up->router_operate_state_unit_.work_switch_.area_difference_flag_);
                uchar router_complete_flag = uchar(p_QueryRouteState_10F4_Up->router_operate_state_unit_.operate_state_word_.router_complete_flag_);

                p_AbstractScriptHost->updateProgress(ProcessState_Processing,  QString(">>>收到事件上报开关为：%1，台区区分开关为：%2，路由完成标志：%3").arg(event_report_flag_10F4).arg(area_difference_flag_10F4).arg(router_complete_flag));

                if (event_report_flag_10F4 == event_report_flag_10F4_  &&  area_difference_flag_10F4==area_difference_flag_10F4_ && router_complete_flag == 0x01)
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed,  QString(">>>Wait_SetAreaIdentification_05F6，10F4查询事件上报开关/台区区分开关/路由完成标志与设置(期望)不一致测试失败，收到事件上报开关为：%1，台区区分开关为：%2,路由完成标志：%3").arg(event_report_flag_10F4).arg(area_difference_flag_10F4).arg(router_complete_flag)+test_name_);
            }
            else
            {
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到其他376.2报文，等待--回复");
        ...
        case Wait_QueryCheckState_Finish:
        {
            if(p_Frame3762Base->afn_== char(0x00) && dtValue3762==1 && p_Frame3762Base->ctrl_field_.dir==kDirUp)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, ">>>>延时10s,然后进行10F4查询路由状态。");
            ...
            else if(p_Frame3762Base->afn_ == 0x10 && dtValue3762==4 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                uchar area_difference_flag_10F4 = uchar(p_QueryRouteState_10F4_Up->router_operate_state_unit_.work_switch_.area_difference_flag_);
                uchar router_complete_flag = uchar(p_QueryRouteState_10F4_Up->router_operate_state_unit_.operate_state_word_.router_complete_flag_);
                //uchar current_state =  uchar(p_QueryRouteState_10F4_Up->router_operate_state_unit_.work_switch_.current_state_);
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString(">>>>收到10F40查询路由运行状态报文，事件上报开关为：%1，台区区分开关为：%2，路由完成标志为：%3").arg(event_report_flag_10F4).arg(area_difference_flag_10F4).arg(router_complete_flag));
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString(">>>>期望10F40查询回来路由运行状态，事件上报开关为：%1，台区区分开关为：%2，路由完成标志为：0(0为未完成)").arg(event_report_flag_10F4_).arg(area_difference_flag_10F4_));
                if (event_report_flag_10F4 == event_report_flag_10F4_ && area_difference_flag_10F4==area_difference_flag_10F4_ && router_complete_flag == 0)
                {
                    tryTimes=0;
                    index=0;
                    emQueryState=Wait_QueryMasterAddr_03F4;
            ...
            else  if(p_Frame3762Base->afn_ == 0x10 && dtValue3762==2 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_timer->stop();
                shared_ptr<Afn10F2> p_QueryNodeInfo_10F2_Up=dynamic_pointer_cast<Afn10F2>(p_Frame3762Base);
                p_AbstractScriptHost->updateProgress(ProcessState_Processing,  QString(">>>收到节点信息的返回报文"));

                if(p_QueryNodeInfo_10F2_Up->node_info_data_unit_.node_total_num_==0 && p_QueryNodeInfo_10F2_Up->node_info_data_unit_.this_node_num_==0)
                {
                    emQueryState = Wait_QueryNetInfo_10F21;
                    tryTimes =0;
                    index =0;
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, ">>>>发送--10F21网络拓扑信息，等待--回复");
                    delay(1*1000); // 等待cco入网，1秒后查询
                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNetInfo_10F21);
                    p_timer->start(CCO_CMD_TIMEOUT_TIME*1000);
                }
                else
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed,  QString(">>>Wait_QueryInitialState_Finish，Wait_QueryNodeInfo_10F2，接收到的节点信息不为0测试失败，%1").arg(test_name_));
            }
            else  if(p_Frame3762Base->afn_ == 0x10 && dtValue3762==21 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_timer->stop();
                shared_ptr<Afn10F21> p_QueryNetInfo_10F21_Up=dynamic_pointer_cast<Afn10F21>(p_Frame3762Base);
                int target_data  = int(p_QueryNetInfo_10F21_Up->network_typelogy_info_unit_.node_total_num_);

                p_AbstractScriptHost->updateProgress(ProcessState_Processing,  QString(">>>收到路由回复节点总数量为：%1").arg(target_data));
                ...
                if(target_data == 1)
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed,  QString(">>>Wait_QueryCheckState_Finish，Wait_QueryNetInfo_10F21，期望路由回复节点总数量和接收路由回复节点总数量不一致测试失败，期望路由回复节点总数量：1，接收路由回复节点总数量：%1").arg(target_data)+test_name_);
            }
            else  if(p_Frame3762Base->afn_ == char(0x10) && dtValue3762==40 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_timer->stop();
            ...
            else if(p_Frame3762Base->afn_ == 0x10 && dtValue3762==7 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                uchar target_data  =p_QuerySlaveNodeIdInfo_10F7->node_id_info_unit_.this_node_num_;
                p_AbstractScriptHost->updateProgress(ProcessState_Processing,  QString(">>>收到从节点信息应答的数量为：%1").arg(target_data));
                if(target_data ==0)
                {
...
//                if(index<p_QuerySlaveNodeIdInfo_10F7->node_id_info_unit_.node_total_num_)
//                {
//                    for(int i=0;i<p_QuerySlaveNodeIdInfo_10F7->node_id_info_unit_.this_node_num_;i++)
//                    {
//                        if(memcmp(staAddr_,p_QuerySlaveNodeIdInfo_10F7->node_id_info_unit_.node_id_info_list_.at(i).node_address_.addr,6)==0)
//                        {
//                            addFlag=false;
//                            p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("addFlag=false"));
//                        }
//                    }
//                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("应答从节点数为%1，从节点总数为%2，等待6秒后重新查询").arg(p_QuerySlaveNodeIdInfo_10F7->node_id_info_unit_.this_node_num_).arg(p_QuerySlaveNodeIdInfo_10F7->node_id_info_unit_.node_total_num_));
...
void Script_CcoParaInit_NetworkingFinish::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_CcoParaInit_NetworkingFinish::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_CcoParaInit_NetworkingFinish::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_QueryNetInfo_10F21)
    {
        p_QueryNetInfo_10F21->ctrl_field_.dir=kDirDown;
        p_QueryNetInfo_10F21->ctrl_field_.prm=kActive;
        p_QueryNetInfo_10F21->ctrl_field_.comn_type=kHplc;

        p_QueryNetInfo_10F21->info_field_.info_field_down.msg_seq=char(msgSeq++);
        p_QueryNetInfo_10F21->info_field_.info_field_down.comu_rate=0;
        p_QueryNetInfo_10F21->info_field_.info_field_down.comu_module_ident=0;

        p_QueryNetInfo_10F21->node_start_no_ =1;//ushort
        p_QueryNetInfo_10F21->node_num_ = 5;//uchar

        sendMsgOct=p_QueryNetInfo_10F21->EncodeFrame();
        sendMsgLog=QString("》》网络拓扑信息10F21：%1\n").arg(QString(sendMsgOct.toHex().toUpper()));
    }
    else if(frame==p_NodeActiveRegister_11F5)
    {
        p_NodeActiveRegister_11F5->ctrl_field_.dir=kDirDown;
        p_NodeActiveRegister_11F5->ctrl_field_.prm=kActive;
        p_NodeActiveRegister_11F5->ctrl_field_.comn_type=kHplc;

        p_NodeActiveRegister_11F5->info_field_.info_field_down.msg_seq=char(msgSeq++);
        p_NodeActiveRegister_11F5->info_field_.info_field_down.comu_rate=0;
        p_NodeActiveRegister_11F5->info_field_.info_field_down.comu_module_ident=0;

        QByteArray startTime=QByteArray::fromHex(QDateTime::currentDateTime().toString("ssmmhhddMMyy").toLatin1());
        memcpy(&p_NodeActiveRegister_11F5->start_time_,startTime,6);
        p_NodeActiveRegister_11F5->last_time_=1;
        p_NodeActiveRegister_11F5->retransmit_times_=0;
        p_NodeActiveRegister_11F5->wait_time_slice_num_=0;

        sendMsgOct=p_NodeActiveRegister_11F5->EncodeFrame();
    ...
    else if(frame==p_SetFreq_05F16)
    {
        ...
        p_SetFreq_05F16->carrier_frequence_range_=char(freq_03F16_);

        sendMsgOct=p_SetFreq_05F16->EncodeFrame();
        sendMsgLog=QString("》》设置频段05F16：%1\n").arg(QString(sendMsgOct.toHex().toUpper()));
    }
    else if(frame==p_SetEventReport_05F2)
    {
        p_SetEventReport_05F2->ctrl_field_.dir=kDirDown;
        p_SetEventReport_05F2->ctrl_field_.prm=kActive;
        p_SetEventReport_05F2->ctrl_field_.comn_type=kHplc;

        p_SetEventReport_05F2->info_field_.info_field_down.msg_seq=char(msgSeq++);
        p_SetEventReport_05F2->info_field_.info_field_down.comu_rate=0;
        p_SetEventReport_05F2->info_field_.info_field_down.comu_module_ident=0;

        p_SetEventReport_05F2->event_report_state_flag_=0x00;

        sendMsgOct=p_SetEventReport_05F2->EncodeFrame();
        sendMsgLog=QString("》》设置事件上报开关05F2：%1\n").arg(QString(sendMsgOct.toHex().toUpper()));
    }
    else if(frame==p_SetAreaIdentification_05F6)
    {
        p_SetAreaIdentification_05F6->ctrl_field_.dir=kDirDown;
        p_SetAreaIdentification_05F6->ctrl_field_.prm=kActive;
        p_SetAreaIdentification_05F6->ctrl_field_.comn_type=kHplc;

        p_SetAreaIdentification_05F6->info_field_.info_field_down.msg_seq=char(msgSeq++);
        p_SetAreaIdentification_05F6->info_field_.info_field_down.comu_rate=0;
        p_SetAreaIdentification_05F6->info_field_.info_field_down.comu_module_ident=0;

        p_SetAreaIdentification_05F6->area_identify_enable_flag_=0x01;

        sendMsgOct=p_SetAreaIdentification_05F6->EncodeFrame();
    ...
    else if(frame==p_QueryRouteState_10F4)
    {
        p_QueryRouteState_10F4->ctrl_field_.dir=kDirDown;
        p_QueryRouteState_10F4->ctrl_field_.prm=kActive;
        p_QueryRouteState_10F4->ctrl_field_.comn_type=kHplc;

        p_QueryRouteState_10F4->info_field_.info_field_down.msg_seq=char(msgSeq++);
        p_QueryRouteState_10F4->info_field_.info_field_down.comu_rate=0;
        p_QueryRouteState_10F4->info_field_.info_field_down.comu_module_ident=0;

        sendMsgOct=p_QueryRouteState_10F4->EncodeFrame();
    ...
    else if(frame==p_QueryModuleInfor_10F7)
    {
        ...
        p_QueryModuleInfor_10F7->node_start_no_ = 1;  //解决查询STA模块ID,返回否认问题
...
void Script_CcoParaInit_NetworkingFinish::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_CcoParaInit_NetworkingFinish::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
    case Wait_BuildNetFinish_Whole:
    ...
    case Wait_QueryInitialState_Finish:
    {
        ...
        case Wait_QueryNetInfo_10F21:
        {
            if(++tryTimes<3)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_QueryInitialState_Finish,Wait_QueryNetInfo_10F21，查询网络拓扑信息（10F21）3次超时失败")+test_name_);
            break;
        }
    ...
    case Wait_SetEventReport_05F2:
    {
        ...
        if(++tryTimes<3)
        {
            ...
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_SetEventReport_05F2，设置路由路由事件上报开关状态（05F2）3次超时失败")+test_name_);
        break;
    }
    case Wait_SetAreaIdentification_05F6:
    {
        ...
        case Wait_QueryFreq_03F16:
        {
            ...
            if(++tryTimes<3)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_QueryParaInit_Finish，Wait_QueryFreq_03F16，设置路由路由台区区分识别开关状态（05F6）3次超时失败")+test_name_);
            break;
        }
        case Wait_QueryRouteState_10F4:
        {
            p_timer->stop();
            if(++tryTimes<3)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_QueryParaInit_Finish，Wait_QueryRouteState_10F4，查询路由运行状态（10F4）3次超时失败")+test_name_);
...
void Script_CcoParaInit_NetworkingFinish::maxAllowTimer_timeoutProc()
{
    p_delayTimer->stop();
    switch(emScriptRunState)
    {
...
void Script_CcoParaInit_NetworkingFinish::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    switch(emScriptRunState)
    {
    ...
    case Wait_QueryInitialState_Finish:
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString(">>>>延时时间到，开始查询路由频段"));
        emQueryState = Wait_QueryFreq_03F16;
        tryTimes =0;
        index =0;
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, ">>>>发送--查询路由频段（03F16），等待--回复");
    ...
    default:
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString(">>>>延时时间到，未收到期望报文测试失败，")+test_name_);
        break;
    }
    }
}
...
bool Script_CcoParaInit_NetworkingFinish::meterIsExist(Address meterAddr)
{
