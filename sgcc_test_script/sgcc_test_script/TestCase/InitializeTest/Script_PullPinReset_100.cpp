#include "Script_PullPinReset_100.h"

Script_PullPinReset_100::Script_PullPinReset_100(QObject *parent) : QObject(parent)
{
...
Script_PullPinReset_100::~Script_PullPinReset_100()
{
...
void Script_PullPinReset_100::execute()
{
...
void Script_PullPinReset_100::stop()
{
...
void Script_PullPinReset_100::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_PullPinReset_100::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_PullPinReset_100::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_PullPinReset_100::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_PullPinReset_100::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_PullPinReset_100::processMsgFromCCO(DvcType dvcType, int dvcId)
{
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_QueryDefaultWorkSwitch_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x08&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    memcpy(&defaultWorkSwitch,&p_QueryWorkSwitch_10F4_Up->router_operate_state_unit_.work_switch_,1);

                    pullPinReset_CCO(p_CtrInfoList,p_AbstractScriptHost);
                    pullPinResetNum++;
                    emScriptRunState=Wait_pullPinReset_Finish;
            ...
            case Wait_pullPinReset_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到03F10上报，拉CCO复位管脚");
                    if(pullPinResetNum<=100)
                    {
                        pullPinReset_CCO(p_CtrInfoList,p_AbstractScriptHost);
                        pullPinResetNum++;
                        emScriptRunState=Wait_pullPinReset_Finish;
...
//                    cco=QString::fromLocal8Bit(ccoAddr);
//                    cco=QString::fromUtf8(reinterpret_cast<char*>(p_CtrInfoList->at(0)->ccoAddr),6);
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("查询主节点地址为%1，不符合要求")
                                                             .arg(QString(QByteArray(p_QueryMasterAddr_03F4_Up->master_node_address_.addr,6).toHex()))
                                                             );
                    }
                }
                ...
                else if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(meterIsExist(p_QueryNodeInfo_10F2_Up->node_info_data_unit_.node_info_group_list_.at(0).node_address_)==true)
                    {
                        ...
                        resultFlag=true;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("拉管脚复位100次测试成功"));
...
void Script_PullPinReset_100::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_PullPinReset_100::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_PullPinReset_100::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_PullPinReset_100::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_PullPinReset_100::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
    ...
    case Wait_pullPinReset_Finish:
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_pullPinReset_Finish timeout!!!");
        break;
    }
    case Wait_HardResetTest_Finish:
    {
...
//    {
//        if(emQueryState==Wait_QueryMasterAddr_03F4)
//        {
//            if(resendFlag==false)
//            {
//                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryMasterAddr_03F4);
//                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "第一次回复超时，重发--查询主节点地址（03F4），等待--回复");
//                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
//                resendFlag = true;
//                resendNum++;
//            }
...
//                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNodeNum_10F1);
//                emQueryState=Wait_QueryNodeNum_10F1;
//                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "第二次回复超时，发下一条--查询从节点数量（10F1），等待--回复");
//                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
//                resendFlag=false;
//            }
//        }
//        else if(emQueryState==Wait_QueryNodeNum_10F1)
//        {
//            if(resendFlag==false)
//            {
//                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNodeNum_10F1);
//                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "第一次回复超时，重发--查询从节点数量（10F1），等待--回复");
//                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
//                resendFlag = true;
//                resendNum++;
//            }
...
//                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNodeInfo_10F2);
//                emQueryState=Wait_QueryNodeInfo_10F2;
//                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "第二次回复超时，发下一条--查询从节点信息（10F2），等待--回复");
//                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
//                resendFlag=false;
//            }
//        }
//        else if(emQueryState==Wait_QueryNodeInfo_10F2)
//        {
//            if(resendFlag==false)
//            {
//                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNodeInfo_10F2);
//                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "第一次回复超时，重发--查询从节点信息（10F2），等待--回复");
//                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
//                resendFlag = true;
//                resendNum++;
//            }
...
//                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "第二次回复超时，发下一条--查询运行频段（03F16），等待--回复");
//                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
//                resendFlag=false;
//            }
//        }
////        else if(emQueryState==Wait_QueryFreq_03F16)
////        {
////            if(resendFlag==false)
////            {
////                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryFreq_03F16);
////                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "第一次回复超时，重发--查询运行频段（03F16），等待--回复");
////                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
////                resendFlag = true;
////                resendNum++;
////            }
...
////                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "第二次回复超时，发下一条--查询路由ID（F0F40），等待--回复");
////                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
////                resendFlag=false;
////            }
////        }
////        else if(emQueryState==Wait_QueryRouterID_F0F40)
////        {
////            if(resendFlag==false)
////            {
////                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryRouterID_F0F40);
////                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "第一次回复超时，重发--查询路由ID（F0F40），等待--回复");
////                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
////                resendFlag = true;
////                resendNum++;
////            }
...
////                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "第二次回复超时，发下一条--查询芯片ID（F0F40），等待--回复");
////                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
////                resendFlag=false;
////            }
////        }
...
////            {
////                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryChipID_F0F40);
////                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "第一次回复超时，重发--查询芯片ID（F0F40），等待--回复");
////                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
////                resendFlag = true;
////                resendNum++;
////            }
...
////                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "第二次回复超时，发下一条--查询路由SN（F0F41），等待--回复");
////                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
////                resendFlag=false;
////            }
////        }
//        else if(emQueryState==Wait_QueryRouterSN_F0F41)
//        {
//            if(resendFlag==false)
//            {
//                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryRouterSN_F0F41);
//                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "第一次回复超时，重发--查询路由SN（F0F41），等待--回复");
//                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
//                resendFlag = true;
//                resendNum++;
//            }
...
//                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "第二次回复超时，发下一条--查询路由工作开关（10F4），等待--回复");
//                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
//                resendFlag=false;
//            }
//        }
//        else if(emQueryState==Wait_QueryWorkSwitch_10F4)
//        {
//            if(resendFlag==false)
//            {
//                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryWorkSwitch_10F4);
//                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "第一次回复超时，重发--查询路由工作开关（10F4），等待--回复");
//                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
//                resendFlag = true;
//                resendNum++;
//            }
//            else
//            {
//                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "第二次回复超时");
//                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "有超时重发情况，重发总次数："+QString::number(resendNum));
//            }
//        }
//        else
//        {
//            p_AbstractScriptHost->updateProgress(ProcessState_Error, "emQueryState error");
//        }
//        break;
//    }
    default:
...
void Script_PullPinReset_100::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_pullPinReset_Finish:
...
void Script_PullPinReset_100::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
}

bool Script_PullPinReset_100::meterIsExist(Address meterAddr)
{
