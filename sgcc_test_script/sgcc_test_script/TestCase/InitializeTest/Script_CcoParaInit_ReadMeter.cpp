#include "Script_CcoParaInit_ReadMeter.h"

Script_CcoParaInit_ReadMeter::Script_CcoParaInit_ReadMeter(QObject *parent) : QObject(parent)
{
...
Script_CcoParaInit_ReadMeter::~Script_CcoParaInit_ReadMeter()
{
...
void Script_CcoParaInit_ReadMeter::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
void Script_CcoParaInit_ReadMeter::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
bool Script_CcoParaInit_ReadMeter::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_CcoParaInit_ReadMeter::execute()
{
    p_AbstractScriptHost->updateProgress(ProcessState_Start,"脚本执行流程描述打印：-----whc(2024年1月20)\r\n"
                                                             ...
                                                             "3.12F1重启路由命令，等待14F1路由主动抄表，回复单相抄表数据，10F4查询路由运行状态，Wait_QueryParaInit_Finish；\r\n"
...
void Script_CcoParaInit_ReadMeter::stop()
{
    p_timer->stop();
    p_delayTimer->stop();
    p_maxAllowTimer->stop();
...
void Script_CcoParaInit_ReadMeter::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_CcoParaInit_ReadMeter::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_CcoParaInit_ReadMeter::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        case Wait_QueryInitialState_Finish:
        {
            ...
            else  if(p_Frame3762Base->afn_== char(0x00) && dtValue3762==1 && p_Frame3762Base->ctrl_field_.dir==kDirUp)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, ">>>>延时90s，进行03F16查询路由频段");
            }
        ...
        case Wait_SetAreaIdentification_05F6:
        {
            ...
            else if(p_Frame3762Base->afn_ == 0x10 && dtValue3762==4 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                {
                    emScriptRunState = Wait_QueryParaInit_Finish;
                    tryTimes =0;
                    index =0;
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed,  QString(">>>Wait_SetAreaIdentification_05F6，10F4查询事件上报开关与设置不一致测试失败，收到事件上报开关为：%1，台区区分开关为：%2").arg(event_report_flag_10F4).arg(area_difference_flag_10F4)+test_name_);
            }
            else
            {
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到其他376.2报文，等待--回复");
        ...
        case Wait_QueryParaInit_Finish:
        {
            if(p_Frame3762Base->afn_== char(0x00) && dtValue3762==1 && p_Frame3762Base->ctrl_field_.dir==kDirUp)
            {
                p_timer->stop();
            ...
            else if(p_Frame3762Base->afn_ == 0x10 && dtValue3762==4 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if (current_state == 0x03) // 14F1下发读取成功，路由收到不下发载波抄表，状态应为其他，
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed,  QString(">>>Wait_QueryParaInit_Finish，路由回复当前状态与期望不一致测试失败，路由当前状态为：%1，期望为：3,%3。").arg(current_state).arg(test_name_));
            }
            else
            {
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到其他376.2报文，等待--回复");
        ...
        case Wait_QueryCheckState_Finish:
        {
            ...
            else if(p_Frame3762Base->afn_ == 0x10 && dtValue3762==4 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString(">>>>期望10F40查询回来路由运行状态，事件上报开关为：%1，台区区分开关为：%2，当前状态为：03").arg(event_report_flag_10F4_).arg(area_difference_flag_10F4_));
                if (event_report_flag_10F4 == event_report_flag_10F4_ && area_difference_flag_10F4==area_difference_flag_10F4_ && current_state == 3)
                {
                    tryTimes=0;
                    index=0;
                    emQueryState=Wait_QueryMasterAddr_03F4;
...
void Script_CcoParaInit_ReadMeter::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_CcoParaInit_ReadMeter::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_CcoParaInit_ReadMeter::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_CcoParaInit_ReadMeter::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_CcoParaInit_ReadMeter::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
    case Wait_BuildNetFinish_Whole:
    ...
    case Wait_SetAreaIdentification_05F6:
    {
        ...
        if(++tryTimes<3)
        {
            ...
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_QueryParaInit_Finish，查询路由工作状态（10F4）3次超时失败")+test_name_);
        break;
    }
    case Wait_QueryParaInit_Finish:
    {
...
void Script_CcoParaInit_ReadMeter::maxAllowTimer_timeoutProc()
{
    p_delayTimer->stop();
    switch(emScriptRunState)
    {
...
void Script_CcoParaInit_ReadMeter::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    switch(emScriptRunState)
    {
    ...
    case Wait_QueryParaInit_Finish:
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString(">>>>Wait_QueryParaInit_Finish，延时时间到，未收到期望报文测试失败，")+test_name_);
        break;
    }
    case Wait_QueryCheckState_Finish:
    {
...
bool Script_CcoParaInit_ReadMeter::meterIsExist(Address meterAddr)
{
