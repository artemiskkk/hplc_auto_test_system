#include "Script_InitializeTest_Temp.h"

Script_InitializeTest_Temp::Script_InitializeTest_Temp(QObject *parent) : QObject(parent)
{
...
Script_InitializeTest_Temp::~Script_InitializeTest_Temp()
{
...
void Script_InitializeTest_Temp::execute()
{
...
void Script_InitializeTest_Temp::stop()
{
...
void Script_InitializeTest_Temp::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_InitializeTest_Temp::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_InitializeTest_Temp::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_InitializeTest_Temp::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_InitializeTest_Temp::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_InitializeTest_Temp::processMsgFromCCO(DvcType dvcType, int dvcId)
{
            ...
            case Wait_HardResetTest_Finish:
            {
                ...
                else if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==char(0x80)&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(uchar(p_QueryFreq_03F16_Up->carrier_frequence_range_)==runFreq)
                    {
                        ...
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--查询路由ID（F0F40），等待--回复");
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    }
                    else
                    {
...
void Script_InitializeTest_Temp::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_InitializeTest_Temp::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_InitializeTest_Temp::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_InitializeTest_Temp::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_InitializeTest_Temp::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
...
void Script_InitializeTest_Temp::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_HardResetTest_Finish:
...
void Script_InitializeTest_Temp::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    p_AbstractScriptHost->updateProgress(ProcessState_Success, "5s到");
...
bool Script_InitializeTest_Temp::meterIsExist(Address meterAddr)
{
