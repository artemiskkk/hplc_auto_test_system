#include "Script_TransparentTrans_NotBuildNet.h"

Script_TransparentTrans_NotBuildNet::Script_TransparentTrans_NotBuildNet(QObject *parent) : QObject(parent)
{
...
Script_TransparentTrans_NotBuildNet::~Script_TransparentTrans_NotBuildNet()
{
...
void Script_TransparentTrans_NotBuildNet::execute()
{
    ...
    else
    {
        ...
        emScriptRunState=Wait_TransmitData_Finish;
        emSetQueryState=Wait_SetDAC_Finish;
...
void Script_TransparentTrans_NotBuildNet::stop()
{
...
void Script_TransparentTrans_NotBuildNet::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_TransparentTrans_NotBuildNet::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_TransparentTrans_NotBuildNet::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_TransparentTrans_NotBuildNet::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_TransparentTrans_NotBuildNet::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_TransparentTrans_NotBuildNet::processMsgFromCCO(DvcType dvcType, int dvcId)
{
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_TransmitData_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x02&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("透传回复数据正常:%1").arg(QString::fromLatin1(p_TransmitData_02F1_Up->frame_content_.toHex().toUpper())));
                        if(++index>4)
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Success,"透传功能测试成功");
                        }
                        else
                        {
                            tryTimes=0;
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("透传回复数据异常:%1").arg(QString::fromLatin1(p_TransmitData_02F1_Up->frame_content_.toHex().toUpper())));
                    }
...
//                                resultFlag=true;
//                                p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("透传功能测试成功,支持不组网透传！"));
//                            }
//                            else
//                            {
...
void Script_TransparentTrans_NotBuildNet::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_TransparentTrans_NotBuildNet::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_TransparentTrans_NotBuildNet::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_TransparentTrans_NotBuildNet::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_TransparentTrans_NotBuildNet::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
        ...
        if(++tryTimes>=3)
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_TransmitData_Finish timeout!!!");
        }
        else
        {
            sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_TransmitData_02F1);
            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "超时重发--转发数据（02F1），等待--确认");
            p_timer->start(STA_CMD_TIMEOUT_TIME*1000);
        }
            break;
        }
...
void Script_TransparentTrans_NotBuildNet::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_TransmitData_Finish:
...
void Script_TransparentTrans_NotBuildNet::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    index=0;
...
QByteArray Script_TransparentTrans_NotBuildNet::getSetFrame(ParaType type)
{
    QByteArray msg;
...
QByteArray Script_TransparentTrans_NotBuildNet::getQueryFrame(ParaType type)
{
    QByteArray msg;
