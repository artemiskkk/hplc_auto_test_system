#include "Script_TransparentTrans_RepeatRead.h"


Script_TransparentTrans_RepeatRead::Script_TransparentTrans_RepeatRead(QObject *parent) : QObject(parent)
{
...
Script_TransparentTrans_RepeatRead::~Script_TransparentTrans_RepeatRead()
{
...
void Script_TransparentTrans_RepeatRead::execute()
{
    QString step_desc = "流程描述: \r";
    step_desc += "1.组网通用流程，SENCE3(需要组网) Wait_BuildNetFinish_Whole\r";
    step_desc += "2.连续下发2帧02F1数据转发命令，通信协议类型0X00，采用645协议，抄读645电表数据 Wait_TransmitData02F1_1_Finish Wait_TransmitData02F1_1_Finish\r";
    step_desc += "3.连续下发2帧02F1数据转发命令，报文内容为应用层扩展报文-参数查询”，抄读645电表 Wait_TransmitData02F1_2_Finish\r";
...
void Script_TransparentTrans_RepeatRead::stop()
{
...
void Script_TransparentTrans_RepeatRead::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_TransparentTrans_RepeatRead::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_TransparentTrans_RepeatRead::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_TransparentTrans_RepeatRead::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
    ...
    if(emScriptRunState==Wait_BuildNetFinish_Whole)
    {
        ...
        else
        {
            if(p_BuildNetwork_GW->emScriptRunState==BuildNetFinish && p_BuildNetwork_GW->buildNetworkResultFlag==true)
            {
                ...
                tryTimes=0;
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "开始第一轮透传--连续下发2帧02F1数据转发命令，通信协议类型0X00，采用645协议，抄读645电表数据");
                emScriptRunState = Wait_TransmitData02F1_1_Finish;
...
void Script_TransparentTrans_RepeatRead::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_TransparentTrans_RepeatRead::processMsgFromCCO(DvcType dvcType, int dvcId)
{
            ...
            case Wait_TransmitData02F1_1_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x02&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    else if(p_TransmitData_02F1_Up->protocol_type_==0x00 && p_TransmitData_02F1_Up->frame_length_==0)
                    {
                        if(++tryTimes>=3)
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_TransmitData02F1_1_Finish透传3次回复数据均为空，表号%0 【GW-CCO-F006-0004-V01】").arg(QString(QByteArray(p_TransmitData_02F1_Up->address_field_.src_addr,6).toHex())));
                        }
                        else
                        {
                            sendMsg(CCO_GW, p_CtrInfoList->at(0)->ctrlID, INSIGNIFICANCE, p_TransmitData_02F1);
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_TransmitData02F1_1_Finish 透传回复数据异常，数据:%1【GW-CCO-F006-0004-V01】").arg(QString::fromLatin1(p_TransmitData_02F1_Up->frame_content_.toHex().toUpper())));
                    }
                }
                ...
                {
//                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_TransmitData02F1_1_Finish 连续下发两条02F1 路由回复否认【GW-CCO-F006-0004-V01】"));
                    p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("连续下发两条02F1 路由回复否认 符合预期"));
                    break;
                }
                else
                {
        ...
        case Wait_TransmitData02F1_2_Finish:
        {
            if(p_Frame3762Base->afn_ == 0x02&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                else if(p_TransmitData_02F1_Up->protocol_type_==0x00 && p_TransmitData_02F1_Up->frame_length_==0)
                {
                    if(++tryTimes>=3)
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_TransmitData02F1_2_Finish透传3次回复数据均为空，表号%0【GW-CCO-F006-0004-V01】").arg(QString(QByteArray(p_TransmitData_02F1_Up->address_field_.src_addr,6).toHex())));
                        break;
                    }
                    else
                    {
                ...
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_TransmitData02F1_2_Finish 透传回复数据异常，数据:%1 【GW-CCO-F006-0004-V01】").arg(QString::fromLatin1(p_TransmitData_02F1_Up->frame_content_.toHex().toUpper())));
                }
            }
            else if(p_Frame3762Base->afn_ == 0x02&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("连续下发两条02F1 路由回复否认 符合预期"));
                break;
            }
...
void Script_TransparentTrans_RepeatRead::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_TransparentTrans_RepeatRead::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_TransparentTrans_RepeatRead::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    if(frame == p_TransmitData_02F1)
    {
        ...
        else if(emScriptRunState == Wait_TransmitData02F1_2_Finish)
        {
            ...
            if(comPrtclType == 0x02)  // 645-07表
            {
                p_TransmitData_02F1->protocol_type_=0x04;             // 使用0x04应用层协议  透传应用层报文给645电表
...
void Script_TransparentTrans_RepeatRead::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_TransparentTrans_RepeatRead::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
        ...
        case Wait_TransmitData02F1_1_Finish:
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_TransmitData02F1_1_Finish timeout!!!");
            break;
        }
        case Wait_TransmitData02F1_2_Finish:
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_TransmitData02F1_2_Finish timeout!!!");
            break;
        }
...
void Script_TransparentTrans_RepeatRead::maxAllowTimer_timeoutProc()
{
    p_maxAllowTimer->stop();
    p_AbstractScriptHost->updateProgress(ProcessState_Error, "State machine run error!!!==p_maxAllowTimer");
}

void Script_TransparentTrans_RepeatRead::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    index=0;
    p_maxAllowTimer->stop();
    if(emScriptRunState == Wait_TransmitData02F1_1_Finish)
    {
        ...
        tryTimes=0;
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "开始第二轮透传--连续下发2帧02F1数据转发命令，报文内容为应用层扩展报文-参数查询”，抄读645电表");
        sendMsg(CCO_GW, p_CtrInfoList->at(0)->ctrlID, INSIGNIFICANCE, p_TransmitData_02F1);
        msgSeq -= 1;
        sendMsg(CCO_GW, p_CtrInfoList->at(0)->ctrlID, INSIGNIFICANCE, p_TransmitData_02F1);
    ...
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("脚本执行成功【GW-CCO-F006-0004-V01】"));
        emScriptRunState = ScriptSuccess;
    }
}

QByteArray Script_TransparentTrans_RepeatRead::getSetFrame(ParaType type)
{
    QByteArray msg;
...
QByteArray Script_TransparentTrans_RepeatRead::getQueryFrame(ParaType type)
{
    QByteArray msg;
