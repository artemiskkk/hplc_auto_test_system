#include "EventReportPrepare.h"
EventReportPrepare::EventReportPrepare(shared_ptr<QList<shared_ptr<CtrInfo>>> p_CtrInfoList,AbstractScriptHost *host,QObject *parent) :
    QObject(parent),p_AbstractScriptHost(host),p_CtrInfoList(p_CtrInfoList)
{
...
EventReportPrepare::~EventReportPrepare()
{

}

void EventReportPrepare::execute()
{
    p_AbstractScriptHost->updateProgress(ProcessState_Start, "事件主动上报准备流程开始!");
...
//    p_AbstractScriptHost->controlDvc(ThreeSTA,QList<int>()<<p_CtrInfoList->at(0)->dvcId,CtrlCmd_SetBaudRate,QList<double>()<<m_Baud);
//    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("发送设置三通工装波特率%1bps，等待确认").arg(QString::number(m_Baud)));
    delay(2000);
    //复位单三通模块
...
//    p_AbstractScriptHost->controlDvc(ThreeSTA,QList<int>()<<p_CtrInfoList->at(0)->dvcId,CtrlCmd_ModuleRST,QList<double>());
//    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("发送复位三通模块，等待确认").arg(QString::number(m_Baud)));
    //等待单三通读表号，120s内
    p_timer->start(120*1000);
}

void EventReportPrepare::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void EventReportPrepare::processCtrlDvcRes(DvcType dvcType, QList<int> , CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
...
void EventReportPrepare::processMsgFromCCO(DvcType dvcType, int dvcId)
{
            ...
            case Wait_BuildNet_Finish:
            {
                ...
                else if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp&&uchar(p_Frame3762Base->info_field_.info_field_up.msg_seq)==msgSeq)
                {
                    ...
                    if(flagParaInit)
                    {
                        ...
                        emit signalNoticeEventReportPrepareState(ProcessState_Failed,state);
                    }
                    else
                    {
                    ...
                    if(ptr->network_typelogy_info_unit_.this_node_num_==2)
                    {
                           ...
                           ||ptr->network_typelogy_info_unit_.network_typelogy_info_List_.last().node_address_==Address(p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->value(p_CtrInfoList->at(0)->keyList.at(index))->mtrAddr))
                        {
                            QString state=QString("事件主动上报流程%1，组网完成")
                                    .arg(metaEnum.valueToKey(emScriptRunState));
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing,state);
                            emit signalNoticeEventReportPrepareState(ProcessState_Processing,state);
                        }
                        else
                        {
...
void EventReportPrepare::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void EventReportPrepare::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void EventReportPrepare::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    if(frame==p_Afn01F2)
    {
        ...
        p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("》》参数初始化01F2：%1\n").arg(QString(sendMsgOct.toHex())));
    }
    else if(frame==p_Afn11F1)
    {
        ...
        p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("》》添加从节点11F1：%1\n").arg(QString(sendMsgOct.toHex())));
    }
    else if(frame==p_Afn10F21)
    {
        ...
        p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("》》查询网络拓扑10F21：%1\n").arg(QString(sendMsgOct.toHex())));
    }
...
void EventReportPrepare::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void EventReportPrepare::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNet_Finish:
        {
            ...
            if(flagParaInit)
            {
                if(++tryTimes>=3)
                {
                    QString state=QString("事件主动上报准备流程%1，参数初始化回复超时")
                            .arg(metaEnum.valueToKey(emScriptRunState));
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing,state);
                    emit signalNoticeEventReportPrepareState(ProcessState_Failed,state);
                }
            ...
            else
            {
                if(++tryTimes>=3)
                {
                    QString state=QString("事件主动上报准备流程%1，添加从节点回复超时")
                            .arg(metaEnum.valueToKey(emScriptRunState));
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing,state);
                    emit signalNoticeEventReportPrepareState(ProcessState_Failed,state);
                }
...
void EventReportPrepare::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        default:
...
void EventReportPrepare::delayTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNet_Finish:
