#ifndef BAUDCONSULTPREPARE_H
#define BAUDCONSULTPREPARE_H

#include <QObject>
#include "../../BuildNetwork_GW.h"
class BaudConsultPrepare : public QObject
{
    Q_OBJECT
public:
    explicit BaudConsultPrepare(shared_ptr<QList<shared_ptr<CtrInfo>>> p_CtrInfoList,AbstractScriptHost *host,int baud=9600,bool isCCO=true,QObject *parent = nullptr);
    ~BaudConsultPrepare();
    enum BaudConsultPrepare_State
    {
        ScriptInit,
...
private:
    Q_ENUM(BaudConsultPrepare_State)
    BaudConsultPrepare_State emScriptRunState=ScriptInit;
    QMetaEnum metaEnum=QMetaEnum::fromType<BaudConsultPrepare_State>();
...
signals:
    void signalNoticeBaudConsultPrepareState(ProcessState,const QString& state);
public:
