#include "Script_20Standard_BaudConsult_Abnormal_CCO.h"
Script_20Standard_BaudConsult_Abnormal_CCO::Script_20Standard_BaudConsult_Abnormal_CCO(QObject *parent) : QObject(parent)
{
...
Script_20Standard_BaudConsult_Abnormal_CCO::~Script_20Standard_BaudConsult_Abnormal_CCO()
{
...
void Script_20Standard_BaudConsult_Abnormal_CCO::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
void Script_20Standard_BaudConsult_Abnormal_CCO::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
bool Script_20Standard_BaudConsult_Abnormal_CCO::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_20Standard_BaudConsult_Abnormal_CCO::execute()
{
    ...
    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "流程描述\n"
                                         ...
                                         "3.波特率协商115200bps，不回复 Wait_BaudConsult_115200bps_Finish\n"
                                         "4.波特率协商19200bps，错误回复   Wait_BaudConsult_19200bps_Finish\n"
                                         "5.波特率协商9600bps，不回复  Wait_BaudConsult_9600bps_Finish\n"
                                         "6.波特率协商4800bps，错误回复  Wait_BaudConsult_4800bps_Finish\n"
                                         "7.波特率协商2400bps，不回复  Wait_BaudConsult_2400bps_Finish\n"
                                         "8.波特率协商1200bps，错误回复  Wait_BaudConsult_1200bps_Finish\n");
...
void Script_20Standard_BaudConsult_Abnormal_CCO::stop()
{
    p_timer->stop();
    p_delayTimer->stop();
...
void Script_20Standard_BaudConsult_Abnormal_CCO::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
    ...
    if(emScriptRunState==Wait_BuildNetFinish_Whole)//场景2-4
    {
        ...
        else
        {
            ...
            connect(p_BaudConsultPrepare,&BaudConsultPrepare::signalNoticeBaudConsultPrepareState,this,&Script_20Standard_BaudConsult_Abnormal_CCO::slotNoticeBaudConsultPrepareState);
...
void Script_20Standard_BaudConsult_Abnormal_CCO::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_20Standard_BaudConsult_Abnormal_CCO::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
        {
            ...
            p_AbstractScriptHost->updateProgress(ProcessState_Failed,QString("收到CCO上报03F10，疑似复位，运行状态为%1"+testCase).arg(metaEnum.valueToKey(emScriptRunState)));
            return;
        }
        switch(emScriptRunState)
        {
...
void Script_20Standard_BaudConsult_Abnormal_CCO::slotNoticeBaudConsultPrepareState(ProcessState pState, const QString &state)
{
    switch(emScriptRunState)
    {
        case Wait_BaudConsultPrepare_Finish:
        {
            if(pState==ProcessState_Processing)
            {
                ...
                m_SpecialType=BaudConsult::Abnormal_NoResponse;
                p_BaudConsult=new BaudConsult(p_CtrInfoList,p_AbstractScriptHost,int(emScriptRunState),true,m_SpecialType);
                connect(p_BaudConsult,&BaudConsult::signalNoticeBaudConsultState,this,&Script_20Standard_BaudConsult_Abnormal_CCO::slotNoticeBaudConsultState);
...
void Script_20Standard_BaudConsult_Abnormal_CCO::slotNoticeBaudConsultState(ProcessState pState, const QString &state)
{
    if(p_BaudConsult!=nullptr)
        delete p_BaudConsult;
...
//                    else if(emScriptRunState==Wait_BaudConsult_1200bps_Finish)
                    ...
                    m_SpecialType=(m_SpecialType==BaudConsult::Abnormal_NoResponse?BaudConsult::Abnormal_WrongResponse:BaudConsult::Abnormal_NoResponse);
                    p_BaudConsult=new BaudConsult(p_CtrInfoList,p_AbstractScriptHost,int(emScriptRunState),true,m_SpecialType);
                    connect(p_BaudConsult,&BaudConsult::signalNoticeBaudConsultState,this,&Script_20Standard_BaudConsult_Abnormal_CCO::slotNoticeBaudConsultState);
                    p_BaudConsult->execute();
                }
...
void Script_20Standard_BaudConsult_Abnormal_CCO::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_20Standard_BaudConsult_Abnormal_CCO::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_20Standard_BaudConsult_Abnormal_CCO::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_20Standard_BaudConsult_Abnormal_CCO::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_20Standard_BaudConsult_Abnormal_CCO::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_20Standard_BaudConsult_Abnormal_CCO::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        default:
...
void Script_20Standard_BaudConsult_Abnormal_CCO::delayTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        default:
