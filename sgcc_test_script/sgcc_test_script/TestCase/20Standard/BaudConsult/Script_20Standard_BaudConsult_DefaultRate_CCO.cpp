#include "Script_20Standard_BaudConsult_DefaultRate_CCO.h"

Script_20Standard_BaudConsult_DefaultRate_CCO::Script_20Standard_BaudConsult_DefaultRate_CCO(QObject *parent) : QObject(parent)
{
...
Script_20Standard_BaudConsult_DefaultRate_CCO::~Script_20Standard_BaudConsult_DefaultRate_CCO()
{
...
void Script_20Standard_BaudConsult_DefaultRate_CCO::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
void Script_20Standard_BaudConsult_DefaultRate_CCO::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
bool Script_20Standard_BaudConsult_DefaultRate_CCO::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_20Standard_BaudConsult_DefaultRate_CCO::execute()
{
    ...
    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "流程描述\n"
                                         ...
                                         "3.波特率协商115200bps Wait_BaudConsult_115200bps_Finish\n"
                                         "4.等待24小时不通信   Wait_24hNoCommunication_Finish\n"
                                         "5.等待115200bps抄表(应该超时)  Wait_ReadMeter_115200bps_Finish\n"
                                         "6.等待9600bps抄表(正常抄读)  Wait_ReadMeter_9600bps_Finish\n");
...
//        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
    ...
    p_maxAllowTimer->start((timerForReachThresld+timerAfterReachThresld+24*3600)*1000);//此处要启动脚本的最大执行时间定时器
}
void Script_20Standard_BaudConsult_DefaultRate_CCO::stop()
{
    p_timer->stop();
    p_delayTimer->stop();
...
void Script_20Standard_BaudConsult_DefaultRate_CCO::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
    ...
    if(emScriptRunState==Wait_BuildNetFinish_Whole)//场景2-4
    {
        ...
        else
        {
            ...
            connect(p_BaudConsultPrepare,&BaudConsultPrepare::signalNoticeBaudConsultPrepareState,this,&Script_20Standard_BaudConsult_DefaultRate_CCO::slotNoticeBaudConsultPrepareState);
...
void Script_20Standard_BaudConsult_DefaultRate_CCO::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_20Standard_BaudConsult_DefaultRate_CCO::processMsgFromCCO(DvcType dvcType, int dvcId)
{
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_ReadMeter_115200bps_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x02&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp&&uchar(p_Frame3762Base->info_field_.info_field_up.msg_seq)==msgSeq)
                {
                    ...
                    {
                        QString state=QString("波特率协商-恢复默认波特率%1，115200bps抄表回复数据为空，符合要求")
                        ...
                        baud=115200;
                        //设置单通工装波特率
                        p_AbstractScriptHost->controlDvc(SingleSTA,QList<int>()<<p_CtrInfoList->at(0)->dvcId,CtrlCmd_SetBaudRate,QList<double>()<<int(baud));
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("发送设置单通工装波特率%1bps，等待确认").arg(QString::number(int(baud))));
                    ...
                    {
                        QString state=QString("波特率协商-恢复默认波特率%1，115200bps抄表回复数据不为空，不符合要求")
                                .arg(metaEnum.valueToKey(emScriptRunState));
...
void Script_20Standard_BaudConsult_DefaultRate_CCO::slotNoticeBaudConsultPrepareState(ProcessState pState, const QString &state)
{
    switch(emScriptRunState)
    {
        case Wait_BaudConsultPrepare_Finish:
        {
            if(pState==ProcessState_Processing)
            {
                ...
                p_BaudConsult=new BaudConsult(p_CtrInfoList,p_AbstractScriptHost,int(emScriptRunState));
                connect(p_BaudConsult,&BaudConsult::signalNoticeBaudConsultState,this,&Script_20Standard_BaudConsult_DefaultRate_CCO::slotNoticeBaudConsultState);
...
void Script_20Standard_BaudConsult_DefaultRate_CCO::slotNoticeBaudConsultState(ProcessState pState, const QString &state)
{
    if(p_BaudConsult!=nullptr)
        delete p_BaudConsult;
            ...
            if(pState==ProcessState_Processing)
            {
                ...
                else
                {
                    if(emScriptRunState==Wait_BaudConsult_115200bps_Finish)
                        emScriptRunState=Wait_24hNoCommunication_Finish;
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("等待24h小时不通信"));
//                    BaudConsult::delay(24*3600*1000);
...
void Script_20Standard_BaudConsult_DefaultRate_CCO::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_20Standard_BaudConsult_DefaultRate_CCO::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_20Standard_BaudConsult_DefaultRate_CCO::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    if(frame==p_Afn02F1)
    {
...
void Script_20Standard_BaudConsult_DefaultRate_CCO::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_20Standard_BaudConsult_DefaultRate_CCO::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_20Standard_BaudConsult_DefaultRate_CCO::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        default:
...
void Script_20Standard_BaudConsult_DefaultRate_CCO::delayTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        default:
