#include "Script_STAHighPowerConfig_TurnOffHighPowerConfig.h"

Script_STAHighPowerConfig_TurnOffHighPowerConfig::Script_STAHighPowerConfig_TurnOffHighPowerConfig(QObject *parent) : QObject(parent)
{
...
Script_STAHighPowerConfig_TurnOffHighPowerConfig::~Script_STAHighPowerConfig_TurnOffHighPowerConfig()
{
...
void Script_STAHighPowerConfig_TurnOffHighPowerConfig::execute()
{
...
void Script_STAHighPowerConfig_TurnOffHighPowerConfig::stop()
{
...
void Script_STAHighPowerConfig_TurnOffHighPowerConfig::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_STAHighPowerConfig_TurnOffHighPowerConfig::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_STAHighPowerConfig_TurnOffHighPowerConfig::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_STAHighPowerConfig_TurnOffHighPowerConfig::processMsg(DvcType dvcType,int id,uchar* data,int datalen)
{
...
void Script_STAHighPowerConfig_TurnOffHighPowerConfig::processCtrlDvcRes(DvcType dvcType,QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
    ...
    switch(emScriptRunState)
    {
        ...
        case Wait_ConfigSuperpowerPara_Normal_F0F21_Finish:
        {
            break;
        }
        case Wait_ConfigSuperpowerPara_Close_F0F21_Finish:
        {
            break;
        }
        case Wait_QueryConfigureSuperpowerParaNormal_Finish:
        {
            break;
        }
        case Wait_QuerySuperpowerPara_ff_ff_Finish:
        {
            break;
        }
        case Wait_QuerySTASuperpowerParaClose_Finish:
        {
            break;
        }
        case ScriptSuccess:
...
void Script_STAHighPowerConfig_TurnOffHighPowerConfig::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_STAHighPowerConfig_TurnOffHighPowerConfig::processMsgFromCCO(DvcType dvcType, int dvcId)
{
...
//                    p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("上报数据长度为%1").arg(p_TransparentTransmit_02F1_Up->frame_length_));
                            ...
                            &&((p_TransparentTransmit_02F1_Up->frame_content_.at(33)>=5&&p_TransparentTransmit_02F1_Up->frame_content_.at(33)<=25)||p_TransparentTransmit_02F1_Up->frame_content_.at(33)==0x00))
                    {
                        ...
                        if(index==p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->size())
                        {
                            if((1-(nodeInfoList.size()/p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->size()))>=0.9)
                            {
                                ...
                                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "配置大功率参数0f_00（F0F21），等待--确认");
                                emScriptRunState=Wait_ConfigSuperpowerPara_Normal_F0F21_Finish;
                    ...
                    else if(p_TransparentTransmit_02F1_Up->frame_length_==0)
                    {
                        if(++tryTimes>=3)
                        {
                            ...
                            index++;
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed,"测试失败【GW-STA-F021-0009-V01】大功率配置-关闭大功率配置");
//                            if(index<=p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->size())
//                            {
...
//                            }
            ...
            case Wait_ConfigSuperpowerPara_Normal_F0F21_Finish:
            {
                ...
                {
                    p_timer->stop();
                    emScriptRunState=Wait_QueryConfigureSuperpowerParaNormal_Finish;
            ...
            case Wait_QueryConfigureSuperpowerParaNormal_Finish:
            {
                if(p_Frame3762Base->afn_ == char(0xF0)&&p_Frame3762Base->dt1_==0x20&&p_Frame3762Base->dt2_==0x02&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(p_QueryConfigSuperpowerPara_F0F22_Up->dac_voltage_==0x0f&&p_QueryConfigSuperpowerPara_F0F22_Up->peak_clipping_factor_==0x00)
                    {
                        ...
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_ConfigSuperpowerPara_F0F21);
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "配置大功率参数ff_ff（F0F21），等待--确认");
                        emScriptRunState=Wait_ConfigSuperpowerPara_Close_F0F21_Finish;
                        p_timer->start(60*1000);
                    }
                    else
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("F0F22查询配置大功率ff_ff，回复数据异常"));
                    }
                }
                else
                {
            ...
            case Wait_ConfigSuperpowerPara_Close_F0F21_Finish:
            {
                if(p_Frame3762Base->afn_ == char(0x00)&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    p_timer->stop();
                    emScriptRunState=Wait_QuerySuperpowerPara_ff_ff_Finish;

                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,0,p_QueryConfigSuperpowerPara_F0F22);
                    p_timer->start(10*1000);
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("发送F0F22查询配置大功率参数-ff_ff，等待--回复"));
                }
                else
                {
                    QByteArray tmpSendMsg=prcsOther3762Msg(uchar(p_Frame3762Base->afn_),p_Frame3762Base->dt1_,p_Frame3762Base->dt2_,uchar(p_Frame3762Base->info_field_.info_field_up.msg_seq));
            ...
            case Wait_QuerySuperpowerPara_ff_ff_Finish:
            {
                ...
                {
                    shared_ptr<AfnF0F22> p_QueryConfigSuperpowerPara_F0F22_Up=dynamic_pointer_cast<AfnF0F22>(p_Frame3762Base);
                    if(p_QueryConfigSuperpowerPara_F0F22_Up->dac_voltage_==char(0xff)&&p_QueryConfigSuperpowerPara_F0F22_Up->peak_clipping_factor_==char(0xff))
                    {
                        qDebug()<<"收到F0F22配置大功率参数(关闭)_Up报文******"<<"状态机的值："<<emScriptRunState;
                        p_timer->stop();
                        index = 0;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "延时10min查询配置大功率参数0f_00，等待--开始查询");
                        p_delayTimer->start(10*60*1000);
                    }
                    else
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("F0F22查询配置大功率，回复数据异常"));
                    }
                }
                else
                {
            ...
            case Wait_QuerySTASuperpowerParaClose_Finish:
            {
                ...
                {
                    p_timer->stop();
                    shared_ptr<Afn02F1> p_QuerySTASuperpowerParaNormal_Up=dynamic_pointer_cast<Afn02F1>(p_Frame3762Base);

                    if(p_QuerySTASuperpowerParaNormal_Up->frame_length_==0x22)
                    {
                        if(p_QuerySTASuperpowerParaNormal_Up->frame_content_.at(28)==nodeParaList.at(index).dac_voltage
                                &&p_QuerySTASuperpowerParaNormal_Up->frame_content_.at(33)==nodeParaList.at(index).peak_clipping_factor)
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("透传回复数据(开启):%1").arg(QString::fromLatin1(p_QuerySTASuperpowerParaNormal_Up->frame_content_.toHex().toUpper())));
                            if(++index==p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->size())
                            {
                                if((1-(nodeInfoList.size()/p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->size()))>=0.9)
                                {
                                    ...
                                    resultFlag=true;
                                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("大功率配置-关闭大功率配置-测试成功;"));
                        ...
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("透传回复数据异常，数据:%1").arg(QString::fromLatin1(p_QuerySTASuperpowerParaNormal_Up->frame_content_.toHex().toUpper())));
                        }
                    }

                    else if(p_QuerySTASuperpowerParaNormal_Up->frame_length_==0)
                    {
                        if(++tryTimes>=3)
                        {
                            NodeInfo_Struct eventST;
...
void Script_STAHighPowerConfig_TurnOffHighPowerConfig::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_STAHighPowerConfig_TurnOffHighPowerConfig::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_STAHighPowerConfig_TurnOffHighPowerConfig::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    sendMsgOct.clear();
...
void Script_STAHighPowerConfig_TurnOffHighPowerConfig::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
        ...
        case Wait_ConfigSuperpowerPara_Normal_F0F21_Finish:
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_ConfigSuperpowerPara_Normal_F0F21_Finish timeout!!!");
            break;
        }
        case Wait_QueryConfigureSuperpowerParaNormal_Finish:
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_QueryConfigureSuperpowerParaNormal_Finish timeout!!!");
            break;
        }
        default:
        {
...
void Script_STAHighPowerConfig_TurnOffHighPowerConfig::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    ...
    case Wait_QuerySuperpowerPara_ff_ff_Finish:
    {
        emScriptRunState = Wait_QuerySTASuperpowerParaClose_Finish;
        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,0,p_TransparentTransmit_02F1);
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "查询STA功率参数，等待--回复2");
        p_timer->start(30*1000);
        break;
    }
    default:
...
QByteArray Script_STAHighPowerConfig_TurnOffHighPowerConfig::getQueryFrame(ParaType type)
{
    QByteArray msg;
...
QByteArray Script_STAHighPowerConfig_TurnOffHighPowerConfig::getSetFrame(ParaType type)
{
    QByteArray msg;
...
void Script_STAHighPowerConfig_TurnOffHighPowerConfig::maxAllowTimer_timeoutProc()
{
    ...
    switch(emScriptRunState)
    {
        ...
        case Wait_ConfigSuperpowerPara_Normal_F0F21_Finish:
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_ConfigSuperpowerPara_Normal_F0F21_Finish maxAllow timeout!!!");
            break;
        }
        case Wait_QueryConfigureSuperpowerParaNormal_Finish:
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_QueryConfigureSuperpowerParaNormal_Finish maxAllow timeout!!!");
            break;
        }
        case Wait_ConfigSuperpowerPara_Close_F0F21_Finish:
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_QuerySTASuperpowerParaNormal_Finish maxAllow timeout!!!");
            break;
        }
        case Wait_QuerySuperpowerPara_ff_ff_Finish:
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_QuerySuperpowerPara_ff_ff_Finish maxAllow timeout!!!");
            break;
        }
        case Wait_QuerySTASuperpowerParaClose_Finish:
        {
