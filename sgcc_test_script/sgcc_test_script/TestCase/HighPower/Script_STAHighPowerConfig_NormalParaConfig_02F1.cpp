#include "Script_STAHighPowerConfig_NormalParaConfig_02F1.h"

Script_STAHighPowerConfig_NormalParaConfig_02F1::Script_STAHighPowerConfig_NormalParaConfig_02F1(QObject *parent) : QObject(parent)
{
...
Script_STAHighPowerConfig_NormalParaConfig_02F1::~Script_STAHighPowerConfig_NormalParaConfig_02F1()
{
...
void Script_STAHighPowerConfig_NormalParaConfig_02F1::execute()
{
...
void Script_STAHighPowerConfig_NormalParaConfig_02F1::stop()
{
...
void Script_STAHighPowerConfig_NormalParaConfig_02F1::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_STAHighPowerConfig_NormalParaConfig_02F1::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_STAHighPowerConfig_NormalParaConfig_02F1::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_STAHighPowerConfig_NormalParaConfig_02F1::processMsg(DvcType dvcType,int id,uchar* data,int datalen)
{
...
void Script_STAHighPowerConfig_NormalParaConfig_02F1::processCtrlDvcRes(DvcType dvcType,QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
    ...
    switch(emScriptRunState)
    {
        ...
        case Wait_SetSuperpowerPara_0B0B_Finish:
        {
            break;
        }
        case Wait_QuerySuperpowerPara_0B0B_Finish:
        {
            break;
        }
        case ScriptSuccess:
...
void Script_STAHighPowerConfig_NormalParaConfig_02F1::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_STAHighPowerConfig_NormalParaConfig_02F1::processMsgFromCCO(DvcType dvcType, int dvcId)
{
...
//                    p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("上报数据长度为%1").arg(p_TransparentTransmit_02F1_Up->frame_length_));
                            ...
                            &&((p_TransparentTransmit_02F1_Up->frame_content_.at(33)>=5&&p_TransparentTransmit_02F1_Up->frame_content_.at(33)<=25)||p_TransparentTransmit_02F1_Up->frame_content_.at(33)==0x00))
                    {
                        ...
                        if(index==p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->size())
                        {
                            if((1-(nodeInfoList.size()/p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->size()))>=0.9)
                            {
                                ...
                                emConfigSuperpowerParaState = Wait_Config_0B0B_Finish;
                                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_TransparentTransmit_02F1);
                                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "02F1配置STA大功率参数_0B0B，等待--确认");
                                emScriptRunState=Wait_SetSuperpowerPara_0B0B_Finish;
                                p_timer->start(STA_CMD_TIMEOUT_TIME*1000);
                            }
                            else
                            {
            ...
            case Wait_SetSuperpowerPara_0B0B_Finish:
            {
                if(p_Frame3762Base->afn_ == char(0x02)&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                            ...
                            &&p_Config_02F1_Up->frame_content_.at(29)==00)
                    {
                        if(index==p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->size())
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "02F1配置STA大功率参数_0B0B完成");
                            index=0;

                            em02F1_State=Query_02F1;
                            sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_TransparentTransmit_02F1);
                            emScriptRunState=Wait_QuerySuperpowerPara_0B0B_Finish;
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--透传查询从节点大功率参数（02F1），等待--回复");
                            p_timer->start(STA_CMD_TIMEOUT_TIME*1000);
                        }
                        else
            ...
            case Wait_QuerySuperpowerPara_0B0B_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x02&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    shared_ptr<Afn02F1> p_TransparentTransmit_02F1_Up=dynamic_pointer_cast<Afn02F1>(p_Frame3762Base);
                    if(p_TransparentTransmit_02F1_Up->frame_length_==0x22
                            &&(p_TransparentTransmit_02F1_Up->frame_content_.at(28)==char(0x0b))
                            &&(p_TransparentTransmit_02F1_Up->frame_content_.at(33)==char(0x0b)))
                    {
                        index++;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("透传回复数据:%1").arg(QString::fromLatin1(p_TransparentTransmit_02F1_Up->frame_content_.toHex().toUpper())));
                        if(index==p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->size())
...
void Script_STAHighPowerConfig_NormalParaConfig_02F1::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_STAHighPowerConfig_NormalParaConfig_02F1::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_STAHighPowerConfig_NormalParaConfig_02F1::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    sendMsgOct.clear();
...
void Script_STAHighPowerConfig_NormalParaConfig_02F1::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
...
void Script_STAHighPowerConfig_NormalParaConfig_02F1::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
...
QByteArray Script_STAHighPowerConfig_NormalParaConfig_02F1::getQueryFrame(ParaType type)
{
    QByteArray msg;
...
QByteArray Script_STAHighPowerConfig_NormalParaConfig_02F1::getSetFrame(ParaType type)
{
    QByteArray msg;
...
void Script_STAHighPowerConfig_NormalParaConfig_02F1::maxAllowTimer_timeoutProc()
{
