#include "Script_STAHighPowerConfig_NormalParaConfig_F0F21.h"

Script_STAHighPowerConfig_NormalParaConfig_F0F21::Script_STAHighPowerConfig_NormalParaConfig_F0F21(QObject *parent) : QObject(parent)
{
...
Script_STAHighPowerConfig_NormalParaConfig_F0F21::~Script_STAHighPowerConfig_NormalParaConfig_F0F21()
{
...
void Script_STAHighPowerConfig_NormalParaConfig_F0F21::execute()
{
...
void Script_STAHighPowerConfig_NormalParaConfig_F0F21::stop()
{
...
void Script_STAHighPowerConfig_NormalParaConfig_F0F21::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_STAHighPowerConfig_NormalParaConfig_F0F21::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_STAHighPowerConfig_NormalParaConfig_F0F21::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_STAHighPowerConfig_NormalParaConfig_F0F21::processMsg(DvcType dvcType,int id,uchar* data,int datalen)
{
...
void Script_STAHighPowerConfig_NormalParaConfig_F0F21::processCtrlDvcRes(DvcType dvcType,QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
    ...
    switch(emScriptRunState)
    {
        ...
        case Wait_SetSuperpowerPara_1010_Finish:
        {
            break;
        }
        case Wait_QuerySuperpowerPara_1010_Finish:
        {
            break;
        }
        case Wait_QuerySuperpowerPara_F0F22_1010_Finish:
        {
            break;
        }
        case ScriptSuccess:
...
void Script_STAHighPowerConfig_NormalParaConfig_F0F21::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_STAHighPowerConfig_NormalParaConfig_F0F21::processMsgFromCCO(DvcType dvcType, int dvcId)
{
...
//                                em02F1_State=Set_02F1;
                                emConfigSuperpowerParaState_F0F21 = Wait_Config_1010_Finish;
                                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_ConfigSuperpowerPara_F0F21);
                                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "F0F21配置STA大功率参数_1010，等待--确认");
                                emScriptRunState=Wait_SetSuperpowerPara_1010_Finish;
                                p_timer->start(STA_CMD_TIMEOUT_TIME*1000);
                            }
                            else
                            {
            ...
            case Wait_QuerySuperpowerPara_1010_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x02&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    shared_ptr<Afn02F1> p_TransparentTransmit_02F1_Up=dynamic_pointer_cast<Afn02F1>(p_Frame3762Base);
                    if(p_TransparentTransmit_02F1_Up->frame_length_==0x22
                            &&(p_TransparentTransmit_02F1_Up->frame_content_.at(28)==0x10)
                            &&(p_TransparentTransmit_02F1_Up->frame_content_.at(33)==0x10))
                    {
                        index++;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("透传回复数据:%1").arg(QString::fromLatin1(p_TransparentTransmit_02F1_Up->frame_content_.toHex().toUpper())));
                        if(index==p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->size())
...
//                                emConfigSuperpowerParaState_F0F21 = Wait_Config_150F_Finish;
                                ...
                                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "F0F22查询大功率参数_1010，等待--确认");
                                emScriptRunState=Wait_QuerySuperpowerPara_F0F22_1010_Finish;
                                p_timer->start(STA_CMD_TIMEOUT_TIME*1000);
                            }
                            else
                            {
            ...
            case Wait_QuerySuperpowerPara_F0F22_1010_Finish:
            {
                ...
                {
                    p_timer->stop();
                    shared_ptr<AfnF0F22> p_QuerySuperpowerPara_F0F22_Up=dynamic_pointer_cast<AfnF0F22>(p_Frame3762Base);
                    if(p_QuerySuperpowerPara_F0F22_Up->dac_voltage_==0x10&&p_QuerySuperpowerPara_F0F22_Up->peak_clipping_factor_==0x10)
                    {
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("F0F22上行回复数据 DAC:%1 削峰因子：%2").arg(p_QuerySuperpowerPara_F0F22_Up->dac_voltage_).arg(p_QuerySuperpowerPara_F0F22_Up->peak_clipping_factor_));
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed,QString("F0F22上行回复数据 DAC:%1 削峰因子：%2").arg(p_QuerySuperpowerPara_F0F22_Up->dac_voltage_).arg(p_QuerySuperpowerPara_F0F22_Up->peak_clipping_factor_));
                    }
...
void Script_STAHighPowerConfig_NormalParaConfig_F0F21::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_STAHighPowerConfig_NormalParaConfig_F0F21::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_STAHighPowerConfig_NormalParaConfig_F0F21::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    sendMsgOct.clear();
...
//        p_TransparentTransmit_02F1->frame_content_=getQueryFrame(DAC);
    ...
    else if(frame==p_ConfigSuperpowerPara_F0F21)
    {
        ...
        {
            case Wait_Config_1010_Finish:
            {
                p_ConfigSuperpowerPara_F0F21->dac_voltage_=0x10;
                p_ConfigSuperpowerPara_F0F21->peak_clipping_factor_=0x10;
                break;
            }
        }
        sendMsgOct=p_ConfigSuperpowerPara_F0F21->EncodeFrame();
        sendMsgLog=QString("》》配置大功率参数F0F21：%1\n").arg(QString(sendMsgOct.toHex().toUpper()));
    }
...
void Script_STAHighPowerConfig_NormalParaConfig_F0F21::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
...
void Script_STAHighPowerConfig_NormalParaConfig_F0F21::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    ...
    case Wait_SetSuperpowerPara_1010_Finish:
    {
        ...
        em02F1_State=Query_02F1;
        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_TransparentTransmit_02F1);
        emScriptRunState=Wait_QuerySuperpowerPara_1010_Finish;
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--透传查询从节点大功率参数（1010），等待--回复");
        p_timer->start(STA_CMD_TIMEOUT_TIME*1000);
        break;
    }
    default:
...
QByteArray Script_STAHighPowerConfig_NormalParaConfig_F0F21::getQueryFrame(ParaType type)
{
    QByteArray msg;
...
QByteArray Script_STAHighPowerConfig_NormalParaConfig_F0F21::getSetFrame(ParaType type)
{
    QByteArray msg;
...
void Script_STAHighPowerConfig_NormalParaConfig_F0F21::maxAllowTimer_timeoutProc()
{
