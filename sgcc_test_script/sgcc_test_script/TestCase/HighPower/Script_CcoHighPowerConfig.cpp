#include "Script_CcoHighPowerConfig.h"

Script_CcoHighPowerConfig::Script_CcoHighPowerConfig(QObject *parent) : QObject(parent)
{
    ...
    p_Afn01F1=make_shared<qgdw_3762_protocol::Afn01F1>();
    ...
    p_AfnF0F12=make_shared<qgdw_3762_protocol::AfnF0F12>();
...
Script_CcoHighPowerConfig::~Script_CcoHighPowerConfig()
{
    p_BuildNetwork_GW->initBuildNetWork();
    if(needPowerOff==true)//断电处理
        powerOffAll12V(p_CtrInfoList,p_AbstractScriptHost,true);
}
void Script_CcoHighPowerConfig::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
void Script_CcoHighPowerConfig::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
bool Script_CcoHighPowerConfig::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
/// 组网完成后，p_BuildNetwork_GW->buildNetworkResultFlag标志为true
/// 执行测试脚本的新操作即可
void Script_CcoHighPowerConfig::execute()
{

    p_AbstractScriptHost->updateProgress(ProcessState_Start, "Test start!");
    p_AbstractScriptHost->updateProgress(ProcessState_Start, "******************************************************");

    userCaseIds=QString("[GW-CCO-F021-0001-V01],");

    QString script_ID,script_Desc,script_CheckKeyPoint;
    script_ID=QString("用例ID：GW-CCO-F021-0001-V01 大功率配置-常规测试（F0F8） \n\n");
    ...
    script_Desc+=QString("F0F8配置CCO大功率参数B（B为正常范围内参数），B≠A，F0F7查询CCO大功率参数，Wait_SetHighPower_F0F8_Finish \n");
    script_Desc+=QString("路由断上电，F0F7查询路由当前功率参数，Wait_RoutePowerOff_Finish \n");
    script_Desc+=QString("01F1路由硬件初始化，F0F7查询路由当前功率参数，Wait_HardWareInit_01F1_Finish \n");
    script_Desc+=QString("01F2路由参数区初始化，F0F7查询路由当前功率参数，Wait_ParaInit_01F2_Finish \n");
    script_Desc+=QString("F0F12擦除flash，F0F7查询路由当前功率参数，Wait_EraseFlash_F0F12_Finish \n");

    script_CheckKeyPoint=QString("检查点1: 路由断上电，硬件初始化，参数初始化，均不改变大功率参数，擦出flash恢复默认值 \n");
...
void Script_CcoHighPowerConfig::stop()
{
    p_timer->stop();
    p_delayTimer->stop();
...
void Script_CcoHighPowerConfig::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_CcoHighPowerConfig::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    ...
    case Wait_SetHighPower_F0F8_Finish:
    {
        QList<int> dvcIdList;
        dvcIdList.append(p_CtrInfoList->at(0)->dvcId);
        DvcType dvc;
        dvc=CCO_GW;
        QList<double> sendParams;sendParams.clear();
        emScriptRunState=Wait_RoutePowerOff_Finish;
        p_AbstractScriptHost->controlDvc(dvc,dvcIdList,CtrlCmd_PowerOn_12V,sendParams);
        break;
    }
    case Wait_RoutePowerOff_Finish:
    {
...
void Script_CcoHighPowerConfig::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        case Wait_QueryHighPower_F0F7_Finish:
        {
            ...
            {
                p_timer->stop();
                emScriptRunState=Wait_SetHighPower_F0F8_Finish;
        ...
        case Wait_SetHighPower_F0F8_Finish:
        {
            ...
            if(p_Frame3762Base->afn_ == 0&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                dvc=CCO_GW;
                QList<double> sendParams;sendParams.clear();
                p_AbstractScriptHost->controlDvc(dvc,dvcIdList,CtrlCmd_PowerOff_12V,sendParams);
            }
            else  if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_timer->stop();
                emScriptRunState=ScriptSuccess;
                p_AbstractScriptHost->updateProgress(ProcessState_Failed,"Wait_SetHighPower_F0F8_Finish ,期望[回复确认]，实际[回复否认]，"+userCaseIds);
        ...
        case Wait_RoutePowerOff_Finish:
        {
            if(p_Frame3762Base->afn_ == char(0xF0)&&p_Frame3762Base->dt1_==0x40&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                {
                    if(p_AfnF0F7->data_unit_up_.parameter_content_.at(0)==0x08&&p_AfnF0F7->data_unit_up_.parameter_content_.at(1)==0x08)
                    {
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_Afn01F1);
                        emScriptRunState=Wait_HardWareInit_01F1_Finish;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--硬件初始化（01F1），等待--回复");
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    }
                    else
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed,QString("Wait_RoutePowerOff_Finish ,期望[回复F0F7 回复0x08 0x08]，实际[回复F0F7 回复%1 %2],").arg(uchar(p_AfnF0F7->data_unit_up_.parameter_content_.at(0))).arg(uchar(p_AfnF0F7->data_unit_up_.parameter_content_.at(1)))+userCaseIds);

                    }
                }
                else
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed,QString("Wait_RoutePowerOff_Finish ,期望[回复F0F7 应答状态0x00]，实际[回复应答状态%1],").arg(uchar(p_AfnF0F7->data_unit_up_.response_state_))+userCaseIds);
            ...
            else  if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_timer->stop();
                emScriptRunState=ScriptSuccess;
                p_AbstractScriptHost->updateProgress(ProcessState_Failed,"Wait_RoutePowerOff_Finish ,期望[回复F0F7]，实际[回复否认]，"+userCaseIds);
            ...
            else  if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Processing,"Wait_RoutePowerOff_Finish [回复03F10]，"+userCaseIds);
        ...
        {
            //示例
            //            if(p_Frame3762Base->afn_ == 0xF0&&p_Frame3762Base->dt1_==0x40&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            //            {
            //                p_timer->stop();
            //                shared_ptr<qgdw_3762_protocol::AfnF0F7> p_AfnF0F7=dynamic_pointer_cast<qgdw_3762_protocol::AfnF0F7>(p_Frame3762Base);
            //                if(p_AfnF0F7->data_unit_up_.response_state_=0x00)
            ...
            //                        emScriptRunState=Wait_HardWareInit_01F1_Finish;
            //                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--参数初始化（01F1），等待--回复");
            //                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
            //                    }
            //                    else
            //                    {
            //                        p_AbstractScriptHost->updateProgress(ProcessState_Failed,QString("Wait_HardWareInit_01F1_Finish ,期望[回复F0F7 回复0x08 0x08]，实际[回复应答状态%1 %2],").arg(uchar(p_AfnF0F7->data_unit_up_.parameter_content_.at(0))).arg(uchar(p_AfnF0F7->data_unit_up_.parameter_content_.at(1)))+userCaseIds);

            //                    }
            //                }
            //                else
            //                {
            //                    p_AbstractScriptHost->updateProgress(ProcessState_Failed,QString("Wait_HardWareInit_01F1_Finish ,期望[回复F0F7 应答状态0x00]，实际[回复应答状态%1],").arg(uchar(p_AfnF0F7->data_unit_up_.response_state_))+userCaseIds);
            ...
            else  if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_timer->stop();
                emScriptRunState=ScriptSuccess;
                p_AbstractScriptHost->updateProgress(ProcessState_Failed,"Wait_HardWareInit_01F1_Finish ,期望[回复F0F7]，实际[回复否认]，"+userCaseIds);

            }
            else  if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing,"Wait_HardWareInit_01F1_Finish [回复03F10]，"+userCaseIds);
        ...
        case Wait_QueryHighPower1_F0F7_Finish:
        {
            ...
            if(p_Frame3762Base->afn_ ==  char(0xF0)&&p_Frame3762Base->dt1_==0x40&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(p_AfnF0F7->data_unit_up_.response_state_==0x00)
                {
                    ...
                    {
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_Afn01F2);
                        emScriptRunState=Wait_ParaInit_01F2_Finish;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--参数初始化（01F2），等待--回复");
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    }
                    else
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed,QString("Wait_QueryHighPower1_F0F7_Finish ,期望[回复F0F7 回复0x08 0x08]，实际[回复F0F7 回复%1 %2],").arg(uchar(p_AfnF0F7->data_unit_up_.parameter_content_.at(0))).arg(uchar(p_AfnF0F7->data_unit_up_.parameter_content_.at(1)))+userCaseIds);
        ...
        case Wait_ParaInit_01F2_Finish:
        {
            ...
            else  if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_timer->stop();
                emScriptRunState=ScriptSuccess;
                p_AbstractScriptHost->updateProgress(ProcessState_Failed,"Wait_ParaInit_01F2_Finish ,期望[回复F0F7]，实际[回复否认]，"+userCaseIds);
            ...
            else  if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_timer->stop();
                emScriptRunState=ScriptSuccess;
                p_AbstractScriptHost->updateProgress(ProcessState_Failed,"Wait_ParaInit_01F2_Finish[回复F0F7]，实际[回复03F10]，"+userCaseIds);
        ...
        case Wait_QueryHighPower2_F0F7_Finish:
        {
            ...
            if(p_Frame3762Base->afn_ == char(0xF0)&&p_Frame3762Base->dt1_==0x40&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(p_AfnF0F7->data_unit_up_.response_state_==0x00)
                {
                    if(p_AfnF0F7->data_unit_up_.parameter_content_.at(0)==0x08&&p_AfnF0F7->data_unit_up_.parameter_content_.at(1)==0x08)
                    {
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_AfnF0F12);
                        emScriptRunState=Wait_EraseFlash_F0F12_Finish;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--擦出Flash（F0F12），等待--回复");
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    }
                    else
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed,QString("Wait_QueryHighPower2_F0F7_Finish ,期望[回复F0F7 回复0x08 0x08]，实际[回复F0F7 回复%1 %2],").arg(uchar(p_AfnF0F7->data_unit_up_.parameter_content_.at(0))).arg(uchar(p_AfnF0F7->data_unit_up_.parameter_content_.at(1)))+userCaseIds);
            ...
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing,"Wait_QueryHighPower2_F0F7_Finish ,期望[回复F0F7],实际[回复03F10]，"+userCaseIds);
            }
            else//收到其它报文的处理
            {
                QByteArray tmpSendMsg=prcsOther3762Msg(uchar(p_Frame3762Base->afn_),p_Frame3762Base->dt1_,p_Frame3762Base->dt2_,uchar(p_Frame3762Base->info_field_.info_field_up.msg_seq));
        ...
        case Wait_EraseFlash_F0F12_Finish:
        {
            ...
            else  if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_timer->stop();
                emScriptRunState=ScriptSuccess;
                p_AbstractScriptHost->updateProgress(ProcessState_Failed,"Wait_EraseFlash_F0F12_Finish ,期望[回复F0F7]，实际[回复否认]，"+userCaseIds);
            ...
            else  if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_timer->stop();
                emScriptRunState=ScriptSuccess;
                p_AbstractScriptHost->updateProgress(ProcessState_Failed,"Wait_EraseFlash_F0F12_Finish[回复F0F7]，实际[回复03F10]，"+userCaseIds);
        ...
        case Wait_QueryHighPower3_F0F7_Finish:
        {
            ...
            if(p_Frame3762Base->afn_ ==  char(0xF0)&&p_Frame3762Base->dt1_==0x40&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(p_AfnF0F7->data_unit_up_.response_state_==0x00)
                {
                    if(p_AfnF0F7->data_unit_up_.parameter_content_.at(0)==0x0f&&p_AfnF0F7->data_unit_up_.parameter_content_.at(1)==0x0a)
                    {
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed,QString("Wait_QueryHighPower3_F0F7_Finish ,期望[回复F0F7 回复0x08 0x08]，实际[回复应答状态%1 %2],").arg(uchar(p_AfnF0F7->data_unit_up_.parameter_content_.at(0))).arg(uchar(p_AfnF0F7->data_unit_up_.parameter_content_.at(1)))+userCaseIds);
...
void Script_CcoHighPowerConfig::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_CcoHighPowerConfig::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_CcoHighPowerConfig::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
//        sendMsgOct=p_QueryAfn_03F11->EncodeFrame();
    ...
    else if(frame==p_Afn01F1)
    {
        p_Afn01F1->ctrl_field_.dir=kDirDown;
        p_Afn01F1->ctrl_field_.prm=kActive;
        p_Afn01F1->ctrl_field_.comn_type=kHplc;

        p_Afn01F1->info_field_.info_field_down.msg_seq=char(msgSeq++);
        p_Afn01F1->info_field_.info_field_down.comu_rate=0;
        p_Afn01F1->info_field_.info_field_down.comu_module_ident=0;

        sendMsgOct=p_Afn01F1->EncodeFrame();
    ...
    else if(frame==p_AfnF0F8)
    {
        ...
        p_AfnF0F8->data_unit_down_.parameter_content_.append(char(0x08));
        p_AfnF0F8->data_unit_down_.parameter_content_.append(char(0x08));

        sendMsgOct=p_AfnF0F8->EncodeFrame();
        sendMsgLog=QString("》》设置路由大功率参数F0F8：%1\n").arg(QString(sendMsgOct.toHex().toUpper()));
    }
    else if(frame==p_AfnF0F12)
    {
        p_AfnF0F12->ctrl_field_.dir=kDirDown;
        p_AfnF0F12->ctrl_field_.prm=kActive;
        p_AfnF0F12->ctrl_field_.comn_type=kHplc;

        p_AfnF0F12->info_field_.info_field_down.msg_seq=char(msgSeq++);
        p_AfnF0F12->info_field_.info_field_down.comu_rate=0;
        p_AfnF0F12->info_field_.info_field_down.comu_module_ident=0;

        sendMsgOct=p_AfnF0F12->EncodeFrame();
        sendMsgLog=QString("》》擦出Flash F0F12：%1\n").arg(QString(sendMsgOct.toHex().toUpper()));
    }
...
void Script_CcoHighPowerConfig::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_CcoHighPowerConfig::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
    case Wait_BuildNetFinish_Whole:
    ...
    case Wait_HardWareInit_01F1_Finish:
    {
        ...
        {
            sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,index,p_Afn01F1);
    ...
    case Wait_EraseFlash_F0F12_Finish:
    {
        ...
        {
            sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,index,p_AfnF0F12);
            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--擦出Flash（AfnF0F12），等待--确认");
        }
        break;
    }
    case Wait_QueryHighPower3_F0F7_Finish:
...
void Script_CcoHighPowerConfig::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
...
void Script_CcoHighPowerConfig::delayTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
    case Wait_RoutePowerOff_Finish:
