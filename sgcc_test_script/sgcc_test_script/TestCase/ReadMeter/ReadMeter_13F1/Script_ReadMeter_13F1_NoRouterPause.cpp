#include "Script_ReadMeter_13F1_NoRouterPause.h"


Script_ReadMeter_13F1_NoRouterPause::Script_ReadMeter_13F1_NoRouterPause(QObject *parent) : QObject(parent)
{
    ...
    p_RestartRouter_12F2 = make_shared<Afn12F2>();
...
Script_ReadMeter_13F1_NoRouterPause::~Script_ReadMeter_13F1_NoRouterPause()
{
    p_BuildNetwork_GW->initBuildNetWork();
    if(needPowerOff==true)//断电处理
        powerOffAll12V(p_CtrInfoList,p_AbstractScriptHost,true);
}
void Script_ReadMeter_13F1_NoRouterPause::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
void Script_ReadMeter_13F1_NoRouterPause::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
bool Script_ReadMeter_13F1_NoRouterPause::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
/// 组网完成后，p_BuildNetwork_GW->buildNetworkResultFlag标志为true
/// 执行测试脚本的新操作即可
void Script_ReadMeter_13F1_NoRouterPause::execute()
{
    p_AbstractScriptHost->updateProgress(ProcessState_Start, "Test start!");
    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "测试编号：【GW-CCO-F003-0008-V01】\n"
                                                                  "测试流程：\n"
                                                                  "1：组网，Wait_11F1_BuildNet_Finish；\n"
                                                                  "2：发送13F1集中器主动抄表下行，Wait_13F1_ReadMeter_Finish；");
    ...
    else//场景1
    {
        ...
        emScriptRunState=Wait_13F1_ReadMeter_Finish;
...
void Script_ReadMeter_13F1_NoRouterPause::stop()
{
    p_timer->stop();
    p_delayTimer->stop();
...
void Script_ReadMeter_13F1_NoRouterPause::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeter_13F1_NoRouterPause::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
...
void Script_ReadMeter_13F1_NoRouterPause::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        case Wait_13F1_ReadMeter_Finish:
        {
        ...
        case Wait_00F1_for_12F3_Recover:
        {
            if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Success, "脚本执行成功，【GW-CCO-F003-0008-V01】");
            }
            else
            {
                QByteArray tmpSendMsg=prcsOther3762Msg(uchar(p_Frame3762Base->afn_),p_Frame3762Base->dt1_,p_Frame3762Base->dt2_,uchar(p_Frame3762Base->info_field_.info_field_up.msg_seq));
...
void Script_ReadMeter_13F1_NoRouterPause::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_13F1_NoRouterPause::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_13F1_NoRouterPause::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    if(frame==p_RouterRecover_12F3)
    {
...
void Script_ReadMeter_13F1_NoRouterPause::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_13F1_NoRouterPause::statisticResult()
{
    index=0;
    ...
    if(p_CtrInfoList->at(0)->successRate[1]>=readMeterSucRateThresld)
    {
        ...
        tryTimes=0;
        emScriptRunState=ScriptSuccess;
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("13F1抄表测试成功，成功率：%1%; 13F1抄表平均耗时：%2秒;").arg(p_CtrInfoList->at(0)->successRate[1]*100).arg(p_CtrInfoList->at(0)->successConsume[1]));
        p_AbstractScriptHost->updateProgress(ProcessState_Success, "脚本执行成功，【GW-CCO-F003-0008-V01】");
    }
    else
    {
...
void Script_ReadMeter_13F1_NoRouterPause::CalcAvrgConsumeTimeLen(uchar rdFlag)
{
...
void Script_ReadMeter_13F1_NoRouterPause::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_ReadMeter_13F1_NoRouterPause::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
    case Wait_13F1_ReadMeter_Finish:
...
void Script_ReadMeter_13F1_NoRouterPause::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
