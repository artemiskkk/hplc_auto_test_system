#include "Script_ReadMeter_13F1_NoConnectedToTheNet_NoArchives.h"

Script_ReadMeter_13F1_NoConnectedToTheNet_NoArchives::Script_ReadMeter_13F1_NoConnectedToTheNet_NoArchives()
{
    ...
    p_DeleteAddr_11F2=make_shared<Afn11F2>();
...
void Script_ReadMeter_13F1_NoConnectedToTheNet_NoArchives::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
//    memcmp(p_tempMeterInfo, p_MeterInfoList, uint(p_MeterInfoList->size()));
//    memcmp(p_paratialMeterInfo, p_MeterInfoList, 10);
...
void Script_ReadMeter_13F1_NoConnectedToTheNet_NoArchives::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}

bool Script_ReadMeter_13F1_NoConnectedToTheNet_NoArchives::config(const QMap<QString, QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ReadMeter_13F1_NoConnectedToTheNet_NoArchives::execute()
{
    p_AbstractScriptHost->updateProgress(ProcessState_Start, "Test start!");
    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "测试编号：【GW-CCO-F003-0003-V01】\n"
                                                                  "测试流程：\n"
                                         "1:组网，Wait_11F1_BuildNet_Finish;\n"
                                         "2:给路由下发12F2，暂停路由命令，Wait_12F12_RouterPause_Finish;\n"
                                         "3:删除从节点，等待从节点离网，Wait_11F2_DeleteAddr_Finish;\n"
                                         "4:下发13F1监控从节点命令，通信协议类型0x02，采用645协议，抄读电表1数据，Wait_13F1_MeterReadOutsideArchives_Finish;\n"
                                         "5:下发13F1监控从节点命令，通信协议类型0x02，采用645协议，抄读不在档案中的电表（也不在环境中，如123456789010）数据，Wait_13F1_ReadVirtualMeter_Finish;"
                                         );
...
void Script_ReadMeter_13F1_NoConnectedToTheNet_NoArchives::stop()
{
    p_timer->stop();
    p_delayTimer->stop();
...
void Script_ReadMeter_13F1_NoConnectedToTheNet_NoArchives::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeter_13F1_NoConnectedToTheNet_NoArchives::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    ...
    case Wait_11F2_DeleteAddr_Finish:
    {
        break;
    }
    ...
    case Wait_13F1_MeterReadOutsideArchives_Finish:
    {
        break;
    }
    case Wait_13F1_ReadVirtualMeter_Finish:
    {
        break;
    }
    case ScriptSuccess:
...
void Script_ReadMeter_13F1_NoConnectedToTheNet_NoArchives::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        try {
            p_Frame3762Base = p_Frame3762Helper->DecodeLocalMsg(&(p_CtrInfoList->at(0)->buf),haveCompleteMsg);
        }
        catch(...) {
        ...
        case Wait_12F12_RouterPause_Finish:
        {
            if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                tryTimes=0;
                index=0;
                sendMsg(CCO_GW,dvcId,index,p_DeleteAddr_11F2);
                emScriptRunState=Wait_11F2_DeleteAddr_Finish;
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--删除从节点（11F2），等待--回复确认");
            ...
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_12F12_RouterPause_Finish，12F1回复异常，期望回复确认，实际回复否认，【GW-CCO-F003-0003-V01】"));
            }
            else//收到其它报文的处理
            {
        ...
        case Wait_11F2_DeleteAddr_Finish:
        {
            ...
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("收到删除从节点回复确认，等待90s节点离网"));
                p_delayTimer->start(90*1000);
            }
            else if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("收到删除从节点回复否认，测试失败"));
            }
            else//收到其它报文的处理
            {
                QByteArray tmpSendMsg=prcsOther3762Msg(uchar(p_Frame3762Base->afn_),p_Frame3762Base->dt1_,p_Frame3762Base->dt2_,uchar(p_Frame3762Base->info_field_.info_field_up.msg_seq));
        ...
        case Wait_13F1_MeterReadOutsideArchives_Finish:
        {
            if(p_Frame3762Base->afn_==0x13&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "Wait_13F1_MeterReadOutsideArchives_Finish，收到13F1上报空数据域，测试继续！");
                    emScriptRunState=Wait_13F1_ReadVirtualMeter_Finish;
                    sendMsg(CCO_GW,dvcId,index,p_MonitorSlaveNode_13F1);
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--抄读虚拟从节点（13F1），等待--点抄完成");
                    p_timer->start(120*1000);
                }
                else
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_13F1_MeterReadOutsideArchives_Finish，13F1回复异常，期望数据域为0，实际数据域不为0：1%，【GW-CCO-F003-0003-V01】").arg(QString::fromLatin1(p_MonitorSlaveNode_13F1_Up->data_field_up_.frame_content_.toHex())));
                }
            }
            else//收到其它报文的处理
            {
        ...
        emScriptRunState=ScriptSuccess;
                    p_AbstractScriptHost->updateProgress(ProcessState_Success, "脚本执行成功，【GW-CCO-F003-0003-V01】");
                }
                else
                {
                    p_timer->stop();
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_13F1_ReadVirtualMeter_Finish，路由上报13F1，数据域不为0：1%，【GW-CCO-F003-0003-V01】").arg(QString::fromLatin1(p_MonitorSlaveNode_13F1_Up->data_field_up_.frame_content_.toHex())));
...
void Script_ReadMeter_13F1_NoConnectedToTheNet_NoArchives::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_13F1_NoConnectedToTheNet_NoArchives::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_13F1_NoConnectedToTheNet_NoArchives::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_DeleteAddr_11F2)
    {
        p_DeleteAddr_11F2->ctrl_field_.dir=kDirDown;
        p_DeleteAddr_11F2->ctrl_field_.prm=kActive;
        p_DeleteAddr_11F2->ctrl_field_.comn_type=kHplc;

        p_DeleteAddr_11F2->info_field_.info_field_down.msg_seq=char(msgSeq++);
        p_DeleteAddr_11F2->info_field_.info_field_down.comu_rate=0;
        p_DeleteAddr_11F2->info_field_.info_field_down.comu_module_ident=0;
        p_DeleteAddr_11F2->node_address_list_.clear();
        ...
        p_DeleteAddr_11F2->node_address_list_.append(nodeAddr);

        p_DeleteAddr_11F2->node_num_=uchar(p_DeleteAddr_11F2->node_address_list_.size());
        sendMsgOct=p_DeleteAddr_11F2->EncodeFrame();
    ...
    else if(frame==p_MonitorSlaveNode_13F1)
    {
        ...
        uchar tmpAddr[6];
        if(emScriptRunState == Wait_13F1_MeterReadOutsideArchives_Finish)
        {
            memcpy(tmpAddr,tmp_645_addr,6);
        }
        else if(emScriptRunState == Wait_13F1_ReadVirtualMeter_Finish)
        {
            memcpy(tmpAddr, tmp_vir_addr, 6);
        }
...
void Script_ReadMeter_13F1_NoConnectedToTheNet_NoArchives::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_13F1_NoConnectedToTheNet_NoArchives::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
    case Wait_BuildNetFinish_Whole:
    ...
    case Wait_12F12_RouterPause_Finish:
    {
        ...
        if(++tryTimes>=3)
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_12F12_RouterPause_Finish，“timeout！”，【GW-CCO-F003-0003-V01】");
        }
        else
        {
            sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_RouterPause_12F2);
    ...
    case Wait_11F2_DeleteAddr_Finish:
    {
        ...
        if(++tryTimes>=3)
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_11F2_DeleteAddr_Finish，“timeout！”，【GW-CCO-F003-0003-V01】");
        }
        else
        {
            sendMsg(CCO_GW,p_CtrInfoList->at(0)->dvcId,index,p_DeleteAddr_11F2);
            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--删除从节点命令（11F2），等待--确认");
        }
        break;
    }
    case Wait_13F1_MeterReadOutsideArchives_Finish:
    {
        p_timer->stop();
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "Wait_13F1_MeterReadOutsideArchives_Finish，120s内未13F1上报空数据域，不符合期望，测试失败！");
        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_13F1_MeterReadOutsideArchives_Finish，“timeout！”，【GW-CCO-F003-0003-V01】");
        break;
    }
    case Wait_13F1_ReadVirtualMeter_Finish:
    {
        p_timer->stop();
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "Wait_13F1_ReadVirtualMeter_Finish，120s内未13F1上报空数据域，不符合期望，测试失败！");
        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_13F1_ReadVirtualMeter_Finish，“timeout！”，【GW-CCO-F003-0003-V01】");
        break;
    }
    default:
    {
...
void Script_ReadMeter_13F1_NoConnectedToTheNet_NoArchives::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
    default:
...
void Script_ReadMeter_13F1_NoConnectedToTheNet_NoArchives::delayTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
    case Wait_BuildNetFinish_Whole:
    ...
    case Wait_11F2_DeleteAddr_Finish:
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "延迟时间到，从节点离线，测试继续");
        emScriptRunState=Wait_13F1_MeterReadOutsideArchives_Finish;
        sendMsg(CCO_GW,p_CtrInfoList->at(0)->dvcId,index,p_MonitorSlaveNode_13F1);
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--抄读不在网、无档案从节点（13F1），等待--点抄完成");
        p_timer->start(120*1000);
    }
    default:
    {
