#include "Script_ReadMeter_13F1_AddrAll9AndAllA.h"



Script_ReadMeter_13F1_AddrAll9AndAllA::Script_ReadMeter_13F1_AddrAll9AndAllA(QObject *parent) : QObject(parent)
{
...
Script_ReadMeter_13F1_AddrAll9AndAllA::~Script_ReadMeter_13F1_AddrAll9AndAllA()
{
...
void Script_ReadMeter_13F1_AddrAll9AndAllA::execute()
{
    p_AbstractScriptHost->updateProgress(ProcessState_Start, "Test start!");
    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "测试编号：【GW-STA-F003-0001-V01】\n"
                                                                  "测试流程：\n"
                                                                  "1：组网，Wait_11F1_BuildNet_Finish；\n"
                                                                  "2：给路由下发12F2，暂停路由命令，Wait_12F12_RouterPause_Finish；\n"
                                                                  "3：下发13F1监控从节点命令，通信协议类型0x02，采用645协议，抄表地址为全9，抄读645表数据，Wait_13F1_MeterReadAddrAll9_Finish；\n"
                                                                  "4：下发13F1监控从节点命令，通信协议类型0x02，采用645协议，抄表地址为全a，抄读645表数据，Wait_13F1_MeterReadAddrAllA_Finish；\n");
...
void Script_ReadMeter_13F1_AddrAll9AndAllA::stop()
{
...
void Script_ReadMeter_13F1_AddrAll9AndAllA::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ReadMeter_13F1_AddrAll9AndAllA::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ReadMeter_13F1_AddrAll9AndAllA::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ReadMeter_13F1_AddrAll9AndAllA::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeter_13F1_AddrAll9AndAllA::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
    ...
    case Wait_13F1_MeterReadAddrAll9_Finish:
    {
        break;
    }
...
void Script_ReadMeter_13F1_AddrAll9AndAllA::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        case Wait_12F12_RouterPause_Finish:
        {
            if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                emScriptRunState=Wait_13F1_MeterReadAddrAll9_Finish;
                sendMsg(CCO_GW,dvcId,index,p_MonitorSlaveNode_13F1);
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--监控从节点（13F1），等待全9地址--点抄完成");
                p_timer->start(95*1000);
            }
            else
            {
        ...
        case Wait_13F1_MeterReadAddrAll9_Finish:
        {
            if(p_Frame3762Base->afn_==0x13&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(p_MonitorSlaveNode_13F1_Up->data_field_up_.frame_content_.size()==0)
                {
                    ...
                    for(int i=0; i<6; i++)
                    {
                        tmp_addr[i] = p_MonitorSlaveNode_13F1_Up->address_field_.src_addr[i];
                    }

                    QString tmp_str = "抄表13F1上报空数据域，上报地址为：" + QString::fromLatin1(tmp_addr.toHex()) + "，符合要求，测试继续！";
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "抄表13F1上报空数据域，符合要求，测试继续！");

                    emScriptRunState = Wait_13F1_MeterReadAddrAllA_Finish;
                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,index,p_MonitorSlaveNode_13F1);
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--监控从节点报文（13F1），等待全A地址--点抄完成");
                    p_timer->start(90*1000);
                }
                else
                {
                    QString tmp_content = "Wait_13F1_MeterReadAddrAll9_Finish，13F1回复异常，期望数据域为0，实际数据域不为0：" + QString::fromLatin1(p_MonitorSlaveNode_13F1_Up->data_field_up_.frame_content_.toHex()) + "，【GW-STA-F003-0001-V01】";
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, tmp_content);
                }
        ...
        case Wait_13F1_MeterReadAddrAllA_Finish:
        {
            ...
            if(p_Frame3762Base->afn_==0x13&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(p_MonitorSlaveNode_13F1_Up->data_field_up_.frame_content_.size() == 0)
                {
                    ...
                    QString tmp_str = "抄表13F1上报，上报地址为：" + QString::fromLatin1(tmp_addr.toHex()) + "，符合预期，测试通过！";
                ...
                {
                    QString tmp_content = "Wait_13F1_MeterReadAddrAllA_Finish，13F1回复异常，期望数据域为0，实际数据域不为0，【GW-STA-F003-0001-V01】";
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, tmp_content);
                }
...
void Script_ReadMeter_13F1_AddrAll9AndAllA::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        {
//            p_timer->stop();
            uchar tmp_all9_addr[6] = {0x99, 0x99, 0x99, 0x99, 0x99, 0x99};
            if(memcmp(MsgBase_645_ptr->addr_, tmp_all9_addr, 6)==0)
            {

                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "Wait_13F1_MeterReadAddrAll9_Finish，收到模块转发的645全9报文，符合预期，虚拟表做不回复处理，测试继续！");
            ...
            else
            {
                ...
                for(int i=0; i<6; i++)
                {
                    tmp_arr[i] = char(MsgBase_645_ptr->addr_[i]);
                }
                QString tmp_str = "Wait_13F1_MeterReadAddrAll9_Finish，收到模块转发的645全9报文，不符合预期: " + QString::fromLatin1(tmp_arr.toHex()) + "，测试失败！！";
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, tmp_str);
            }
        ...
        case Wait_13F1_MeterReadAddrAllA_Finish:
        {
            uchar tmp_all9_addr[6] = {0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA};
            if(memcmp(MsgBase_645_ptr->addr_, tmp_all9_addr, 6)==0)
            {
//                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "Wait_13F1_MeterReadAddrAllA_Finish，收到模块转发的645全A报文，符合预期，虚拟表正常处理，测试继续！");
            }
        ...
        default:
        {
            ...
            {
//                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "default，收到模块转发的645全9报文，符合预期，虚拟表做不回复处理，测试继续！");
                break;
            }
...
void Script_ReadMeter_13F1_AddrAll9AndAllA::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_13F1_AddrAll9AndAllA::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
//        uchar comPrtclType=p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(1)->prtcl;
        ...
        if(comPrtclType==0x02)
        {
            ...
            if(emScriptRunState == Wait_13F1_MeterReadAddrAll9_Finish)
            {
                uchar addr[6] = {0x99, 0x99, 0x99, 0x99, 0x99, 0x99};
                memcpy(p_ReadData_0x11->addr_,addr,6);
            }
            else if(emScriptRunState == Wait_13F1_MeterReadAddrAllA_Finish)
            {
                uchar addr[6] = {0xaa, 0xaa, 0xaa, 0xaa, 0xaa, 0xaa};
                memcpy(p_ReadData_0x11->addr_,addr,6);
            }
            else
            {
...
void Script_ReadMeter_13F1_AddrAll9AndAllA::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_13F1_AddrAll9AndAllA::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
    case Wait_BuildNetFinish_Whole:
    ...
    case Wait_13F1_MeterReadAddrAll9_Finish:
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "Wait_13F1_MeterReadAddrAll9_Finish，90s超时时间到，无13F1空数据域上报！测试失败！！");
        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_13F1_MeterReadAddrAll9_Finish，timeout！【GW-STA-F003-0001-V01】");
    ...
    {
        p_timer->stop();
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "Wait_13F1_MeterReadAddrAllA_Finish，90s超时时间到，无13F1空数据域上报！测试失败！");
        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_13F1_MeterReadAddrAllA_Finish timeout! 【GW-STA-F003-0001-V01】");
        break;
    }
    default:
    {
...
void Script_ReadMeter_13F1_AddrAll9AndAllA::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_13F1_MeterReadAddrAll9_Finish:
...
void Script_ReadMeter_13F1_AddrAll9AndAllA::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
...
void Script_ReadMeter_13F1_AddrAll9AndAllA::statisticResult()
{
    index=0;
    CalcAvrgConsumeTimeLen(1);
    QString failedMeter = GenerateFailedMeterStr_ReadMeter(p_CtrInfoList->at(0),1);
...
void Script_ReadMeter_13F1_AddrAll9AndAllA::CalcAvrgConsumeTimeLen(uchar rdFlag)
{
...
bool Script_ReadMeter_13F1_AddrAll9AndAllA::extractAndProcess3762Frame(QByteArray& buf,QByteArray& completeFrame)
{
    completeFrame.clear();
