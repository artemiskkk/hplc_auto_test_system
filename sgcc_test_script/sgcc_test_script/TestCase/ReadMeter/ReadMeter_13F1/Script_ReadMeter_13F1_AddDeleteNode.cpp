#include "Script_ReadMeter_13F1_AddDeleteNode.h"

Script_ReadMeter_13F1_AddDeleteNode::Script_ReadMeter_13F1_AddDeleteNode(QObject *parent) : QObject(parent)
{
...
Script_ReadMeter_13F1_AddDeleteNode::~Script_ReadMeter_13F1_AddDeleteNode()
{
    // 先停止所有定时器
    if(p_timer != nullptr) {
        p_timer->stop();
...
void Script_ReadMeter_13F1_AddDeleteNode::execute()
{
...
void Script_ReadMeter_13F1_AddDeleteNode::stop()
{
    // 先停止所有定时器
    if(p_timer != nullptr)
        p_timer->stop();
...
void Script_ReadMeter_13F1_AddDeleteNode::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ReadMeter_13F1_AddDeleteNode::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ReadMeter_13F1_AddDeleteNode::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ReadMeter_13F1_AddDeleteNode::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeter_13F1_AddDeleteNode::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ReadMeter_13F1_AddDeleteNode::processMsgFromCCO(DvcType dvcType, int dvcId)
{
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_00F1_for_12F2_Pause:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    sendMsg(CCO_GW,dvcId,INSIGNIFICANCE,p_AddSlaveNode_11F1);
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--添加从节点（11F1），等待--确认");
                    p_timer->start(10*1000);//10s超时
            ...
            case Wait_Finish_13F1:
            {
                if(p_Frame3762Base->afn_==0x13&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    else
                    {
                        ...
                        if(flagNeedDelete==true)
                        {
                            flagNeedDelete=false;
                            sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,index,p_DeleteSlaveNode_11F2);
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--删除从节点（11F2），等待--回复");
                        }
                ...
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing,"回复否认");
                }
                else
                {
                    QByteArray tmpSendMsg=prcsOther3762Msg(uchar(p_Frame3762Base->afn_),p_Frame3762Base->dt1_,p_Frame3762Base->dt2_,uchar(p_Frame3762Base->info_field_.info_field_up.msg_seq));
...
void Script_ReadMeter_13F1_AddDeleteNode::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_ReadMeter_13F1_AddDeleteNode::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_13F1_AddDeleteNode::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_AddSlaveNode_11F1)
    {
        ...
        QString node="001999997777";
        NodeParameter nodePara;
        memcpy(nodePara.node_address_.addr,QByteArray::fromHex(node.toLatin1()),6);
        nodePara.protocol_type_=0x02;
    ...
    else if(frame==p_DeleteSlaveNode_11F2)
    {
        ...
        if(index==1)
        {
            memcpy(nodeAddr.addr,p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(index)->mtrAddr,6);
        }
        else
        {
            QString node="001999997777";
            memcpy(nodeAddr.addr,QByteArray::fromHex(node.toLatin1()),6);
        }
...
void Script_ReadMeter_13F1_AddDeleteNode::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_13F1_AddDeleteNode::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
        ...
        case Wait_Finish_13F1:
        {
            ...
            if(++index>=times)
            {
                // 所有电表都处理完了，调用statisticResult结束测试，而不是无限循环
                p_timer->stop();
                p_maxAllowTimer->stop();
                statisticResult();
                break;
...
void Script_ReadMeter_13F1_AddDeleteNode::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_Finish_13F1:
        {
            ...
            {
                p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("Script_ReadMeter_13F1_AddDeleteNode 13F1抄表成功率：%1%; 13F1抄表平均耗时：%2秒;").arg(p_CtrInfoList->at(0)->successRate[1]*100).arg(p_CtrInfoList->at(0)->successConsume[1]));
                resultFlag=true;
                emScriptRunState=ScriptSuccess;
            }
            else
...
void Script_ReadMeter_13F1_AddDeleteNode::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
...
void Script_ReadMeter_13F1_AddDeleteNode::statisticResult()
{
    index=0;
    CalcAvrgConsumeTimeLen(1);
    QString failedMeter = GenerateFailedMeterStr_ReadMeter(p_CtrInfoList->at(0),1);
    ...
    if(p_CtrInfoList->at(0)->successRate[1]>=readMeterSucRateThresld)
    {
        ...
        QString failedMeter = GenerateFailedMeterStr_ReadMeter(p_CtrInfoList->at(0),1);
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, failedMeter);
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("13F1抄表成功率：%1%; 13F1抄表平均耗时：%2秒;").arg(p_CtrInfoList->at(0)->successRate[1]*100).arg(p_CtrInfoList->at(0)->successConsume[1]));
        p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("13F1抄表时添加和删除从节点测试成功;"));
    ...
    else
    {
        if(++tryTimes>=3)
        {
            ...
            p_AbstractScriptHost->updateProgress(ProcessState_Processing, failedMeter);
            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("13F1抄表成功率：%1%; 13F1抄表平均耗时：%2秒;").arg(p_CtrInfoList->at(0)->successRate[1]*100).arg(p_CtrInfoList->at(0)->successConsume[1]));
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("13F1抄表时添加和删除从节点测试失败;"));
        }
        else
        {
...
void Script_ReadMeter_13F1_AddDeleteNode::CalcAvrgConsumeTimeLen(uchar rdFlag)
{
...
bool Script_ReadMeter_13F1_AddDeleteNode::extractAndProcess3762Frame(QByteArray& buf,QByteArray& completeFrame)
{
    completeFrame.clear();
