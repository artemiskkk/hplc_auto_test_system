#include "Script_ReadMeter_13F1_DelayRelated.h"

Script_ReadMeter_13F1_DelayRelated::Script_ReadMeter_13F1_DelayRelated(QObject *parent) : QObject(parent)
{
...
Script_ReadMeter_13F1_DelayRelated::~Script_ReadMeter_13F1_DelayRelated()
{
...
void Script_ReadMeter_13F1_DelayRelated::execute()
{
...
void Script_ReadMeter_13F1_DelayRelated::stop()
{
...
void Script_ReadMeter_13F1_DelayRelated::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ReadMeter_13F1_DelayRelated::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ReadMeter_13F1_DelayRelated::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ReadMeter_13F1_DelayRelated::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeter_13F1_DelayRelated::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ReadMeter_13F1_DelayRelated::processMsgFromCCO(DvcType dvcType, int dvcId)
{
            ...
            case Wait_Finish_13F1:
            {
                ...
                else if(p_Frame3762Base->afn_==0x14&&p_Frame3762Base->dt1_==0x04&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                 //   p_timer->stop();
                    ...
                    msgSeq14F3=p_DelayCorrect_14F3_Up->info_field_.info_field_up.msg_seq;
                ...
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, "收到03F10上报，路由复位");
                }
                else
                {
                    QByteArray tmpSendMsg=prcsOther3762Msg(uchar(p_Frame3762Base->afn_),p_Frame3762Base->dt1_,p_Frame3762Base->dt2_,uchar(p_Frame3762Base->info_field_.info_field_up.msg_seq));
...
void Script_ReadMeter_13F1_DelayRelated::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_ReadMeter_13F1_DelayRelated::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_13F1_DelayRelated::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_MonitorSlaveNode_13F1)
    {
        ...
        if(comPrtclType==0x02)
        {
            ...
            p_MonitorSlaveNode_13F1->data_field_down_.delay_tag_=(index%2==0?0x01:0x02);//精度1s或0.1s
        ...
        else if(comPrtclType==0x03)
        {
            ...
            p_MonitorSlaveNode_13F1->data_field_down_.delay_tag_=0x01;//精度1s
            p_MonitorSlaveNode_13F1->data_field_down_.sub_node_num_=0x00;
    ...
    else if(frame==p_DelayCorrect_14F3)
    {
        ...
        p_DelayCorrect_14F3->info_field_.info_field_down.msg_seq=msgSeq14F3;
...
void Script_ReadMeter_13F1_DelayRelated::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_13F1_DelayRelated::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
        ...
        case Wait_Finish_13F1:
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_13F1_UP timeout");
            //需要判断是否切换电表
//            if(++index>=times)
//            {
//                index=0;
...
void Script_ReadMeter_13F1_DelayRelated::maxAllowTimer_timeoutProc()
{
...
void Script_ReadMeter_13F1_DelayRelated::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
...
void Script_ReadMeter_13F1_DelayRelated::CalcAvrgConsumeTimeLen(uchar rdFlag)
{
...
void Script_ReadMeter_13F1_DelayRelated::statisticResult()
{
    index=0;
    CalcAvrgConsumeTimeLen(1);
    QString failedMeter = GenerateFailedMeterStr_ReadMeter(p_CtrInfoList->at(0),1);
...
bool Script_ReadMeter_13F1_DelayRelated::extractAndProcess3762Frame(QByteArray& buf,QByteArray& completeFrame)
{
    completeFrame.clear();
