#include "Script_ReadMeter_13F1_SubsidiaryNode.h"

Script_ReadMeter_13F1_SubsidiaryNode::Script_ReadMeter_13F1_SubsidiaryNode(QObject *parent) : QObject(parent)
{
...
Script_ReadMeter_13F1_SubsidiaryNode::~Script_ReadMeter_13F1_SubsidiaryNode()
{
...
void Script_ReadMeter_13F1_SubsidiaryNode::execute()
{
...
void Script_ReadMeter_13F1_SubsidiaryNode::stop()
{
...
void Script_ReadMeter_13F1_SubsidiaryNode::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ReadMeter_13F1_SubsidiaryNode::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ReadMeter_13F1_SubsidiaryNode::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ReadMeter_13F1_SubsidiaryNode::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeter_13F1_SubsidiaryNode::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ReadMeter_13F1_SubsidiaryNode::processMsgFromCCO(DvcType dvcType, int dvcId)
{
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_00F1_for_12F3_Recover:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Success, "带附属节点地址13F1抄读测试完成，支持带附属节点地址抄读！");
...
void Script_ReadMeter_13F1_SubsidiaryNode::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_ReadMeter_13F1_SubsidiaryNode::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_13F1_SubsidiaryNode::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_MonitorSlaveNode_13F1)
    {
        ...
        if(comPrtclType==0x02)
        {
            ...
            p_MonitorSlaveNode_13F1->data_field_down_.sub_node_address_list_.clear();
            if(index<7)
            {
                for(int i=0;i<index;i++)
                {
                    Address subAddress;
                    memcpy(subAddress.addr,p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(i)->mtrAddr,6);
                    p_MonitorSlaveNode_13F1->data_field_down_.sub_node_address_list_.append(subAddress);
                }
            }
            else if(index>=7)
            {
                ...
                {
                    Address subAddress;
                    memcpy(subAddress.addr,p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(index-7+i)->mtrAddr,6);
                    p_MonitorSlaveNode_13F1->data_field_down_.sub_node_address_list_.append(subAddress);
                }
            }
            p_MonitorSlaveNode_13F1->data_field_down_.sub_node_num_=uchar(p_MonitorSlaveNode_13F1->data_field_down_.sub_node_address_list_.size());
...
void Script_ReadMeter_13F1_SubsidiaryNode::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_13F1_SubsidiaryNode::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
        ...
        case Wait_Finish_13F1:
        {
            ...
            {
                index=0;
                if(failCount==times-1)
                {
                    if(p_maxAllowTimer!=nullptr)
                        p_maxAllowTimer->stop();
                    emScriptRunState=ScriptSuccess;
                    p_AbstractScriptHost->updateProgress(ProcessState_Success, "带附属节点地址13F1抄读测试完成，不支持带附属节点地址抄读！");
                }
...
void Script_ReadMeter_13F1_SubsidiaryNode::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_Finish_13F1:
...
void Script_ReadMeter_13F1_SubsidiaryNode::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
...
void Script_ReadMeter_13F1_SubsidiaryNode::CalcAvrgConsumeTimeLen(uchar rdFlag)
{
...
void Script_ReadMeter_13F1_SubsidiaryNode::statisticResult()
{
    index=0;
    CalcAvrgConsumeTimeLen(1);
    QString failedMeter = GenerateFailedMeterStr_ReadMeter(p_CtrInfoList->at(0),1);
...
bool Script_ReadMeter_13F1_SubsidiaryNode::extractAndProcess3762Frame(QByteArray& buf,QByteArray& completeFrame)
{
    completeFrame.clear();
