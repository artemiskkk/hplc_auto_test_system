#include "Script_ReadMeter_13F1_NodeNotInRouter.h"

Script_ReadMeter_13F1_NodeNotInRouter::Script_ReadMeter_13F1_NodeNotInRouter(QObject *parent) : QObject(parent)
{
...
Script_ReadMeter_13F1_NodeNotInRouter::~Script_ReadMeter_13F1_NodeNotInRouter()
{
...
void Script_ReadMeter_13F1_NodeNotInRouter::execute()
{
...
void Script_ReadMeter_13F1_NodeNotInRouter::stop()
{
...
void Script_ReadMeter_13F1_NodeNotInRouter::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ReadMeter_13F1_NodeNotInRouter::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ReadMeter_13F1_NodeNotInRouter::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ReadMeter_13F1_NodeNotInRouter::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeter_13F1_NodeNotInRouter::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ReadMeter_13F1_NodeNotInRouter::processMsgFromCCO(DvcType dvcType, int dvcId)
{
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_00F1_for_11F2:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "等待5s，开始暂停路由");
            ...
            case Wait_Finish_13F1:
            {
                if(p_Frame3762Base->afn_==0x13&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    else
                    {
                        ...
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("index = %1 times = %2 flagNeedRead = %3").arg(index).arg(times).arg(flagNeedRead));
                ...
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到否认帧！");
                    shared_ptr<Afn00F2> ptr=dynamic_pointer_cast<Afn00F2>(p_Frame3762Base);
                    if(ptr->error_code_ == 7)
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "表不在档案内，回复否认！！!");
                        notInRouterFlag = true;
                    }
                }
                else
                {
...
void Script_ReadMeter_13F1_NodeNotInRouter::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_13F1_NodeNotInRouter::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_13F1_NodeNotInRouter::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    if(frame==p_DeleteNode_11F2)
    {
        ...
        Address deleteMeterAddr;
        memcpy(deleteMeterAddr.addr,p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(index)->mtrAddr,6);
        p_DeleteNode_11F2->node_address_list_.append(deleteMeterAddr);
    ...
    else if(frame==p_MonitorSlaveNode_13F1)
    {
        ...
        comPrtclType=p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(index)->prtcl;
...
void Script_ReadMeter_13F1_NodeNotInRouter::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_13F1_NodeNotInRouter::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
        ...
        case Wait_Finish_13F1:
        {
            ...
            if(++index>=times)
            {
                ...
                if(p_maxAllowTimer!=nullptr)
                    p_maxAllowTimer->stop();
                emScriptRunState=ScriptSuccess;
                p_AbstractScriptHost->updateProgress(ProcessState_Success, "13F1抄表抄读不在档案中从节点测试完成，不在档案内电表无法抄读！");
                return;
            }
            sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,index,p_MonitorSlaveNode_13F1);
...
void Script_ReadMeter_13F1_NodeNotInRouter::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_Finish_13F1:
...
void Script_ReadMeter_13F1_NodeNotInRouter::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    if(emScriptRunState==Wait_00F1_for_11F2)
    {
...
void Script_ReadMeter_13F1_NodeNotInRouter::CalcAvrgConsumeTimeLen(uchar rdFlag)
{
...
void Script_ReadMeter_13F1_NodeNotInRouter::statisticResult()
{
    index=0;
    CalcAvrgConsumeTimeLen(1);
    if(p_CtrInfoList->at(0)->successRate[1] == 1.0)
    {
        tryTimes=0;
        if(p_maxAllowTimer!=nullptr)
            p_maxAllowTimer->stop();
        emScriptRunState = ScriptSuccess;
        p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("13F1抄表-不在档案内支持抄读且成功率：%1%; 平均耗时：%2秒;").arg(p_CtrInfoList->at(0)->successRate[1]*100).arg(p_CtrInfoList->at(0)->successConsume[1]));
    }
    else
    {
        QString failedMeter = GenerateFailedMeterStr_ReadMeter(p_CtrInfoList->at(0),1);
        ...
        if(notInRouterFlag)
        {
            tryTimes=0;
            resultFlag=true;
            QString deleteMeterNo=QString(QByteArray(reinterpret_cast<char*>(p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(0)->mtrAddr),6).toHex());
            if(failedMeter.contains(deleteMeterNo))
            {
                if(p_maxAllowTimer!=nullptr)
                    p_maxAllowTimer->stop();
                emScriptRunState = ScriptSuccess;
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("13F1抄表-不在档案内不支持抄读，成功率：%1%，平均耗时：%2秒;").arg(p_CtrInfoList->at(0)->successRate[1]*100).arg(p_CtrInfoList->at(0)->successConsume[1]));
                p_AbstractScriptHost->updateProgress(ProcessState_Success, "不在档案内的电表抄读失败，符合预期，测试通过\n");
                return;
            }
        }
        if(++tryTimes>=3)
        {
            ...
            if(failedMeter.contains(deleteMeterNo))
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("13F1抄表-不在档案内不支持抄读，成功率：%1%，平均耗时：%2秒;").arg(p_CtrInfoList->at(0)->successRate[1]*100).arg(p_CtrInfoList->at(0)->successConsume[1]));
            }
            else
            {
                ...
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("13F1抄表-不在档案内支持抄读但成功率：%1%，平均耗时：%2秒;").arg(p_CtrInfoList->at(0)->successRate[1]*100).arg(p_CtrInfoList->at(0)->successConsume[1]));
                }
            }
//            if(!failedMeter.contains(deleteMeterNo))
//            {
//                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("失败表不包含不在档案内的电表%1\n").arg(deleteMeterNo));
//                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("13F1抄表-不在档案内测试失败，支持抄读但成功率：%1%，平均耗时：%2秒;").arg(p_CtrInfoList->at(0)->successRate[1]*100).arg(p_CtrInfoList->at(0)->successConsume[1]));
//            }
//            else
//            {
//                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("失败表包含不在档案内的电表%1\n").arg(deleteMeterNo));
//                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("13F1抄表-不在档案内测试失败，不支持抄读，成功率：%1%，平均耗时：%2秒;").arg(p_CtrInfoList->at(0)->successRate[1]*100).arg(p_CtrInfoList->at(0)->successConsume[1]));
//            }
        }
        else
        {
