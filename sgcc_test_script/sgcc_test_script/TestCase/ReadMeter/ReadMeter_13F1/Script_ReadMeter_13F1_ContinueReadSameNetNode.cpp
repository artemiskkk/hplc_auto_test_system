#include "Script_ReadMeter_13F1_ContinueReadSameNetNode.h"

Script_ReadMeter_13F1_ContinueReadSameNetNode::Script_ReadMeter_13F1_ContinueReadSameNetNode(QObject *parent) : QObject(parent)
{
    ...
    p_timer0=new QTimer(this);
    connect(p_timer,SIGNAL(timeout()),this,SLOT(timer_timeoutProc()));
    connect(p_maxAllowTimer,SIGNAL(timeout()),this,SLOT(maxAllowTimer_timeoutProc()));
    connect(p_delayTimer,SIGNAL(timeout()),this,SLOT(delayTimer_timeoutProc()));
    connect(p_timer0,SIGNAL(timeout()),this,SLOT(timer0_timeroutProc()));
}
Script_ReadMeter_13F1_ContinueReadSameNetNode::~Script_ReadMeter_13F1_ContinueReadSameNetNode()
{
    p_BuildNetwork_GW->initBuildNetWork();
    if(needPowerOff==true)//断电处理
        powerOffAll12V(p_CtrInfoList,p_AbstractScriptHost,true);
}
void Script_ReadMeter_13F1_ContinueReadSameNetNode::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
void Script_ReadMeter_13F1_ContinueReadSameNetNode::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
bool Script_ReadMeter_13F1_ContinueReadSameNetNode::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
/// 组网完成后，p_BuildNetwork_GW->buildNetworkResultFlag标志为true
/// 执行测试脚本的新操作即可
void Script_ReadMeter_13F1_ContinueReadSameNetNode::execute()
{
    p_AbstractScriptHost->updateProgress(ProcessState_Start, "Test start!");
    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "测试编号：【GW-CCO-F003-0005-V01】\n"
                                                                  "测试流程：\n"
                                                                  "1：组网，Wait_11F1_BuildNet_Finish；\n"
                                                                  "2：给路由下发12F2，暂停路由命令，Wait_12F12_RouterPause_Finish；\n"
                                                                  "3：连续下发2帧13F1监控从节点命令（针对某一个入网节点，376.2报文序列号不一致），通信协议类型0x02,采用645协议，抄读645电表数据，等待路由返回应答；（第一条命令在未回复结果之前，下发第二条），Wait_13F1_ReadMeterRepeat_Finish;");
...
void Script_ReadMeter_13F1_ContinueReadSameNetNode::stop()
{
    p_timer->stop();
    p_delayTimer->stop();
...
void Script_ReadMeter_13F1_ContinueReadSameNetNode::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeter_13F1_ContinueReadSameNetNode::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
...
void Script_ReadMeter_13F1_ContinueReadSameNetNode::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        case Wait_13F1_ReadMeterRepeat_Finish:
        {
            if(p_Frame3762Base->afn_==0x13&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                    ...
                    if(p_MonitorSlaveNode_13F1_Up->data_field_up_.protocol_type_==0x02)
                    {
                        ...
                        if(MsgBase_645_ptr!=nullptr)
                        {
                            if(MsgBase_645_ptr->ctrlCode_==NORMAL_RESP)
                            {
                                ...
                                {
                                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "第一次13F1上报成功，测试继续");
                                    times++;
                ...
                else if(p_MonitorSlaveNode_13F1_Up->data_field_up_.frame_length_==0 && times > 0)
                {
                    p_timer0->stop();
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, "13F1不应该有其他二次上报，不符合预期，脚本执行失败，【GW-CCO-F003-0005-V01】");
                }
                else
                {
                    p_timer->stop();
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_13F1_MeterReadOutsideArchives_Finish，13F1回复异常，期望回复帧序号为A的13F1上行报文，实际回复帧序号为B或上行报文数量不为1，【GW-CCO-F003-0005-V01】"));
...
void Script_ReadMeter_13F1_ContinueReadSameNetNode::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_ReadMeter_13F1_ContinueReadSameNetNode::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_13F1_ContinueReadSameNetNode::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_ReadMeter_13F1_ContinueReadSameNetNode::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_13F1_ContinueReadSameNetNode::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
    case Wait_BuildNetFinish_Whole:
    ...
    {
        p_timer->stop();
        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_13F1_MeterReadOutsideArchives_Finish，“timeout！”，【GW-CCO-F003-0005-V01】");
        break;
    }
    default:
    {
...
void Script_ReadMeter_13F1_ContinueReadSameNetNode::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
    default:
...
void Script_ReadMeter_13F1_ContinueReadSameNetNode::delayTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
    case Wait_BuildNetFinish_Whole:
...
void Script_ReadMeter_13F1_ContinueReadSameNetNode::timer0_timeroutProc()
{
    p_timer0->stop();
    switch(emScriptRunState)
    {
        case Wait_13F1_ReadMeterRepeat_Finish:
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "60s内无其他13F1上报，符合期望，测试成功！");
            if(p_maxAllowTimer!=nullptr)
                p_maxAllowTimer->stop();
            emScriptRunState = ScriptSuccess;
            p_AbstractScriptHost->updateProgress(ProcessState_Success, "脚本执行成功，【GW-CCO-F003-0005-V01】");
            break;
        }
        default:
        {
...
bool Script_ReadMeter_13F1_ContinueReadSameNetNode::extractAndProcess3762Frame(QByteArray& buf,QByteArray& completeFrame)
{
    completeFrame.clear();
