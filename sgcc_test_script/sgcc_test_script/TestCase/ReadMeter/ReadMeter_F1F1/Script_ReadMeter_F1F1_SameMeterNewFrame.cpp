#include "Script_ReadMeter_F1F1_SameMeterNewFrame.h"

Script_ReadMeter_F1F1_SameMeterNewFrame::Script_ReadMeter_F1F1_SameMeterNewFrame(QObject *parent) : QObject(parent)
{
...
Script_ReadMeter_F1F1_SameMeterNewFrame::~Script_ReadMeter_F1F1_SameMeterNewFrame()
{
...
void Script_ReadMeter_F1F1_SameMeterNewFrame::execute()
{
...
void Script_ReadMeter_F1F1_SameMeterNewFrame::stop()
{
...
void Script_ReadMeter_F1F1_SameMeterNewFrame::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ReadMeter_F1F1_SameMeterNewFrame::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ReadMeter_F1F1_SameMeterNewFrame::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ReadMeter_F1F1_SameMeterNewFrame::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeter_F1F1_SameMeterNewFrame::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ReadMeter_F1F1_SameMeterNewFrame::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        case Wait_00F1_for_12F2_Pause:
        {
            ...
            //    times=ushort(p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->size());
                ...
                emScriptRunState=Wait_00F2_for_sameF1F1;
        ...
        case Wait_00F2_for_sameF1F1:
        {
            if(p_Frame3762Base->afn_==0x00&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(p_Deny_Up->error_code_==0x6F)
                {
                  //  emScriptRunState=ScriptSuccess;
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("路由回复否认，错误码111"));
                    denyFlag=true;
                }
                else
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, "路由回复否认报文的错误码不符合要求："+QString::number(p_Deny_Up->error_code_));
                }
            }
            else if(p_Frame3762Base->afn_==char(0xF1)&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("收到第%0条F1F1并发回复报文，表号%1").arg(++index).arg(QString(QByteArray(p_ParallelReadMeter_F1F1_Up->address_field_.src_addr,6).toHex())));
                if(index==maxParallelNum)
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("连续下发%0条F1F1抄同一只表，路由未回否认").arg(maxParallelNum));
                }
            }
            else
            {
...
//                    {
//                        parallelCount--;
//                        if(readInfoList.at(currentMeterIndex).readFlag!=ReadSuccess)
//                        {
...
//                    {
//                        emScriptRunState=ScriptSuccess;
//                        p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("并发抄表测试完成，不允许并发给同一块电表多个读表帧，回复否认(111)，符合要求;"));
...
void Script_ReadMeter_F1F1_SameMeterNewFrame::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_ReadMeter_F1F1_SameMeterNewFrame::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_F1F1_SameMeterNewFrame::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_ReadMeter_F1F1_SameMeterNewFrame::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_F1F1_SameMeterNewFrame::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
    ...
    case Wait_00F2_for_sameF1F1:
    {
        if(denyFlag==true)
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Success, "并发抄表测试完成，不允许并发给同一块电表多个读表帧，回复否认(111)，符合要求");
...
void Script_ReadMeter_F1F1_SameMeterNewFrame::maxAllowTimer_timeoutProc()
{
...
void Script_ReadMeter_F1F1_SameMeterNewFrame::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    index=0;
    p_maxAllowTimer->stop();
...
void Script_ReadMeter_F1F1_SameMeterNewFrame::readInfoInit()
{
...
bool Script_ReadMeter_F1F1_SameMeterNewFrame::isMeterExist(Address address)
{
...
int Script_ReadMeter_F1F1_SameMeterNewFrame::getReadInfo(Address address)
{
    for(int i=0;i<readInfoList.size();i++)
    {
        if(address==readInfoList.at(i).meterNo)
...
double Script_ReadMeter_F1F1_SameMeterNewFrame::calSuccessRate()
{
...
QString Script_ReadMeter_F1F1_SameMeterNewFrame::calCostTime()
{
...
QString Script_ReadMeter_F1F1_SameMeterNewFrame::getFailMeterNo()
{
    QString failMeterNo;
    for(int i=0;i<readInfoList.size();i++)
    {
...
bool Script_ReadMeter_F1F1_SameMeterNewFrame::isAllRead(int meterID)
{
    bool sus=true;
    for(int i=0;i<readInfoList.at(meterID).dataUnitList.size();i++)
    {
...
bool Script_ReadMeter_F1F1_SameMeterNewFrame::isAllMeterReadSuccess()
{
    bool sus=true;
    for(int i=0;i<readInfoList.size();i++)
    {
...
void Script_ReadMeter_F1F1_SameMeterNewFrame::CalcAvrgConsumeTimeLen(uchar rdFlag)
{
...
bool Script_ReadMeter_F1F1_SameMeterNewFrame::extractAndProcess3762Frame(QByteArray& buf,QByteArray& completeFrame)
{
    completeFrame.clear();
