#include "Script_ReadMeter_F1F1_MultipleFrames.h"

Script_ReadMeter_F1F1_MultipleFrames::Script_ReadMeter_F1F1_MultipleFrames(QObject *parent) : QObject(parent)
{
...
Script_ReadMeter_F1F1_MultipleFrames::~Script_ReadMeter_F1F1_MultipleFrames()
{
...
void Script_ReadMeter_F1F1_MultipleFrames::execute()
{
    ...
    userCaseIds=QString("[GW-CCO-F005-0013-V01],");

    QString script_ID,script_Desc,script_CheckKeyPoint;

    script_ID=QString("用例ID：GW-CCO-F005-0010-V01 终端并发抄表-多条OOP \n\n");

    script_Desc+=QString("等待组网完成 Wait_BuildNetFinish_Whole \n");
    script_Desc+=QString("设置暂停路由，  Wait_00F1_for_12F2_Pause \n");
    script_Desc+=QString("测试1条OOP 5个OAD  Wait_F1F1_MeterRead1oop5oad_Finish \n");
    script_Desc+=QString("测试5条OOP 5个OAD  Wait_F1F1_MeterRead5oop1oad_Finish \n");
    script_Desc+=QString("等待脚本运行结束  ScriptSuccess \n");

    script_CheckKeyPoint=QString("检查点1：多条OOP也能抄回来 \n");
...
void Script_ReadMeter_F1F1_MultipleFrames::stop()
{
...
void Script_ReadMeter_F1F1_MultipleFrames::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ReadMeter_F1F1_MultipleFrames::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ReadMeter_F1F1_MultipleFrames::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ReadMeter_F1F1_MultipleFrames::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeter_F1F1_MultipleFrames::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ReadMeter_F1F1_MultipleFrames::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        case Wait_00F1_for_12F2_Pause:
        {
            if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                bool isTest=false;//本测试用例目的是测试oop表，下列遍历是为了确保表存在oop协议的，如果不存在则测试终止
                for(int i=2;i<readInfoList.size();i++)
                {
                    if(readInfoList.at(i).protocolType==0x03)
                    {
                        isTest=true;
                        break;
                    }
                ...
                if(!isTest)
                { p_AbstractScriptHost->updateProgress(ProcessState_Failed, "环境表计不存在oop表，测试停止");
                    break;
                }


                emScriptRunState=Wait_F1F1_MeterRead1oop5oad_Finish;
                //1

                sendMsg(CCO_GW,dvcId,meterIndex,p_ParallelReadMeter_F1F1);
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--并发抄表(1条oop5个oad)（F1F1），等待--抄表完成");
        ...
        case Wait_F1F1_MeterRead1oop5oad_Finish:
        {
            if(p_Frame3762Base->afn_==char(0xF1)&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(currentMeterIndex!=-1)
                {
                    ...
                    if(p_ParallelReadMeter_F1F1_Up->unit_up_.frame_length_!=0)////////////////////////////////////////////
                    {
                        ...
                        if(readInfoList.at(currentMeterIndex).protocolType==OOP)
                        {
                            ...
                            while(res)
                            {
                                ...
                                if(MsgBase_OOP_ptr==nullptr)continue;
                                ...
                                {
                                    if(MsgBase_OOP_ptr->service_type_==GET_RESPONSE_SERVER && MsgBase_OOP_ptr->service_sub_type_==uchar(GetResponseType::kGetResponseNormalList))
                                    {
                                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("F1F1并发抄表类型OOP 有效;"));

                                        shared_ptr<GetResponseNormalList> p_GetResponseNormalList=dynamic_pointer_cast<GetResponseNormalList>(MsgBase_OOP_ptr);

                                        if(p_GetResponseNormalList->list_result_normal_.size()==5)
                                        {
                                            oop_index=5;
                                            continue;
                                        }
                                        else
                                        {    oop_index=p_GetResponseNormalList->list_result_normal_.size();
                                            continue;
                                        }
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_F1F1_MeterRead1oop5oad_Finish 抄读协议为时，期望[回复oop数据]，实际[回复数据域为空],")+userCaseIds);
                        break;
                    }
                }
                else
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_F1F1_MeterRead1oop5oad_Finish F1F1 并发抄表上报数据地址非目标地址"));
                    break;

                }
                if(oop_index==5)
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("F1F1并发抄表-1个OOP5OAD成功"));
                else
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_F1F1_MeterRead1oop5oad_Finish,F1F1并发抄表-1个OOP5OAD,期望[回复oop数据5个oad],实际[回复数据域oop数据%1个oad],").arg(oop_index)+userCaseIds);
                ...
                index=0;
                meterIndex=0;
                emScriptRunState=Wait_F1F1_MeterRead5oop1oad_Finish;
                sendMsg(CCO_GW,dvcId,meterIndex,p_ParallelReadMeter_F1F1);
        ...
        case Wait_F1F1_MeterRead5oop1oad_Finish:
        {
            if(p_Frame3762Base->afn_==char(0xF1)&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_timer->stop();
                ...
                if(currentMeterIndex!=-1)
                {
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_F1F1_MeterRead5oop1oad_Finish 1条oop1个oad(共5条)，期望[回复5条oop]，实际[回复数据域为空]")+userCaseIds);
                        break;
                    }
                }
                else
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_F1F1_MeterRead1oop5oad_Finish F1F1  1条oop1个oad(共5条)，期望[回复5条oop],实际[并发抄表上报数据地址非目标地址]")+userCaseIds);
                    break;
                ...
                if(oop_index==5)
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_F1F1_MeterRead1oop5oad_Finish F1F1 1条oop1个oad(共5条),期望[回复5条oop]，实际[回复%1条oop]").arg(oop_index)+userCaseIds);
...
void Script_ReadMeter_F1F1_MultipleFrames::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_ReadMeter_F1F1_MultipleFrames::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_F1F1_MultipleFrames::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_ParallelReadMeter_F1F1)
    {
        ...
        for(int i=2;i<readInfoList.size();i++)
        {
            if(readInfoList.at(i).protocolType==OOP)
            {
                uchar tmpAddr[6];
                memcpy(tmpAddr,readInfoList.at(i).meterNo.addr,6);
                uchar tmpCcoAddr[6];
                ...
                if(emScriptRunState==Wait_F1F1_MeterRead5oop1oad_Finish)
                {
                    ...
                    for(int i=0;i<4;i++)msg698.append(tmpMsg);
                ...
                {
                    shared_ptr<GetRequestNormalList> p_GetRequestNormalList_ReadData=make_shared<GetRequestNormalList>();
                    p_GetRequestNormalList_ReadData->ctrl_field_.dir = 0;
                    p_GetRequestNormalList_ReadData->ctrl_field_.prm = 1;
                    p_GetRequestNormalList_ReadData->ctrl_field_.fra = 0;
                    p_GetRequestNormalList_ReadData->ctrl_field_.res = 0;
                    p_GetRequestNormalList_ReadData->ctrl_field_.sc = 0;
                    p_GetRequestNormalList_ReadData->ctrl_field_.func = 3;

                    p_GetRequestNormalList_ReadData->address_field_.sa.addr_type = 0;
                    p_GetRequestNormalList_ReadData->address_field_.sa.logic_addr = 0;
                    p_GetRequestNormalList_ReadData->address_field_.sa.addr_len = 5;
                    p_GetRequestNormalList_ReadData->address_field_.sa.address = QString((QByteArray(reinterpret_cast<char*>(tmpAddr),6).toHex()));
                    p_GetRequestNormalList_ReadData->address_field_.ca.address = 0x00;
                    ...
                    oad.element_index=1;
                    for(int i=0;i<5;i++)
                        p_GetRequestNormalList_ReadData->list_oad_.append(oad);


                    p_GetRequestNormalList_ReadData->piid_.reserve = 0;
                    p_GetRequestNormalList_ReadData->piid_.serve_priority = 0;
                    p_GetRequestNormalList_ReadData->piid_.serve_seq = 1;

                    p_GetRequestNormalList_ReadData->time_tag_field_.optional_ = 0;

                    QByteArray msg698=p_GetRequestNormalList_ReadData->EncodeFrame();
...
void Script_ReadMeter_F1F1_MultipleFrames::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_F1F1_MultipleFrames::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
    case Wait_BuildNetFinish_Whole:
    ...
    case Wait_F1F1_MeterRead1oop5oad_Finish:
    {
        ...
        p_AbstractScriptHost->updateProgress(ProcessState_Failed, userCaseIds+ QString("并发抄表1条oop5个oad时，F1F1并发抄表超时未回复").arg(uchar(protocol)));
    ...
    case Wait_F1F1_MeterRead5oop1oad_Finish:
    {
        if(p_maxAllowTimer!=nullptr) p_maxAllowTimer->stop();
        emScriptRunState=ScriptSuccess;
        p_AbstractScriptHost->updateProgress(ProcessState_Failed, userCaseIds+ QString("并发抄表5条oop5个oad时，F1F1并发抄表超时未回复").arg(uchar(protocol)));
...
void Script_ReadMeter_F1F1_MultipleFrames::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
    case Wait_F1F1_MeterRead1oop5oad_Finish:
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "Wait_F1F1_MeterRead1oop5oad_Finish timeout!!!"+QString("  F1F1并发抄表超时未回复"));
    ...
    case Wait_F1F1_MeterRead5oop1oad_Finish:
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "Wait_F1F1_MeterRead5oop1oad_Finish timeout!!!"+QString("  F1F1并发抄表超时未回复"));
        p_maxAllowTimer->stop();
        p_AbstractScriptHost->updateProgress(ProcessState_Error, "State machine run error!!!==p_maxAllowTimer");
        break;
    }
...
void Script_ReadMeter_F1F1_MultipleFrames::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
}

void Script_ReadMeter_F1F1_MultipleFrames::readInfoInit()
{
...
bool Script_ReadMeter_F1F1_MultipleFrames::isMeterExist(Address address)
{
...
int Script_ReadMeter_F1F1_MultipleFrames::getReadInfo(Address address)
{
    for(int i=0;i<readInfoList.size();i++)
    {
        if(address==readInfoList.at(i).meterNo)
...
double Script_ReadMeter_F1F1_MultipleFrames::calSuccessRate()
{
...
QString Script_ReadMeter_F1F1_MultipleFrames::calCostTime()
{
...
QString Script_ReadMeter_F1F1_MultipleFrames::getFailMeterNo()
{
    QString failMeterNo;
    int count=0;
    for(int i=0;i<readInfoList.size();i++)
...
bool Script_ReadMeter_F1F1_MultipleFrames::isAllRead(int meterID)
{
    bool sus=true;
...
bool Script_ReadMeter_F1F1_MultipleFrames::isAllMeterReadSuccess()
{
    bool sus=true;
    for(int i=0;i<readInfoList.size();i++)
    {
...
void Script_ReadMeter_F1F1_MultipleFrames::CalcAvrgConsumeTimeLen(uchar rdFlag)
{
...
bool Script_ReadMeter_F1F1_MultipleFrames::extractAndProcess3762Frame(QByteArray& buf,QByteArray& completeFrame)
{
    completeFrame.clear();
