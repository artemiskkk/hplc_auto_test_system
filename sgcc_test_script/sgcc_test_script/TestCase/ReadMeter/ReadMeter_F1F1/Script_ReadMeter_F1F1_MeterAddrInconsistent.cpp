#include "Script_ReadMeter_F1F1_MeterAddrInconsistent.h"
Script_ReadMeter_F1F1_MeterAddrInconsistent::Script_ReadMeter_F1F1_MeterAddrInconsistent(QObject *parent) : QObject(parent)
{
...
Script_ReadMeter_F1F1_MeterAddrInconsistent::~Script_ReadMeter_F1F1_MeterAddrInconsistent()
{
...
void Script_ReadMeter_F1F1_MeterAddrInconsistent::execute()
{
    ...
    userCaseIds=QString("[GW-CCO-F005-0011-V01],");

    QString script_ID,script_Desc,script_CheckKeyPoint;

    script_ID=QString("用例ID：GW-CCO-F005-0010-V01 终端并发抄表-表地址不一致 \n\n");

    script_Desc+=QString("等待组网完成 Wait_BuildNetFinish_Whole \n");
    script_Desc+=QString("设置暂停路由，  Wait_00F1_for_12F2_Pause \n");
    script_Desc+=QString("测试97协议抄读645表  Wait_F1F1_MeterRead97_Finish \n");
    script_Desc+=QString("测试07协议抄读645表  Wait_F1F1_MeterRead07_Finish \n");
    script_Desc+=QString("等待脚本运行结束  ScriptSuccess \n");

    script_CheckKeyPoint=QString("检查点1：97和07协议抄读，只能抄回第一个数据项 \n");
...
void Script_ReadMeter_F1F1_MeterAddrInconsistent::stop()
{
...
void Script_ReadMeter_F1F1_MeterAddrInconsistent::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ReadMeter_F1F1_MeterAddrInconsistent::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ReadMeter_F1F1_MeterAddrInconsistent::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ReadMeter_F1F1_MeterAddrInconsistent::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeter_F1F1_MeterAddrInconsistent::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
    ...
    case Wait_F1F1_MeterRead97_Finish:
    {
        break;
    }
...
void Script_ReadMeter_F1F1_MeterAddrInconsistent::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        case Wait_00F1_for_12F2_Pause:
        {
            if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(!isTest)
                { p_AbstractScriptHost->updateProgress(ProcessState_Failed, "环境表计不存在645表，测试停止");
                    break;
                }

                //                if(times!=1||p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(0)->prtcl!=2)
                //                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, "环境表计不等于1或者测试对象非645表，测试停止");

                protocol=RMP97;
                emScriptRunState=Wait_F1F1_MeterRead97_Finish;
                //1
        ...
        case Wait_F1F1_MeterRead97_Finish:
        {
            if(p_Frame3762Base->afn_==char(0xF1)&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(currentMeterIndex!=-1)
                {
                                        ...
                                        if(memcmp(p_ReadData_0x91->addr_,srcAddr.addr,6)==0)
                                        {
                                            for(int i=0;i<readInfoList.at(currentMeterIndex).dataUnitList.size();i++)
                                            {
                                                if(memcmp(readInfoList.at(currentMeterIndex).dataUnitList.at(i).dataID,p_ReadData_0x91->di,4)==0)
                                                {
                                                    if(i==1)
                                                    {
                                                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_F1F1_MeterRead97_Finish 抄读协议为%1时，第2个数据项地址错误也能抄回").arg(uchar(protocol)));
                                                        break;
                                                    }
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_F1F1_MeterRead97_Finish 抄读协议为%1时，F1F1回复数据域为空").arg(uchar(protocol)));
                        break;
                    }
                }
                ...
                index=0;
                meterIndex=0;
                emScriptRunState=Wait_F1F1_MeterRead07_Finish;
                sendMsg(CCO_GW,dvcId,meterIndex,p_ParallelReadMeter_F1F1);
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--并发抄表（F1F1），等待--抄表完成");
                //开始计时
                p_timer->start((maxMonitorTime*2)*1000);
            ...
            {
                 p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("收到否认帧，cco不支持换表地址抄读"));
        ...
        case Wait_F1F1_MeterRead07_Finish:
        {
            if(p_Frame3762Base->afn_==char(0xF1)&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_timer->stop();
                ...
                if(currentMeterIndex!=-1)
                {
                                        ...
                                        if(memcmp(p_ReadData_0x91->addr_,srcAddr.addr,6)==0)
                                        {
                                            for(int i=0;i<readInfoList.at(currentMeterIndex).dataUnitList.size();i++)
                                            {
                                                if(memcmp(readInfoList.at(currentMeterIndex).dataUnitList.at(i).dataID,p_ReadData_0x91->di,4)==0)
                                                {
                                                    if(i==1)
                                                    {
                                                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_F1F1_MeterRead07_Finish 抄读协议为%1时，第2个数据项地址错误也能抄回").arg(uchar(protocol)));
                                                        break;
                                                    }
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_F1F1_MeterRead07_Finish 抄读协议为%1时，F1F1回复数据域为空").arg(uchar(protocol)));
                        break;
                    }
                }
...
void Script_ReadMeter_F1F1_MeterAddrInconsistent::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_ReadMeter_F1F1_MeterAddrInconsistent::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_F1F1_MeterAddrInconsistent::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_ReadMeter_F1F1_MeterAddrInconsistent::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_F1F1_MeterAddrInconsistent::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
    case Wait_BuildNetFinish_Whole:
    ...
    case Wait_F1F1_MeterRead97_Finish:
    {
        ...
        p_AbstractScriptHost->updateProgress(ProcessState_Failed, userCaseIds+ QString("抄表协议协议%1时，F1F1并发抄表超时未回复").arg(uchar(protocol)));
...
void Script_ReadMeter_F1F1_MeterAddrInconsistent::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
    case Wait_F1F1_MeterRead97_Finish:
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "Wait_F1F1_MeterRead97_Finish timeout!!!"+QString("  F1F1抄表-协议为97时抄读失败"));
    ...
    case Wait_F1F1_MeterRead07_Finish:
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "Wait_F1F1_MeterRead07_Finish timeout!!!"+QString("  F1F1抄表-协议为07时抄读失败"));
        p_maxAllowTimer->stop();
        p_AbstractScriptHost->updateProgress(ProcessState_Error, "State machine run error!!!==p_maxAllowTimer");
        break;
    }
...
void Script_ReadMeter_F1F1_MeterAddrInconsistent::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
}

void Script_ReadMeter_F1F1_MeterAddrInconsistent::readInfoInit()
{
...
bool Script_ReadMeter_F1F1_MeterAddrInconsistent::isMeterExist(Address address)
{
    for(int i=0;i<readInfoList.size();i++)
    {
...
int Script_ReadMeter_F1F1_MeterAddrInconsistent::getReadInfo(Address address)
{
    for(int i=0;i<readInfoList.size();i++)
    {
        if(address==readInfoList.at(i).meterNo)
...
double Script_ReadMeter_F1F1_MeterAddrInconsistent::calSuccessRate()
{
...
QString Script_ReadMeter_F1F1_MeterAddrInconsistent::calCostTime()
{
...
QString Script_ReadMeter_F1F1_MeterAddrInconsistent::getFailMeterNo()
{
    QString failMeterNo;
    int count=0;
    for(int i=0;i<readInfoList.size();i++)
...
bool Script_ReadMeter_F1F1_MeterAddrInconsistent::isAllRead(int meterID)
{
    bool sus=true;
    for(int i=0;i<readInfoList.at(meterID).dataUnitList.size();i++)
    {
...
bool Script_ReadMeter_F1F1_MeterAddrInconsistent::isAllMeterReadSuccess()
{
    bool sus=true;
    for(int i=0;i<readInfoList.size();i++)
    {
...
void Script_ReadMeter_F1F1_MeterAddrInconsistent::CalcAvrgConsumeTimeLen(uchar rdFlag)
{
...
bool Script_ReadMeter_F1F1_MeterAddrInconsistent::extractAndProcess3762Frame(QByteArray& buf,QByteArray& completeFrame)
{
    completeFrame.clear();
