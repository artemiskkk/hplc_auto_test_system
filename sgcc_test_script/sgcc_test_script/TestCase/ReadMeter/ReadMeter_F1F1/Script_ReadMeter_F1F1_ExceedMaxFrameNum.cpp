#include "Script_ReadMeter_F1F1_ExceedMaxFrameNum.h"

Script_ReadMeter_F1F1_ExceedMaxFrameNum::Script_ReadMeter_F1F1_ExceedMaxFrameNum(QObject *parent) : QObject(parent)
{
...
Script_ReadMeter_F1F1_ExceedMaxFrameNum::~Script_ReadMeter_F1F1_ExceedMaxFrameNum()
{
...
void Script_ReadMeter_F1F1_ExceedMaxFrameNum::execute()
{
...
void Script_ReadMeter_F1F1_ExceedMaxFrameNum::stop()
{
...
void Script_ReadMeter_F1F1_ExceedMaxFrameNum::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ReadMeter_F1F1_ExceedMaxFrameNum::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ReadMeter_F1F1_ExceedMaxFrameNum::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ReadMeter_F1F1_ExceedMaxFrameNum::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeter_F1F1_ExceedMaxFrameNum::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ReadMeter_F1F1_ExceedMaxFrameNum::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_Finish_F1F1:
            {
                ...
                {
                    if(flagReadInfoInit==true)
                    {
                        flagReadInfoInit=false;
                        if(p_maxAllowTimer!=nullptr) p_maxAllowTimer->stop();
                        emScriptRunState=ScriptSuccess;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("F1F1并发抄表-最大帧数测试成功，最大帧数%3;").arg(maxFrameNum));
                        p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("F1F1并发抄表-最大帧数测试成功，最大帧数%3;").arg(maxFrameNum));

                        return;
//                        maxParallelNum=5;//切换最大并发数
//                        readInfoInit();
//                        index=0;
//                        meterIndex=0;
//                        times=ushort(p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->size());
                                        ...
                                        if(MsgBase_645_ptr->ctrlCode_==NORMAL_RESP)
                                        {
                                            ...
                                            if(memcmp(p_ReadData_0x91->addr_,srcAddr.addr,6)==0)
                                            {
                                                for(int i=0;i<readInfoList.at(currentMeterIndex).dataUnitList.size();i++)
                                                {
                                                    ...
                                                    {
                                                        readInfoList[currentMeterIndex].dataUnitList[i].isThisIDReadSuccess=true;
...
//                                        {
//                                            if(readInfoList.at(currentMeterIndex).dataUnitList.at(idIndex).isThisIDReadSuccess==true)
//                                            {
//                                                idIndex+=maxFrameNum;
//                                            }
//                                            readInfoList[currentMeterIndex].dataUnitList[idIndex].isThisIDReadSuccess=true;
//                                            endTime=QDateTime::currentDateTime().toString(CURRENT_TIME_FORMAT);
//                                            spendTime=calculateConsumeLen(startTime,endTime);
//                                            readInfoList[currentMeterIndex].dataUnitList[idIndex].costTime=spendTime.toDouble();
//                                            idIndex++;
//                                        }
                                    }
                                    if(isThisMeterReadSuccess(currentMeterIndex)==true)
                    ...
                    if(parallelCount==0&&meterIndex>=readInfoList.size())
                    {
                        if(p_maxAllowTimer!=nullptr) p_maxAllowTimer->stop();
                        emScriptRunState=ScriptSuccess;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("F1F1并发抄表成功率：%1%; 抄表平均耗时：%2秒;最大帧数%3").arg(calSuccessRate()*100).arg(calCostTime()).arg(maxFrameNum));
                        if(calSuccessRate() >= readMeterSucRateThresld)
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("F1F1并发抄表-最大帧数测试成功，成功率：%1%，平均耗时：%2秒，最大帧数%3;").arg(calSuccessRate()*100).arg(calCostTime()).arg(maxFrameNum));
                        }
                        else
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("失败电表清单如下%1\n").arg(getFailMeterNo()));
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("F1F1并发抄表-最大帧数测试失败，成功率：%1%，平均耗时：%2秒，最大帧数%3;").arg(calSuccessRate()*100).arg(calCostTime()).arg(maxFrameNum));

                        }
                    }
                    else
                    {
                        if(isAllDataIDRead(currentMeterIndex)==false)
                        {
                        ...
                        while(parallelCount<maxParallelNum)
                        {
                            if(meterIndex>=readInfoList.size())
                                break;
                            if(isAllDataIDRead(meterIndex)==false)
                            {
                                sendMsg(CCO_GW,dvcId,meterIndex,p_ParallelReadMeter_F1F1);
                                parallelCount++;
                                meterIndex++;
                ...
                else if(p_Frame3762Base->afn_==0x00&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(maxFrameNum==0)
                    {
                        if(p_maxAllowTimer!=nullptr) p_maxAllowTimer->stop();
                        emScriptRunState=ScriptSuccess;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("F1F1并发抄表-最大帧数测试失败，最大帧数%3;").arg(maxFrameNum));
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("F1F1并发抄表-最大帧数测试失败，最大帧数%3;").arg(maxFrameNum));
                    }
                    if(p_Deny_Up->error_code_==0x6E)
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("并发抄表-最大帧数测试，回复否认(110)"));
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("并发抄表-最大帧数测试，当前最大帧数%1").arg(maxFrameNum));
                        parallelCount--;
                        meterIndex--;
                        maxFrameNum--;
                        for(int i=0;i<readInfoList.at(meterIndex).dataUnitList.size();i++)
                        {
                            readInfoList[meterIndex].dataUnitList[i].notRead=true;
                        }
...
void Script_ReadMeter_F1F1_ExceedMaxFrameNum::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_ReadMeter_F1F1_ExceedMaxFrameNum::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_F1F1_ExceedMaxFrameNum::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
            ...
            for(int i=0;i<readInfoList.at(meterID).dataUnitList.size();i++)
            {
                if(frameCount<maxFrameNum)
                {
                    if(readInfoList.at(meterID).dataUnitList.at(i).notRead==true)
                    {
                        ...
                        readInfoList[meterID].dataUnitList[i].notRead=false;
                        frameCount++;
                    }
                }
                else
...
void Script_ReadMeter_F1F1_ExceedMaxFrameNum::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_F1F1_ExceedMaxFrameNum::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_ReadMeter_F1F1_ExceedMaxFrameNum::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_Finish_F1F1:
...
void Script_ReadMeter_F1F1_ExceedMaxFrameNum::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    index=0;
    p_maxAllowTimer->stop();
...
void Script_ReadMeter_F1F1_ExceedMaxFrameNum::readInfoInit()
{
...
bool Script_ReadMeter_F1F1_ExceedMaxFrameNum::isMeterExist(Address address)
{
...
int Script_ReadMeter_F1F1_ExceedMaxFrameNum::getReadInfo(Address address)
{
    for(int i=0;i<readInfoList.size();i++)
    {
        if(address==readInfoList.at(i).meterNo)
...
double Script_ReadMeter_F1F1_ExceedMaxFrameNum::calSuccessRate()
{
...
QString Script_ReadMeter_F1F1_ExceedMaxFrameNum::calCostTime()
{
...
QString Script_ReadMeter_F1F1_ExceedMaxFrameNum::getFailMeterNo()
{
    QString failMeterNo;
    int count=0;
    for(int i=0;i<readInfoList.size();i++)
...
bool Script_ReadMeter_F1F1_ExceedMaxFrameNum::isAllDataIDRead(int meterID)
{
    bool sus=true;
    for(int i=0;i<readInfoList.at(meterID).dataUnitList.size();i++)
    {
...
bool Script_ReadMeter_F1F1_ExceedMaxFrameNum::isThisMeterReadSuccess(int meterID)
{
    bool sus=true;
    for(int i=0;i<readInfoList.at(meterID).dataUnitList.size();i++)
    {
        if(readInfoList.at(meterID).dataUnitList.at(i).isThisIDReadSuccess==false)
        {
            sus=false;
            break;
        }
...
bool Script_ReadMeter_F1F1_ExceedMaxFrameNum::isAllMeterReadSuccess()
{
    bool sus=true;
    for(int i=0;i<readInfoList.size();i++)
    {
...
void Script_ReadMeter_F1F1_ExceedMaxFrameNum::CalcAvrgConsumeTimeLen(uchar rdFlag)
{
...
bool Script_ReadMeter_F1F1_ExceedMaxFrameNum::extractAndProcess3762Frame(QByteArray& buf,QByteArray& completeFrame)
{
    completeFrame.clear();
