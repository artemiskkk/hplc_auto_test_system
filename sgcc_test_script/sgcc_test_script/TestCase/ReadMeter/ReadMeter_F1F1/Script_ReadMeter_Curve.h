#ifndef SCRIPT_READMETER_CURVE_H
#define SCRIPT_READMETER_CURVE_H

#include <QObject>
#include "TestCase/BuildNetwork_GW.h"
enum Script_ReadMeter_Curve_RunState
{
    ReadMeterInit,
...
namespace Namespace_ReadMeter_Curve
{
    struct ReadDataUnit
    {
        QByteArray dataID;
...
using namespace Namespace_ReadMeter_Curve;
class Script_ReadMeter_Curve : public QObject, public AbstractScript,public DynamicCreate<Script_ReadMeter_Curve>
{
    Q_OBJECT
//    Q_PLUGIN_METADATA(IID "com.topscomm.AbstractScript")
//    Q_INTERFACES(AbstractScript)
public:
    explicit Script_ReadMeter_Curve(QObject *parent = nullptr);
    ~Script_ReadMeter_Curve();

    Script_ReadMeter_Curve_RunState emScriptRunState;
    ...
    int maxFrameNum=4;//单帧最大报文数
    int maxParallelNum=4;//最大并行数
    int readPeriod=15;//15分钟曲线
    int maxReadRound=10;//最大轮次

    int parallelCount=0;//并行数
    int meterIndex=0;//电表序号
    int currentReadRound=0;//当前轮次
...
#endif // SCRIPT_READMETER_CURVE_H
