#include "Script_ReadMeter_F1F1_SubsidiaryNode.h"

Script_ReadMeter_F1F1_SubsidiaryNode::Script_ReadMeter_F1F1_SubsidiaryNode(QObject *parent) : QObject(parent)
{
...
Script_ReadMeter_F1F1_SubsidiaryNode::~Script_ReadMeter_F1F1_SubsidiaryNode()
{
...
void Script_ReadMeter_F1F1_SubsidiaryNode::execute()
{
...
void Script_ReadMeter_F1F1_SubsidiaryNode::stop()
{
...
void Script_ReadMeter_F1F1_SubsidiaryNode::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ReadMeter_F1F1_SubsidiaryNode::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ReadMeter_F1F1_SubsidiaryNode::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ReadMeter_F1F1_SubsidiaryNode::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeter_F1F1_SubsidiaryNode::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ReadMeter_F1F1_SubsidiaryNode::processMsgFromCCO(DvcType dvcType, int dvcId)
{
            ...
            case Wait_Finish_F1F1:
            {
                if(p_Frame3762Base->afn_==char(0xF1)&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(isAllMeterReadSuccess())
                    {
                        ...
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("当前F1F1并发抄表成功率：%1%;").arg(calSuccessRate()*100));
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("F1F1并发抄表成功率：%1%; 抄表平均耗时：%2秒;").arg(calSuccessRate()*100).arg(calCostTime()));
                        p_AbstractScriptHost->updateProgress(ProcessState_Success, "并发抄表，支持带附属节点地址！");
                    }
                    else
                    {
                        parallelCount--;
...
bool Script_ReadMeter_F1F1_SubsidiaryNode::extractAndProcess3762Frame(QByteArray& buf,QByteArray& completeFrame)
{
    completeFrame.clear();
...
void Script_ReadMeter_F1F1_SubsidiaryNode::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_ReadMeter_F1F1_SubsidiaryNode::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_F1F1_SubsidiaryNode::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_ParallelReadMeter_F1F1)
    {
        ...
        p_ParallelReadMeter_F1F1->unit_down_.subsidiary_node_address_list_.clear();
        if(meterID<7)
        {
            for(int i=0;i<meterID;i++)
            {
                Address subAddress;
                memcpy(subAddress.addr,readInfoList.at(i).meterNo.addr,6);
                p_ParallelReadMeter_F1F1->unit_down_.subsidiary_node_address_list_.append(subAddress);
            }
        }
        else if(meterID>=7)
        {
            ...
            {
                Address subAddress;
                memcpy(subAddress.addr,readInfoList.at(meterID-7+i).meterNo.addr,6);
                p_ParallelReadMeter_F1F1->unit_down_.subsidiary_node_address_list_.append(subAddress);
            }
        }
        p_ParallelReadMeter_F1F1->unit_down_.subsidiary_node_num_=uchar(p_ParallelReadMeter_F1F1->unit_down_.subsidiary_node_address_list_.size());
        ...
        if(readInfoList.at(meterID).protocolType==DLT645_2007)
        {
            ...
            p_ParallelReadMeter_F1F1->unit_down_.protocol_type_=DLT645_2007;
        ...
        else if(readInfoList.at(meterID).protocolType==OOP)
        {
            ...
            //p_ParallelReadMeter_F1F1->unit_down_.subsidiary_node_num_=0x00;
            p_ParallelReadMeter_F1F1->unit_down_.protocol_type_=OOP;
...
void Script_ReadMeter_F1F1_SubsidiaryNode::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_F1F1_SubsidiaryNode::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
        ...
        case Wait_Finish_F1F1:
        {
            ...
            if(failCount>=times)
            {
                ...
                if(p_maxAllowTimer!=nullptr) p_maxAllowTimer->stop();
                emScriptRunState=ScriptSuccess;
                p_AbstractScriptHost->updateProgress(ProcessState_Success, "并发抄表，不支持带附属节点地址！");
            }
            break;
        }
        case Wait_00F1_for_12F3_Recover:
...
void Script_ReadMeter_F1F1_SubsidiaryNode::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_Finish_F1F1:
...
void Script_ReadMeter_F1F1_SubsidiaryNode::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    index=0;
    p_maxAllowTimer->stop();
...
void Script_ReadMeter_F1F1_SubsidiaryNode::readInfoInit()
{
...
bool Script_ReadMeter_F1F1_SubsidiaryNode::isMeterExist(Address address)
{
...
int Script_ReadMeter_F1F1_SubsidiaryNode::getReadInfo(Address address)
{
    for(int i=0;i<readInfoList.size();i++)
    {
        if(address==readInfoList.at(i).meterNo)
...
double Script_ReadMeter_F1F1_SubsidiaryNode::calSuccessRate()
{
...
QString Script_ReadMeter_F1F1_SubsidiaryNode::calCostTime()
{
...
QString Script_ReadMeter_F1F1_SubsidiaryNode::getFailMeterNo()
{
    QString failMeterNo;
    for(int i=0;i<readInfoList.size();i++)
    {
...
bool Script_ReadMeter_F1F1_SubsidiaryNode::isAllRead(int meterID)
{
    bool sus=true;
    for(int i=0;i<readInfoList.at(meterID).dataUnitList.size();i++)
    {
...
bool Script_ReadMeter_F1F1_SubsidiaryNode::isAllMeterReadSuccess()
{
    bool sus=true;
    for(int i=0;i<readInfoList.size();i++)
    {
...
void Script_ReadMeter_F1F1_SubsidiaryNode::CalcAvrgConsumeTimeLen(uchar rdFlag)
{
