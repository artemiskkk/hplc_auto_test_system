#include "Script_ReadMeter_F1F1_RelayAddress.h"

Script_ReadMeter_F1F1_RelayAddress::Script_ReadMeter_F1F1_RelayAddress(QObject *parent) : QObject(parent)
{
...
Script_ReadMeter_F1F1_RelayAddress::~Script_ReadMeter_F1F1_RelayAddress()
{
...
void Script_ReadMeter_F1F1_RelayAddress::execute()
{
...
void Script_ReadMeter_F1F1_RelayAddress::stop()
{
...
void Script_ReadMeter_F1F1_RelayAddress::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ReadMeter_F1F1_RelayAddress::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ReadMeter_F1F1_RelayAddress::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ReadMeter_F1F1_RelayAddress::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeter_F1F1_RelayAddress::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ReadMeter_F1F1_RelayAddress::processMsgFromCCO(DvcType dvcType, int dvcId)
{
            ...
            case Wait_Finish_F1F1:
            {
                if(p_Frame3762Base->afn_==char(0xF1)&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(isAllMeterReadSuccess())
                    {
                        ...
                        p_AbstractScriptHost->updateProgress(ProcessState_Success, "并发抄表，支持带中继地址！");
                    }
                    else
                    {
                        parallelCount--;
...
void Script_ReadMeter_F1F1_RelayAddress::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_ReadMeter_F1F1_RelayAddress::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_F1F1_RelayAddress::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_ParallelReadMeter_F1F1)
    {
        ...
        p_ParallelReadMeter_F1F1->address_field_.delay_addr.clear();
        if(meterID<7)
        {
            for(int i=0;i<meterID;i++)
            {
                shared_ptr<Address> relayAddress=make_shared<Address>();
                memcpy(relayAddress->addr,p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(i)->mtrAddr,6);
                p_ParallelReadMeter_F1F1->address_field_.delay_addr.append(relayAddress);
            }
        }
        else if(meterID>=7)
        {
            ...
            {
                shared_ptr<Address> relayAddress=make_shared<Address>();
                memcpy(relayAddress->addr,p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(meterID-7+i)->mtrAddr,6);
                p_ParallelReadMeter_F1F1->address_field_.delay_addr.append(relayAddress);
            }
        }
        p_ParallelReadMeter_F1F1->info_field_.info_field_down.relay_level=uchar(p_ParallelReadMeter_F1F1->address_field_.delay_addr.size());
...
void Script_ReadMeter_F1F1_RelayAddress::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_F1F1_RelayAddress::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
        ...
        case Wait_Finish_F1F1:
        {
            ...
            if(failCount>=times)
            {
                ...
                if(p_maxAllowTimer!=nullptr) p_maxAllowTimer->stop();
                emScriptRunState=ScriptSuccess;
                p_AbstractScriptHost->updateProgress(ProcessState_Success, "并发抄表，不支持带中继地址！");
            }
            break;
        }
        case Wait_00F1_for_12F3_Recover:
...
void Script_ReadMeter_F1F1_RelayAddress::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_Finish_F1F1:
...
void Script_ReadMeter_F1F1_RelayAddress::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    index=0;
    p_maxAllowTimer->stop();
...
void Script_ReadMeter_F1F1_RelayAddress::readInfoInit()
{
...
bool Script_ReadMeter_F1F1_RelayAddress::isMeterExist(Address address)
{
...
int Script_ReadMeter_F1F1_RelayAddress::getReadInfo(Address address)
{
    for(int i=0;i<readInfoList.size();i++)
    {
        if(address==readInfoList.at(i).meterNo)
...
double Script_ReadMeter_F1F1_RelayAddress::calSuccessRate()
{
...
QString Script_ReadMeter_F1F1_RelayAddress::calCostTime()
{
...
QString Script_ReadMeter_F1F1_RelayAddress::getFailMeterNo()
{
    QString failMeterNo;
    int count=0;
    for(int i=0;i<readInfoList.size();i++)
...
bool Script_ReadMeter_F1F1_RelayAddress::isAllRead(int meterID)
{
    bool sus=true;
    for(int i=0;i<readInfoList.at(meterID).dataUnitList.size();i++)
    {
...
bool Script_ReadMeter_F1F1_RelayAddress::isAllMeterReadSuccess()
{
    bool sus=true;
    for(int i=0;i<readInfoList.size();i++)
    {
...
void Script_ReadMeter_F1F1_RelayAddress::CalcAvrgConsumeTimeLen(uchar rdFlag)
{
...
bool Script_ReadMeter_F1F1_RelayAddress::extractAndProcess3762Frame(QByteArray& buf,QByteArray& completeFrame)
{
    completeFrame.clear();
