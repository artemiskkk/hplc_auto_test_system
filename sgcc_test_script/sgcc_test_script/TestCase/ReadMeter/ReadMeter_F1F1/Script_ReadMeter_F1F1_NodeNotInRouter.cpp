#include "Script_ReadMeter_F1F1_NodeNotInRouter.h"

Script_ReadMeter_F1F1_NodeNotInRouter::Script_ReadMeter_F1F1_NodeNotInRouter(QObject *parent) : QObject(parent)
{
    ...
    p_DeleteNode_11F2=make_shared<Afn11F2>();
...
Script_ReadMeter_F1F1_NodeNotInRouter::~Script_ReadMeter_F1F1_NodeNotInRouter()
{
...
void Script_ReadMeter_F1F1_NodeNotInRouter::execute()
{
    ...
    else
    {
        readInfoInit();
        tryTimes=0;
        emScriptRunState=Wait_00F1_for_11F2;
        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_DeleteNode_11F2);
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--删除从节点（11F2），等待--确认");
        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
    }
    p_maxAllowTimer->start((timerForReachThresld+timerAfterReachThresld)*1000);

}
void Script_ReadMeter_F1F1_NodeNotInRouter::stop()
{
...
void Script_ReadMeter_F1F1_NodeNotInRouter::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ReadMeter_F1F1_NodeNotInRouter::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ReadMeter_F1F1_NodeNotInRouter::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ReadMeter_F1F1_NodeNotInRouter::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
    ...
    if(emScriptRunState==Wait_BuildNetFinish_Whole)
    {
        ...
        else
        {
            if(p_BuildNetwork_GW->emScriptRunState==BuildNetFinish&&p_BuildNetwork_GW->buildNetworkResultFlag==true)
            {
                ...
                tryTimes=0;
                emScriptRunState=Wait_00F1_for_11F2;
                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,id,p_DeleteNode_11F2);
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--删除从节点（11F2），等待--确认");
                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
            }
        }
...
void Script_ReadMeter_F1F1_NodeNotInRouter::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ReadMeter_F1F1_NodeNotInRouter::processMsgFromCCO(DvcType dvcType, int dvcId)
{
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_00F1_for_11F2:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing,"删除从节点，回复确认");
            ...
            case Wait_Finish_F1F1:
            {
                if(p_Frame3762Base->afn_==char(0xF1)&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("parallelCount%1，meterIndex%2，readInfoList.size()%3").arg(parallelCount).arg(parallelCount).arg(meterIndex).arg(readInfoList.size()));
                    if(parallelCount==1&&0!=readInfoList.size())
                    {
                        if(p_maxAllowTimer!=nullptr) p_maxAllowTimer->stop();
                        emScriptRunState=ScriptSuccess;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("当前F1F1并发抄表成功率：%1%;").arg(calSuccessRate()*100));
                        QString failMeterNo=getFailMeterNo();
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("失败电表清单如下%1\n").arg(failMeterNo));

                        QString deleteMeterNo=QString(QByteArray(readInfoList.at(0).meterNo.addr,6).toHex());
                        if(failMeterNo.contains(deleteMeterNo))
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("不在档案内的电表%1抄读失败\n").arg(deleteMeterNo));
                            p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("F1F1并发抄表-不在档案内不支持抄读，成功率：%1%，平均耗时：%2秒;").arg(calSuccessRate()*100).arg(calCostTime()));
                        }
                        else
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("不在档案内的电表%1抄读成功\n").arg(deleteMeterNo));
                            if(calSuccessRate() >= readMeterSucRateThresld)
                            {
                                p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("F1F1并发抄表-不在档案内支持抄读且成功率：%1%，平均耗时：%2秒;").arg(calSuccessRate()*100).arg(calCostTime()));
                            }
                            else
                            {
                                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("F1F1并发抄表-不在档案内支持抄读但成功率：%1%，平均耗时：%2秒;").arg(calSuccessRate()*100).arg(calCostTime()));
                            }
                        }
//                        if(calSuccessRate() >= readMeterSucRateThresld)
//                        {
//                            p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("F1F1并发抄表-不在档案内测试成功，支持抄读且成功率：%1%，平均耗时：%2秒;").arg(calSuccessRate()*100).arg(calCostTime()));
//                        }
//                        else
//                        {
//                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("失败电表清单如下%1\n").arg(getFailMeterNo()));
...
//                            if(!failMeterNo.contains(deleteMeterNo))
//                            {
//                                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("失败表不包含不在档案内的电表%1\n").arg(deleteMeterNo));
//                                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("F1F1并发抄表-不在档案内测试失败，支持抄读但成功率：%1%，平均耗时：%2秒;").arg(calSuccessRate()*100).arg(calCostTime()));
//                            }
//                            else
//                            {
//                                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("失败表包含不在档案内的电表%1\n").arg(deleteMeterNo));
//                                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("F1F1并发抄表-不在档案内测试失败，不支持抄读，成功率：%1%，平均耗时：%2秒;").arg(calSuccessRate()*100).arg(calCostTime()));
//                            }
//                        }
                    }
                    else
            ...
            case Wait_Finish_NodeNotInRouter:
            {
                if(p_Frame3762Base->afn_==char(0xF1)&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    if(p_maxAllowTimer!=nullptr) p_maxAllowTimer->stop();
                    emScriptRunState=ScriptSuccess;
                    p_AbstractScriptHost->updateProgress(ProcessState_Success,QString("并发抄表测试完成，支持不在档案内的电表抄读！"));
                }
                break;
            }
...
void Script_ReadMeter_F1F1_NodeNotInRouter::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_ReadMeter_F1F1_NodeNotInRouter::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_F1F1_NodeNotInRouter::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    if(frame==p_DeleteNode_11F2)
    {
        p_DeleteNode_11F2->ctrl_field_.dir=kDirDown;
        p_DeleteNode_11F2->ctrl_field_.prm=kActive;
        p_DeleteNode_11F2->ctrl_field_.comn_type=kHplc;

        p_DeleteNode_11F2->info_field_.info_field_down.msg_seq=char(msgSeq++);
        p_DeleteNode_11F2->info_field_.info_field_down.comu_rate=0;
        p_DeleteNode_11F2->info_field_.info_field_down.comu_module_ident=0;

        p_DeleteNode_11F2->node_address_list_.clear();
        p_DeleteNode_11F2->node_address_list_.append(readInfoList.at(0).meterNo);
        p_DeleteNode_11F2->node_num_=uchar(p_DeleteNode_11F2->node_address_list_.size());

        sendMsgOct=p_DeleteNode_11F2->EncodeFrame();
...
void Script_ReadMeter_F1F1_NodeNotInRouter::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_F1F1_NodeNotInRouter::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
        ...
        case Wait_Finish_F1F1:
        {
            p_timer->stop();
            emScriptRunState=ScriptSuccess;
            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("F1F1回复超时，当前F1F1并发抄表成功率：%1%;").arg(calSuccessRate()*100));
            QString failMeterNo=getFailMeterNo();
            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("失败电表清单如下%1\n").arg(failMeterNo));
            ...
            else
            {
                ...
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("不在档案内的电表%1抄读失败\n").arg(deleteMeterNo));
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("F1F1并发抄表-不在档案内不支持抄读，成功率：%1%，平均耗时：%2秒;").arg(calSuccessRate()*100).arg(calCostTime()));
                }
                else
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("不在档案内的电表%1抄读成功\n").arg(deleteMeterNo));
        ...
        case Wait_Finish_NodeNotInRouter:
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "Wait_Finish_NodeNotInRouter timeout!!! Read Next Meter");
            emScriptRunState=ScriptSuccess;
            p_AbstractScriptHost->updateProgress(ProcessState_Success,QString("并发抄表测试完成，不支持不在档案内的电表抄读！"));
            break;
        }
        case Wait_00F1_for_12F3_Recover:
        {
...
void Script_ReadMeter_F1F1_NodeNotInRouter::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_Finish_F1F1:
...
void Script_ReadMeter_F1F1_NodeNotInRouter::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    index=0;
    p_maxAllowTimer->stop();
...
void Script_ReadMeter_F1F1_NodeNotInRouter::readInfoInit()
{
  ...
  //  nodeInfo=readInfoList.at(0);
}
bool Script_ReadMeter_F1F1_NodeNotInRouter::isMeterExist(Address address)
{
...
int Script_ReadMeter_F1F1_NodeNotInRouter::getReadInfo(Address address)
{
    for(int i=0;i<readInfoList.size();i++)
    {
        if(address==readInfoList.at(i).meterNo)
...
double Script_ReadMeter_F1F1_NodeNotInRouter::calSuccessRate()
{
...
QString Script_ReadMeter_F1F1_NodeNotInRouter::calCostTime()
{
...
QString Script_ReadMeter_F1F1_NodeNotInRouter::getFailMeterNo()
{
    QString failMeterNo;
    int count=0;
    for(int i=0;i<readInfoList.size();i++)
...
bool Script_ReadMeter_F1F1_NodeNotInRouter::isAllRead(int meterID)
{
    bool sus=true;
    for(int i=0;i<readInfoList.at(meterID).dataUnitList.size();i++)
    {
...
bool Script_ReadMeter_F1F1_NodeNotInRouter::isAllMeterReadSuccess()
{
    bool sus=true;
    for(int i=0;i<readInfoList.size();i++)
    {
...
void Script_ReadMeter_F1F1_NodeNotInRouter::CalcAvrgConsumeTimeLen(uchar rdFlag)
{
...
bool Script_ReadMeter_F1F1_NodeNotInRouter::extractAndProcess3762Frame(QByteArray& buf,QByteArray& completeFrame)
{
    completeFrame.clear();
