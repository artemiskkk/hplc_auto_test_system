#ifndef SCRIPT_READMETER_F1F1_EXCEEDMAXFRAMENUM_H
#define SCRIPT_READMETER_F1F1_EXCEEDMAXFRAMENUM_H

#include <QObject>
#include "TestCase/BuildNetwork_GW.h"
enum Script_ReadMeter_F1F1_ExceedMaxFrameNum_RunState
{
    ReadMeterInit,
...
namespace Namespace_ReadMeter_F1F1
{
    ...
    struct ReadDataUnit
    {
        QByteArray dataID;
        bool notRead=true;
        bool isThisIDReadSuccess=false;
        double costTime;
    };
...
class Script_ReadMeter_F1F1_ExceedMaxFrameNum : public QObject, public AbstractScript,public DynamicCreate<Script_ReadMeter_F1F1_ExceedMaxFrameNum>
{
    Q_OBJECT
//    Q_PLUGIN_METADATA(IID "com.topscomm.AbstractScript")
//    Q_INTERFACES(AbstractScript)
public:
    explicit Script_ReadMeter_F1F1_ExceedMaxFrameNum(QObject *parent = nullptr);
    ~Script_ReadMeter_F1F1_ExceedMaxFrameNum();

    Script_ReadMeter_F1F1_ExceedMaxFrameNum_RunState emScriptRunState;
    ...
    //并发抄表参数
    int maxFrameNum=18;//单帧最大报文数 路由最大支持13条
    int maxParallelNum=1;//最大并行数16
    int parallelCount=0;//并行数
    int meterIndex=0;//电表序号
    bool flagReadInfoInit=true;
public:
    void  execute();
    void  stop();
...
private:
    ...
    bool isAllDataIDRead(int meterID);
    bool isThisMeterReadSuccess(int meterID);
...
#endif // SCRIPT_READMETER_F1F1_EXCEEDMAXFRAMENUM_H
