#include "Script_ReadMeter_F1F1_NotBuildNet.h"

Script_ReadMeter_F1F1_NotBuildNet::Script_ReadMeter_F1F1_NotBuildNet(QObject *parent) : QObject(parent)
{
...
Script_ReadMeter_F1F1_NotBuildNet::~Script_ReadMeter_F1F1_NotBuildNet()
{
...
void Script_ReadMeter_F1F1_NotBuildNet::execute()
{
...
void Script_ReadMeter_F1F1_NotBuildNet::stop()
{
...
void Script_ReadMeter_F1F1_NotBuildNet::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ReadMeter_F1F1_NotBuildNet::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ReadMeter_F1F1_NotBuildNet::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ReadMeter_F1F1_NotBuildNet::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeter_F1F1_NotBuildNet::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ReadMeter_F1F1_NotBuildNet::processMsgFromCCO(DvcType dvcType, int dvcId)
{
            ...
            case Wait_Finish_F1F1:
            {
                if(p_Frame3762Base->afn_==char(0xF1)&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(parallelCount==0&&meterIndex>=readInfoList.size())
                    {
                        ...
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("F1F1并发抄表-不组网测试成功，成功率：%1%，平均耗时：%2秒，最大并行数%3;").arg(calSuccessRate()*100).arg(calCostTime()).arg(maxParallelNum));
                        }
                        else
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("失败电表清单如下%1\n").arg(getFailMeterNo()));
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("F1F1并发抄表-不组网测试失败，成功率：%1%，平均耗时：%2秒，最大并行数%3;").arg(calSuccessRate()*100).arg(calCostTime()).arg(maxParallelNum));
                ...
                else if(p_Frame3762Base->afn_==0x00&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(p_Deny_Up->error_code_==0x09)
                    {
                        if(p_maxAllowTimer!=nullptr) p_maxAllowTimer->stop();
                        emScriptRunState=ScriptSuccess;
                        p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("并发抄表测试完成，未组网并发抄表回复否认(09)，符合要求;").arg(calSuccessRate()*100).arg(calCostTime()));
                    }
                }
                else
                {
...
void Script_ReadMeter_F1F1_NotBuildNet::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_ReadMeter_F1F1_NotBuildNet::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_F1F1_NotBuildNet::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_ReadMeter_F1F1_NotBuildNet::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_F1F1_NotBuildNet::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_ReadMeter_F1F1_NotBuildNet::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_Finish_F1F1:
...
void Script_ReadMeter_F1F1_NotBuildNet::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    index=0;
    p_maxAllowTimer->stop();
...
void Script_ReadMeter_F1F1_NotBuildNet::readInfoInit()
{
...
bool Script_ReadMeter_F1F1_NotBuildNet::isMeterExist(Address address)
{
...
int Script_ReadMeter_F1F1_NotBuildNet::getReadInfo(Address address)
{
    for(int i=0;i<readInfoList.size();i++)
    {
        if(address==readInfoList.at(i).meterNo)
...
double Script_ReadMeter_F1F1_NotBuildNet::calSuccessRate()
{
...
QString Script_ReadMeter_F1F1_NotBuildNet::calCostTime()
{
...
QString Script_ReadMeter_F1F1_NotBuildNet::getFailMeterNo()
{
    QString failMeterNo;
    int count=0;
    for(int i=0;i<readInfoList.size();i++)
...
bool Script_ReadMeter_F1F1_NotBuildNet::isAllRead(int meterID)
{
    bool sus=true;
    for(int i=0;i<readInfoList.at(meterID).dataUnitList.size();i++)
    {
...
bool Script_ReadMeter_F1F1_NotBuildNet::isAllMeterReadSuccess()
{
    bool sus=true;
    for(int i=0;i<readInfoList.size();i++)
    {
...
void Script_ReadMeter_F1F1_NotBuildNet::CalcAvrgConsumeTimeLen(uchar rdFlag)
{
...
bool Script_ReadMeter_F1F1_NotBuildNet::extractAndProcess3762Frame(QByteArray& buf,QByteArray& completeFrame)
{
    completeFrame.clear();
