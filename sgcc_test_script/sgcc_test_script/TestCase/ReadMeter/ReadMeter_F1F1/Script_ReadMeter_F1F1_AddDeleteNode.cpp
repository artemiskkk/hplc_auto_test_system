#include "Script_ReadMeter_F1F1_AddDeleteNode.h"

Script_ReadMeter_F1F1_AddDeleteNode::Script_ReadMeter_F1F1_AddDeleteNode(QObject *parent) : QObject(parent)
{
...
Script_ReadMeter_F1F1_AddDeleteNode::~Script_ReadMeter_F1F1_AddDeleteNode()
{
...
void Script_ReadMeter_F1F1_AddDeleteNode::execute()
{
...
void Script_ReadMeter_F1F1_AddDeleteNode::stop()
{
...
void Script_ReadMeter_F1F1_AddDeleteNode::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ReadMeter_F1F1_AddDeleteNode::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ReadMeter_F1F1_AddDeleteNode::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ReadMeter_F1F1_AddDeleteNode::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeter_F1F1_AddDeleteNode::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ReadMeter_F1F1_AddDeleteNode::processMsgFromCCO(DvcType dvcType, int dvcId)
{
            ...
            case Wait_00F1_for_12F2_Pause:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    while(parallelCount<maxParallelNum)
                    {
                        ...
                        {
                            //删除
                            addDeleteNode=readInfoList.at(meterIndex);
                            sendMsg(CCO_GW,dvcId,INSIGNIFICANCE,p_DeleteSlaveNode_11F2);
                            runStep++;
                        }
                   }
                    //开始计时
                    ...
                    else
                    {
                        while(parallelCount<maxParallelNum)
                        {
                            if(meterIndex>=readInfoList.size())
                            {
                                ...
                                //最后一条发完了之后，开始计时，当超时之后如果并发数仍大于0，则进行提示。
...
void Script_ReadMeter_F1F1_AddDeleteNode::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_ReadMeter_F1F1_AddDeleteNode::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_F1F1_AddDeleteNode::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_ReadMeter_F1F1_AddDeleteNode::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_F1F1_AddDeleteNode::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_ReadMeter_F1F1_AddDeleteNode::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_Finish_F1F1:
...
void Script_ReadMeter_F1F1_AddDeleteNode::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    index=0;
    p_maxAllowTimer->stop();
...
void Script_ReadMeter_F1F1_AddDeleteNode::readInfoInit()
{
...
bool Script_ReadMeter_F1F1_AddDeleteNode::isMeterExist(Address address)
{
...
int Script_ReadMeter_F1F1_AddDeleteNode::getReadInfo(Address address)
{
    for(int i=0;i<readInfoList.size();i++)
    {
        if(address==readInfoList.at(i).meterNo)
...
double Script_ReadMeter_F1F1_AddDeleteNode::calSuccessRate()
{
...
QString Script_ReadMeter_F1F1_AddDeleteNode::calCostTime()
{
...
QString Script_ReadMeter_F1F1_AddDeleteNode::getFailMeterNo()
{
    QString failMeterNo;
    int count=0;
    for(int i=0;i<readInfoList.size();i++)
...
bool Script_ReadMeter_F1F1_AddDeleteNode::isAllRead(int meterID)
{
    bool sus=true;
    for(int i=0;i<readInfoList.at(meterID).dataUnitList.size();i++)
    {
...
bool Script_ReadMeter_F1F1_AddDeleteNode::isAllMeterReadSuccess()
{
    bool sus=true;
    for(int i=0;i<readInfoList.size();i++)
    {
...
void Script_ReadMeter_F1F1_AddDeleteNode::CalcAvrgConsumeTimeLen(uchar rdFlag)
{
...
bool Script_ReadMeter_F1F1_AddDeleteNode::extractAndProcess3762Frame(QByteArray& buf,QByteArray& completeFrame)
{
    completeFrame.clear();
