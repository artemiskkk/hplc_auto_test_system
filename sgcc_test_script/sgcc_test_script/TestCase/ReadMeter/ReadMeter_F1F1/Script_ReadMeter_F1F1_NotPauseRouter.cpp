#include "Script_ReadMeter_F1F1_NotPauseRouter.h"

Script_ReadMeter_F1F1_NotPauseRouter::Script_ReadMeter_F1F1_NotPauseRouter(QObject *parent) : QObject(parent)
{
...
Script_ReadMeter_F1F1_NotPauseRouter::~Script_ReadMeter_F1F1_NotPauseRouter()
{
...
void Script_ReadMeter_F1F1_NotPauseRouter::execute()
{
...
void Script_ReadMeter_F1F1_NotPauseRouter::stop()
{
...
void Script_ReadMeter_F1F1_NotPauseRouter::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ReadMeter_F1F1_NotPauseRouter::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ReadMeter_F1F1_NotPauseRouter::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
        ...
        {
            this->maxParallelNumTemp = (*paraDic)["maxParallelNum"].toUShort();
        }
...
void Script_ReadMeter_F1F1_NotPauseRouter::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeter_F1F1_NotPauseRouter::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ReadMeter_F1F1_NotPauseRouter::processMsgFromCCO(DvcType dvcType, int dvcId)
{
        ...
        switch(emScriptRunState)
        {
            ...
            {                
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    maxParallelNum=maxParallelNumTemp;/////////////////////////////
            ...
            case Wait_Finish_F1F1:
            {
                if(p_Frame3762Base->afn_==char(0xF1)&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(parallelCount==0&&meterIndex>=readInfoList.size())
                    {
                        ...
                        {
                            if(denyCount>0)
                                p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("F1F1并发抄表-未暂停路由测试成功，有09否认，成功率：%1%; 抄表平均耗时：%2秒;").arg(calSuccessRate()*100).arg(calCostTime()));
                            else
                                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("F1F1并发抄表-未暂停路由测试失败，无09否认，成功率：%1%; 抄表平均耗时：%2秒;").arg(calSuccessRate()*100).arg(calCostTime()));
                        }
                        else
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("失败电表清单如下%1\n").arg(getFailMeterNo()));
                            if(denyCount>0)
                                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("F1F1并发抄表-未暂停路由测试失败，有09否认，成功率：%1%; 抄表平均耗时：%2秒;").arg(calSuccessRate()*100).arg(calCostTime()));
                            else
                                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("F1F1并发抄表-未暂停路由测试失败，无09否认，成功率：%1%; 抄表平均耗时：%2秒;").arg(calSuccessRate()*100).arg(calCostTime()));
                        }
                    }
                ...
                else if(p_Frame3762Base->afn_==0x00&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(p_Deny_Up->error_code_==0x09)
                    {
                        denyCount++;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("并发抄表-未暂停路由，回复否认(09)，CCO模块忙，不允许抄表"));

                        emScriptRunState=Wait_00F1_for_12F2_Pause;
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_RouterPause_12F2);
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--路由暂停（12F1），等待--确认");
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    }
                }
                else if(p_Frame3762Base->afn_==0x14&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
...
void Script_ReadMeter_F1F1_NotPauseRouter::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_ReadMeter_F1F1_NotPauseRouter::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_F1F1_NotPauseRouter::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    if(frame==p_RouterRestart_12F1)
    {
...
void Script_ReadMeter_F1F1_NotPauseRouter::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_F1F1_NotPauseRouter::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_ReadMeter_F1F1_NotPauseRouter::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_Finish_F1F1:
...
void Script_ReadMeter_F1F1_NotPauseRouter::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    index=0;
    p_maxAllowTimer->stop();
...
void Script_ReadMeter_F1F1_NotPauseRouter::readInfoInit()
{
...
bool Script_ReadMeter_F1F1_NotPauseRouter::isMeterExist(Address address)
{
...
int Script_ReadMeter_F1F1_NotPauseRouter::getReadInfo(Address address)
{
    for(int i=0;i<readInfoList.size();i++)
    {
        if(address==readInfoList.at(i).meterNo)
...
double Script_ReadMeter_F1F1_NotPauseRouter::calSuccessRate()
{
...
QString Script_ReadMeter_F1F1_NotPauseRouter::calCostTime()
{
...
QString Script_ReadMeter_F1F1_NotPauseRouter::getFailMeterNo()
{
    QString failMeterNo;
    int count=0;
    for(int i=0;i<readInfoList.size();i++)
...
bool Script_ReadMeter_F1F1_NotPauseRouter::isAllRead(int meterID)
{
    bool sus=true;
    for(int i=0;i<readInfoList.at(meterID).dataUnitList.size();i++)
    {
...
bool Script_ReadMeter_F1F1_NotPauseRouter::isAllMeterReadSuccess()
{
    bool sus=true;
    for(int i=0;i<readInfoList.size();i++)
    {
...
void Script_ReadMeter_F1F1_NotPauseRouter::CalcAvrgConsumeTimeLen(uchar rdFlag)
{
...
bool Script_ReadMeter_F1F1_NotPauseRouter::extractAndProcess3762Frame(QByteArray& buf,QByteArray& completeFrame)
{
    completeFrame.clear();
