#include "Script_ReadMeter_F1F1_SearchMeter.h"

Script_ReadMeter_F1F1_SearchMeter::Script_ReadMeter_F1F1_SearchMeter(QObject *parent) : QObject(parent)
{
...
Script_ReadMeter_F1F1_SearchMeter::~Script_ReadMeter_F1F1_SearchMeter()
{
...
void Script_ReadMeter_F1F1_SearchMeter::execute()
{
...
void Script_ReadMeter_F1F1_SearchMeter::stop()
{
...
void Script_ReadMeter_F1F1_SearchMeter::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ReadMeter_F1F1_SearchMeter::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ReadMeter_F1F1_SearchMeter::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ReadMeter_F1F1_SearchMeter::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeter_F1F1_SearchMeter::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ReadMeter_F1F1_SearchMeter::processMsgFromCCO(DvcType dvcType, int dvcId)
{
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_00F1_for_12F2_Pause:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(searchMeterReportFlag==false)
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("发送--激活主动注册（11F5），等待--确认"));
                        sendMsg(dvcType,dvcId,INSIGNIFICANCE,p_ActiveRegister_11F5);
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                        p_delayTimer->start(activeRegisterLastTime*65*1000);
                    }
                }
                else
                {
                    ...
                    if(currentMeterIndex!=-1)
                    {
                        ...
                        if(p_ParallelReadMeter_F1F1_Up->unit_up_.frame_length_!=0)
                        {
                            ...
                            if(p_ParallelReadMeter_F1F1_Up->unit_up_.protocol_type_==DLT645_2007)
                            {
                                ...
                                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("收到F1F1并发抄表回复（645），表号%0，耗时：%1秒;").arg(QString(QByteArray(readInfoList.at(currentMeterIndex).meterNo.addr,6).toHex())).arg(spendTime));
                            }
                            else if(p_ParallelReadMeter_F1F1_Up->unit_up_.protocol_type_==OOP)
                            {
                                ...
                                if(isAllRead(currentMeterIndex)==true)
                                    readInfoList[currentMeterIndex].readFlag=ReadSuccess;
                                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("收到F1F1并发抄表回复（OOP），表号%0，耗时：%1秒;").arg(QString(QByteArray(readInfoList.at(currentMeterIndex).meterNo.addr,6).toHex())).arg(spendTime));
                            }
                        }
                    }
...
//                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("当前F1F1并发抄表成功率：%1%;").arg(calSuccessRate()*100));
//                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("并发抄表全部完成，抄表平均耗时：%2秒;").arg(calCostTime()));
//                    }
                    //3
                    if(parallelCount==0&&meterIndex>=readInfoList.size())
                    {
                        ...
                        {
                            if(searchMeterReportFlag == true)
                            {
                                if(p_maxAllowTimer!=nullptr) p_maxAllowTimer->stop();
                                emScriptRunState=ScriptSuccess;
                                p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("F1F1并发抄表-搜表测试成功，收到主动注册的电表，抄表成功率：%1%，平均耗时：%2秒，最大并行数%3;").arg(calSuccessRate()*100).arg(calCostTime()).arg(maxParallelNum));
                            }
                            else
                            {
                                if(searchMeterEndFlag == true)
                                {
                                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("F1F1并发抄表-搜表测试失败，搜表已结束未收到上报，抄表成功率：%1%达到要求，平均耗时：%2秒，最大并行数%3;").arg(calSuccessRate()*100).arg(calCostTime()).arg(maxParallelNum));
                                }
                                else
                                {
                                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("抄表成功率达到要求，暂未收到搜表上报，搜表还未结束再等待3min"));
                        ...
                        else
                        {
                            if(searchMeterReportFlag==true)
                            {
                                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("F1F1并发抄表-搜表测试失败，收到主动注册的电表，抄表成功率：%1%未达到要求，平均耗时：%2秒，最大并行数%3;").arg(calSuccessRate()*100).arg(calCostTime()).arg(maxParallelNum));
                            }
                            else
                            {
                                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("F1F1并发抄表-搜表测试失败，未收到主动注册的电表，抄表成功率：%1%未达到要求，平均耗时：%2秒，最大并行数%3;").arg(calSuccessRate()*100).arg(calCostTime()).arg(maxParallelNum));
                            }
                        }
                    }
                    else
                ...
                {
                    p_timer->stop();
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "激活主动注册收到确认");
                }
                else if(p_Frame3762Base->afn_==0x06&&p_Frame3762Base->dt1_==0x04&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(p_ReportMissionState_Up->router_work_task_change_==0x02)
                    {
                        p_delayTimer->stop();
...
void Script_ReadMeter_F1F1_SearchMeter::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_ReadMeter_F1F1_SearchMeter::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_F1F1_SearchMeter::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_ReadMeter_F1F1_SearchMeter::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_F1F1_SearchMeter::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
        ...
        case Wait_Finish_F1F1:
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "Wait_F1F1_UP timeout!!!");
            //需要判断是否切换电表
//            if(++index>=times)
//            {
//                index=0;
...
void Script_ReadMeter_F1F1_SearchMeter::maxAllowTimer_timeoutProc()
{
...
void Script_ReadMeter_F1F1_SearchMeter::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    if(finalWaitFlag == false)
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("搜表持续时间到后未上报结束"));
    }
    else
    {
        if(searchMeterReportFlag == false)
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("抄表成功率达到要求后再等待3min依然没有搜表上报"));
        }
        else
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Success, "抄表成功率达到要求后再等待3min收到搜表上报");
            if(p_maxAllowTimer!=nullptr) p_maxAllowTimer->stop();
            emScriptRunState = ScriptSuccess;
        }
    }
...
void Script_ReadMeter_F1F1_SearchMeter::readInfoInit()
{
...
bool Script_ReadMeter_F1F1_SearchMeter::isMeterExist(Address address)
{
...
int Script_ReadMeter_F1F1_SearchMeter::getReadInfo(Address address)
{
    for(int i=0;i<readInfoList.size();i++)
    {
        if(address==readInfoList.at(i).meterNo)
...
double Script_ReadMeter_F1F1_SearchMeter::calSuccessRate()
{
...
QString Script_ReadMeter_F1F1_SearchMeter::calCostTime()
{
...
QString Script_ReadMeter_F1F1_SearchMeter::getFailMeterNo()
{
    QString failMeterNo;
    int count=0;
    for(int i=0;i<readInfoList.size();i++)
...
bool Script_ReadMeter_F1F1_SearchMeter::isAllRead(int meterID)
{
    bool sus=true;
    for(int i=0;i<readInfoList.at(meterID).dataUnitList.size();i++)
    {
...
bool Script_ReadMeter_F1F1_SearchMeter::isAllMeterReadSuccess()
{
    bool sus=true;
    for(int i=0;i<readInfoList.size();i++)
    {
...
void Script_ReadMeter_F1F1_SearchMeter::CalcAvrgConsumeTimeLen(uchar rdFlag)
{
...
bool Script_ReadMeter_F1F1_SearchMeter::extractAndProcess3762Frame(QByteArray& buf,QByteArray& completeFrame)
{
    completeFrame.clear();
