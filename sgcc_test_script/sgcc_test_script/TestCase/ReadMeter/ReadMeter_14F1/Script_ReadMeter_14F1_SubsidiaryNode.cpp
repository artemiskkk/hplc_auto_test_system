#include "Script_ReadMeter_14F1_SubsidiaryNode.h"

Script_ReadMeter_14F1_SubsidiaryNode::Script_ReadMeter_14F1_SubsidiaryNode(QObject *parent) : QObject(parent)
{
...
Script_ReadMeter_14F1_SubsidiaryNode::~Script_ReadMeter_14F1_SubsidiaryNode()
{
...
void Script_ReadMeter_14F1_SubsidiaryNode::execute()
{
...
void Script_ReadMeter_14F1_SubsidiaryNode::stop()
{
...
void Script_ReadMeter_14F1_SubsidiaryNode::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ReadMeter_14F1_SubsidiaryNode::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ReadMeter_14F1_SubsidiaryNode::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ReadMeter_14F1_SubsidiaryNode::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeter_14F1_SubsidiaryNode::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ReadMeter_14F1_SubsidiaryNode::processMsgFromCCO(DvcType dvcType, int dvcId)
{
            ...
            case Wait_Finish_14F1:
            {
                if(p_Frame3762Base->afn_==0x14&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(currentMeterIndex!=-1)
                    {
                        ...
                        //计数
                        failCount++;
                        if(failCount>readInfoList.size()&&calSuccessCount()==1)
                        {
                            ...
                            p_AbstractScriptHost->updateProgress(ProcessState_Success,QString("14F1抄表测试完毕，不支持带附属节点地址抄读！"));
                        }
                    }
                    else
                    {
                ...
                else if(p_Frame3762Base->afn_==0x06&&p_Frame3762Base->dt1_==0x04&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(p_ReportMissionState_Up->router_work_task_change_==0x01)
                    {
                        ...
                        emScriptRunState=ScriptSuccess;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("当前路由请求抄读14F1成功率：%1%;").arg(calSuccessRate()*100));
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("14F1抄表成功率：%1%; 14F1抄表平均耗时：%2秒;").arg(calSuccessRate()*100).arg(calCostTime()));
                        p_AbstractScriptHost->updateProgress(ProcessState_Success,QString("14F1抄表测试完毕，支持带附属节点地址抄读！"));
                    }
                }
                else
                {
...
void Script_ReadMeter_14F1_SubsidiaryNode::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_ReadMeter_14F1_SubsidiaryNode::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_14F1_SubsidiaryNode::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_RouterRequestRead_14F1)
    {
        ...
        p_RouterRequestRead_14F1->router_request_read_unit_.subsidiary_node_address_list_.clear();
        if(meterID<7)
        {
            for(int i=0;i<meterID;i++)
            {
                ...
                p_RouterRequestRead_14F1->router_request_read_unit_.subsidiary_node_address_list_.append(subAddress);
            }
        }
        else if(meterID>=7)
        {
        ...
        p_RouterRequestRead_14F1->router_request_read_unit_.subsidiary_node_num_=uchar(p_RouterRequestRead_14F1->router_request_read_unit_.subsidiary_node_address_list_.size());
...
void Script_ReadMeter_14F1_SubsidiaryNode::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_14F1_SubsidiaryNode::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_ReadMeter_14F1_SubsidiaryNode::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_Finish_14F1:
...
void Script_ReadMeter_14F1_SubsidiaryNode::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    index=0;
    p_maxAllowTimer->stop();
...
void Script_ReadMeter_14F1_SubsidiaryNode::readInfoInit()
{
    readInfoList.clear();
...
bool Script_ReadMeter_14F1_SubsidiaryNode::isMeterExist(Address address)
{
...
int Script_ReadMeter_14F1_SubsidiaryNode::getReadInfo(Address address)
{
    for(int i=0;i<readInfoList.size();i++)
    {
        if(address==readInfoList.at(i).meterNo)
...
double Script_ReadMeter_14F1_SubsidiaryNode::calSuccessRate()
{
...
int Script_ReadMeter_14F1_SubsidiaryNode::calSuccessCount()
{
    int successCount=0;
    for(int i=0;i<readInfoList.size();i++)
    {
...
QString Script_ReadMeter_14F1_SubsidiaryNode::calCostTime()
{
...
QString Script_ReadMeter_14F1_SubsidiaryNode::getFailMeterNo()
{
    QString failMeterNo;
    for(int i=0;i<readInfoList.size();i++)
    {
...
void Script_ReadMeter_14F1_SubsidiaryNode::CalcAvrgConsumeTimeLen(uchar rdFlag)
{
...
bool Script_ReadMeter_14F1_SubsidiaryNode::extractAndProcess3762Frame(QByteArray& buf,QByteArray& completeFrame)
{
    completeFrame.clear();
