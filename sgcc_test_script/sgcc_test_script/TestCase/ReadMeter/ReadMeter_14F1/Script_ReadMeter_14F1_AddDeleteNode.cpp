#include "Script_ReadMeter_14F1_AddDeleteNode.h"

Script_ReadMeter_14F1_AddDeleteNode::Script_ReadMeter_14F1_AddDeleteNode(QObject *parent) : QObject(parent)
{
...
Script_ReadMeter_14F1_AddDeleteNode::~Script_ReadMeter_14F1_AddDeleteNode()
{
...
void Script_ReadMeter_14F1_AddDeleteNode::execute()
{
...
void Script_ReadMeter_14F1_AddDeleteNode::stop()
{
...
void Script_ReadMeter_14F1_AddDeleteNode::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ReadMeter_14F1_AddDeleteNode::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ReadMeter_14F1_AddDeleteNode::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ReadMeter_14F1_AddDeleteNode::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeter_14F1_AddDeleteNode::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ReadMeter_14F1_AddDeleteNode::processMsgFromCCO(DvcType dvcType, int dvcId)
{
            ...
            case Wait_Finish_14F1:
            {
                if(p_Frame3762Base->afn_==0x14&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(currentMeterIndex!=-1)
                    {
                        ...
                        if(runStep==0)
                        {                            
                            addDeleteNode=readInfoList.at(currentMeterIndex);
                            sendMsg(dvcType,dvcId,currentMeterIndex,p_DeleteSlaveNode_11F2);
                            readInfoList.removeAt(currentMeterIndex);
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("发送--删除从节点%1，等待--确认").arg(QString(QByteArray(addDeleteNode.meterNo.addr,6).toHex())));
                            p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                            runStep++;
                        }
                        else if(runStep==1)
                        {
                            sendMsg(dvcType,dvcId,currentMeterIndex,p_AddSlaveNode_11F1);
                            readInfoList.append(addDeleteNode);
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("发送--添加从节点%1，等待--确认").arg(QString(QByteArray(addDeleteNode.meterNo.addr,6).toHex())));
                            p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                            runStep++;
                        }
                    }
                ...
                else if(p_Frame3762Base->afn_==0x06&&p_Frame3762Base->dt1_==0x04&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(p_ReportMissionState_Up->router_work_task_change_==0x01)
                    {
                        ...
                        if(calSuccessRate() >= readMeterSucRateThresld)
                            p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("14F1抄表-添加删除节点，测试成功，14F1抄表成功率：%1%; 14F1抄表平均耗时：%2秒;").arg(calSuccessRate()*100).arg(calCostTime()));
                        else
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("14F1抄表-添加删除节点，测试失败，14F1抄表成功率：%1%; 14F1抄表平均耗时：%2秒;").arg(calSuccessRate()*100).arg(calCostTime()));
                    }
                    else
                    {
...
void Script_ReadMeter_14F1_AddDeleteNode::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_ReadMeter_14F1_AddDeleteNode::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_14F1_AddDeleteNode::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_AddSlaveNode_11F1)
    {
        ...
        memcpy(nodePara.node_address_.addr,addDeleteNode.meterNo.addr,6);
        nodePara.protocol_type_=char(addDeleteNode.protocolType);
    ...
    else if(frame==p_DeleteSlaveNode_11F2)
    {
        ...
        p_DeleteSlaveNode_11F2->node_address_list_.append(addDeleteNode.meterNo);
...
void Script_ReadMeter_14F1_AddDeleteNode::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_14F1_AddDeleteNode::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_ReadMeter_14F1_AddDeleteNode::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_Finish_14F1:
...
void Script_ReadMeter_14F1_AddDeleteNode::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    index=0;
    p_maxAllowTimer->stop();
...
void Script_ReadMeter_14F1_AddDeleteNode::readInfoInit()
{
    readInfoList.clear();
...
bool Script_ReadMeter_14F1_AddDeleteNode::isMeterExist(Address address)
{
...
int Script_ReadMeter_14F1_AddDeleteNode::getReadInfo(Address address)
{
    for(int i=0;i<readInfoList.size();i++)
    {
        if(address==readInfoList.at(i).meterNo)
...
double Script_ReadMeter_14F1_AddDeleteNode::calSuccessRate()
{
...
QString Script_ReadMeter_14F1_AddDeleteNode::calCostTime()
{
...
QString Script_ReadMeter_14F1_AddDeleteNode::getFailMeterNo()
{
    QString failMeterNo;
    for(int i=0;i<readInfoList.size();i++)
    {
...
void Script_ReadMeter_14F1_AddDeleteNode::CalcAvrgConsumeTimeLen(uchar rdFlag)
{
...
Address Script_ReadMeter_14F1_AddDeleteNode::extractAddressFromAfn06F2(shared_ptr<Afn06F2> p_ReportReadData_Up)
{
    Address srcAddr;
...
bool Script_ReadMeter_14F1_AddDeleteNode::extractAndProcess3762Frame(QByteArray& buf,QByteArray& completeFrame)
{
    completeFrame.clear();
