#include "Script_ReadMeter_14F1_RestartPauseRocover2.h"

Script_ReadMeter_14F1_RestartPauseRocover2::Script_ReadMeter_14F1_RestartPauseRocover2(QObject *parent) : QObject(parent)
{
...
Script_ReadMeter_14F1_RestartPauseRocover2::~Script_ReadMeter_14F1_RestartPauseRocover2()
{
...
void Script_ReadMeter_14F1_RestartPauseRocover2::execute()
{
...
void Script_ReadMeter_14F1_RestartPauseRocover2::stop()
{
...
void Script_ReadMeter_14F1_RestartPauseRocover2::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ReadMeter_14F1_RestartPauseRocover2::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ReadMeter_14F1_RestartPauseRocover2::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ReadMeter_14F1_RestartPauseRocover2::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeter_14F1_RestartPauseRocover2::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ReadMeter_14F1_RestartPauseRocover2::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_00F1_for_12F1_Restart:
            {
                ...
                {
                    if(runStep==0||runStep==4||runStep==7)
                    {
                ...
                else if(p_Frame3762Base->afn_==0x06&&p_Frame3762Base->dt1_==0x04&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(p_ReportMissionState_Up->router_work_task_change_==0x01)
                    {
                        ...
                        if(calSuccessRate() >= readMeterSucRateThresld)
                            p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("14F1抄表-重启暂停恢复2，测试成功，14F1抄表成功率：%1%; 14F1抄表平均耗时：%2秒;").arg(calSuccessRate()*100).arg(calCostTime()));
                        else
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("14F1抄表-重启暂停恢复2，测试失败，14F1抄表成功率：%1%; 14F1抄表平均耗时：%2秒;").arg(calSuccessRate()*100).arg(calCostTime()));
                    }
                    else
                    {
...
void Script_ReadMeter_14F1_RestartPauseRocover2::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_ReadMeter_14F1_RestartPauseRocover2::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_14F1_RestartPauseRocover2::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_ReadMeter_14F1_RestartPauseRocover2::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_14F1_RestartPauseRocover2::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_ReadMeter_14F1_RestartPauseRocover2::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_Finish_14F1:
...
void Script_ReadMeter_14F1_RestartPauseRocover2::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    index=0;
    p_maxAllowTimer->stop();
...
void Script_ReadMeter_14F1_RestartPauseRocover2::readInfoInit()
{
    readInfoList.clear();
...
bool Script_ReadMeter_14F1_RestartPauseRocover2::isMeterExist(Address address)
{
...
int Script_ReadMeter_14F1_RestartPauseRocover2::getReadInfo(Address address)
{
    for(int i=0;i<readInfoList.size();i++)
    {
        if(address==readInfoList.at(i).meterNo)
...
double Script_ReadMeter_14F1_RestartPauseRocover2::calSuccessRate()
{
...
QString Script_ReadMeter_14F1_RestartPauseRocover2::calCostTime()
{
...
QString Script_ReadMeter_14F1_RestartPauseRocover2::getFailMeterNo()
{
    QString failMeterNo;
    for(int i=0;i<readInfoList.size();i++)
    {
...
void Script_ReadMeter_14F1_RestartPauseRocover2::CalcAvrgConsumeTimeLen(uchar rdFlag)
{
...
Address Script_ReadMeter_14F1_RestartPauseRocover2::extractAddressFromAfn06F2(shared_ptr<Afn06F2> p_ReportReadData_Up)
{
    Address srcAddr;
...
bool Script_ReadMeter_14F1_RestartPauseRocover2::extractAndProcess3762Frame(QByteArray& buf,QByteArray& completeFrame)
{
    completeFrame.clear();
