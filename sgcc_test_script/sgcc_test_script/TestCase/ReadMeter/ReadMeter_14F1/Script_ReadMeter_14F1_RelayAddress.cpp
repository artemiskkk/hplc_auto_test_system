#include "Script_ReadMeter_14F1_RelayAddress.h"

Script_ReadMeter_14F1_RelayAddress::Script_ReadMeter_14F1_RelayAddress(QObject *parent) : QObject(parent)
{
...
Script_ReadMeter_14F1_RelayAddress::~Script_ReadMeter_14F1_RelayAddress()
{
...
void Script_ReadMeter_14F1_RelayAddress::execute()
{
...
void Script_ReadMeter_14F1_RelayAddress::stop()
{
...
void Script_ReadMeter_14F1_RelayAddress::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ReadMeter_14F1_RelayAddress::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ReadMeter_14F1_RelayAddress::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ReadMeter_14F1_RelayAddress::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeter_14F1_RelayAddress::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ReadMeter_14F1_RelayAddress::processMsgFromCCO(DvcType dvcType, int dvcId)
{
            ...
            case Wait_Finish_14F1:
            {
                if(p_Frame3762Base->afn_==0x14&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(currentMeterIndex!=-1)
                    {
                        ...
                        if(failCount>readInfoList.size()&&calSuccessCount()==1)
                        {
                            ...
                            p_AbstractScriptHost->updateProgress(ProcessState_Success,QString("14F1抄表测试完毕，不支持带中继地址抄读！"));
                        }
                    }
                    else
                    {
                ...
                else if(p_Frame3762Base->afn_==0x06&&p_Frame3762Base->dt1_==0x04&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(p_ReportMissionState_Up->router_work_task_change_==0x01)
                    {
                        ...
                        p_AbstractScriptHost->updateProgress(ProcessState_Success,QString("14F1抄表测试完毕，支持带中继地址抄读！"));
                    }
                }
                else
                {
...
void Script_ReadMeter_14F1_RelayAddress::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_ReadMeter_14F1_RelayAddress::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_14F1_RelayAddress::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_RouterRequestRead_14F1)
    {
        ...
        p_RouterRequestRead_14F1->address_field_.delay_addr.clear();
        if(meterID<7)
        {
            for(int i=0;i<meterID;i++)
            {
                ...
                p_RouterRequestRead_14F1->address_field_.delay_addr.append(relayAddress);
            }
        }
        else if(meterID>=7)
        {
        ...
        p_RouterRequestRead_14F1->info_field_.info_field_down.relay_level=uchar(p_RouterRequestRead_14F1->address_field_.delay_addr.size());
...
void Script_ReadMeter_14F1_RelayAddress::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_14F1_RelayAddress::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_ReadMeter_14F1_RelayAddress::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_Finish_14F1:
...
void Script_ReadMeter_14F1_RelayAddress::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    index=0;
    p_maxAllowTimer->stop();
...
void Script_ReadMeter_14F1_RelayAddress::readInfoInit()
{
    readInfoList.clear();
...
bool Script_ReadMeter_14F1_RelayAddress::isMeterExist(Address address)
{
...
int Script_ReadMeter_14F1_RelayAddress::getReadInfo(Address address)
{
    for(int i=0;i<readInfoList.size();i++)
    {
        if(address==readInfoList.at(i).meterNo)
...
double Script_ReadMeter_14F1_RelayAddress::calSuccessRate()
{
...
int Script_ReadMeter_14F1_RelayAddress::calSuccessCount()
{
    int successCount=0;
    for(int i=0;i<readInfoList.size();i++)
    {
...
QString Script_ReadMeter_14F1_RelayAddress::calCostTime()
{
...
QString Script_ReadMeter_14F1_RelayAddress::getFailMeterNo()
{
    QString failMeterNo;
    for(int i=0;i<readInfoList.size();i++)
    {
...
void Script_ReadMeter_14F1_RelayAddress::CalcAvrgConsumeTimeLen(uchar rdFlag)
{
...
bool Script_ReadMeter_14F1_RelayAddress::extractAndProcess3762Frame(QByteArray& buf,QByteArray& completeFrame)
{
    completeFrame.clear();
