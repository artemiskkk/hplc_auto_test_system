#include "Script_ReadMeter_14F1_MeterReadParaInit.h"


Script_ReadMeter_14F1_MeterReadParaInit::Script_ReadMeter_14F1_MeterReadParaInit(QObject *parent) : QObject(parent)
{
    ...
    p_AddArchives_11F1 = make_shared<Afn11F1>();
...
Script_ReadMeter_14F1_MeterReadParaInit::~Script_ReadMeter_14F1_MeterReadParaInit()
{
...
void Script_ReadMeter_14F1_MeterReadParaInit::execute()
{
    p_AbstractScriptHost->updateProgress(ProcessState_Start, "Test start!");
    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "测试编号：【GW-CCO-F004-0017-V01】\n"
                                                                  "测试流程：\n"
                                                                  "1：组网，Wait_11F1_BuildNet_Finish；\n"
                                                                  "2.给路由下发12F1重启路由，Wait_12F1_RouterRestart_Finish；\n"
                                                                  "3.14F1下行，抄表标志可以抄读，抄读数据项1，Wait_14F1_MeterRequest_Finish；\n"
                                                                  "  等待06F2上报，最大超时时间30S，回复确认，路由14F1上行请求第1块表；\n"
                                                                  "4.01F2参数初始化，回复确认；\n"
                                                                  "  10F4查询路由运行状态，工作状态：抄表，Wait_01F2_RouterParaInit_Finish；\n"
                                                                  "5.11F1添加档案，路由不再发送14F1的上行请求报文；\n"
                                                                  "  在组网过程中，10F4查询路由运行状态，路由完成标志：未完成，工作状态：抄表；\n"
                                                                  "  组完完成，10F4查询路由运行状态，路由完成标志：完成，工作状态：学习，Wait_10F4_QueryRouterRunStateFinal_Finish；\n");
...
void Script_ReadMeter_14F1_MeterReadParaInit::stop()
{
...
void Script_ReadMeter_14F1_MeterReadParaInit::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ReadMeter_14F1_MeterReadParaInit::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ReadMeter_14F1_MeterReadParaInit::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ReadMeter_14F1_MeterReadParaInit::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeter_14F1_MeterReadParaInit::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ReadMeter_14F1_MeterReadParaInit::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        case Wait_12F1_RouterRestart_Finish:
        {
            ...
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到12F1回复确认，符合预期，测试继续！");
                QList<MeterInfoForSingleNet*> meterList = p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values();
                if (meterList.size() < 2 || meterList.at(1) == nullptr)
                {
                    emScriptRunState = ScriptSuccess;
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed,
                        QString("用例配置错误：抄表-14F1-抄表中参数初始化至少需要2块表档案，当前%1块，已停止本用例避免程序异常退出。").arg(meterList.size()));
                    break;
                }
                memcpy(tmp_addr,meterList.at(1)->mtrAddr,6);
            ...
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_12F1_RouterRestart_Finish，重启路由12F1回复否认，测试失败！");
            }
            else
            {
...
//                if(times_all == 2) //上报抄读两次工装上的虚拟表
//                {
//                    //p_AbstractScriptHost->updateProgress(ProcessState_Processing, "14F1上报数与档案数一致，符合预期，测试继续！");
//                    emScriptRunState = Wait_01F2_RouterParaInit_Finish;
//                    sendMsg(CCO_GW,dvcId,INSIGNIFICANCE,p_ParaInit_01F2);
//                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--路由档案初始化(01F2)--等待回复确认");
//                    p_timer->start(30*1000);
//                }

                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到14F1路由请求抄表，符合预期，测试继续！");
                ...
                Address srcAddr;
                memcpy(srcAddr.addr,p_RouterRequestRead_14F1_Up->address_field_.src_addr,6);
            ...
            else if(p_Frame3762Base->afn_==0x06&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                {
                    //01F2参数初始化
                    sendMsg(CCO_GW,dvcId,INSIGNIFICANCE,p_Confirm_00F1);
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--确认！");
                ...
                if(times_all == p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->size())
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "06F2上报数与档案数一致，符合预期，测试继续！");
                    emScriptRunState = Wait_01F2_RouterParaInit_Finish;
                    sendMsg(CCO_GW,dvcId,INSIGNIFICANCE,p_ParaInit_01F2);
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--路由档案初始化(01F2)--等待回复确认");
                    p_timer->start(30*1000);
...
//            }
        ...
        case Wait_01F2_RouterParaInit_Finish:
        {
            if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_timer->stop();
                tryTimes=0;
                tmp_state = false;
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到路由参数初始化回复确认，测试继续！");
                sendMsg(CCO_GW,dvcId,INSIGNIFICANCE,p_QueryRouterState_10F4);
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "查询路由运行状态（10F4），等待--完成");
                p_timer->start(30*1000);
                p_maxAllowTimer->start(90*1000);
            }
            else if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_01F2_RouterParaInit_Finish，01F2回复异常，期望回复确认，实际回复否认，【GW-CCO-F004-0017-V01】");
            ...
            else if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x08&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(uchar(p_QueryRouterState_10F4_Up->router_operate_state_unit_.work_switch_.work_state)==0x00)
                {
                    p_maxAllowTimer->stop();
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "Wait_01F2_RouterParaInit_Finish，查询路由运行状态回复：抄表，符合要求，测试继续！");
                    emScriptRunState=Wait_10F4_QueryRouterRunStateFinal_Finish;
                    tmp_state = true;
                    sendMsg(CCO_GW,dvcId,INSIGNIFICANCE,p_AddArchives_11F1);
                    p_timer->start(30*1000);
                }
                else
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "Wait_01F2_RouterParaInit_Finish，查询路由运行状态回复非抄表，等待5秒后继续查询！");
        ...
        case Wait_10F4_QueryRouterRunStateFinal_Finish:
        {
            if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_timer->stop();
                tryTimes=0;
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "Wait_10F4_QueryRouterRunStateFinal_Finish，收到路由添加档案回复确认，测试继续！");
                sendMsg(CCO_GW,dvcId,INSIGNIFICANCE,p_QueryRouterState_10F4);
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "查询路由运行状态（10F4），等待--完成");
                p_timer->start(30*1000);
            }
            ...
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_10F4_QueryRouterRunStateFinal_Finish，12F1回复异常，期望回复确认，实际回复否认，【GW-CCO-F004-0017-V01】");
            ...
            else if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x08&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(uchar(p_QueryRouterState_10F4_Up->router_operate_state_unit_.work_switch_.work_state)==0x01)
                {
                    p_maxAllowTimer->stop();
                    p_delayTimer->stop();
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "查询路由运行状态回复：学习，符合要求，测试通过！");
                    p_AbstractScriptHost->updateProgress(ProcessState_Success, "脚本执行成功，【GW-CCO-F004-0017-V01】");
                    emScriptRunState = ScriptSuccess;
                }
                else
                {
                    p_maxAllowTimer->stop();
                    p_delayTimer->stop();
//                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "查询路由运行状态回复非学习，等待15秒后继续查询！");
//                    p_delayTimer->start(15*1000);
//                    p_maxAllowTimer->start(120*1000);
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "查询路由运行状态回复符合要求，测试通过！");
                    p_AbstractScriptHost->updateProgress(ProcessState_Success, "脚本执行成功，【GW-CCO-F004-0017-V01】");
                    emScriptRunState = ScriptSuccess;
...
void Script_ReadMeter_14F1_MeterReadParaInit::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_ReadMeter_14F1_MeterReadParaInit::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_14F1_MeterReadParaInit::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame == p_AddArchives_11F1)
    {
        p_AddArchives_11F1->ctrl_field_.dir=kDirDown;
        p_AddArchives_11F1->ctrl_field_.prm=kActive;
        p_AddArchives_11F1->ctrl_field_.comn_type=kHplc;

        p_AddArchives_11F1->info_field_.info_field_down.msg_seq=char(msgSeq++);
        p_AddArchives_11F1->info_field_.info_field_down.comu_rate=0;
        p_AddArchives_11F1->info_field_.info_field_down.comu_module_ident=0;

        p_AddArchives_11F1->node_num_=1;
        NodeParameter node;
        node.protocol_type_=0x02;
        memcpy(node.node_address_.addr,tmp_addr,6);
        p_AddArchives_11F1->node_parameter_list_.append(node);

        sendMsgOct=p_AddArchives_11F1->EncodeFrame();
...
void Script_ReadMeter_14F1_MeterReadParaInit::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_14F1_MeterReadParaInit::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
    case Wait_BuildNetFinish_Whole:
    ...
    case Wait_14F1_MeterRequest_Finish:
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "01F2，Wait_14F1_MeterRequest_Finish，“timeout！”，【GW-CCO-F004-0017-V01】");
        break;
    }
    case Wait_01F2_RouterParaInit_Finish:
    {
        ...
        {
            if(tmp_state == false)
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "10F4，Wait_01F2_RouterParaInit_Finish，“timeout！”，【GW-CCO-F004-0017-V01】");
            else
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "01F2，Wait_01F2_RouterParaInit_Finish，“timeout！”，【GW-CCO-F004-0017-V01】");
        }
        else
        {
            if(tmp_state == false)
    ...
    case Wait_10F4_QueryRouterRunStateFinal_Finish:
    {
        if(++tryTimes>=3)
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_10F4_QueryRouterRunStateFinal_Finish，“timeout！”，【GW-CCO-F004-0017-V01】");
        }
        else
        {
            sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryRouterState_10F4);
...
void Script_ReadMeter_14F1_MeterReadParaInit::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
    case Wait_14F1_MeterRequest_Finish:
...
void Script_ReadMeter_14F1_MeterReadParaInit::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    ...
    else if(emScriptRunState==Wait_01F2_RouterParaInit_Finish)
    {
        p_delayTimer->stop();
        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryRouterState_10F4);
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "查询路由运行状态（10F4），等待--完成");
    }
    else if(emScriptRunState == Wait_10F4_QueryRouterRunStateFinal_Finish)
    {
        p_delayTimer->stop();
        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryRouterState_10F4);
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "查询路由运行状态（10F4），等待--完成");
...
void Script_ReadMeter_14F1_MeterReadParaInit::readInfoInit()
{
    readInfoList.clear();
...
bool Script_ReadMeter_14F1_MeterReadParaInit::isMeterExist(Address address)
{
...
int Script_ReadMeter_14F1_MeterReadParaInit::getReadInfo(Address address)
{
    for(int i=0;i<readInfoList.size();i++)
    {
        if(address==readInfoList.at(i).meterNo)
...
double Script_ReadMeter_14F1_MeterReadParaInit::calSuccessRate()
{
...
QString Script_ReadMeter_14F1_MeterReadParaInit::calCostTime()
{
...
QString Script_ReadMeter_14F1_MeterReadParaInit::getFailMeterNo()
{
    QString failMeterNo;
    int count=0;
    for(int i=0;i<readInfoList.size();i++)
...
void Script_ReadMeter_14F1_MeterReadParaInit::CalcAvrgConsumeTimeLen(uchar rdFlag)
{
...
Address Script_ReadMeter_14F1_MeterReadParaInit::extractAddressFromAfn06F2(shared_ptr<Afn06F2> p_ReportReadData_Up)
{
    Address srcAddr;
...
bool Script_ReadMeter_14F1_MeterReadParaInit::extractAndProcess3762Frame(QByteArray& buf,QByteArray& completeFrame)
{
    completeFrame.clear();
