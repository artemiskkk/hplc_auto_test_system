#include "Script_ReadMeter_14F1_NotBuildNet.h"

Script_ReadMeter_14F1_NotBuildNet::Script_ReadMeter_14F1_NotBuildNet(QObject *parent) : QObject(parent)
{
...
Script_ReadMeter_14F1_NotBuildNet::~Script_ReadMeter_14F1_NotBuildNet()
{
...
void Script_ReadMeter_14F1_NotBuildNet::execute()
{
...
void Script_ReadMeter_14F1_NotBuildNet::stop()
{
...
void Script_ReadMeter_14F1_NotBuildNet::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ReadMeter_14F1_NotBuildNet::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ReadMeter_14F1_NotBuildNet::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ReadMeter_14F1_NotBuildNet::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeter_14F1_NotBuildNet::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ReadMeter_14F1_NotBuildNet::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        case Wait_HardInit_Finish:
        {
            ...
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到03F10上报，发送--路由重启（12F1），等待--确认");
                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_RouterRestart_12F1);
                emScriptRunState=Wait_00F1_for_12F1_Restart;
                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
            }
        ...
        case Wait_00F1_for_12F1_Restart:
        {
            ...
            {
                p_timer->stop();
                emScriptRunState=Wait_10s_AfterRestart;
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到确认，等待20s看是否有14F1");
        ...
        case Wait_00F1_for_12F2_Pause:
        {
            ...
            else if(p_Frame3762Base->afn_==0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("统计%1min内14F1上报次数").arg(waitTime));
        ...
        case Wait_Finish_14F1:
        {
            if(p_Frame3762Base->afn_==0x14&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("收到14F1请求，回复抄读数据项，表号")+QByteArray(p_RouterRequestRead_14F1_Up->node_address_.addr,6).toHex());
                    ...
                    if(p_ReportReadData_Up->report_data_unit_.node_protocol_type_==DLT645_2007)
                    {
                        ...
                        for(int i=0;i<readInfoList.at(currentMeterIndex).dataUnitList.size();i++)
                        {
                            if(memcmp(readInfoList.at(currentMeterIndex).dataUnitList.at(i).dataID,p_ReadData_0x91->di,4)==0)
                            {
                                ...
                                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("收到06F2上报抄读数据，抄表耗时：%1秒;").arg(spendTime));
...
void Script_ReadMeter_14F1_NotBuildNet::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_ReadMeter_14F1_NotBuildNet::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_14F1_NotBuildNet::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_ReadMeter_14F1_NotBuildNet::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_14F1_NotBuildNet::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_ReadMeter_14F1_NotBuildNet::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_Finish_14F1:
...
void Script_ReadMeter_14F1_NotBuildNet::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    if(emScriptRunState==Wait_10s_AfterRestart)
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "重启后20s内未收到14F1请求，符合要求，继续等待");
        emScriptRunState=Wait_Finish_14F1;
    }
    else if(emScriptRunState==Wait_00F1_for_12F2_Pause)
    {
...
void Script_ReadMeter_14F1_NotBuildNet::readInfoInit()
{
    readInfoList.clear();
...
bool Script_ReadMeter_14F1_NotBuildNet::isMeterExist(Address address)
{
...
int Script_ReadMeter_14F1_NotBuildNet::getReadInfo(Address address)
{
    for(int i=0;i<readInfoList.size();i++)
    {
        if(address==readInfoList.at(i).meterNo)
...
double Script_ReadMeter_14F1_NotBuildNet::calSuccessRate()
{
...
QString Script_ReadMeter_14F1_NotBuildNet::calCostTime()
{
...
QString Script_ReadMeter_14F1_NotBuildNet::getFailMeterNo()
{
    QString failMeterNo;
    int count=0;
    for(int i=0;i<readInfoList.size();i++)
...
void Script_ReadMeter_14F1_NotBuildNet::CalcAvrgConsumeTimeLen(uchar rdFlag)
{
...
bool Script_ReadMeter_14F1_NotBuildNet::extractAndProcess3762Frame(QByteArray& buf,QByteArray& completeFrame)
{
    completeFrame.clear();
