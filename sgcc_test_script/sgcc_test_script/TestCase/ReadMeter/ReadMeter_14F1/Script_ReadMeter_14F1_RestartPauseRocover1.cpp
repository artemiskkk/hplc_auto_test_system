#include "Script_ReadMeter_14F1_RestartPauseRocover1.h"

Script_ReadMeter_14F1_RestartPauseRocover1::Script_ReadMeter_14F1_RestartPauseRocover1(QObject *parent) : QObject(parent)
{
...
Script_ReadMeter_14F1_RestartPauseRocover1::~Script_ReadMeter_14F1_RestartPauseRocover1()
{
...
void Script_ReadMeter_14F1_RestartPauseRocover1::execute()
{
...
void Script_ReadMeter_14F1_RestartPauseRocover1::stop()
{
...
void Script_ReadMeter_14F1_RestartPauseRocover1::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ReadMeter_14F1_RestartPauseRocover1::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ReadMeter_14F1_RestartPauseRocover1::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ReadMeter_14F1_RestartPauseRocover1::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeter_14F1_RestartPauseRocover1::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ReadMeter_14F1_RestartPauseRocover1::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_00F1_for_12F1_Restart:
            {
                ...
                {
                    if(emCtrlState==Recover)//6
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到确认，发送--路由恢复（12F3），等待--确认");
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_RouterRecover_12F3);
                        emScriptRunState=Wait_00F1_for_12F3_Recover;
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                        emCtrlState=Restart;
                    }
                    else if(emCtrlState==Restart||emCtrlState==End)
                    {
                        emScriptRunState=Wait_Finish_14F1;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到确认，等待14F1路由主动请求抄表");
                    }
                }
                else if(p_Frame3762Base->afn_==0x14&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("收到14F1路由请求抄表，表号%1").arg(QString(QByteArray(p_RouterRequestRead_14F1_Up->node_address_.addr,6).toHex())));
                }
                else
                {
            ...
            case Wait_00F1_for_12F2_Pause:
            {
                if(p_Frame3762Base->afn_==0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    else if(emCtrlState==Restart)//5
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到确认，发送--路由重启（12F1），等待--确认");
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_RouterRestart_12F1);
                        emScriptRunState=Wait_00F1_for_12F1_Restart;
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                        emCtrlState=Recover;
            ...
            case Wait_Finish_14F1:
            {
                if(p_Frame3762Base->afn_==0x14&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("收到14F1路由请求抄表，表号%1，回复抄读数据项").arg(QString(QByteArray(p_RouterRequestRead_14F1_Up->node_address_.addr,6).toHex())));
                        sendMsg(dvcType,dvcId,currentMeterIndex,p_RouterRequestRead_14F1);
                    }
                    else
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("收到14F1路由请求抄表，表号%1不在档案").arg(QString(QByteArray(p_RouterRequestRead_14F1_Up->node_address_.addr,6).toHex())));
                    }
                }
                else if(p_Frame3762Base->afn_==0x06&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(isAddrEmpty) {
                         p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到06F2上报抄读数据，提取地址无效(全0)，跳过此帧");
                         continue;
                    }
                        ...
                        if(p_ReportReadData_Up->report_data_unit_.node_protocol_type_==DLT645_2007)
                        {
                            ...
                            for(int i=0;i<readInfoList.at(currentMeterIndex).dataUnitList.size();i++)
                            {
                                if(memcmp(readInfoList.at(currentMeterIndex).dataUnitList.at(i).dataID,p_ReadData_0x91->di,4)==0)
                                {
                                    ...
                                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("收到06F2上报抄读数据(645)，抄表耗时：%1秒;").arg(spendTime));
                                    //发送确认或否认
                                    sendMsg(dvcType,dvcId,INSIGNIFICANCE,p_Confirm_00F1);
                                    break;
                                }
                                else
                                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("收到06F2上报抄读数据，645报文中DI有误"));
                            }
                        }
                        else if(p_ReportReadData_Up->report_data_unit_.node_protocol_type_==OOP)
                        {
                            ...
                            for(int i=0;i<readInfoList.at(currentMeterIndex).dataUnitList.size();i++)
                            {
                                if(p_GetResponseNormal->a_result_normal_.oad==oad)
                                {
                                    ...
                                    readInfoList[currentMeterIndex].dataUnitList[i].costTime=spendTime.toDouble();
                                    (p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values())[currentMeterIndex]->timeConsumList[1]=spendTime.toDouble();
                                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("收到06F2上报抄读数据(OOP)，抄表耗时：%1秒;").arg(spendTime));
                                    ...
                                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("收到06F2上报抄读数据，OOP报文中oad有误"));
                            }
                        }

                        if(emCtrlState==Restart)//2
                        {
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("收到06F2上报抄读数据，表号%1不在档案").arg(QString(QByteArray(srcAddr.addr,6).toHex())));
                    }
                ...
                else if(p_Frame3762Base->afn_==0x06&&p_Frame3762Base->dt1_==0x04&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(p_ReportMissionState_Up->router_work_task_change_==0x01)
                    {
                        ...
                        if(calSuccessRate() >= readMeterSucRateThresld)
                            p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("14F1抄表-重启暂停恢复1，测试成功，14F1抄表成功率：%1%; 14F1抄表平均耗时：%2秒;").arg(calSuccessRate()*100).arg(calCostTime()));
                        else
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("14F1抄表-重启暂停恢复1，测试失败，14F1抄表成功率：%1%; 14F1抄表平均耗时：%2秒;").arg(calSuccessRate()*100).arg(calCostTime()));
                    }
                    else
                    {
...
void Script_ReadMeter_14F1_RestartPauseRocover1::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_ReadMeter_14F1_RestartPauseRocover1::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_14F1_RestartPauseRocover1::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_ReadMeter_14F1_RestartPauseRocover1::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_14F1_RestartPauseRocover1::timer_timeoutProc()
{
    p_timer->stop();
    ...
    switch(emScriptRunState)
    {
        ...
        case Wait_00F1_for_12F2_Pause:
        {
            if(++tryTimes>=3)
            {
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_00F1_for_12F2_Pause timeout, 已重试3次！");
            }
            else
            {
                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_RouterPause_12F2);
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, 
                    QString("发送--路由暂停命令（12F2），第%1次重试").arg(tryTimes));
                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
            }
            break;
        }
        case Wait_00F1_for_12F1_Restart:
        {
            if(++tryTimes>=3)
            {
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_00F1_for_12F1_Restart timeout, 已重试3次！");
            }
            else
            {
                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_RouterRestart_12F1);
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, 
                    QString("发送--路由重启命令（12F1），第%1次重试").arg(tryTimes));
                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
            }
            break;
        }
        case Wait_00F1_for_12F3_Recover:
        {
            if(++tryTimes>=3)
            {
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_00F1_for_12F3_Recover timeout, 已重试3次！");
            }
            else
            {
                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_RouterRecover_12F3);
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, 
                    QString("发送--路由恢复命令（12F3），第%1次重试").arg(tryTimes));
                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
            }
            break;
        }
...
void Script_ReadMeter_14F1_RestartPauseRocover1::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_Finish_14F1:
...
void Script_ReadMeter_14F1_RestartPauseRocover1::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    index=0;
    p_maxAllowTimer->stop();
...
void Script_ReadMeter_14F1_RestartPauseRocover1::readInfoInit()
{
    readInfoList.clear();
...
bool Script_ReadMeter_14F1_RestartPauseRocover1::isMeterExist(Address address)
{
...
int Script_ReadMeter_14F1_RestartPauseRocover1::getReadInfo(Address address)
{
    for(int i=0;i<readInfoList.size();i++)
    {
        if(address==readInfoList.at(i).meterNo)
...
double Script_ReadMeter_14F1_RestartPauseRocover1::calSuccessRate()
{
...
QString Script_ReadMeter_14F1_RestartPauseRocover1::calCostTime()
{
...
QString Script_ReadMeter_14F1_RestartPauseRocover1::getFailMeterNo()
{
    QString failMeterNo;
    for(int i=0;i<readInfoList.size();i++)
    {
...
void Script_ReadMeter_14F1_RestartPauseRocover1::CalcAvrgConsumeTimeLen(uchar rdFlag)
{
...
Address Script_ReadMeter_14F1_RestartPauseRocover1::extractAddressFromAfn06F2(shared_ptr<Afn06F2> p_ReportReadData_Up)
{
    Address srcAddr;
...
bool Script_ReadMeter_14F1_RestartPauseRocover1::extractAndProcess3762Frame(QByteArray& buf,QByteArray& completeFrame)
{
    completeFrame.clear();

    // 首先跳过所有 0xFE 和 0x1E 前导符（645帧的前导符）
    ...
    if ((unsigned char)buf[frameLength - 1] != 0x16) {
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "帧结束标志错误，跳过此0x68");
        buf.remove(0, 1);
        return extractAndProcess3762Frame(buf, completeFrame);
    }
