#include "Script_ReadMeter_14F1_3762InfoAreaFrameSeqNotMatch.h"


Script_ReadMeter_14F1_3762InfoAreaFrameSeqNotMatch::Script_ReadMeter_14F1_3762InfoAreaFrameSeqNotMatch(QObject *parent) : QObject(parent)
{
...
Script_ReadMeter_14F1_3762InfoAreaFrameSeqNotMatch::~Script_ReadMeter_14F1_3762InfoAreaFrameSeqNotMatch()
{
...
void Script_ReadMeter_14F1_3762InfoAreaFrameSeqNotMatch::execute()
{
    p_AbstractScriptHost->updateProgress(ProcessState_Start, "Test start!");
    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "测试编号：【GW-CCO-F004-0013-V01】\n"
                                                                  "测试流程：\n"
                                                                  "1：组网，Wait_11F1_BuildNet_Finish；\n"
                                                                  "2：给路由下发12F1重启路由，Wait_12F1_RouterRestart_Finish；\n"
                                                                  "3：路由14F1上行请求表1，14F1下行（报文序列号不匹配），抄读标志可以抄读，Wait_14F1_MeterRequest_Finish；\n"
                                                                  "   等待06F2上报，最大超时时间30S，回复确认；\n"
                                                                  "   路由14F1上行继续请求第1块表，14F1下行，抄表标志抄读成功；\n"
                                                                  "   遍历档案所有表；\n"
                                                                  "4：06F3上报成功率100%，任务变动类型为1（抄表任务结束），回复确认，Wait_06F3_ActiveReport_Finish；\n"
...
void Script_ReadMeter_14F1_3762InfoAreaFrameSeqNotMatch::stop()
{
...
void Script_ReadMeter_14F1_3762InfoAreaFrameSeqNotMatch::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ReadMeter_14F1_3762InfoAreaFrameSeqNotMatch::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ReadMeter_14F1_3762InfoAreaFrameSeqNotMatch::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ReadMeter_14F1_3762InfoAreaFrameSeqNotMatch::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeter_14F1_3762InfoAreaFrameSeqNotMatch::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ReadMeter_14F1_3762InfoAreaFrameSeqNotMatch::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        case Wait_00F1_for_12F2_Pause:
        {
            ...
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_12F1_RouterRestart_Finish，12F1回复异常，期望回复确认，实际回复否认，【GW-CCO-F004-0013-V01】");
            }
            else
            {
                QByteArray tmpSendMsg=prcsOther3762Msg(uchar(p_Frame3762Base->afn_),p_Frame3762Base->dt1_,p_Frame3762Base->dt2_,uchar(p_Frame3762Base->info_field_.info_field_up.msg_seq));
            ...
            else if(p_Frame3762Base->afn_==0x06&&p_Frame3762Base->dt1_==0x04&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(p_ReportMissionState_Up->router_work_task_change_==0x01)
                {
                    ...
                    if(calSuccessRate() >= readMeterSucRateThresld)
                    {
                        ...
                        p_AbstractScriptHost->updateProgress(ProcessState_Success, "脚本执行成功，【GW-CCO-F004-0013-V01】");
                    }
                    else
                ...
                {
                    QString tmp_str = "Wait_06F3_ActiveReport_Finish，工作状态回复异常，期望回复学习01，实际回复：" + QString::number(p_ReportMissionState_Up->router_work_task_change_) + "，【GW-CCO-F004-0013-V01】" ;
...
void Script_ReadMeter_14F1_3762InfoAreaFrameSeqNotMatch::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_ReadMeter_14F1_3762InfoAreaFrameSeqNotMatch::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_14F1_3762InfoAreaFrameSeqNotMatch::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_RouterRequestRead_14F1)
    {
        ...
        p_RouterRequestRead_14F1->info_field_.info_field_down.msg_seq=char(msgSeq++); //为使报文序列号不匹配,报文序列号作加一处理
...
void Script_ReadMeter_14F1_3762InfoAreaFrameSeqNotMatch::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_14F1_3762InfoAreaFrameSeqNotMatch::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
    case Wait_BuildNetFinish_Whole:
    ...
    case Wait_00F1_for_12F2_Pause:
    {
        if(++tryTimes>=3)
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_12F1_RouterRestart_Finish，“timeout！”，【GW-CCO-F004-0013-V01】");
        }
        else
        {
            sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_RouterPause_12F2);
...
void Script_ReadMeter_14F1_3762InfoAreaFrameSeqNotMatch::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
    case Wait_14F1_MeterRequest_Finish:
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "Wait_14F1_MeterRequest_Finish，“timeout！”，【GW-CCO-F004-0013-V01】"+QString("  14F1抄表成功率：%1%").arg(calSuccessRate()*100));
...
void Script_ReadMeter_14F1_3762InfoAreaFrameSeqNotMatch::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
...
void Script_ReadMeter_14F1_3762InfoAreaFrameSeqNotMatch::readInfoInit()
{
    readInfoList.clear();
...
bool Script_ReadMeter_14F1_3762InfoAreaFrameSeqNotMatch::isMeterExist(Address address)
{
...
int Script_ReadMeter_14F1_3762InfoAreaFrameSeqNotMatch::getReadInfo(Address address)
{
    for(int i=0;i<readInfoList.size();i++)
    {
        if(address==readInfoList.at(i).meterNo)
...
double Script_ReadMeter_14F1_3762InfoAreaFrameSeqNotMatch::calSuccessRate()
{
...
QString Script_ReadMeter_14F1_3762InfoAreaFrameSeqNotMatch::calCostTime()
{
...
QString Script_ReadMeter_14F1_3762InfoAreaFrameSeqNotMatch::getFailMeterNo()
{
    QString failMeterNo;
    int count=0;
    for(int i=0;i<readInfoList.size();i++)
...
void Script_ReadMeter_14F1_3762InfoAreaFrameSeqNotMatch::CalcAvrgConsumeTimeLen(uchar rdFlag)
{
...
bool Script_ReadMeter_14F1_3762InfoAreaFrameSeqNotMatch::extractAndProcess3762Frame(QByteArray& buf,QByteArray& completeFrame)
{
    completeFrame.clear();
...
Address Script_ReadMeter_14F1_3762InfoAreaFrameSeqNotMatch::extractAddressFromAfn06F2(shared_ptr<Afn06F2> p_ReportReadData_Up)
{
    Address srcAddr;
