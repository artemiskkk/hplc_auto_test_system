#include "Script_ReadMeter_14F1_SearchMeter.h"

Script_ReadMeter_14F1_SearchMeter::Script_ReadMeter_14F1_SearchMeter(QObject *parent) : QObject(parent)
{
...
Script_ReadMeter_14F1_SearchMeter::~Script_ReadMeter_14F1_SearchMeter()
{
...
void Script_ReadMeter_14F1_SearchMeter::execute()
{
...
void Script_ReadMeter_14F1_SearchMeter::stop()
{
...
void Script_ReadMeter_14F1_SearchMeter::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ReadMeter_14F1_SearchMeter::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ReadMeter_14F1_SearchMeter::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
        ...
        if(paraDic->keys().contains("activeRegisterLastTime"))
        {
            this->activeRegisterLastTime = (*paraDic)["activeRegisterLastTime"].toUShort();
        }
...
void Script_ReadMeter_14F1_SearchMeter::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeter_14F1_SearchMeter::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ReadMeter_14F1_SearchMeter::processMsgFromCCO(DvcType dvcType, int dvcId)
{
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_00F1_for_12F1_Restart:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到确认，路由主动请求抄表（14F1），等待--抄表完成");
            ...
            case Wait_Finish_14F1:
            {
                if(p_Frame3762Base->afn_==0x14&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("收到第%1个14F1请求，表号%2，进行回复").arg(++requestNum_14F1).arg(QString(QByteArray(p_RouterRequestRead_14F1_Up->node_address_.addr,6).toHex())));
                        sendMsg(dvcType,dvcId,currentMeterIndex,p_RouterRequestRead_14F1);
                        if(searchMeterFlag==false)
                        {
                            sendMsg(dvcType,dvcId,currentMeterIndex,p_ActiveRegister_11F5);
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("发送--激活主动注册（11F5），等待--确认"));
                            p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                        }
                    }
                    else
                    {
                       // continue;
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "收到14F1请求不在档案中的表："+ QByteArray(p_RouterRequestRead_14F1_Up->node_address_.addr,6).toHex());
                    }
                }
                ...
                else if(p_Frame3762Base->afn_==0x06&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(currentMeterIndex!=-1)
                    {
                        ...
                        if(p_ReportReadData_Up->report_data_unit_.node_protocol_type_==DLT645_2007)
                        {
                            ...
                            if(MsgBase_645_ptr==nullptr)
                                // continue;
                                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "收到06F2上报645报文为空");
                            if(MsgBase_645_ptr->ctrlCode_!=NORMAL_RESP)
                                // continue;
                                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "收到06F2上报645报文控制码非0x91");
                            ...
                            if(0!=memcmp(p_ReadData_0x91->addr_,srcAddr.addr,6))
                                // continue;
                                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "收到06F2上报报文源地址和里边的645报文地址不一致");
                            for(int i=0;i<readInfoList.at(currentMeterIndex).dataUnitList.size();i++)
                            {
                                if(memcmp(readInfoList.at(currentMeterIndex).dataUnitList.at(i).dataID,p_ReadData_0x91->di,4)==0)
                                {
                                    ...
                                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("收到第%0个06F2抄表上报(645)，抄表耗时：%1，表号%2").arg(++reportNum_readMeter).arg(spendTime).arg(QString(QByteArray(srcAddr.addr,6).toHex())));
                                    //发送确认或否认
                                   ...
                                   // continue;
                                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, "收到06F2上报645报文DI和14F1请求抄读的DI不一致");
                            }
                        }
                        else if(p_ReportReadData_Up->report_data_unit_.node_protocol_type_==OOP)
                        {
                            ...
                            if(MsgBase_OOP_ptr==nullptr)
                               // continue;
                                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "收到06F2上报OOP报文为空");
                            if(0!=memcmp(QByteArray::fromHex(MsgBase_OOP_ptr->address_field_.sa.address.toLatin1()),readInfoList.at(currentMeterIndex).meterNo.addr,6))
                                //continue;
                                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "收到06F2上报报文源地址和里边的OOP报文SA地址不一致");
                            if(MsgBase_OOP_ptr->service_type_!=GET_RESPONSE_SERVER || MsgBase_OOP_ptr->service_sub_type_!=uchar(GetResponseType::kGetResponseNormal))
                                //continue;
                                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "收到06F2上报OOP报文服务类型非133(读取响应) 或 服务子类型非1(GetResponseNormal)");
                            ...
                            for(int i=0;i<readInfoList.at(currentMeterIndex).dataUnitList.size();i++)
                            {
                                if(p_GetResponseNormal->a_result_normal_.oad==oad)
                                {
                                    ...
                                    readInfoList[currentMeterIndex].dataUnitList[i].costTime=spendTime.toDouble();
                                    (p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values())[currentMeterIndex]->timeConsumList[1]=spendTime.toDouble();
                                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("收到第%0个06F2抄表上报(OOP)，抄表耗时：%1，表号%2").arg(++reportNum_readMeter).arg(spendTime).arg(QString(QByteArray(srcAddr.addr,6).toHex())));
                                   ...
                                   // continue;
                                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, "收到06F2上报OOP报文OAD非正向有功电能(00100200)");
                            }
                        }
                    }
                    else
                    {
                        //continue;
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "收到06F2上报不在档案中的表："+ QByteArray(p_ReportReadData_Up->address_field_.src_addr,6).toHex());
                    }
                ...
                else if(p_Frame3762Base->afn_==0x06&&p_Frame3762Base->dt1_==0x04&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(p_ReportMissionState_Up->router_work_task_change_==0x01)
                    {
                        ...
                        if(calSuccessRate() >= readMeterSucRateThresld)
                            p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("14F1抄表-搜表，测试成功，14F1抄表成功率：%1%; 14F1抄表平均耗时：%2秒;").arg(calSuccessRate()*100).arg(calCostTime()));
                        else
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("14F1抄表-搜表，测试失败，14F1抄表成功率：%1%; 14F1抄表平均耗时：%2秒;").arg(calSuccessRate()*100).arg(calCostTime()));
                    }
                ...
                else if(p_Frame3762Base->afn_ == 0x06&&p_Frame3762Base->dt1_==0x08&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("上报注册从节点第%0个，表号%1；").arg(++reportNum_search).arg(QString(QByteArray(p_ReportRegisterNode_Up->report_node_info_unit_.report_node_address_.addr,6).toHex())));
...
void Script_ReadMeter_14F1_SearchMeter::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_ReadMeter_14F1_SearchMeter::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_14F1_SearchMeter::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_ActiveRegister_11F5)
    {
        ...
        p_ActiveRegister_11F5->last_time_=activeRegisterLastTime;
...
void Script_ReadMeter_14F1_SearchMeter::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_14F1_SearchMeter::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_ReadMeter_14F1_SearchMeter::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_Finish_14F1:
...
void Script_ReadMeter_14F1_SearchMeter::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    index=0;
    p_maxAllowTimer->stop();
...
void Script_ReadMeter_14F1_SearchMeter::readInfoInit()
{
    readInfoList.clear();
...
bool Script_ReadMeter_14F1_SearchMeter::isMeterExist(Address address)
{
...
int Script_ReadMeter_14F1_SearchMeter::getReadInfo(Address address)
{
    for(int i=0;i<readInfoList.size();i++)
    {
        if(address==readInfoList.at(i).meterNo)
...
double Script_ReadMeter_14F1_SearchMeter::calSuccessRate()
{
...
QString Script_ReadMeter_14F1_SearchMeter::calCostTime()
{
...
QString Script_ReadMeter_14F1_SearchMeter::getFailMeterNo()
{
    QString failMeterNo;
    int count=0;
    for(int i=0;i<readInfoList.size();i++)
...
void Script_ReadMeter_14F1_SearchMeter::CalcAvrgConsumeTimeLen(uchar rdFlag)
{
...
Address Script_ReadMeter_14F1_SearchMeter::extractAddressFromAfn06F2(shared_ptr<Afn06F2> p_ReportReadData_Up)
{
    Address srcAddr;
...
bool Script_ReadMeter_14F1_SearchMeter::extractAndProcess3762Frame(QByteArray& buf,QByteArray& completeFrame)
{
    completeFrame.clear();
