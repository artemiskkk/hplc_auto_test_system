#include "Script_ReadMeter_14F1_Delay14F3DownNoResponse.h"


Script_ReadMeter_14F1_Delay14F3DownNoResponse::Script_ReadMeter_14F1_Delay14F3DownNoResponse(QObject *parent) : QObject(parent)
{
...
Script_ReadMeter_14F1_Delay14F3DownNoResponse::~Script_ReadMeter_14F1_Delay14F3DownNoResponse()
{
...
void Script_ReadMeter_14F1_Delay14F3DownNoResponse::execute()
{
    p_AbstractScriptHost->updateProgress(ProcessState_Start, "Test start!");
    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "测试编号：【GW-CCO-F004-0008-V01】\n"
                                                                  "测试流程：\n"
                                                                  ...
                                                                  "   等待14F3上报，不回复14F3下行，\n"
                                                                  "   等待06F2上报，最大超时时间40S，回复确认；\n"
                                                                  "   14F1下行，抄表标志抄读成功；\n"
                                                                  "   等待06F3上报，任务变动类型1，回复确认；\n");
...
void Script_ReadMeter_14F1_Delay14F3DownNoResponse::stop()
{
...
void Script_ReadMeter_14F1_Delay14F3DownNoResponse::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ReadMeter_14F1_Delay14F3DownNoResponse::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ReadMeter_14F1_Delay14F3DownNoResponse::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ReadMeter_14F1_Delay14F3DownNoResponse::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeter_14F1_Delay14F3DownNoResponse::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ReadMeter_14F1_Delay14F3DownNoResponse::processMsgFromCCO(DvcType dvcType, int dvcId)
{
...
//                int currentMeterIndex=1;
            ...
            else if(p_Frame3762Base->afn_==0x06&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "Wait_Finish_14F1，收到06F2上报，符合预期，测试继续！");
...
//                        p_AbstractScriptHost->updateProgress(ProcessState_Success, "脚本执行成功，【GW-CCO-F004-0008-V01】");
//                    else
//                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("14F1抄表-延时不响应，测试失败，14F1抄表成功率：%1%; 14F1抄表平均耗时：%2秒;").arg(calSuccessRate()*100).arg(calCostTime()));
//                }
//                else
//                {
...
//            }
            else if(p_Frame3762Base->afn_==0x14&&p_Frame3762Base->dt1_==0x04&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到--请求依通信延时修正通信数据（14F3）");
                sendMsg(CCO_GW,dvcId,INSIGNIFICANCE,p_DelayCorrect_14F3);
                emScriptRunState=Wait_14F1_MeterRequest_Finish;
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "响应请求依通信延时修正通信数据（14F3），等待06F3上报");
        ...
        case Wait_14F1_MeterRequest_Finish:
        {
            ...
            else if(p_Frame3762Base->afn_==0x06&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                //p_timer->stop();
                shared_ptr<Afn06F2> p_ReportReadData_Up=dynamic_pointer_cast<Afn06F2>(p_Frame3762Base);
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "Wait_14F1_MeterRequest_Finish，收到06F2上报，符合期待，测试继续！");
                Address srcAddr = extractAddressFromAfn06F2(p_ReportReadData_Up);
                int currentMeterIndex=getReadInfo(srcAddr);
                if(currentMeterIndex!=-1)
                {
...
void Script_ReadMeter_14F1_Delay14F3DownNoResponse::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_ReadMeter_14F1_Delay14F3DownNoResponse::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_14F1_Delay14F3DownNoResponse::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_RouterRequestRead_14F1)
    {
        ...
        if(readInfoList.at(meterID).protocolType==DLT645_2007)
        {
            ...
            if(emScriptRunState == Wait_Finish_14F1)
            {
...
void Script_ReadMeter_14F1_Delay14F3DownNoResponse::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_14F1_Delay14F3DownNoResponse::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
    case Wait_BuildNetFinish_Whole:
    ...
    case Wait_Finish_14F1:
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_Finish_14F1, timeout!!!");
        break;
    }
    case Wait_00F1_for_12F3_Recover:
    {
    ...
    case Wait_14F1_MeterRequest_Finish:
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_14F1_MeterRequest_Finish, timeout!!!");
        break;
    }
    default:
    {
...
void Script_ReadMeter_14F1_Delay14F3DownNoResponse::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
    case Wait_Finish_14F1:
...
void Script_ReadMeter_14F1_Delay14F3DownNoResponse::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    index=0;
    p_maxAllowTimer->stop();
...
void Script_ReadMeter_14F1_Delay14F3DownNoResponse::readInfoInit()
{
    readInfoList.clear();
...
bool Script_ReadMeter_14F1_Delay14F3DownNoResponse::isMeterExist(Address address)
{
...
int Script_ReadMeter_14F1_Delay14F3DownNoResponse::getReadInfo(Address address)
{
    for(int i=0;i<readInfoList.size();i++)
    {
        if(address==readInfoList.at(i).meterNo)
...
double Script_ReadMeter_14F1_Delay14F3DownNoResponse::calSuccessRate()
{
...
QString Script_ReadMeter_14F1_Delay14F3DownNoResponse::calCostTime()
{
...
QString Script_ReadMeter_14F1_Delay14F3DownNoResponse::getFailMeterNo()
{
    QString failMeterNo;
    for(int i=0;i<readInfoList.size();i++)
    {
...
void Script_ReadMeter_14F1_Delay14F3DownNoResponse::CalcAvrgConsumeTimeLen(uchar rdFlag)
{
...
Address Script_ReadMeter_14F1_Delay14F3DownNoResponse::extractAddressFromAfn06F2(shared_ptr<Afn06F2> p_ReportReadData_Up)
{
    Address srcAddr;
...
bool Script_ReadMeter_14F1_Delay14F3DownNoResponse::extractAndProcess3762Frame(QByteArray& buf,QByteArray& completeFrame)
{
    completeFrame.clear();
