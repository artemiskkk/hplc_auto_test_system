#include "Script_ReadMeter_14F1_SuccessFailFlag.h"

Script_ReadMeter_14F1_SuccessFailFlag::Script_ReadMeter_14F1_SuccessFailFlag(QObject *parent) : QObject(parent)
{
...
Script_ReadMeter_14F1_SuccessFailFlag::~Script_ReadMeter_14F1_SuccessFailFlag()
{
...
void Script_ReadMeter_14F1_SuccessFailFlag::execute()
{
...
void Script_ReadMeter_14F1_SuccessFailFlag::stop()
{
...
void Script_ReadMeter_14F1_SuccessFailFlag::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ReadMeter_14F1_SuccessFailFlag::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ReadMeter_14F1_SuccessFailFlag::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ReadMeter_14F1_SuccessFailFlag::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeter_14F1_SuccessFailFlag::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ReadMeter_14F1_SuccessFailFlag::processMsgFromCCO(DvcType dvcType, int dvcId)
{
                ...
                else if(p_Frame3762Base->afn_==0x06&&p_Frame3762Base->dt1_==0x04&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(p_ReportMissionState_Up->router_work_task_change_==0x01)
                    {
                        ...
                        if(calSuccessRate() >= readMeterSucRateThresld)
                            p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("14F1抄表-成功失败标志，测试成功，14F1抄表成功率：%1%; 14F1抄表平均耗时：%2秒;").arg(calSuccessRate()*100).arg(calCostTime()));
                        else
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("14F1抄表-成功失败标志，测试失败，14F1抄表成功率：%1%; 14F1抄表平均耗时：%2秒;").arg(calSuccessRate()*100).arg(calCostTime()));
                    }
                    else
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "路由上报工况变动");
//                        QByteArray tmpSendMsg=prcsOther3762Msg(uchar(p_Frame3762Base->afn_),p_Frame3762Base->dt1_,p_Frame3762Base->dt2_,uchar(p_Frame3762Base->info_field_.info_field_up.msg_seq));
//                        sendSrcMsg(dvcType,dvcId,tmpSendMsg);
                    }
                    break;
...
void Script_ReadMeter_14F1_SuccessFailFlag::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_ReadMeter_14F1_SuccessFailFlag::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ReadMeter_14F1_SuccessFailFlag::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_RouterRequestRead_14F1)
    {
        ...
        if(readInfoList.at(meterID).protocolType==DLT645_2007)
        {
            ...
            if(readInfoList.at(meterID).readFlag==Reading)
            {
                ...
                if(meterID%2==1)
                {
                    readInfoList[meterID].readFlag=ReadFail;
                }
                else
                {
                    bool needRead=false;
            ...
            else if(readInfoList.at(meterID).readFlag==ReadFail)
            {
                bool needRead=false;
                for(int i=0;i<readInfoList.at(meterID).dataUnitList.size();i++)
                {
                ...
                if(readInfoList.at(meterID).readFlag==ReadSuccess)
                    p_RouterRequestRead_14F1->router_request_read_unit_.read_flag_=char(readInfoList.at(meterID).readFlag);
                else
                    p_RouterRequestRead_14F1->router_request_read_unit_.read_flag_=0x02;//可以抄读
            }
...
void Script_ReadMeter_14F1_SuccessFailFlag::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ReadMeter_14F1_SuccessFailFlag::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_ReadMeter_14F1_SuccessFailFlag::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_Finish_14F1:
...
void Script_ReadMeter_14F1_SuccessFailFlag::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    index=0;
    p_maxAllowTimer->stop();
...
void Script_ReadMeter_14F1_SuccessFailFlag::readInfoInit()
{
    readInfoList.clear();
...
bool Script_ReadMeter_14F1_SuccessFailFlag::isMeterExist(Address address)
{
...
int Script_ReadMeter_14F1_SuccessFailFlag::getReadInfo(Address address)
{
    for(int i=0;i<readInfoList.size();i++)
    {
        if(address==readInfoList.at(i).meterNo)
...
double Script_ReadMeter_14F1_SuccessFailFlag::calSuccessRate()
{
...
QString Script_ReadMeter_14F1_SuccessFailFlag::calCostTime()
{
...
QString Script_ReadMeter_14F1_SuccessFailFlag::getFailMeterNo()
{
    QString failMeterNo;
    for(int i=0;i<readInfoList.size();i++)
    {
...
void Script_ReadMeter_14F1_SuccessFailFlag::CalcAvrgConsumeTimeLen(uchar rdFlag)
{
...
Address Script_ReadMeter_14F1_SuccessFailFlag::extractAddressFromAfn06F2(shared_ptr<Afn06F2> p_ReportReadData_Up)
{
    Address srcAddr;
...
bool Script_ReadMeter_14F1_SuccessFailFlag::extractAndProcess3762Frame(QByteArray& buf,QByteArray& completeFrame)
{
    completeFrame.clear();
