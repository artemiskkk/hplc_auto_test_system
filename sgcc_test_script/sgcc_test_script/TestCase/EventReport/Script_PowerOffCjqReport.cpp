#include "Script_PowerOffCjqReport.h"

Script_PowerOffCjqReport::Script_PowerOffCjqReport(QObject *parent) : QObject(parent)
{
...
Script_PowerOffCjqReport::~Script_PowerOffCjqReport()
{
...
void Script_PowerOffCjqReport::execute()
{
...
void Script_PowerOffCjqReport::stop()
{
...
void Script_PowerOffCjqReport::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_PowerOffCjqReport::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_PowerOffCjqReport::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_PowerOffCjqReport::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
    ...
    if(emScriptRunState==Wait_BuildNetFinish_Whole)
    {
        ...
        else
        {
            if(p_BuildNetwork_GW->emScriptRunState==BuildNetFinish&&p_BuildNetwork_GW->buildNetworkResultFlag==true)
            {
                ...
                powerOff220V_CJQ(p_CtrInfoList,p_AbstractScriptHost);
                //时间再定
...
void Script_PowerOffCjqReport::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_PowerOffCjqReport::processMsgFromCCO(DvcType dvcType, int dvcId)
{
...
void Script_PowerOffCjqReport::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_PowerOffCjqReport::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_PowerOffCjqReport::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_PowerOffCjqReport::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_PowerOffCjqReport::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_PowerOffCjqReport::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_EventReport_Finish:
...
void Script_PowerOffCjqReport::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    calPowerOnReportRate();
}

void Script_PowerOffCjqReport::calPowerOnReportRate()
{
