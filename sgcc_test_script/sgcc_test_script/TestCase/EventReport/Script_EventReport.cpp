#include "Script_EventReport.h"

Script_EventReport::Script_EventReport(QObject *parent) : QObject(parent)
{
...
Script_EventReport::~Script_EventReport()
{
...
void Script_EventReport::execute()
{
...
void Script_EventReport::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_EventReport::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_EventReport::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_EventReport::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
    ...
    if(emScriptRunState==Wait_BuildNetFinish_Whole)
    {
       ...
       //     {
            ...
            emScriptRunState=Wait_HardwareInit_Finish;
...
void Script_EventReport::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
  ...
  //  QList<int> sendParams;
    ...
    case Wait_EventReport_Finish:
    {
        ...
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Error, QString("dvcType error:processCtrlDvcRes()"));
        }
        break;
    }
    case ScriptSuccess:
    {
        ...
        else if(dvcType == ThreeSTA)
        {
            if(recvEventNum >= 2)
            {
                p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("收到%1个电表事件上报").arg(recvEventNum));
            }
            else if(recvEventNum == 1)
            {
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("只收到1个电表事件上报，不符合要求"));
            }
            else
            {
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("未收到电表事件上报"));
            }
        }
        else
        {
...
void Script_EventReport::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        case Wait_HardwareInit_Finish:
        {
            ...
            {
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到确认，拉高单通事件管脚");
                sendParams.clear();
                idList.clear();
        ...
        case Wait_EventReport_Finish:
        {
            if(p_Frame3762Base->afn_ == 0x06&&p_Frame3762Base->dt1_==0x10&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(p_EventReport_06F5_Up->report_event_unit_.frame_content_.at(0)==0x01)//停电
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("收到表号%1停电上报").arg(QString(QByteArray(eventST.reportNodeAddress.addr,6).toHex())));
                }
                else if(p_EventReport_06F5_Up->report_event_unit_.frame_content_.at(0)==0x02)//复电
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("收到表号%1复电上报").arg(QString(QByteArray(eventST.reportNodeAddress.addr,6).toHex())));
                }
                else
                {
//                    PowerEvent_Struct eventST;
...
//                    otherEventReportList.append(eventST);
                 //   receiveEventFlag = true;
                    QString eventMsg = p_EventReport_06F5_Up->report_event_unit_.frame_content_.toHex();
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("收到第%1个电表事件：").arg(++recvEventNum)+eventMsg);
                    eventMsgList.append(eventMsg).append("\n");
                }
            ...
            else if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "组网完成，上报电表事件汇总：\n"+eventMsgList);
                ...
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("当前网络规模%0，组网未完成，60s后再查询").arg(p_QueryNetScale_10F9_Up->network_scale_));
                    p_delayTimer->start(60*1000);
                }
            }
            else
...
void Script_EventReport::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_EventReport::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_EventReport::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_EventReport::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_EventReport::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
    ...
    case Wait_HardwareInit_Finish:
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_HardwareInit_03F10 timeout!!!");
        break;
    }
    case Wait_EventReport_Finish:
    {
...
void Script_EventReport::maxAllowTimer_timeoutProc()
{
    p_maxAllowTimer->stop();
    switch(emScriptRunState)
    {
    case Wait_EventReport_Finish:
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "组网到达最大时间，上报电表事件汇总：\n"+eventMsgList);
        emScriptRunState = ScriptSuccess;
        sendParams.clear();
        idList.clear();
        idList = getDvcIdList(SingleSTA);
...
void Script_EventReport::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
...
void Script_EventReport::calPowerOnReportRate()
{
...
QList<int> Script_EventReport::getDvcIdList(DvcType dvcType)
{
    QList<int> dvcIdList;
    dvcIdList.clear();
    switch (dvcType)
