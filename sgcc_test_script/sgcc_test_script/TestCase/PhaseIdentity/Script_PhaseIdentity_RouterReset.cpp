#include "Script_PhaseIdentity_RouterReset.h"

Script_PhaseIdentity_RouterReset::Script_PhaseIdentity_RouterReset(QObject *parent) : QObject(parent)
{
...
Script_PhaseIdentity_RouterReset::~Script_PhaseIdentity_RouterReset()
{
...
void Script_PhaseIdentity_RouterReset::execute()
{
    ...
    else
    {
        ...
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("等待%1s，开始查询").arg(delayTime));
...
void Script_PhaseIdentity_RouterReset::stop()
{
...
void Script_PhaseIdentity_RouterReset::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_PhaseIdentity_RouterReset::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_PhaseIdentity_RouterReset::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_PhaseIdentity_RouterReset::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_PhaseIdentity_RouterReset::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_PhaseIdentity_RouterReset::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_PhaseIdentity_10F2_Finish:
            {
                ...
                {
                    p_timer->stop();
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到03F10！");
                ...
                {
                    p_timer->stop();
                    shared_ptr<Afn10F21> p_QueryNetTopoInfo_10F21_Up=std::dynamic_pointer_cast<Afn10F21>(p_Frame3762Base);
                    if(p_QueryNetTopoInfo_10F21_Up->network_typelogy_info_unit_.node_total_num_==p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->size()+1)
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "网络拓扑信息与档案一致，符合要求！");
                        index=0;
                        times=ushort(p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->size());
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("查询网络拓扑信息（%1轮）！").arg(++index));
            ...
            case Wait_PhaseIdentity_10F31_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x40&&p_Frame3762Base->dt2_==0x03&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    index+=num;
                    if(index<p_QueryPhaseInfo_10F31_Up->node_phase_info_unit_.node_total_num_)
                    {
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryPhaseInfo_10F31);
                        emScriptRunState=Wait_PhaseIdentity_10F31_Finish;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--查询从节点相位信息（10F31），等待--确认");
            ...
            case Wait_PhaseIdentity_13F1_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x13&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(p_MonitorSlaveNode_13F1_Up->data_field_up_.frame_length_==0)
                    {
                        ...
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("13F1查询表号%1相位，抄读失败！").arg(QString(QByteArray(p_MonitorSlaveNode_13F1_Up->address_field_.src_addr,6).toHex())));
                    ...
                    if(p_MonitorSlaveNode_13F1_Up->info_field_.info_field_up.meter_channel_character==0x04)//三相
                        nodeInfo.nodePhase=getNodePhase(Query_13F1,7);
                    else
                        nodeInfo.nodePhase=getNodePhase(Query_13F1,p_MonitorSlaveNode_13F1_Up->info_field_.info_field_up.measured_phase_flag);
                    ...
                    else
                    {
                        ...
                        if(!identityFailList_10F2.isEmpty())
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("10F2查询，存在从节点相位识别错误！\n失败表如下：\n%1").arg(identityFailList_10F2));
                        }
                        if(!identityFailList_10F31.isEmpty())
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("10F31查询，存在从节点相位识别错误！\n失败表如下：\n%1").arg(identityFailList_10F31));
                        }
                        if(!identityFailList_13F1.isEmpty())
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("13F1查询，存在从节点相位识别错误！\n失败表如下：\n%1").arg(identityFailList_13F1));
                        }
                        nodeInfo10F31List_10F31.clear();
                        nodeInfoGroupList_10F2.clear();
                        if(emCmdType==NormalCmd)
                        {
//                            if(identityFailList_10F2.isEmpty()&&identityFailList_10F31.isEmpty()&&identityFailList_13F1.isEmpty())
//                            {
                                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "所有从节点相位识别正确！");
                                index=0;
                                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_AddSlaveNode_11F1);
                                emScriptRunState=Wait_PhaseIdentity_10F2_Finish;
                                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--添加从节点（11F1），等待--确认");
...
//                            else
//                            {
//                                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "存在从节点相位识别错误！");
//                                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("相位识别测试失败！"));
//                            }
                        }
...
//                            {
//                                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "存在从节点相位识别错误！");
//                                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("相位识别-重复添加从节点，测试失败！"));
//                            }
                        }
                        else if(emCmdType==HardReset)
                        {
                            if(identityFailList_10F2.isEmpty()&&identityFailList_10F31.isEmpty()&&identityFailList_13F1.isEmpty())
                            {
                                ...
                                resultFlag=true;
                                p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("相位识别-硬件复位，测试成功！"));
                            }
                            else
                            {
                                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "存在从节点相位识别错误！");
                                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("相位识别-硬件复位，测试失败！"));
                            }
                        }
                    }
                }
...
void Script_PhaseIdentity_RouterReset::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_PhaseIdentity_RouterReset::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_PhaseIdentity_RouterReset::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_AddSlaveNode_11F1)
    {
        ...
        if(index+num<p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->size())
        {
            for(int i=0;i<num;i++)
            {
    ...
    else if(frame==p_QueryNetTopoInfo_10F21)
    {
        ...
        p_QueryNetTopoInfo_10F21->node_num_=num;
    ...
    else if(frame==p_QueryPhaseInfo_10F31)
    {
        ...
        p_QueryPhaseInfo_10F31->node_start_no_=index;
        p_QueryPhaseInfo_10F31->node_num_=num;
...
void Script_PhaseIdentity_RouterReset::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_PhaseIdentity_RouterReset::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_PhaseIdentity_RouterReset::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_PhaseIdentity_10F2_Finish:
...
void Script_PhaseIdentity_RouterReset::delayTimer_timeoutProc()
{
    p_delayTimer->stop();    
...
void Script_PhaseIdentity_RouterReset::generateStandardList(Script_PhaseIdentity_RouterReset::QueryType emQueryType)
{
    if(emQueryType==Query_10F2)
    {
        ...
        for(int i=0;i<nodeInfoGroupList_10F2.size();i++)
        {
            ...
            for(int j=0;j<p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().size();j++)
            {
                ...
                {
                    if(uchar(nodeInfoST.nodePhase)!=p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(j)->realPhase)
                    {
                        nodeInfoST.phaseIsRight=false;
                        nodeInfoST.initPhase=getNodePhase(Query_10F2,p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(j)->realPhase);
                        break;
                    }
                    else
...
void Script_PhaseIdentity_RouterReset::getFailList(Script_PhaseIdentity_RouterReset::QueryType emQueryType)
{
    if(emQueryType==Query_10F2)
    {
        identityFailList_10F2.clear();
        for(int i=0;i<nodeInfoList_10F2.size();i++)
        {
            if(nodeInfoList_10F2.at(i).phaseIsRight==false)
                identityFailList_10F2+=QString(QByteArray(nodeInfoList_10F2.at(i).nodeAddress.addr,6).toHex())+QString(" 实测相位%1 档案相位为%2\n").arg(getPhaseInfo(nodeInfoList_10F2.at(i).nodePhase)).arg(getPhaseInfo(nodeInfoList_10F2.at(i).initPhase));
        }
    }
    else if(emQueryType==Query_10F31)
    {
        identityFailList_10F31.clear();
        for(int i=0;i<nodeInfoList_10F31.size();i++)
        {
            if(nodeInfoList_10F31.at(i).phaseIsRight==false)
                identityFailList_10F31+=QString(QByteArray(nodeInfoList_10F31.at(i).nodeAddress.addr,6).toHex())+QString(" 实测相位%1 档案相位为%2\n").arg(getPhaseInfo(nodeInfoList_10F31.at(i).nodePhase)).arg(getPhaseInfo(nodeInfoList_10F31.at(i).initPhase));
        }
    }
    else if(emQueryType==Query_13F1)
    {
        identityFailList_13F1.clear();
        for(int i=0;i<nodeInfoList_13F1.size();i++)
        {
            if(nodeInfoList_13F1.at(i).phaseIsRight==false)
                identityFailList_13F1+=QString(QByteArray(nodeInfoList_13F1.at(i).nodeAddress.addr,6).toHex())+QString(" 实测相位%1 档案相位为%2\n").arg(getPhaseInfo(nodeInfoList_13F1.at(i).nodePhase)).arg(getPhaseInfo(nodeInfoList_13F1.at(i).initPhase));
        }
    }
}
Script_PhaseIdentity_RouterReset::Phase Script_PhaseIdentity_RouterReset::getNodePhase(Script_PhaseIdentity_RouterReset::QueryType emQueryType, uchar phase)
{
...
QString Script_PhaseIdentity_RouterReset::getPhaseInfo(Script_PhaseIdentity_RouterReset::Phase phase)
{
    if(phase==A)
        return "A相";
    else if(phase==B)
