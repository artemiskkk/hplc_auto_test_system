#include "Script_PhaseIdentity_AddNodeRepeat.h"

Script_PhaseIdentity_AddNodeRepeat::Script_PhaseIdentity_AddNodeRepeat(QObject *parent) : QObject(parent)
{
...
Script_PhaseIdentity_AddNodeRepeat::~Script_PhaseIdentity_AddNodeRepeat()
{
...
void Script_PhaseIdentity_AddNodeRepeat::execute()
{
...
void Script_PhaseIdentity_AddNodeRepeat::stop()
{
...
void Script_PhaseIdentity_AddNodeRepeat::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_PhaseIdentity_AddNodeRepeat::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_PhaseIdentity_AddNodeRepeat::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_PhaseIdentity_AddNodeRepeat::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_PhaseIdentity_AddNodeRepeat::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_PhaseIdentity_AddNodeRepeat::processMsgFromCCO(DvcType dvcType, int dvcId)
{
...
//                            }
                        ...
                        else if(emCmdType==AddNode)
                        {
                            if(identityFailList_10F2.isEmpty()&&identityFailList_10F31.isEmpty()&&identityFailList_13F1.isEmpty())
                            {
                                ...
                                resultFlag=true;
                                p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("相位识别-重复添加从节点，测试成功！"));
                            }
                            else
                            {
                                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "存在从节点相位识别错误！");
...
void Script_PhaseIdentity_AddNodeRepeat::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_PhaseIdentity_AddNodeRepeat::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_PhaseIdentity_AddNodeRepeat::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_PhaseIdentity_AddNodeRepeat::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_PhaseIdentity_AddNodeRepeat::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_PhaseIdentity_AddNodeRepeat::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_PhaseIdentity_10F2_Finish:
...
void Script_PhaseIdentity_AddNodeRepeat::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNodeInfo_10F2);
    emScriptRunState=Wait_PhaseIdentity_10F2_Finish;
...
void Script_PhaseIdentity_AddNodeRepeat::generateStandardList(Script_PhaseIdentity_AddNodeRepeat::QueryType emQueryType)
{
...
void Script_PhaseIdentity_AddNodeRepeat::getFailList(Script_PhaseIdentity_AddNodeRepeat::QueryType emQueryType)
{
    if(emQueryType==Query_10F2)
    {
...
Script_PhaseIdentity_AddNodeRepeat::Phase Script_PhaseIdentity_AddNodeRepeat::getNodePhase(Script_PhaseIdentity_AddNodeRepeat::QueryType emQueryType, uchar phase)
{
...
QString Script_PhaseIdentity_AddNodeRepeat::getPhaseInfo(Script_PhaseIdentity_AddNodeRepeat::Phase phase)
{
    if(phase==A)
        return "A相";
    else if(phase==B)
