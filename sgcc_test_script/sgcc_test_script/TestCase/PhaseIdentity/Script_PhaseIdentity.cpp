#include "Script_PhaseIdentity.h"

Script_PhaseIdentity::Script_PhaseIdentity(QObject *parent) : QObject(parent)
{
...
Script_PhaseIdentity::~Script_PhaseIdentity()
{
...
void Script_PhaseIdentity::execute()
{
...
void Script_PhaseIdentity::stop()
{
...
void Script_PhaseIdentity::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_PhaseIdentity::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_PhaseIdentity::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_PhaseIdentity::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_PhaseIdentity::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_PhaseIdentity::processMsgFromCCO(DvcType dvcType, int dvcId)
{
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_PhaseIdentity_10F2_Finish:
            {
                ...
                else if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    else if(emCmdType==BuildNet)
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "硬件初始化，回复确认！");
                    }
                    else
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("emCmdType error:%1").arg(emCmdType));
                    }
                }
                ...
                {
                    p_timer->stop();
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "硬件初始化，收到03F10！");
                    index=0;
...
void Script_PhaseIdentity::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_PhaseIdentity::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_PhaseIdentity::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_PhaseIdentity::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_PhaseIdentity::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_PhaseIdentity::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_PhaseIdentity_10F2_Finish:
...
void Script_PhaseIdentity::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
...
void Script_PhaseIdentity::generateStandardList(Script_PhaseIdentity::QueryType emQueryType)
{
...
void Script_PhaseIdentity::getFailList(Script_PhaseIdentity::QueryType emQueryType)
{
    QString failInfo;
    if(emQueryType==Query_10F2)
    {
...
Script_PhaseIdentity::Phase Script_PhaseIdentity::getNodePhase(Script_PhaseIdentity::QueryType emQueryType, uchar phase)
{
...
Script_PhaseIdentity::Phase Script_PhaseIdentity::getThreePhase_10F31(uchar phase_sequence)
{
    if(phase_sequence==0)
        return ABC;
...
Script_PhaseIdentity::Phase Script_PhaseIdentity::getThreePhase_config(uchar config)
{
    if(config==7)
        return ABC;
    else if(config==8)
...
QString Script_PhaseIdentity::getPhaseInfo(Script_PhaseIdentity::Phase phase)
{
    if(phase==A)
        return "A相";
    else if(phase==B)
