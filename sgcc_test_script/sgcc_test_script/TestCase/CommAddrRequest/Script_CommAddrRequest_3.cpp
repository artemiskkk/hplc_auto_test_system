#include "Script_CommAddrRequest_3.h"
#include <QDataStream>
#include <QDir>

Script_CommAddrRequest_3::Script_CommAddrRequest_3(QObject *parent) : QObject(parent)
{
...
Script_CommAddrRequest_3::~Script_CommAddrRequest_3()
{
    p_timer->stop();
...
QList<int> Script_CommAddrRequest_3::getDvcIdList(DvcType dvcType)
{
    QList<int> dvcIdList;
    dvcIdList.clear();
 //   switch (dvcType)
...
void  Script_CommAddrRequest_3::execute()
{
...
void  Script_CommAddrRequest_3::stop()
{
    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "Test stop!");
}

void Script_CommAddrRequest_3::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void  Script_CommAddrRequest_3::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
 //   p_BuildNetwork->setHost(host);
}

bool  Script_CommAddrRequest_3::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_CommAddrRequest_3::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
    ...
    else if(dvcType==SingleSTA || dvcType==ThreeSTA)
    {
        ...
        if(emScriptRunState==Wait_StaReadAddr||emScriptRunState==Wait_OOPReadMeter_Finish)
        {
...
void Script_CommAddrRequest_3::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
//    }
    ...
    case Init_ResetSta:
    {
        if (dvcType == SingleSTA)
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("单通复位成功，等待OOP读表号"));
...
void  Script_CommAddrRequest_3::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
...
void  Script_CommAddrRequest_3::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
    default:
...
void Script_CommAddrRequest_3::processMsgFromMeter698(int id)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        if(MsgBase_698_ptr->service_type_==GET_REQUEST_CLIENT && MsgBase_698_ptr->service_sub_type_==uchar(GetRequestType::kGetRequestNormal))
        {
            ...
            {
               // p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到OOP读表号报文，应答，10s后抄表...");
                sendMsg(SingleSTA,id,p_GetResponseNormal_ReadAddr);
//                p_timer->start(10*1000);
                p_AbstractScriptHost->updateProgress(ProcessState_Success, "OOP测试成功");//KEYID_OI响应数据太大，解析失败，所以地址请求应答后，说明通信正常，直接判成功，不进行读数据
                emScriptRunState=ScriptComplete;
            }
            else
            {
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到OOP读数据报文，应答");
        ...
        else if(MsgBase_698_ptr->service_type_==GET_RESPONSE_SERVER && MsgBase_698_ptr->service_sub_type_==uchar(GetResponseType::kGetResponseNormal))
        {
            ...
            if(p_GetResponseNormal_Up->a_result_normal_.oad.OI==KEYID_OI && p_GetResponseNormal_Up->a_result_normal_.get_result_ptr->choice_==GetResultChoice::kData)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Success, "OOP抄表成功");
                emScriptRunState=ScriptComplete;
            }
            else
            {
...
void Script_CommAddrRequest_3::sendMsg(DvcType dvcType,int id,shared_ptr<void> frame)
{
    sendMsgOct.clear();

    if(frame==p_GetResponseNormal_ReadAddr)
    {
...
void Script_CommAddrRequest_3::sendMsg(DvcType dvcType, int id, QByteArray msg)
{
