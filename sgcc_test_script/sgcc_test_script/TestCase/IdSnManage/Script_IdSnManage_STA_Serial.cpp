#include "Script_IdSnManage_STA_Serial.h"

Script_IdSnManage_STA_Serial::Script_IdSnManage_STA_Serial(QObject *parent) : QObject(parent)
{
...
Script_IdSnManage_STA_Serial::~Script_IdSnManage_STA_Serial()
{
...
void Script_IdSnManage_STA_Serial::execute()
{
...
void Script_IdSnManage_STA_Serial::stop()
{
...
void Script_IdSnManage_STA_Serial::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_IdSnManage_STA_Serial::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_IdSnManage_STA_Serial::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_IdSnManage_STA_Serial::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
    ...
    if(emScriptRunState==Wait_BuildNetFinish_Whole)
    {
        ...
        //    if(p_BuildNetwork_GW->emScriptRunState==BuildNetFinish&&p_BuildNetwork_GW->resultFlag==true)
            {
                emScriptRunState = SetChipID;
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--串口设置STA芯片ID");
    ...
    else
    {
        ...
        else if(dvcType==SingleSTA || dvcType==ThreeSTA)
        {
            for(int i=0; i<p_CtrInfoList->at(0)->keyList.size(); i++)
            {
                ...
                {
                    if(emScriptRunState==SetChipID || emScriptRunState==SetStaID || emScriptRunState==SetSN)
                    {
                        p_CtrInfoList->at(0)->buf.append(recvTempData);
                        processMsgFromCCO(dvcType,id);
                    }
...
void Script_IdSnManage_STA_Serial::processCtrlDvcRes(DvcType dvcType, QList<int> snList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_IdSnManage_STA_Serial::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        case SetChipID:
        {
            ...
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到确认，串口查询STA设置后的芯片ID");
                sendMsg(SingleSTA,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryChipID_F0F40);
                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
            }
            else if(p_Frame3762Base->afn_==char(0xf0) && dtValue3762==40 && p_Frame3762Base->ctrl_field_.dir==kDirUp)
            {
                ...
                {
                    if(chipId == QString(p_QueryChipID_F0F40_Up->read_chip_id_unit_up_.id_content_.toHex()))
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("STA芯片ID为%1，符合要求，串口设置STA模块ID").arg(QString(p_QueryChipID_F0F40_Up->read_chip_id_unit_up_.id_content_.toHex())));
                        sendMsg(SingleSTA,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_SetStaID_F0F39);
                        emScriptRunState = SetStaID;
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    }
                    else
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("STA芯片ID为%1，不符合要求").arg(QString(p_QueryChipID_F0F40_Up->read_chip_id_unit_up_.id_content_.toHex())));
                    }
                }
                else
                {
        ...
        case SetStaID:
        {
            ...
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到确认，串口查询STA设置后的模块ID");
                sendMsg(SingleSTA,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryStaID_F0F40);
                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
            }
            else if(p_Frame3762Base->afn_==char(0xf0) && dtValue3762==40 && p_Frame3762Base->ctrl_field_.dir==kDirUp)
            {
                p_timer->stop();
                shared_ptr<AfnF0F40> p_QueryStaID_F0F40_Up = dynamic_pointer_cast<AfnF0F40>(p_Frame3762Base);
                if(memcmp(p_QueryStaID_F0F40_Up->read_chip_id_unit_up_.mac_address_.addr,p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(0)->mtrAddr,6)==0
                        && p_QueryStaID_F0F40_Up->read_chip_id_unit_up_.id_type_==0x02
                        && p_QueryStaID_F0F40_Up->read_chip_id_unit_up_.id_format_==0x02)
                {
                    if(staId == QString(p_QueryStaID_F0F40_Up->read_chip_id_unit_up_.id_content_.toHex()))
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("STA模块ID为%1，符合要求，串口设置STA生产序列号SN").arg(QString(p_QueryStaID_F0F40_Up->read_chip_id_unit_up_.id_content_.toHex())));
                        sendMsg(SingleSTA,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_SetSn_F0F42);
                        emScriptRunState = SetSN;
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    }
                    else
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("STA芯片ID为%1，不符合要求").arg(QString(p_QueryStaID_F0F40_Up->read_chip_id_unit_up_.id_content_.toHex())));
                    }
                }
                else
                {
        ...
        case SetSN:
        {
            ...
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到确认，串口查询STA设置后的SN");
                sendMsg(SingleSTA,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QuerySn_F0F41);
                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
            }
            else if(p_Frame3762Base->afn_==char(0xf0) && dtValue3762==41 && p_Frame3762Base->ctrl_field_.dir==kDirUp)
            {
                ...
                {
                    if(sn==QString(p_QuerySn_F0F41_Up->sn_content_.toHex()).toUpper())
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("STA生产序列号SN为%1，符合要求，路由硬件初始化").arg(QString(p_QuerySn_F0F41_Up->sn_content_.toHex()).toUpper()));
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_HardReset_01F1);
                        p_timer->start(CCO_CMD_TIMEOUT_TIME*1000);
                        emScriptRunState = QueryCco_After;
                    }
                    else
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("STA生产序列号SN为%1，不符合要求").arg(QString(p_QuerySn_F0F41_Up->sn_content_.toHex()).toUpper()));
                    }
                }
                else
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("F0F41回复MAC地址或设备类型不符合要求"));
                }
            }
            else
            {
        ...
        case QueryCco_After:
        {
            ...
            else if(p_Frame3762Base->afn_==0x10 && dtValue3762==40 && p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(flag_10F40 == false)
                {
                    shared_ptr<Afn10F40> p_QueryChipID_10F40_Up = dynamic_pointer_cast<Afn10F40>(p_Frame3762Base);
                    if(memcmp(p_QueryChipID_10F40_Up->read_chip_id_unit_up_.mac_address_.addr,p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(0)->mtrAddr,6)==0
                            && p_QueryChipID_10F40_Up->read_chip_id_unit_up_.id_type_==0x01
                            && p_QueryChipID_10F40_Up->read_chip_id_unit_up_.device_type_==0x03)
                    {
                        if(chipId == QString(p_QueryChipID_10F40_Up->read_chip_id_unit_up_.id_content_.toHex()))
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("STA芯片ID为%1，符合要求，发送--查询STA模块ID（10F40），等待--回复").arg(QString(p_QueryChipID_10F40_Up->read_chip_id_unit_up_.id_content_.toHex())));
                            sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryStaID_10F40);
                            p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                            flag_10F40 = true;
                        }
                        else
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("STA芯片ID为%1，不符合要求").arg(QString(p_QueryChipID_10F40_Up->read_chip_id_unit_up_.id_content_.toHex())));
                        }
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    {
                        if(staId == QString(p_QueryStaID_10F40_Up->read_chip_id_unit_up_.id_content_.toHex()))
                        {
                            ...
                            p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("STA串口设置芯片ID、模块ID、生产序列号SN测试成功"));
                        }
                        else
                        {
...
void Script_IdSnManage_STA_Serial::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_IdSnManage_STA_Serial::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_IdSnManage_STA_Serial::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_QueryChipID_10F40)
    {
        ...
        memcpy(p_QueryChipID_10F40->read_chip_id_unit_down_.mac_address_.addr,p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(0)->mtrAddr,6);
    ...
    else if(frame==p_SetChipID_F0F39)
    {
        ...
        memcpy(p_SetChipID_F0F39->config_chip_id_unit_.mac_address_.addr,p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(0)->mtrAddr,6);
    ...
    else if(frame==p_SetStaID_F0F39)
    {
        ...
        memcpy(p_SetStaID_F0F39->config_chip_id_unit_.mac_address_.addr,p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(0)->mtrAddr,6);
    ...
    else if(frame==p_QueryChipID_F0F40)
    {
        ...
        memcpy(p_QueryChipID_F0F40->read_chip_id_unit_down_.mac_address_.addr,p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(0)->mtrAddr,6);
    ...
    else if(frame==p_QueryStaID_F0F40)
    {
        ...
        memcpy(p_QueryStaID_F0F40->read_chip_id_unit_down_.mac_address_.addr,p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(0)->mtrAddr,6);
    ...
    else if(frame==p_SetSn_F0F42)
    {
        ...
        memcpy(p_SetSn_F0F42->mac_address_.addr,p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(0)->mtrAddr,6);
    ...
    else if(frame==p_QuerySn_F0F41)
    {
        ...
        memcpy(p_QuerySn_F0F41->mac_address_.addr,p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(0)->mtrAddr,6);
...
void Script_IdSnManage_STA_Serial::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_IdSnManage_STA_Serial::timer_timeoutProc()
{
    p_timer->stop();
    p_AbstractScriptHost->updateProgress(ProcessState_Failed, "p_timer timeout");
}

void Script_IdSnManage_STA_Serial::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        default:
...
void Script_IdSnManage_STA_Serial::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
...
QByteArray Script_IdSnManage_STA_Serial::getQueryFrame()
{
    QByteArray msg;
