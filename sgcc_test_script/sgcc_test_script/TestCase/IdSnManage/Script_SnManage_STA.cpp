#include "Script_SnManage_STA.h"

Script_SnManage_STA::Script_SnManage_STA(QObject *parent) : QObject(parent)
{
    ...
    p_QueryStaSn_F0F41=make_shared<AfnF0F41>();
  //  p_SetStaSn_F0F42=make_shared<AfnF0F42>();
...
Script_SnManage_STA::~Script_SnManage_STA()
{
...
void Script_SnManage_STA::execute()
{
    ...
    {
        index=0;
        emScriptRunState=Wait_SetStaSn_Finish;
        sendMsg(ReadCtrlDvc,p_CtrInfoList->at(0)->ctrlID,index,p_TransmitData_02F1);
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--透传设置STA生产序列号Sn，等待--确认");
        p_timer->start(STA_CMD_TIMEOUT_TIME*1000);
    }
    p_maxAllowTimer->start((timerForReachThresld+timerAfterReachThresld)*1000);

}
void Script_SnManage_STA::stop()
{
...
void Script_SnManage_STA::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_SnManage_STA::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_SnManage_STA::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_SnManage_STA::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_SnManage_STA::processCtrlDvcRes(DvcType dvcType, QList<int> snList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_SnManage_STA::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_SetStaSn_Finish:
            {
                QByteArray tmpSendMsg=prcsOther3762Msg(uchar(p_Frame3762Base->afn_),p_Frame3762Base->dt1_,p_Frame3762Base->dt2_,uchar(p_Frame3762Base->info_field_.info_field_up.msg_seq));
                sendSrcMsg(dvcType,dvcId,tmpSendMsg);
                break;
            }
            case Wait_QueryStaSn_Finish:
            {
                QByteArray tmpSendMsg=prcsOther3762Msg(uchar(p_Frame3762Base->afn_),p_Frame3762Base->dt1_,p_Frame3762Base->dt2_,uchar(p_Frame3762Base->info_field_.info_field_up.msg_seq));
                sendSrcMsg(dvcType,dvcId,tmpSendMsg);
                break;
...
void Script_SnManage_STA::processMsgFromReadCtrlDvc(DvcType dvcType, int dvcId)
{
    if(dvcId!=p_CtrInfoList->at(0)->ctrlID)
        return;
    bool haveCompleteMsg=true;
            ...
            case Wait_SetStaSn_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x02&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(p_TransmitData_02F1_Up->frame_content_.size()!=0)
                    {
                        ...
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "模块SN设置成功");
                            tryTimes=0;
                            emScriptRunState=Wait_QueryStaSn_Finish;
                        ...
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "模块SN设置失败");
                        }
                    }
                    else
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "模块SN设置失败,02F1上报为空");
                    }
                }
                else
                {
            ...
            case Wait_QueryStaSn_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x02&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(p_TransmitData_02F1_Up->frame_content_.size()!=0)
                    {
                        ...
                        if(p_ChkParam_Up->paramInfoList.at(0).id==29&&QString(p_ChkParam_Up->paramInfoList.at(0).idCntnt.toHex()).toUpper()==dstSn)
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("STA生产序列号Sn为%1，符合要求").arg(QString(p_ChkParam_Up->paramInfoList.at(0).idCntnt.toHex()).toUpper()));
                            snIndex++;
                            ...
                            else
                            {
                                ...
                                resultFlag=true;
                                p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("STA生产序列号Sn管理测试成功;"));
                            }
                        }
                        else
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("模块SN查询失败%1").arg(QString(p_ChkParam_Up->paramInfoList.at(0).idCntnt.toHex()).toUpper()));
                        }
                    }
                    else
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "模块SN查询失败,02F1上报为空");
                    }
                }
                else
                {
...
void Script_SnManage_STA::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_SnManage_STA::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_SnManage_STA::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    if(frame==p_TransmitData_02F1)
    {
        ...
        if(emScriptRunState==Wait_SetStaSn_Finish)
        {
            p_TransmitData_02F1->frame_content_=getSetFrame();
        }
        else if(emScriptRunState==Wait_QueryStaSn_Finish)
        {
...
void Script_SnManage_STA::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_SnManage_STA::timer_timeoutProc()
{
    if(emScriptRunState==Wait_SetStaSn_Finish)
    {
        if(++tryTimes<3)
...
void Script_SnManage_STA::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
...
void Script_SnManage_STA::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    p_maxAllowTimer->stop();
}

QByteArray Script_SnManage_STA::getSetFrame()
{
    QByteArray msg;
    shared_ptr<SetParam_Down> p_SetParam_Down=make_shared<SetParam_Down>();
    QList<ParamInfo> setParamInfoList;
    ParamInfo tmpParamInfo;
    tmpParamInfo.id=29;//SN
    tmpParamInfo.idCntnt=QByteArray::fromHex(snList.at(snIndex).toLatin1());
...
QByteArray Script_SnManage_STA::getQueryFrame()
{
    QByteArray msg;
