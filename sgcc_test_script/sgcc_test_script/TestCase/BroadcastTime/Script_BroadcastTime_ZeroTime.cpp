#include "Script_BroadcastTime_ZeroTime.h"

Script_BroadcastTime_ZeroTime::Script_BroadcastTime_ZeroTime(QObject *parent) : QObject(parent)
{
...
Script_BroadcastTime_ZeroTime::~Script_BroadcastTime_ZeroTime()
{
...
void Script_BroadcastTime_ZeroTime::execute()
{
...
void Script_BroadcastTime_ZeroTime::stop()
{
...
void Script_BroadcastTime_ZeroTime::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_BroadcastTime_ZeroTime::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_BroadcastTime_ZeroTime::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_BroadcastTime_ZeroTime::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_BroadcastTime_ZeroTime::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_BroadcastTime_ZeroTime::processMsgFromCCO(DvcType dvcType, int dvcId)
{
...
void Script_BroadcastTime_ZeroTime::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_BroadcastTime_ZeroTime::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_BroadcastTime_ZeroTime::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_BroadcastTime_ZeroTime::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_BroadcastTime_ZeroTime::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_BroadcastTime_ZeroTime::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_Finish_Broadcast:
...
void Script_BroadcastTime_ZeroTime::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    index=0;
    p_maxAllowTimer->stop();
    ...
    if(calSuccessRate()>=netSucRateThresld)
    {
        emScriptRunState=ScriptSuccess;
        resultFlag=true;
        p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("广播校时成功率：%1%，时间无偏移！").arg(calSuccessRate()*100));
    }
    else
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, getFailMeterNo());
...
void Script_BroadcastTime_ZeroTime::meterInfoInit()
{
    meterInfoList.clear();
...
bool Script_BroadcastTime_ZeroTime::isMeterExist(Address address)
{
...
int Script_BroadcastTime_ZeroTime::getMeterInfo(Address address)
{
    for(int i=0;i<meterInfoList.size();i++)
    {
        if(address==meterInfoList.at(i).meterNo)
...
double Script_BroadcastTime_ZeroTime::calSuccessRate()
{
...
QString Script_BroadcastTime_ZeroTime::getFailMeterNo()
{
    QString failMeterNo;
    for(int i=0;i<meterInfoList.size();i++)
    {
