#include "Script_BroadcastTime_NotBuildNet.h"

Script_BroadcastTime_NotBuildNet::Script_BroadcastTime_NotBuildNet(QObject *parent) : QObject(parent)
{
    ...
    p_ParamInit_01F2=make_shared<Afn01F2>();
...
Script_BroadcastTime_NotBuildNet::~Script_BroadcastTime_NotBuildNet()
{
...
void Script_BroadcastTime_NotBuildNet::execute()
{
...
void Script_BroadcastTime_NotBuildNet::stop()
{
...
void Script_BroadcastTime_NotBuildNet::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_BroadcastTime_NotBuildNet::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_BroadcastTime_NotBuildNet::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_BroadcastTime_NotBuildNet::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
    ...
    if(emScriptRunState==Wait_BuildNetFinish_Whole)
    {
        ...
        else
        {
            ...
            p_BuildNetwork_GW->initBuildNetWork();
            meterInfoInit();
            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送-05F3广播校时");
            sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_Broadcast_05F3);
            emScriptRunState = Wait_00F1_For_05F3_Broadcast;
            p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
...
void Script_BroadcastTime_NotBuildNet::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_BroadcastTime_NotBuildNet::processMsgFromCCO(DvcType dvcType, int dvcId)
{
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_00F1_For_05F3_Broadcast:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("广播校时收到确认"));
...
void Script_BroadcastTime_NotBuildNet::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
            ...
            default:
            {
                ...
                else
                {
                    ...
                    if(MsgBase_645_ptr->ctrlCode_==BROADCAST)
                    {
                        ...
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("%1芯片，广播校时-不组网，不应该收到校时报文").arg(chipType));
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("%1芯片，广播校时-不组网，不应该收到校时报文").arg(chipType));
                        }
                    }
                    else
                    {
...
void Script_BroadcastTime_NotBuildNet::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_BroadcastTime_NotBuildNet::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_AddSlaveNode_11F1)
    {
        ...
        char addr[6] = {0x38,0x39,0x38,0x39,0x38,0x39};
        memcpy(nodeParameter.node_address_.addr,addr,6);
        nodeParameter.protocol_type_ = 2;

        p_AddSlaveNode_11F1->node_num_=1;
        p_AddSlaveNode_11F1->node_parameter_list_.append(nodeParameter);

        sendMsgOct=p_AddSlaveNode_11F1->EncodeFrame();
        sendMsgLog=QString("》》添加从节点11F1：%1\n").arg(QString(sendMsgOct.toHex().toUpper()));
    }
    else if(frame==p_ParamInit_01F2)
    {
...
void Script_BroadcastTime_NotBuildNet::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_BroadcastTime_NotBuildNet::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
    ...
    case Wait_ParaInit_Finish:
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("发送--添加从节点（11F1），等待--确认，1min后广播校时"));
    ...
    case Wait_Finish_Broadcast:
    {
        if(chipType=="GY")
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "广播校时时间到，失败列表：" + getFailMeterNo());
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("广播校时成功率：%1%").arg(calSuccessRate()*100));
        }
        else
        {
            if(calSuccessRate()!=0.0)
            {
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("广播校时成功率：%1%").arg(calSuccessRate()*100));
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("%1芯片，广播校时-不组网，不应该收到校时报文").arg(chipType));
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("%1芯片，广播校时-不组网，不应该收到校时报文").arg(chipType));
            }
            else
            {
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("广播校时成功率：%1%").arg(calSuccessRate()*100));
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("%1芯片，广播校时-不组网，未收到校时报文，符合要求").arg(chipType));
                p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("%1芯片，广播校时-不组网，广播校时测试通过").arg(chipType));
            }
        }
        break;
    }
...
void Script_BroadcastTime_NotBuildNet::maxAllowTimer_timeoutProc()
{
...
void Script_BroadcastTime_NotBuildNet::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送-05F3广播校时");
    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_Broadcast_05F3);
...
//        emScriptRunState=ScriptSuccess;
//        resultFlag=true;
//        p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("广播校时成功率：%1%，支持不组网广播校时！").arg(calSuccessRate()*100));
//    }
//    else
//    {
//        p_AbstractScriptHost->updateProgress(ProcessState_Processing, getFailMeterNo());
...
void Script_BroadcastTime_NotBuildNet::meterInfoInit()
{
    meterInfoList.clear();
...
bool Script_BroadcastTime_NotBuildNet::isMeterExist(Address address)
{
...
int Script_BroadcastTime_NotBuildNet::getMeterInfo(Address address)
{
    for(int i=0;i<meterInfoList.size();i++)
    {
        if(address==meterInfoList.at(i).meterNo)
...
double Script_BroadcastTime_NotBuildNet::calSuccessRate()
{
...
QString Script_BroadcastTime_NotBuildNet::getFailMeterNo()
{
    QString failMeterNo;
    for(int i=0;i<meterInfoList.size();i++)
    {
