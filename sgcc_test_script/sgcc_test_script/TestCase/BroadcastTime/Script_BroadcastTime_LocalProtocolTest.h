#ifndef SCRIPT_BROADCASTTIME_LOCALPROTOCOLTEST_H
#define SCRIPT_BROADCASTTIME_LOCALPROTOCOLTEST_H
...
/**
 * @brief The Script_BroadcastTime_LocalProtocolTest_RunState 运行状态机
 */
enum Script_BroadcastTime_LocalProtocolTest_RunState
{
    BroadcastInit,
    Wait_BuildNetFinish_Whole,
    Wait_SendBroadcast_05F3_Finish_1,
    Wait_SendBroadcast_05F3_Finish_2,
    Wait_SendBroadcast_05F3_Finish_3,
    Wait_SendBroadcast_05F3_Finish_4,
    Wait_SendBroadcast_05F3_Finish_5,
...
/**
 * @brief The Script_BroadcastTime_LocalProtocolTest 广播校时05F3-本地格式规范性测试 【GW-CCO-F008-0002-V01】
 * refs #3243 新增修改点 05F3的是广播校时命令的数据长度与实际报文长度不符，以及05F3这整条报文长度小于17字节都是异常的
 */
using namespace Namespace_BroadcastTime;
class Script_BroadcastTime_LocalProtocolTest :  public QObject, public AbstractScript, public DynamicCreate<Script_BroadcastTime_LocalProtocolTest>
{
    Q_OBJECT
public:
    explicit Script_BroadcastTime_LocalProtocolTest(QObject *parent = nullptr);
    ~Script_BroadcastTime_LocalProtocolTest();
private:
    Script_BroadcastTime_LocalProtocolTest_RunState emScriptRunState = BroadcastInit;
    ...
    //校时参数
    QList<MeterInfoBroadcast_Struct> meterInfoList;   // 需要进行校时的虚拟表--单通+三通
    int checkTime=60;//秒
    double calSuccessRate();
    int getMeterInfo(Address address);
    bool broadcast_falg = false;
...
#endif // SCRIPT_BROADCASTTIME_LOCALPROTOCOLTEST_H
