#include "Script_BroadcastTime.h"

Script_BroadcastTime::Script_BroadcastTime(QObject *parent) : QObject(parent)
{
...
Script_BroadcastTime::~Script_BroadcastTime()
{
...
void Script_BroadcastTime::execute()
{
...
void Script_BroadcastTime::stop()
{
...
void Script_BroadcastTime::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_BroadcastTime::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_BroadcastTime::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_BroadcastTime::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_BroadcastTime::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_BroadcastTime::processMsgFromCCO(DvcType dvcType, int dvcId)
{
...
void Script_BroadcastTime::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_BroadcastTime::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_BroadcastTime::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_BroadcastTime::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_BroadcastTime::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_BroadcastTime::maxAllowTimer_timeoutProc()
{
...
void Script_BroadcastTime::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    index=0;
    p_maxAllowTimer->stop();
...
void Script_BroadcastTime::meterInfoInit()
{
    meterInfoList.clear();
...
bool Script_BroadcastTime::isMeterExist(Address address)
{
...
int Script_BroadcastTime::getMeterInfo(Address address)
{
    for(int i=0;i<meterInfoList.size();i++)
    {
        if(address==meterInfoList.at(i).meterNo)
...
double Script_BroadcastTime::calSuccessRate()
{
...
QString Script_BroadcastTime::getFailMeterNo()
{
    QString failMeterNo;
    for(int i=0;i<meterInfoList.size();i++)
    {
