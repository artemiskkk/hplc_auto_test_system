#include "Script_BroadcastTime_DelayRelated.h"

Script_BroadcastTime_DelayRelated::Script_BroadcastTime_DelayRelated(QObject *parent) : QObject(parent)
{
    ...
    p_QueryDelayRelated_03F9=make_shared<Afn03F9>();
...
Script_BroadcastTime_DelayRelated::~Script_BroadcastTime_DelayRelated()
{
...
void Script_BroadcastTime_DelayRelated::execute()
{
...
void Script_BroadcastTime_DelayRelated::stop()
{
...
void Script_BroadcastTime_DelayRelated::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_BroadcastTime_DelayRelated::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_BroadcastTime_DelayRelated::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_BroadcastTime_DelayRelated::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
    ...
    if(emScriptRunState==Wait_BuildNetFinish_Whole)
    {
        ...
        else
        {
            if(p_BuildNetwork_GW->emScriptRunState==BuildNetFinish&&p_BuildNetwork_GW->buildNetworkResultFlag==true)
            {
                tryTimes=0;
                meterInfoInit();
                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryDelayRelated_03F9);
                emScriptRunState=Wait_QueryDelayTime_Finish;
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--广播通信时长查询（03F9），等待--确认");
                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
...
void Script_BroadcastTime_DelayRelated::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_BroadcastTime_DelayRelated::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_QueryDelayTime_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();//接收到一条完整报文
                    shared_ptr<Afn03F9> p_QueryDelayRelated_03F9_Up=dynamic_pointer_cast<Afn03F9>(p_Frame3762Base);
                    delayTime=p_QueryDelayRelated_03F9_Up->data_unit_up_.broadcast_communication_time_;/////
                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_Broadcast_05F3);
                    emScriptRunState=Wait_00F1_For_05F3_Broadcast;
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--广播校时（05F3），等待--确认");
                    p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
            ...
            case Wait_00F1_For_05F3_Broadcast:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    p_timer->start(maxBroadcastWaitTime*1000);
...
void Script_BroadcastTime_DelayRelated::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_BroadcastTime_DelayRelated::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_BroadcastTime_DelayRelated::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    if(frame==p_Broadcast_05F3)
    {
        ...
        QByteArray setTime=QByteArray::fromHex(QDateTime::currentDateTime().addSecs(delayTime).toString("yyMMddhhmmss").toLatin1());
    ...
    else if(frame==p_QueryDelayRelated_03F9)
    {
        p_QueryDelayRelated_03F9->ctrl_field_.dir=kDirDown;
        p_QueryDelayRelated_03F9->ctrl_field_.prm=kActive;
        p_QueryDelayRelated_03F9->ctrl_field_.comn_type=kHplc;

        p_QueryDelayRelated_03F9->info_field_.info_field_down.msg_seq=char(msgSeq++);
        p_QueryDelayRelated_03F9->info_field_.info_field_down.comu_rate=0;
        p_QueryDelayRelated_03F9->info_field_.info_field_down.comu_module_ident=0;

        QByteArray msg645;
        shared_ptr<Rqst_BroadCastTiming_0x08> p_BroadCastTiming_0x08=make_shared<Rqst_BroadCastTiming_0x08>(addr,4);
        memset(p_BroadCastTiming_0x08->addr_,char(0x99),6);
        QByteArray setTime=QByteArray::fromHex(QDateTime::currentDateTime().toString("yyMMddhhmmss").toLatin1());
        memcpy(p_BroadCastTiming_0x08->dateTime,setTime,6);
        msg645.append(p_BroadCastTiming_0x08->EncodeFrame());

        p_QueryDelayRelated_03F9->data_unit_down_.delay_related_flag_=0x01;//精确度为s
        p_QueryDelayRelated_03F9->data_unit_down_.communtication_protocol_=DLT645_2007;
        p_QueryDelayRelated_03F9->data_unit_down_.frame_content_=msg645;
        p_QueryDelayRelated_03F9->data_unit_down_.frame_length_=uchar(msg645.size());

        sendMsgOct=p_QueryDelayRelated_03F9->EncodeFrame();
        sendMsgLog=QString("》》广播通信时长查询03F9：%1\n").arg(QString(sendMsgOct.toHex().toUpper()));
    }
...
void Script_BroadcastTime_DelayRelated::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_BroadcastTime_DelayRelated::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_BroadcastTime_DelayRelated::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_Finish_Broadcast:
...
void Script_BroadcastTime_DelayRelated::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    index=0;
    p_maxAllowTimer->stop();
    ...
    if(calSuccessRate()>=netSucRateThresld)
    {
        emScriptRunState=ScriptSuccess;
        resultFlag=true;
        p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("广播校时成功率：%1%，延时相关广播！").arg(calSuccessRate()*100));
    }
    else
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, getFailMeterNo());
...
void Script_BroadcastTime_DelayRelated::meterInfoInit()
{
    meterInfoList.clear();
...
bool Script_BroadcastTime_DelayRelated::isMeterExist(Address address)
{
...
int Script_BroadcastTime_DelayRelated::getMeterInfo(Address address)
{
    for(int i=0;i<meterInfoList.size();i++)
    {
        if(address==meterInfoList.at(i).meterNo)
...
double Script_BroadcastTime_DelayRelated::calSuccessRate()
{
...
QString Script_BroadcastTime_DelayRelated::getFailMeterNo()
{
    QString failMeterNo;
    for(int i=0;i<meterInfoList.size();i++)
    {
