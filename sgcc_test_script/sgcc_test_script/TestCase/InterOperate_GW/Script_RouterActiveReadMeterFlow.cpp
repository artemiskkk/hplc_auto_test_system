#include "Script_RouterActiveReadMeterFlow.h"

Script_RouterActiveReadMeterFlow::Script_RouterActiveReadMeterFlow(QObject *parent) : InteroperateBase_GW(parent)
{

}
Script_RouterActiveReadMeterFlow::~Script_RouterActiveReadMeterFlow()
{
    stop();
}
void Script_RouterActiveReadMeterFlow::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_InteroperateInit_GW->setHost(host);
}
void Script_RouterActiveReadMeterFlow::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
bool Script_RouterActiveReadMeterFlow::config(const QMap<QString,QString> *paraDic)
{
    bool result = true;
    if(paraDic!=nullptr)
    {
...
void Script_RouterActiveReadMeterFlow::execute()
{
    p_AbstractScriptHost->updateProgress(ProcessState_Start, "路由模块主动抄表流程: 开始测试!");
    emScriptRunState=ScriptInit;
    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "重启 开始...");
    emScriptRunState=Wait_RestartRouter_Finish;
    QString msg=QString("68 0F 00 43 00 00 28 32 00 5D 12 01 00 0D 16").replace(" ","");
...
void Script_RouterActiveReadMeterFlow::slotStartFollowTest()
{

}
void Script_RouterActiveReadMeterFlow::stop()
{
    p_timer->stop();
...
void Script_RouterActiveReadMeterFlow::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_RouterActiveReadMeterFlow::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
...
void Script_RouterActiveReadMeterFlow::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        switch(emScriptRunState)
        {
            case Wait_RestartRouter_Finish:
            {
            ...
            case Wait_ReadMeter_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x14&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    QByteArray msg=QByteArray::fromHex(protocol==0x02?QString("68 2F 00 03 04 01 28 32 00 05 88 88 88 99 99 04 04 33 00 00 00 00 14 01 00 02 01 10 68 04 33 00 00 00 00 68 11 04 33 33 34 33 E9 16 00 7C 16").replace(" ","").toLatin1()
                                                                    :QString("68 39 00 03 04 01 28 32 00 05 88 88 88 99 99 04 04 33 00 00 00 00 14 01 00 02 01 1A 68 18 00 43 05 04 33 00 00 00 00 10 64 D1 05 02 00 01 00 10 02 00 00 91 04 16 A7 00 16").replace(" ","").toLatin1());
                ...
                else if(p_Frame3762Base->afn_ == 0x06&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    sendSrcMsg(dvcType,dvcId,QByteArray::fromHex(QString("68 0F 00 43 00 00 28 32 00 5E 12 02 00 0F 16 ").replace(" ","").toLatin1()));
                    msgSeq=0x5E;
                    p_timer->start(CCO_CMD_TIMEOUT_TIME*1000);
            ...
            case Wait_PauseRouter_Finish:
            {
                ...
                {
                    p_timer->stop();
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("发送“暂停”(AFN=12H-F2)暂停抄表 结束"));
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("路由模块主动抄表流程:流程测试成功"));
                    p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("路由模块主动抄表流程:流程测试成功"));
                }
                else
                {
                    QByteArray tmpSendMsg=prcsOther3762Msg(uchar(p_Frame3762Base->afn_),p_Frame3762Base->dt1_,p_Frame3762Base->dt2_,uchar(p_Frame3762Base->info_field_.info_field_up.msg_seq));
...
void Script_RouterActiveReadMeterFlow::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_RouterActiveReadMeterFlow::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    QString stringDvcType;
    if(dvcType == SingleSTA)
    {
...
void Script_RouterActiveReadMeterFlow::sendMsg(DvcType , int , int , shared_ptr<void> )
{

}

void Script_RouterActiveReadMeterFlow::timer_timeout()
{
    switch(emScriptRunState)
    {
        default:
...
void Script_RouterActiveReadMeterFlow::maxAllowTimer_timeout()
{
    p_maxAllowTimer->stop();
    p_AbstractScriptHost->updateProgress(ProcessState_Error, QString("timeout p_maxAllowTimer!!!"));
}

void Script_RouterActiveReadMeterFlow::delayTimer_timeout()
{
