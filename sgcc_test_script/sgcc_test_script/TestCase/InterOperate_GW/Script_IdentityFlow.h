#ifndef SCRIPT_IDENTITYFLOW_H
#define SCRIPT_IDENTITYFLOW_H

#include <QObject>
#include "../InteroperateBase_GW.h"
enum Script_IdentityFlow_RunState
{
    ScriptInit,
    Wait_InteroperateInit_Finish,
    Wait_CommandProcess_Finish,
    ScriptFinish
};
class Script_IdentityFlow : public InteroperateBase_GW, public AbstractScript,public DynamicCreate<Script_IdentityFlow>
{
    Q_OBJECT
public:
    explicit Script_IdentityFlow(QObject *parent = nullptr);
    ~Script_IdentityFlow();
private:
    Script_IdentityFlow_RunState emScriptRunState=ScriptInit;
private:
