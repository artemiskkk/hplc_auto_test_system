#include "Script_ConcentratorActiveReadMeterFlow.h"

Script_ConcentratorActiveReadMeterFlow::Script_ConcentratorActiveReadMeterFlow(QObject *parent) : InteroperateBase_GW(parent)
{

}

Script_ConcentratorActiveReadMeterFlow::~Script_ConcentratorActiveReadMeterFlow()
{
    stop();
}
void Script_ConcentratorActiveReadMeterFlow::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_InteroperateInit_GW->setHost(host);
}
void Script_ConcentratorActiveReadMeterFlow::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
bool Script_ConcentratorActiveReadMeterFlow::config(const QMap<QString,QString> *paraDic)
{
    bool result = true;
    if(paraDic!=nullptr)
    {
...
void Script_ConcentratorActiveReadMeterFlow::execute()
{
    p_AbstractScriptHost->updateProgress(ProcessState_Start, "集中器主动抄表: 开始测试!");
    ...
    QString msg=QString("68 0F 00 43 00 00 28 32 00 59 12 02 00 0A 16").replace(" ","");
...
void Script_ConcentratorActiveReadMeterFlow::slotStartFollowTest()
{

}
void Script_ConcentratorActiveReadMeterFlow::stop()
{
    p_timer->stop();
...
void Script_ConcentratorActiveReadMeterFlow::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ConcentratorActiveReadMeterFlow::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
...
void Script_ConcentratorActiveReadMeterFlow::processMsgFromCCO(DvcType dvcType, int dvcId)
{
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_RecoverRouter_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp&&uchar(p_Frame3762Base->info_field_.info_field_up.msg_seq)==msgSeq)
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("集中器主动抄表:流程测试成功"));
                }
                else
                {
                    QByteArray tmpSendMsg=prcsOther3762Msg(uchar(p_Frame3762Base->afn_),p_Frame3762Base->dt1_,p_Frame3762Base->dt2_,uchar(p_Frame3762Base->info_field_.info_field_up.msg_seq));
...
void Script_ConcentratorActiveReadMeterFlow::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_ConcentratorActiveReadMeterFlow::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    QString stringDvcType;
    if(dvcType == SingleSTA)
    {
...
void Script_ConcentratorActiveReadMeterFlow::sendMsg(DvcType , int , int , shared_ptr<void> )
{

}

void Script_ConcentratorActiveReadMeterFlow::timer_timeout()
{
    switch(emScriptRunState)
    {
        default:
...
void Script_ConcentratorActiveReadMeterFlow::maxAllowTimer_timeout()
{
    p_maxAllowTimer->stop();
    p_AbstractScriptHost->updateProgress(ProcessState_Error, QString("timeout p_maxAllowTimer!!!"));
}

void Script_ConcentratorActiveReadMeterFlow::delayTimer_timeout()
{
