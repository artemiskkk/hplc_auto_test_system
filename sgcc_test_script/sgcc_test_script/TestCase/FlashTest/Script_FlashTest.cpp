#include "Script_FlashTest.h"

Script_FlashTest::Script_FlashTest(QObject *parent) : QObject(parent)
{
    ...
    memset(ccoAddr,char(0xFF),6);
}
Script_FlashTest::~Script_FlashTest()
{
...
void Script_FlashTest::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_FlashTest::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_FlashTest::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_FlashTest::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_FlashTest::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_FlashTest::processMsgFromCCO(DvcType dvcType, int dvcId)
{
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_QueryInitFreq_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==char(0x80)&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                 ...
                 //   runFreq=uchar(p_QueryFreq_03F16_Up->carrier_frequence_range_);
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("初始运行频段为%1").arg(uchar(p_QueryFreq_03F16_Up->carrier_frequence_range_)));
            ...
            case Wait_EraseFlash_Finish:
            {
                ...
                else if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==0x08&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing,  QString("查询主节点地址为全F，符合要求"));
                ...
                else if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==char(0x80)&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();
                    shared_ptr<Afn03F16> p_QueryFreq_03F16_Up=dynamic_pointer_cast<Afn03F16>(p_Frame3762Base);
                    if(p_QueryFreq_03F16_Up->carrier_frequence_range_==0x02)
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("频段恢复默认2，符合要求"));
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed,QString("频段为%1，未恢复默认2，不符合要求").arg(uchar(p_QueryFreq_03F16_Up->carrier_frequence_range_)));
                    }
                }
...
void Script_FlashTest::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_FlashTest::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_FlashTest::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_FlashTest::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_FlashTest::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_FlashTest::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_EraseFlash_Finish:
...
void Script_FlashTest::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    index=0;
...
bool Script_FlashTest::meterIsExist(Address meterAddr)
{
