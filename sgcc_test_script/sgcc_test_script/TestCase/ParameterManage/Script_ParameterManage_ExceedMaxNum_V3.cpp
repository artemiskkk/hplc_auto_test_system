#include "Script_ParameterManage_ExceedMaxNum_V3.h"

Script_ParameterManage_ExceedMaxNum_V3::Script_ParameterManage_ExceedMaxNum_V3(QObject *parent) : QObject(parent)
{
...
Script_ParameterManage_ExceedMaxNum_V3::~Script_ParameterManage_ExceedMaxNum_V3()
{
...
void Script_ParameterManage_ExceedMaxNum_V3::execute()
{
...
void Script_ParameterManage_ExceedMaxNum_V3::stop()
{
...
void Script_ParameterManage_ExceedMaxNum_V3::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ParameterManage_ExceedMaxNum_V3::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ParameterManage_ExceedMaxNum_V3::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ParameterManage_ExceedMaxNum_V3::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ParameterManage_ExceedMaxNum_V3::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ParameterManage_ExceedMaxNum_V3::processMsgFromCCO(DvcType dvcType, int dvcId)
{
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_ParaInit_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到确认，发送--查询从节点数量（10F1），等待--回复");
            ...
            case Wait_QueryNodeNum_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("参数初始化后从节点数量不为0，不符合要求：%1").arg(p_QueryNodeNum_10F1_Up->node_total_num_));
                    }
                }
                else
                {
            ...
            case Wait_AddSlaveNode_Finish:
            {
                ...
                else if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(p_Deny_Up->error_code_ == 0)
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到否认，错误状态字为0（通信超时）");
                    }
                    else
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "收到否认，错误状态字不为0："+QString::number(p_Deny_Up->error_code_));
                        return;
                    }
                    index+=num;
            ...
            case Wait_QueryNodeInfo_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(p_QueryNodeInfo_10F2_Up->node_info_data_unit_.node_total_num_ != maxNodeNum)
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("查询从节点信息10F2回复节点总数量%1不等于最大支持数%2").arg(p_QueryNodeInfo_10F2_Up->node_info_data_unit_.node_total_num_).arg(maxNodeNum));
                        return;
                    }
                    if(p_QueryNodeInfo_10F2_Up->node_info_data_unit_.this_node_num_ == 0)
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("查询从节点信息10F2回复本次应答数量为0"));
                        return;
                    }
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到10F2回复，发送--查询从节点信息（10F2），等待--回复");
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNodeInfo_10F2);
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    }
                    else
                    {
                        ...
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("查询全部从节点信息正确，符合要求"));
                            index=0;
...
void Script_ParameterManage_ExceedMaxNum_V3::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ParameterManage_ExceedMaxNum_V3::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ParameterManage_ExceedMaxNum_V3::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_ParameterManage_ExceedMaxNum_V3::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ParameterManage_ExceedMaxNum_V3::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_ParameterManage_ExceedMaxNum_V3::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
...
void Script_ParameterManage_ExceedMaxNum_V3::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    p_maxAllowTimer->stop();
}

void Script_ParameterManage_ExceedMaxNum_V3::generateParameterList()
{
    parameterList.clear();
...
void Script_ParameterManage_ExceedMaxNum_V3::generateExceedParameterList()
{
    exceedParameterList.clear();
    quint64 initNo=202100002048;
...
bool Script_ParameterManage_ExceedMaxNum_V3::isAllNodeInfoRight()
{
    ...
    for(auto item : nodeInfoList_10F2)
    {
        ...
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("10F2回复从节点信息有重复：%1").arg(QString(QByteArray(item.node_address_.addr,6).toHex())));
            return false;
        }
    }
    ...
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("10F2回复从节点信息汇总数量%1不等于最大支持节点数%2").arg(nodeInfoList_10F2.size()).arg(maxNodeNum));
        return false;
    }
    for(int i=0;i<nodeInfoList_10F2.size();i++)
    {
        for(int j=0;j<parameterList.size();j++)
        {
            ...
            else
            {
                ...
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("10F2回复的表号%1在添加的档案中找不到").arg(QString(QByteArray(nodeInfoList_10F2.at(i).node_address_.addr,6).toHex())));
