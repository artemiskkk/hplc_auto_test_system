#include "Script_ParameterManage_RepeatParameter.h"

Script_ParameterManage_RepeatParameter::Script_ParameterManage_RepeatParameter(QObject *parent) : QObject(parent)
{
...
Script_ParameterManage_RepeatParameter::~Script_ParameterManage_RepeatParameter()
{
...
void Script_ParameterManage_RepeatParameter::execute()
{
...
void Script_ParameterManage_RepeatParameter::stop()
{
...
void Script_ParameterManage_RepeatParameter::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ParameterManage_RepeatParameter::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ParameterManage_RepeatParameter::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ParameterManage_RepeatParameter::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ParameterManage_RepeatParameter::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ParameterManage_RepeatParameter::processMsgFromCCO(DvcType dvcType, int dvcId)
{
            ...
            case Wait_AddSlaveNode_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    else
                    {
                        ...
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--添加重复载波从节点（11F1），等待--确认");
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    }
                }
                else
...
void Script_ParameterManage_RepeatParameter::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ParameterManage_RepeatParameter::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ParameterManage_RepeatParameter::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_ParameterManage_RepeatParameter::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ParameterManage_RepeatParameter::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_ParameterManage_RepeatParameter::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
...
void Script_ParameterManage_RepeatParameter::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    p_maxAllowTimer->stop();
}

void Script_ParameterManage_RepeatParameter::generateParameterList()
{
    parameterList.clear();
...
void Script_ParameterManage_RepeatParameter::generateRepeatParameterList()
{
...
bool Script_ParameterManage_RepeatParameter::isAllNodeInfoRight()
{
