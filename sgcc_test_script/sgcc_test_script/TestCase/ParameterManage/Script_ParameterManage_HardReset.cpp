#include "Script_ParameterManage_HardReset.h"

Script_ParameterManage_HardReset::Script_ParameterManage_HardReset(QObject *parent) : QObject(parent)
{
...
Script_ParameterManage_HardReset::~Script_ParameterManage_HardReset()
{
...
void Script_ParameterManage_HardReset::execute()
{
...
void Script_ParameterManage_HardReset::stop()
{
...
void Script_ParameterManage_HardReset::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ParameterManage_HardReset::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ParameterManage_HardReset::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ParameterManage_HardReset::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ParameterManage_HardReset::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ParameterManage_HardReset::processMsgFromCCO(DvcType dvcType, int dvcId)
{
                ...
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(p_QueryNodeNum_10F1_Up->node_total_num_==0)
                    {
                        ...
                        else
                        {
                            ...
                            resultFlag=true;
                            p_AbstractScriptHost->updateProgress(ProcessState_Success, "档案管理-无档案硬件初始化,测试完成！");
                        }
                    }
                    else
                    {
...
void Script_ParameterManage_HardReset::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ParameterManage_HardReset::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ParameterManage_HardReset::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_ParameterManage_HardReset::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ParameterManage_HardReset::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_ParameterManage_HardReset::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
...
void Script_ParameterManage_HardReset::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    p_maxAllowTimer->stop();
}

void Script_ParameterManage_HardReset::generateParameterList()
{
    parameterList.clear();
...
bool Script_ParameterManage_HardReset::isAllNodeInfoRight()
{
