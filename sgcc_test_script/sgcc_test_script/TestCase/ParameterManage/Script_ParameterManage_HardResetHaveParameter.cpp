#include "Script_ParameterManage_HardResetHaveParameter.h"

Script_ParameterManage_HardResetHaveParameter::Script_ParameterManage_HardResetHaveParameter(QObject *parent) : QObject(parent)
{
...
Script_ParameterManage_HardResetHaveParameter::~Script_ParameterManage_HardResetHaveParameter()
{
...
void Script_ParameterManage_HardResetHaveParameter::execute()
{
...
void Script_ParameterManage_HardResetHaveParameter::stop()
{
...
void Script_ParameterManage_HardResetHaveParameter::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ParameterManage_HardResetHaveParameter::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ParameterManage_HardResetHaveParameter::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ParameterManage_HardResetHaveParameter::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ParameterManage_HardResetHaveParameter::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ParameterManage_HardResetHaveParameter::processMsgFromCCO(DvcType dvcType, int dvcId)
{
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_QueryNodeNum_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    else
                    {
                        ...
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("档案管理-无档案硬件初始化，测试失败！"));
                    }
                }
                else
                {
            ...
            case Wait_HardReset_Finish:
            {
                ...
                else if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    index=1;
                    nodeInfoList_10F2.clear();
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到03F10，发送--查询从节点信息（10F2），等待--回复");
                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNodeInfo_10F2);
                    emScriptRunState=Wait_QueryNodeInfo_Finish;
                    p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                }
                ...
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    else
                    {
                        ...
                        if(isAllNodeInfoRight()==true)
                        {
                            ...
                            index=0;
                            if(++hardResetIndex<=maxHardResetTimes)
                            {
                            ...
                            else
                            {
                                ...
                                resultFlag=true;
                                p_AbstractScriptHost->updateProgress(ProcessState_Success, "档案管理-有档案硬件初始化,测试完成！");
                            }
                        }
                        else
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("查询从节点信息，不符合要求！"));//可以考虑添加展示
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("档案管理-有档案硬件初始化，测试失败！"));
                        }
                    }
                }
                else
...
void Script_ParameterManage_HardResetHaveParameter::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ParameterManage_HardResetHaveParameter::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ParameterManage_HardResetHaveParameter::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_ParameterManage_HardResetHaveParameter::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ParameterManage_HardResetHaveParameter::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
    ...
    case Wait_HardReset_Finish:
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait 03F10 After HardwareInit timeout!!!");
        break;
    }
    default:
    {
...
void Script_ParameterManage_HardResetHaveParameter::maxAllowTimer_timeoutProc()
{
...
void Script_ParameterManage_HardResetHaveParameter::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    p_maxAllowTimer->stop();
}

void Script_ParameterManage_HardResetHaveParameter::generateParameterList()
{
    parameterList.clear();
...
bool Script_ParameterManage_HardResetHaveParameter::isAllNodeInfoRight()
{
