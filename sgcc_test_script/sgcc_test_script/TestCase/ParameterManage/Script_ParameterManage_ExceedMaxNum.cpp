#include "Script_ParameterManage_ExceedMaxNum.h"

Script_ParameterManage_ExceedMaxNum::Script_ParameterManage_ExceedMaxNum(QObject *parent) : QObject(parent)
{
...
Script_ParameterManage_ExceedMaxNum::~Script_ParameterManage_ExceedMaxNum()
{
...
void Script_ParameterManage_ExceedMaxNum::execute()
{
...
void Script_ParameterManage_ExceedMaxNum::stop()
{
...
void Script_ParameterManage_ExceedMaxNum::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ParameterManage_ExceedMaxNum::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ParameterManage_ExceedMaxNum::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ParameterManage_ExceedMaxNum::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ParameterManage_ExceedMaxNum::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ParameterManage_ExceedMaxNum::processMsgFromCCO(DvcType dvcType, int dvcId)
{
            ...
            case Wait_QueryNodeNum_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    else
                    {
                        ...
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("Wait_QueryNodeNum_Finish 参数初始化后查询从节点数量回不为0，测试失败【GW-CCO-F016-0008-V01】"));
                    }
                }
                else
                {
            ...
            case Wait_AddSlaveNode_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    else
                    {
                        tryTimes = 0;
                        index=0;
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,1,p_AddSlaveNode_11F1);
                        emScriptRunState=Wait_AddSlaveNode_Finish;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--添加载波从节点（11F1），等待--确认");
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    }
...
//                            resultFlag=true;
//                            p_AbstractScriptHost->updateProgress(ProcessState_Success, "档案管理-超过最大数量,测试完成！");
                        }
                        else
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("查询从节点信息，不符合要求！"));
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString(" Wait_QueryNodeInfo_Finish 查询从节点信息与添加档案不一致 【GW-CCO-F016-0008-V01】"));
                        }
                    }
                }
                else
            ...
            case Wait_ParaInit_Finish_1:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    tryTimes = 0;
                    sendMsg(CCO_GW, p_CtrInfoList->at(0)->ctrlID, INSIGNIFICANCE, p_QueryNodeInfo_10F2);
                    emScriptRunState=Wait_QueryNodeInfo_Finish_1;
            ...
            case Wait_QueryNodeInfo_Finish_1:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    shared_ptr<Afn10F2> p_QueryNodeInfo_10F2_Up = dynamic_pointer_cast<Afn10F2>(p_Frame3762Base);
                    if(p_QueryNodeInfo_10F2_Up->node_info_data_unit_.node_total_num_ == 0)
                    {
                        emScriptRunState=ScriptSuccess;
                        resultFlag=true;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "档案管理-超过最大数量, 测试完成！");
                        p_AbstractScriptHost->updateProgress(ProcessState_Success, "脚本执行成功【GW-CCO-F016-0008-V01】");
                    }
                    else
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString(" Wait_QueryNodeInfo_Finish_1 参数初始化后查询从节点信息档案数量不为0 【GW-CCO-F016-0008-V01】"));
                    }
                }
                else
                {
...
void Script_ParameterManage_ExceedMaxNum::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ParameterManage_ExceedMaxNum::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ParameterManage_ExceedMaxNum::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    if(frame==p_AddSlaveNode_11F1)
    {
        ...
        {
            if(index+num<exceedParameterList.size())
            {
                for(int i=0;i<num;i++)
                {
                    NodeParameter nodePara;
                    memcpy(nodePara.node_address_.addr,exceedParameterList.at(index+i).node_address_.addr,6);
                    nodePara.protocol_type_=exceedParameterList.at(index+i).protocol_type_;
                    p_AddSlaveNode_11F1->node_parameter_list_.append(nodePara);
                }
            }
            else
            {
                for(int i=0;i<exceedParameterList.size()-index;i++)
                {
                    NodeParameter nodePara;
                    memcpy(nodePara.node_address_.addr,exceedParameterList.at(index+i).node_address_.addr,6);
                    nodePara.protocol_type_=exceedParameterList.at(index+i).protocol_type_;
...
void Script_ParameterManage_ExceedMaxNum::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ParameterManage_ExceedMaxNum::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_BuildNetFinish_Whole timeout【GW-CCO-F016-0010-V01】");
            break;
        }
        // 新增 20240122  yjt
        case Wait_ParaInit_Finish:
        {
            if(++tryTimes>=3)
            {
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_ParaInit_Finish timeout 【GW-CCO-F016-0010-V01】");
            }
            else
            {
                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_ParaInit_01F2);
        ...
        case Wait_AddSlaveNode_Finish:
        {
            if(++tryTimes>=3)
            {
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_AddSlaveNode_Finish timeout 【GW-CCO-F016-0010-V01】");
            }
            else
            {
                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_AddSlaveNode_11F1);
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "再次发送--添加载波从节点（11F1），等待--确认");
                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
            }
            break;
        }
        case Wait_QueryNodeInfo_Finish:
        {
            ...
            {
                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNodeInfo_10F2);
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "再次发送--查询从节点信息（10F2），等待--回复");
                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
            }
            break;
        }
        case Wait_ParaInit_Finish_1:
        {
            if(++tryTimes>=3)
            {
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_ParaInit_Finish_1 timeout 【GW-CCO-F016-0010-V01】");
            }
            else
            {
                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_ParaInit_01F2);
        ...
        case Wait_QueryNodeInfo_Finish_1:
        {
            if(++tryTimes>=3)
            {
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "Wait_QueryNodeInfo_Finish_1 timeout 【GW-CCO-F016-0010-V01】");
            }
            else
            {
                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNodeInfo_10F2);
...
void Script_ParameterManage_ExceedMaxNum::maxAllowTimer_timeoutProc()
{
...
void Script_ParameterManage_ExceedMaxNum::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    p_maxAllowTimer->stop();
}

void Script_ParameterManage_ExceedMaxNum::generateParameterList()
{
    parameterList.clear();
    ...
    if(parameterList.size()<maxNodeNum)
    {
        ...
        {
            NodeParameter nodePara;
            memcpy(nodePara.node_address_.addr,QByteArray::fromHex(QString::number(++initNo).toLatin1()),6);
            if(i%2==0)
            {
...
void Script_ParameterManage_ExceedMaxNum::generateExceedParameterList()
{
    exceedParameterList.clear();
    quint64 initNo=202100001024;
    for(int i=0;i<exceedNodeNum;i++)
    {
        NodeParameter nodePara;
        QString meter_virtual = QString::number(++initNo);
        // 地址中增加16进制的 a-f
        meter_virtual.replace(0, 1, QString::number(i%16, 16));
        meter_virtual.replace(5, 1, QString::number((i+1)%16, 16));
        meter_virtual.replace(11, 1, QString::number((i+2)%16, 16));

        memcpy(nodePara.node_address_.addr, QByteArray::fromHex(meter_virtual.toLatin1()), 6);
        if(i%2==0)
        {
            nodePara.protocol_type_=DLT645_2007;
        }
        ...
        exceedParameterList.append(nodePara);
    }
}

bool Script_ParameterManage_ExceedMaxNum::isAllNodeInfoRight()
{
