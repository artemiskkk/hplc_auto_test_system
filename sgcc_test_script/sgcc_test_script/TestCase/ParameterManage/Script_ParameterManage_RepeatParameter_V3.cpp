#include "Script_ParameterManage_RepeatParameter_V3.h"

Script_ParameterManage_RepeatParameter_V3::Script_ParameterManage_RepeatParameter_V3(QObject *parent) : QObject(parent)
{
...
Script_ParameterManage_RepeatParameter_V3::~Script_ParameterManage_RepeatParameter_V3()
{
...
void Script_ParameterManage_RepeatParameter_V3::execute()
{
...
void Script_ParameterManage_RepeatParameter_V3::stop()
{
...
void Script_ParameterManage_RepeatParameter_V3::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ParameterManage_RepeatParameter_V3::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ParameterManage_RepeatParameter_V3::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ParameterManage_RepeatParameter_V3::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ParameterManage_RepeatParameter_V3::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ParameterManage_RepeatParameter_V3::processMsgFromCCO(DvcType dvcType, int dvcId)
{
...
//                            resultFlag=true;
//                            p_AbstractScriptHost->updateProgress(ProcessState_Success, "档案管理-无档案参数初始化,测试完成！");
//                        }
                    }
                    else
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("查询从节点数量，不符合要求！"));//可以考虑添加展示
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("档案管理-重复档案，测试失败！"));
                    }
                }
                else
                {
            ...
            case Wait_AddSlaveNode_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(index<maxNodeNum)
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到确认，继续添加从节点");
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_AddSlaveNode_11F1);
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    }
                    else
                    {
                        index=0;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到确认，从节点已添加完，开始添加重复从节点");
                        emScriptRunState=Wait_AddSlaveNode_Repeat_Finish;
            ...
            case Wait_AddSlaveNode_Repeat_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, "添加重复从节点收到确认，不符合要求");
                }
                else if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();
                    ...
                    {
                        index+=num;
                        if(index<repeatNodeNum)
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到否认，错误状态字为6(表号重复)，符合要求，继续添加重复从节点");
                            sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,1,p_AddSlaveNode_11F1);
                            p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                        }
                        else
                        {
                            ...
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到否认，重复从节点添加完毕，开始查询从节点信息（10F2），等待--回复");
            ...
            case Wait_QueryNodeInfo_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--继续查询从节点信息（10F2），等待--回复");
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNodeInfo_10F2);
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    }
                    else
                    {
                        ...
                        if(isAllNodeInfoRight()==true)
                        {
                            ...
                            emScriptRunState=ScriptSuccess;
                            resultFlag=true;
                            p_AbstractScriptHost->updateProgress(ProcessState_Success, "档案管理-重复档案,测试完成！");
                        }
                        else
                        {
...
void Script_ParameterManage_RepeatParameter_V3::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ParameterManage_RepeatParameter_V3::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ParameterManage_RepeatParameter_V3::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    if(frame==p_AddSlaveNode_11F1)
    {
        ...
        {
            if(index+num<repeatParameterList.size())
            {
                for(int i=0;i<num;i++)
                {
                    NodeParameter nodePara;
                    memcpy(nodePara.node_address_.addr,repeatParameterList.at(index+i).node_address_.addr,6);
                    nodePara.protocol_type_=repeatParameterList.at(index+i).protocol_type_;
                    p_AddSlaveNode_11F1->node_parameter_list_.append(nodePara);
                }
            }
            else
            {
                for(int i=0;i<repeatParameterList.size()-index;i++)
                {
                    NodeParameter nodePara;
                    memcpy(nodePara.node_address_.addr,repeatParameterList.at(index+i).node_address_.addr,6);
                    nodePara.protocol_type_=repeatParameterList.at(index+i).protocol_type_;
...
void Script_ParameterManage_RepeatParameter_V3::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ParameterManage_RepeatParameter_V3::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_ParameterManage_RepeatParameter_V3::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
...
void Script_ParameterManage_RepeatParameter_V3::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    p_maxAllowTimer->stop();
}

void Script_ParameterManage_RepeatParameter_V3::generateParameterList()
{
    parameterList.clear();
...
void Script_ParameterManage_RepeatParameter_V3::generateRepeatParameterList()
{
    repeatParameterList.clear();
    quint64 initNo=202100000000;
    for(int i=0;i<repeatNodeNum;i++)
    {
        NodeParameter nodePara;
        memcpy(nodePara.node_address_.addr,QByteArray::fromHex(QString::number(++initNo).toLatin1()),6);
        if(i%2==0)
        ...
        repeatParameterList.append(nodePara);
    }
}

bool Script_ParameterManage_RepeatParameter_V3::isAllNodeInfoRight()
{
