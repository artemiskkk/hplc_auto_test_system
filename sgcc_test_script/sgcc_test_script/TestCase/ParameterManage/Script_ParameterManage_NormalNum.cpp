#include "Script_ParameterManage_NormalNum.h"

Script_ParameterManage_NormalNum::Script_ParameterManage_NormalNum(QObject *parent) : QObject(parent)
{
...
Script_ParameterManage_NormalNum::~Script_ParameterManage_NormalNum()
{
...
void Script_ParameterManage_NormalNum::execute()
{
    ...
    else
    {
        ...
        emScriptRunState=Wait_ParaInit_First_Finish;
...
void Script_ParameterManage_NormalNum::stop()
{
...
void Script_ParameterManage_NormalNum::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ParameterManage_NormalNum::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ParameterManage_NormalNum::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ParameterManage_NormalNum::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ParameterManage_NormalNum::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ParameterManage_NormalNum::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_ParaInit_First_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    emScriptRunState=Wait_QueryNodeNum_First_Finish;
            ...
            case Wait_QueryNodeNum_First_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(p_QueryNodeNum_10F1_Up->node_total_num_==0)
                    {
                        ...
                        emScriptRunState=Wait_AddSlaveNode_First_Finish;
                    ...
                    else
                    {
                        ...
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("档案管理-正常数量，测试失败！"));
                    }
                }
                else
                {
            ...
            case Wait_AddSlaveNode_First_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();
                    ...
                    else
                    {
                        ...
                        emScriptRunState=Wait_QueryNodeInfo_First_Finish;
            ...
            case Wait_QueryNodeInfo_First_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    else
                    {
                        ...
                        if(isAllNodeInfoRight()==true)
                        {
                            ...
                            emScriptRunState=Wait_DeleteSlaveNode_Finish;
            ...
            case Wait_DeleteSlaveNode_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    index+=num;
                    ...
                    else
                    {
                        index=0;
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNodeNum_10F1);
                        emScriptRunState=Wait_QueryNodeNum_Second_Finish;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--查询从节点数量（10F1），等待--回复");
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    }
                }
            ...
            case Wait_QueryNodeNum_Second_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();
                    ...
                    if(p_QueryNodeNum_10F1_Up->node_total_num_==0)
                    {
                        ...
                        index=0;
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_AddSlaveNode_11F1);
                        emScriptRunState=Wait_AddSlaveNode_Second_Finish;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--添加载波从节点（11F1），等待--确认");
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    }
                    else
            ...
            case Wait_AddSlaveNode_Second_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();
                    ...
                    else
                    {
                        ...
                        nodeInfoList_10F2.clear();
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNodeInfo_10F2);
                        emScriptRunState=Wait_QueryNodeInfo_Second_Finish;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--查询从节点信息（10F2），等待--回复");
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    }
                }
            ...
            case Wait_QueryNodeInfo_Second_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    shared_ptr<Afn10F2> p_QueryNodeInfo_10F2_Up=dynamic_pointer_cast<Afn10F2>(p_Frame3762Base);
                    ...
                    else
                    {
                        ...
                        if(isAllNodeInfoRight()==true)
                        {
                            ...
                            index=0;
                            sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_ParaInit_01F2);
                            emScriptRunState=Wait_ParaInit_Second_Finish;
            ...
            case Wait_ParaInit_Second_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();
                    ...
                    index=0;
                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNodeNum_10F1);
                    emScriptRunState=Wait_QueryNodeNum_Third_Finish;
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--查询从节点数量（10F1），等待--回复");
                    p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                }
                else
            ...
            case Wait_QueryNodeNum_Third_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();
                    ...
                    if(p_QueryNodeNum_10F1_Up->node_total_num_==0)
                    {
                        ...
                        resultFlag=true;
                        p_AbstractScriptHost->updateProgress(ProcessState_Success, "档案管理-正常数量,测试完成！");
                    }
                    else
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("查询从节点数量，不符合要求！"));//可以考虑添加展示
...
void Script_ParameterManage_NormalNum::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ParameterManage_NormalNum::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ParameterManage_NormalNum::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_ParameterManage_NormalNum::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ParameterManage_NormalNum::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_ParameterManage_NormalNum::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
...
void Script_ParameterManage_NormalNum::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    p_maxAllowTimer->stop();
}

void Script_ParameterManage_NormalNum::generateParameterList()
{
    parameterList.clear();
...
bool Script_ParameterManage_NormalNum::isAllNodeInfoRight()
{
