#include "Script_ParameterManage_ParameterInit.h"

Script_ParameterManage_ParameterInit::Script_ParameterManage_ParameterInit(QObject *parent) : QObject(parent)
{
...
Script_ParameterManage_ParameterInit::~Script_ParameterManage_ParameterInit()
{
...
void Script_ParameterManage_ParameterInit::execute()
{
...
void Script_ParameterManage_ParameterInit::stop()
{
...
void Script_ParameterManage_ParameterInit::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ParameterManage_ParameterInit::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ParameterManage_ParameterInit::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ParameterManage_ParameterInit::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ParameterManage_ParameterInit::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ParameterManage_ParameterInit::processMsgFromCCO(DvcType dvcType, int dvcId)
{
            ...
            case Wait_QueryNodeNum_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    else
                    {
                        ...
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("档案管理-无档案参数初始化，测试失败！"));
                    }
                }
                else
                {
...
void Script_ParameterManage_ParameterInit::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ParameterManage_ParameterInit::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ParameterManage_ParameterInit::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_ParameterManage_ParameterInit::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ParameterManage_ParameterInit::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_ParameterManage_ParameterInit::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
...
void Script_ParameterManage_ParameterInit::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    p_maxAllowTimer->stop();
}

void Script_ParameterManage_ParameterInit::generateParameterList()
{
    parameterList.clear();
...
bool Script_ParameterManage_ParameterInit::isAllNodeInfoRight()
{
