#include "Script_SoftwareRecord_STA_Hunan.h"

Script_SoftwareRecord_STA_Hunan::Script_SoftwareRecord_STA_Hunan(QObject *parent) : QObject(parent)
{
...
Script_SoftwareRecord_STA_Hunan::~Script_SoftwareRecord_STA_Hunan()
{
...
void Script_SoftwareRecord_STA_Hunan::execute()
{
...
void Script_SoftwareRecord_STA_Hunan::stop()
{
...
void Script_SoftwareRecord_STA_Hunan::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_SoftwareRecord_STA_Hunan::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_SoftwareRecord_STA_Hunan::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_SoftwareRecord_STA_Hunan::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_SoftwareRecord_STA_Hunan::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_SoftwareRecord_STA_Hunan::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    Q_UNUSED(dvcType)
...
void Script_SoftwareRecord_STA_Hunan::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_SoftwareRecord_STA_Hunan::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_SoftwareRecord_STA_Hunan::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    if(frame==p_Rqst_ReadData_0x11)
    {
        ...
        else if(emScriptRunState==Wait_QuerySoftWareCompare_645_Finish)
        {
...
void Script_SoftwareRecord_STA_Hunan::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_SoftwareRecord_STA_Hunan::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_SoftwareRecord_STA_Hunan::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
...
void Script_SoftwareRecord_STA_Hunan::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
}

void Script_SoftwareRecord_STA_Hunan::softwareInfoInit()
{
    //加载程序文件
