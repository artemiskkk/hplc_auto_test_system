#include "Script_TaskConfig_Normal_Hunan.h"

Script_TaskConfig_Normal_Hunan::Script_TaskConfig_Normal_Hunan(QObject *parent) : QObject(parent)
{
...
Script_TaskConfig_Normal_Hunan::~Script_TaskConfig_Normal_Hunan()
{
...
void Script_TaskConfig_Normal_Hunan::execute()
{
...
void Script_TaskConfig_Normal_Hunan::stop()
{
...
void Script_TaskConfig_Normal_Hunan::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_TaskConfig_Normal_Hunan::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_TaskConfig_Normal_Hunan::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_TaskConfig_Normal_Hunan::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_TaskConfig_Normal_Hunan::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_TaskConfig_Normal_Hunan::processMsgFromCCO(DvcType dvcType, int dvcId)
{
                ...
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x40&&p_Frame3762Base->dt2_==0x0C&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    else
                    {
                        if(failStateSta.isEmpty())
                        {
                            ...
                            p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("任务配置-正常，测试成功"));
                        }
                        else
                        {
                            failStateSta.prepend("配置状态失败STA如下：\n");
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing,failStateSta);
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("任务配置-正常，测试失败"));
                        }
                    }
                }
                else
...
void Script_TaskConfig_Normal_Hunan::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_TaskConfig_Normal_Hunan::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_TaskConfig_Normal_Hunan::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_TaskConfig_Normal_Hunan::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_TaskConfig_Normal_Hunan::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_TaskConfig_Normal_Hunan::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
...
void Script_TaskConfig_Normal_Hunan::delayTimer_timeoutProc()
{
    p_delayTimer->stop();    
...
QList<DataIdLen> Script_TaskConfig_Normal_Hunan::dataIdInit(QString dataId)
{
...
void Script_TaskConfig_Normal_Hunan::curveDataInit()
{
    //645数据项
    ...
    /////////////645
    //645 单
    curveData_645_05F103.set_flag_=setFlagEnable;
    ...
    /////////////OOP
    //OOP 单
    curveData_OOP_05F103.set_flag_=setFlagEnable;
