#include "Script_MinuteFreeze_NormalConfigRead_Hunan.h"

Script_MinuteFreeze_NormalConfigRead_Hunan::Script_MinuteFreeze_NormalConfigRead_Hunan(QObject *parent) : QObject(parent)
{
...
Script_MinuteFreeze_NormalConfigRead_Hunan::~Script_MinuteFreeze_NormalConfigRead_Hunan()
{
...
void Script_MinuteFreeze_NormalConfigRead_Hunan::execute()
{
...
void Script_MinuteFreeze_NormalConfigRead_Hunan::stop()
{
...
void Script_MinuteFreeze_NormalConfigRead_Hunan::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_MinuteFreeze_NormalConfigRead_Hunan::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_MinuteFreeze_NormalConfigRead_Hunan::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_MinuteFreeze_NormalConfigRead_Hunan::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_MinuteFreeze_NormalConfigRead_Hunan::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_MinuteFreeze_NormalConfigRead_Hunan::processMsgFromCCO(DvcType dvcType, int dvcId)
{
...
//                        tryTimes=0;
//                        index=1;
//                        moduleConfig10F103_Hunan.clear();
...
//                        p_timer->start(STA_CMD_TIMEOUT_TIME*1000);
    ...
    ////                    moduleConfig10F103_Hunan.size();
    //                    if(isSuccess(moduleConfig10F103_Hunan)==true)
    //                    {
    //                        emScriptRunState=Wait_Delay_3_Finish;
    ////                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryConfigStatus_STA_10f103);
    //                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "延时2min，确保每个节点开始冻结");
    ...
    //                    else
    //                    {
    //                        p_AbstractScriptHost->updateProgress(ProcessState_Failed,QString("节点配置失败,失败地址1%").arg(getFailMeterNo()));
    //                    }
                    }
                }
                else
                        ...
                        else if(p_ReadCurveData_STA_F1F100_Up->unit_up_.protocol_type_==OOP)
                        {
                            ...
                            while(msgBuf.size()>0)
                            {
                                ...
                                if(0!=memcmp(QByteArray::fromHex(MsgBase_OOP_ptr->address_field_.sa.address.toLatin1()),allMeterList.at(currentMeterIndex).meterNo.addr,6))
                                    continue;
                                if(MsgBase_OOP_ptr->service_type_!=GET_RESPONSE_SERVER && MsgBase_OOP_ptr->service_sub_type_!=uchar(GetResponseType::kGetResponseRecord))
                                    continue;
                                shared_ptr<GetResponseRecord> p_GetResponseRecord=dynamic_pointer_cast<GetResponseRecord>(MsgBase_OOP_ptr);
    //                            待修改？？？？？？？？？？？？？？？？？？？？？？？？？？？？？？
                                OAD oad;
                                oad.OI=0x5002;
                                oad.attribute.feature=0x00;
                                oad.attribute.seq=2;
                                oad.element_index=0x00;
                                if(p_GetResponseRecord->a_result_record_.oad==oad)
                                {
                                    for(int i=0;i<allMeterList.at(currentMeterIndex).dataUnitList.size();i++)
                                    {
                                        ...
                                        OAD csd_oad;
                                        csd_oad.OI=uchar(oi[1])+ushort(short(oi[0])<<8);
                                        csd_oad.attribute.feature=(allMeterList.at(currentMeterIndex).dataUnitList.at(i).dataIdLen.data_id_[2]>>4)&0x0f;
                                        csd_oad.attribute.seq=allMeterList.at(currentMeterIndex).dataUnitList.at(i).dataIdLen.data_id_[2]&0x0f;
                                        csd_oad.element_index=uchar(allMeterList.at(currentMeterIndex).dataUnitList.at(i).dataIdLen.data_id_[3]);

                                        for(int j=0;j<p_GetResponseRecord->a_result_record_.rcsd_ptr->list_csd_.size();j++)
                                        {
                                            //返回数据项与已知数据项一致
                                            shared_ptr<CsdOad> csdOad=dynamic_pointer_cast<CsdOad>(p_GetResponseRecord->a_result_record_.rcsd_ptr->list_csd_.at(j));
                                            if(csdOad->oad_==csd_oad)
                                            {
                                                //保存的对应的数据项
//                                                p_GetResponseRecord->a_result_record_.response_data_ptr
                                                break;
                                            }
                                            else
                                                continue;
                                    ...
                                    //保存数据项对应的数据
                                    shared_ptr<RecordResponseData> p_RecordResponseData = dynamic_pointer_cast<RecordResponseData>(p_GetResponseRecord->a_result_record_.response_data_ptr);
                                    for(int n=0;n<p_RecordResponseData->list_record_row_.size();n++)
                                    {
                                        p_RecordResponseData->list_record_row_.at(n).list_data;
                                    }
                                }
...
//                                    if(p_GetResponseNormal->a_result_normal_.oad==csd_oad)
//                                    {
...
//                                        {
//                                            std::shared_ptr<FollowReportNormal> value=dynamic_pointer_cast<FollowReportNormal>(p_GetResponseRecord);

//                                            std::shared_ptr<GetResultData> value_ptr_=dynamic_pointer_cast<GetResultData>(p_GetResponseRecord);

//                                            std::shared_ptr<DataBasic> value_ptr=dynamic_pointer_cast<DataBasic>(p_GetResponseRecord);
...
//                            startDateTime=QDateTime::currentDateTime();
                            ...
                            curTime_start_oop=QByteArray::fromHex(startDateTime.addSecs(pointNumIndex*60).toString("yyyyMMddhhmmss").toLatin1());
                            curTime_end_oop=QByteArray::fromHex(startDateTime.addSecs((pointNumIndex*60)+(realPeriod*60)).toString("yyyyMMddhhmmss").toLatin1());
...
////                         {
////                             display.append(QString("表号：%1 ").arg(QString(QByteArray(allMeterList.at(i).meterNo.addr,6).toHex())));
////                             for(int j=0;j<allMeterList.at(i).dataUnitList.size();j++)
////                             {
////                                 if(allMeterList.at(i).protocolType==DLT645_2007)
////                                     display.append(QString("数据项%0：%1 %2 ").arg(j+1).arg(QString(QByteArray(allMeterList.at(i).dataUnitList.at(j).dataIdLen.data_id_,4).toHex())).arg(QString(allMeterList.at(i).dataUnitList.at(j).roundList.at(0).replyData.toHex())));
////                                 else
////                                     display.append(QString("OAD%0：%1 %2 ").arg(j+1).arg(QString(QByteArray(allMeterList.at(i).dataUnitList.at(j).dataIdLen.data_id_,4).toHex())).arg(QString(allMeterList.at(i).dataUnitList.at(j).roundList.at(0).replyData.toHex())));
////                             }
////                             display.append("\n");
////                         }
////                         p_AbstractScriptHost->updateProgress(ProcessState_Processing, display);
                ...
                {
                    p_timer->stop();
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, "F1F100回复否认，测试失败！！！");
            ...
            case Wait_SetCurveConfig_STA_05F103_4_Finish:
            {
                ...
                {
                    p_timer->stop();
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到设置STA模块曲线配置（05F103_oop_00），确认回复");

                    emScriptRunState=Wait_Delay_5_Finish;
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "延时3min等待环境运行");
                    //记录当前时间
                    startDateTime=QDateTime::currentDateTime();
                ...
                if(currentMeterIndex!=-1)
                {
                    ...
                    if(p_ReadCurveData_STA_F1F100_Up->unit_up_.protocol_type_==DLT645_2007)
                    {
                        ...
                        while(msgBuf.size()>0)
                        {
                            ...
                            if(MsgBase_645_ptr->ctrlCode_==NORMAL_RESP)
                            {
                                p_AbstractScriptHost->updateProgress(ProcessState_Failed, "收到645从站回复报文,响应异常！！！");
                                /////////////
                                shared_ptr<RspsNormal_ReadData_0x91> p_ReadData_0x91=dynamic_pointer_cast<RspsNormal_ReadData_0x91>(MsgBase_645_ptr);
                                if(memcmp(p_ReadData_0x91->addr_,srcAddr.addr,6)!=0)
                                    continue;
                            ...
                            {
                                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到从站回复否认D1，正常应答。");
                                shared_ptr<RspsAbNormal_ReadData_0xD1> p_RspsAbNormal_0xD1=dynamic_pointer_cast<RspsAbNormal_ReadData_0xD1>(MsgBase_645_ptr);
                                if(memcmp(p_RspsAbNormal_0xD1->addr_,srcAddr.addr,6)!=0)
                                    continue;
                            }
...
//                        QByteArray msgBuf=p_ReadCurveData_STA_F1F100_Up->unit_up_.frame_content_;
//                        //判断返回数据类型
//                        if(p_ReadCurveData_STA_F1F100_Up->unit_up_.protocol_type_==OOP)
//                        {
//                            //解析oop报文，OOP回复数据域为空；
//                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到oop_F1F100抄读曲线数据，正常回复");

//                            if(index<allMeterList.size())
//                            {
//                                emScriptRunState=Wait_ReadCurveData_STA_F1F100_2_Finish;
//                                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_ReadCurveData_STA_F1F100);
//                                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--并发抄读STA曲线数据（F1F100），等待--回复");
//                                p_timer->start(STA_CMD_TIMEOUT_TIME*1000);
//                                index++;
//                            }
...
//                            }
//                        }
//                        else if(p_ReadCurveData_STA_F1F100_Up->unit_up_.protocol_type_==DLT645_2007)
//                        {
//                            //循环判断每条645报文
//                            while(msgBuf.size()>0)
//                            {
...
//                                if(MsgBase_645_ptr->ctrlCode_==NORMAL_RESP)
//                                {
//                                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("测试失败,645回复报文错误！！！！应回D1否认。"));
//                                }
//                                else if(MsgBase_645_ptr->ctrlCode_==AbNORMAL_RESP_ReadData)
//                                {
//                                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到645表回否认。");
//                                    shared_ptr<RspsAbNormal_ReadData_0xD1> p_RspsAbNormal_0xD1=dynamic_pointer_cast<RspsAbNormal_ReadData_0xD1>(MsgBase_645_ptr);
//                                    if(memcmp(p_RspsAbNormal_0xD1->addr_,srcAddr.addr,6)!=0)
//                                        continue;
//                                }
//                                else
//                                {
//                                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, "测试失败，接收数据异常！！！");
//                                }
//                            }
...
void Script_MinuteFreeze_NormalConfigRead_Hunan::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_MinuteFreeze_NormalConfigRead_Hunan::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_MinuteFreeze_NormalConfigRead_Hunan::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
//            p_GetRequestNormal_ReadData->get_record_.rcsd_ptr->GetCsdSize();
            ...
            oad->oad_.OI=0x2021;
            oad->oad_.attribute.feature=0;
            oad->oad_.attribute.seq=2;
            oad->oad_.element_index=0;
...
void Script_MinuteFreeze_NormalConfigRead_Hunan::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_MinuteFreeze_NormalConfigRead_Hunan::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_MinuteFreeze_NormalConfigRead_Hunan::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_00F1_For_05F102_Finish:
...
void Script_MinuteFreeze_NormalConfigRead_Hunan::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    switch(emScriptRunState)
    {
...
//            curTime=QByteArray::fromHex(QDateTime::currentDateTime().toString("mmhhddMMyy").toLatin1());
            curTime_start_oop=QByteArray::fromHex(QDateTime::currentDateTime().toString("yyyyMMddhhmmss").toLatin1());
            curTime_end_oop=QByteArray::fromHex(QDateTime::currentDateTime().addSecs(realPeriod*60).toString("yyyyMMddhhmmss").toLatin1());
...
//            p_delayTimer->start(2*60*1000);//记得更改回来
            break;
        }
        case Wait_Delay_4_Finish:
        {
...
void Script_MinuteFreeze_NormalConfigRead_Hunan::meterInfoListInit()
{
...
bool Script_MinuteFreeze_NormalConfigRead_Hunan::isSuccess(QList<ModuleConfig10F103_Hunan> moduleConfig10F103_Hunan)
{
...
QString Script_MinuteFreeze_NormalConfigRead_Hunan::getFailMeterNo()
{
    QString failMeterNo;
    for(int i=0;i<moduleConfig10F103_Hunan.size();i++)
    {
...
int Script_MinuteFreeze_NormalConfigRead_Hunan::getMeterNoIndex(Address address)
{
    for(int i=0;i<allMeterList.size();i++)
    {
...
bool Script_MinuteFreeze_NormalConfigRead_Hunan::isNotReplyEmpty(QByteArray data)
{
    int count=0;
...
void Script_MinuteFreeze_NormalConfigRead_Hunan::readInfoClear()
{
    for(int i=0;i<allMeterList.size();i++)
    {
...
QList<DataIdLen> Script_MinuteFreeze_NormalConfigRead_Hunan::dataIdInit(QString dataId)
{
