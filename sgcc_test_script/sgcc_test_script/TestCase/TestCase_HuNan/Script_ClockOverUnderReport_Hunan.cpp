#include "Script_ClockOverUnderReport_Hunan.h"

Script_ClockOverUnderReport_Hunan::Script_ClockOverUnderReport_Hunan(QObject *parent) : QObject(parent)
{
...
Script_ClockOverUnderReport_Hunan::~Script_ClockOverUnderReport_Hunan()
{
...
void Script_ClockOverUnderReport_Hunan::execute()
{
...
void Script_ClockOverUnderReport_Hunan::stop()
{
...
void Script_ClockOverUnderReport_Hunan::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ClockOverUnderReport_Hunan::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_ClockOverUnderReport_Hunan::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ClockOverUnderReport_Hunan::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ClockOverUnderReport_Hunan::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ClockOverUnderReport_Hunan::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        case Wait_00F1_For_05F3_01_Broadcast:
        {
            if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_timer->stop();//接收到一条完整报文
                ...
                emScriptRunState=Wait_06F5_2_Report;
                p_delayTimer->start(delayTime*1000);
//                p_maxAllowTimer->start(timerForReachThresld*1000);
            }
            else
        ...
        case Wait_00F1_For_05F3_00_Broadcast:
        {
            if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_timer->stop();//接收到一条完整报文
                ...
                emScriptRunState=Wait_06F5_3_Report;
                p_delayTimer->start(delayTime*1000);
//                p_maxAllowTimer->start(timerForReachThresld*1000);
            }
            else
...
void Script_ClockOverUnderReport_Hunan::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
        ...
        case Wait_06F5_Report:
        {
            ...
            {
//                p_timer->stop();
//                shared_ptr<dlt_645_Protocol::Rqst_BroadCastTiming_0x08> p_Rqst_BroadCastTiming_0x08 = std::dynamic_pointer_cast<dlt_645_Protocol::Rqst_BroadCastTiming_0x08>(MsgBase_645_ptr);
                timingMsgFlag=true;
            }
//            else
//            {
...
void Script_ClockOverUnderReport_Hunan::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ClockOverUnderReport_Hunan::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_Broadcast_05F3)
    {
        ...
        QByteArray setTime;
        if(emScriptRunState==Wait_00F1_For_05F3_Broadcast)
        {
            setTime=QByteArray::fromHex(QDateTime::currentDateTime().addSecs(checkTime).toString("yyMMddhhmmss").toLatin1());
        }
        else if(emScriptRunState==Wait_00F1_For_05F3_01_Broadcast)
        {
            setTime=QByteArray::fromHex(QDateTime::currentDateTime().addSecs(-checkTime).toString("yyMMddhhmmss").toLatin1());
        }
        else if(emScriptRunState==Wait_00F1_For_05F3_00_Broadcast)
        {
            setTime=QByteArray::fromHex(QDateTime::currentDateTime().toString("yyMMddhhmmss").toLatin1());
        }
...
void Script_ClockOverUnderReport_Hunan::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ClockOverUnderReport_Hunan::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_ClockOverUnderReport_Hunan::maxAllowTimer_timeoutProc()
{
...
void Script_ClockOverUnderReport_Hunan::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    index=0;
    p_AbstractScriptHost->updateProgress(ProcessState_Processing, getFailMeterNo());
    ...
    case Wait_06F5_Report:
    {
        ...
        {
            if(meterInfoList.at(0).offsetFlag==uchar(0x02)&&timingMsgFlag==true)
            {
                tryTimes=0;
                emScriptRunState=Wait_00F1_For_05F3_01_Broadcast;
                timingMsgFlag=false;
                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_Broadcast_05F3);
//                emScriptRunState=Wait_00F1_For_05F3_01_Broadcast;
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--广播校时（05F3），等待--确认");
            ...
            {
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("校时时间大于电表事间5min以上,失败。偏差方式为：%1，虚拟表没有收到转发的校时报文标志为：%2").arg(meterInfoList.at(0).offsetFlag).arg(timingMsgFlag));
            }
            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("湖南精确校时测试，电能表低于集中器时间成功"));
            meterInfoList.clear();//sgl
        }
        else
        {
    ...
    case Wait_06F5_2_Report:
    {
        if(meterInfoList.size()>0)
        {
            if(meterInfoList.at(0).offsetFlag==uchar(0x01)&&timingMsgFlag==true)
            {
                tryTimes=0;
                emScriptRunState=Wait_00F1_For_05F3_00_Broadcast;
                timingMsgFlag=false;
                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_Broadcast_05F3);
//                emScriptRunState=Wait_00F1_For_05F3_00_Broadcast;
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--广播校时（05F3），等待--确认");
            ...
            {
                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("校时时间小于电表事间5min以上,失败。偏差方式为：%1，虚拟表没有收到转发的校时报文标志为：%2").arg(meterInfoList.at(0).offsetFlag).arg(timingMsgFlag));
            }
            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("湖南精确校时测试，电能表低于集中器时间成功"));
            meterInfoList.clear();//sgl
        }
    ...
    case Wait_06F5_3_Report:
    {
        if(meterInfoList.size()==0&&timingMsgFlag==true)
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("湖南时钟超差上报测试成功"));
            emScriptRunState=ScriptSuccess;
            p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("湖南时钟超差上报测试成功"));
        }
        else
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("不应上报，但实际上报%1").arg(meterInfoList.size()));
        }
        break;
    }
    default:
...
void Script_ClockOverUnderReport_Hunan::meterInfoInit()
{
    meterInfoList.clear();
...
bool Script_ClockOverUnderReport_Hunan::isMeterExist(Address address)
{
...
int Script_ClockOverUnderReport_Hunan::getMeterInfo(Address address)
{
    for(int i=0;i<meterInfoList.size();i++)
    {
        if(address==meterInfoList.at(i).meterNo)
...
double Script_ClockOverUnderReport_Hunan::calSuccessRate()
{
...
QString Script_ClockOverUnderReport_Hunan::getFailMeterNo()
{
    QString failMeterNo;
    failMeterNo.clear();
    for(int i=0;i<meterInfoList.size();i++)
