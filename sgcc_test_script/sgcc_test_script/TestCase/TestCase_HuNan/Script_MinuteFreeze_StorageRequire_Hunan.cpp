#include "Script_MinuteFreeze_StorageRequire_Hunan.h"
Script_MinuteFreeze_StorageRequire_Hunan::Script_MinuteFreeze_StorageRequire_Hunan(QObject *parent) : QObject(parent)
{
...
Script_MinuteFreeze_StorageRequire_Hunan::~Script_MinuteFreeze_StorageRequire_Hunan()
{
...
void Script_MinuteFreeze_StorageRequire_Hunan::execute()
{
...
void Script_MinuteFreeze_StorageRequire_Hunan::stop()
{
...
void Script_MinuteFreeze_StorageRequire_Hunan::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_MinuteFreeze_StorageRequire_Hunan::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_MinuteFreeze_StorageRequire_Hunan::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_MinuteFreeze_StorageRequire_Hunan::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_MinuteFreeze_StorageRequire_Hunan::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_MinuteFreeze_StorageRequire_Hunan::processMsgFromCCO(DvcType dvcType, int dvcId)
{
...
void Script_MinuteFreeze_StorageRequire_Hunan::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_MinuteFreeze_StorageRequire_Hunan::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_MinuteFreeze_StorageRequire_Hunan::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_SetCurveConfig_STA_05F103)
    {
        ...
        if(emScriptRunState==Wait_SetCurveConfig_STA_05F103_Finish)//配置645表，单三相，冻结01或禁止冻结00
        {
...
void Script_MinuteFreeze_StorageRequire_Hunan::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_MinuteFreeze_StorageRequire_Hunan::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_MinuteFreeze_StorageRequire_Hunan::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_00F1_For_05F102_Finish:
...
void Script_MinuteFreeze_StorageRequire_Hunan::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    switch(emScriptRunState)
    {
...
//            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "延时15分钟时间到");
            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("延时%1分钟时间到").arg(delayTimer));
            index=0;
            parallelCount=0;
            while(parallelCount<maxParallelNum)
            {
...
void Script_MinuteFreeze_StorageRequire_Hunan::meterInfoListInit()
{
...
bool Script_MinuteFreeze_StorageRequire_Hunan::isSuccess(QList<ModuleConfig10F103_Hunan> moduleConfig10F103_Hunan)
{
...
QString Script_MinuteFreeze_StorageRequire_Hunan::getFailMeterNo()
{
    QString failMeterNo;
    for(int i=0;i<moduleConfig10F103_Hunan.size();i++)
    {
...
int Script_MinuteFreeze_StorageRequire_Hunan::getMeterNoIndex(Address address)
{
...
bool Script_MinuteFreeze_StorageRequire_Hunan::isNotReplyEmpty(QByteArray data)
{
    int count=0;
...
void Script_MinuteFreeze_StorageRequire_Hunan::readInfoClear()
{
    for(int i=0;i<allMeterList.size();i++)
    {
...
QList<DataIdLen> Script_MinuteFreeze_StorageRequire_Hunan::dataIdInit(QString dataId)
{
...
//                p_timer->stop();
//                shared_ptr<Afn10F103_Hunan> p_QueryConfigStatus_STA_10f103_Up=dynamic_pointer_cast<Afn10F103_Hunan>(p_Frame3762Base);
//                moduleConfig10F103_Hunan.append(p_QueryConfigStatus_STA_10f103_Up->module_config_unit_.node_info_list_);
//                index+=node_num;
//                if(index<p_QueryConfigStatus_STA_10f103_Up->module_config_unit_.node_total_num_)
//                {
//                    emScriptRunState=Wait_QueryConfigStatus_STA_10f103_Finish;
...
//        p_SetRouterTimePeriod_05F102->period_value_=0x02;
//        p_SetRouterTimePeriod_05F102->period_unit_=0x02;
...
//            //单相表
//            p_SetCurveConfig_STA_05F103->curve_config_data_.single_meter_=0x00;
//            p_SetCurveConfig_STA_05F103->curve_config_data_.single_data_id_num_=uchar(dataIDNum);
...
//            //三相表
//            p_SetCurveConfig_STA_05F103->curve_config_data_.tri_meter_=0x01;
//            p_SetCurveConfig_STA_05F103->curve_config_data_.tri_data_id_num_=uchar(dataIDNum);
...
//            }
//        }
//        else if(emScriptRunState==Wait_SetCurveConfig_STA_05F103_2_Finish)//配置oop表，单三相，冻结01
//        {
//            if(emScriptRunState==Wait_SetCurveConfig_STA_05F103_2_Finish)
//            {
//                p_SetCurveConfig_STA_05F103->curve_config_data_.set_flag_=0x01;
...
//        uchar tmpAddr[6];
////        memcpy(tmpAddr,p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(index)->mtrAddr,6);
//        memcpy(tmpAddr,allMeterList.at(index).meterNo.addr,6);
...
//        if(allMeterList.at(index).protocolType==DLT645_2007)
//        {
//            shared_ptr<Rqst_ReadData_0x11> p_Rqst_ReadData_0x11=make_shared<Rqst_ReadData_0x11>(addr,4,0);
//            memcpy(p_Rqst_ReadData_0x11->addr_,tmpAddr,6);
//            QByteArray msg645;
...
//            curTime=QByteArray::fromHex(QDateTime::currentDateTime().toString("mmhhddMMyy").toLatin1());
//            curTime_start_oop=QByteArray::fromHex(QDateTime::currentDateTime().toString("yyyyMMddhhmmss").toLatin1());
//            curTime_end_oop=QByteArray::fromHex(QDateTime::currentDateTime().addSecs(realPeriod).toString("yyyyMMddhhmmss").toLatin1());
//            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("延时2min时间到,记录当前时间为：%1").arg(QString(QDateTime::currentDateTime().toString("yyyyMMddhhmm"))));
//            emScriptRunState=Wait_Delay_4_Finish;
//            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("等待环境运行%1min").arg((delayTimer)));
//            p_delayTimer->start(delayTimer*60*1000);
//            break;
...
//                meterMode.protocolType=DLT645_2007;
////                singleSTA_645List.append(meterMode);
//                for(int n=0;n<dataIDNum;n++)//与数据项个数对应
//                {
//                    ReadDataUnit readData;
//                    readData.dataIdLen=singleDataId_645_List.at(n);
////                    readData.roundList.append(roundList);
