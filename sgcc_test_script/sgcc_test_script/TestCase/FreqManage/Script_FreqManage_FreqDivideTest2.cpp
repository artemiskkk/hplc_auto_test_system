#include "Script_FreqManage_FreqDivideTest2.h"

Script_FreqManage_FreqDivideTest2::Script_FreqManage_FreqDivideTest2(QObject *parent) : QObject(parent)
{
...
void Script_FreqManage_FreqDivideTest2::execute()
{
...
void Script_FreqManage_FreqDivideTest2::stop()
{
...
void Script_FreqManage_FreqDivideTest2::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_FreqManage_FreqDivideTest2::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_FreqManage_FreqDivideTest2::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_FreqManage_FreqDivideTest2::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_FreqManage_FreqDivideTest2::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_FreqManage_FreqDivideTest2::processMsgFromCCO(DvcType dvcType, int dvcId)
{
                ...
                else if(p_Frame3762Base->afn_ ==char(0xF0)&&p_Frame3762Base->dt1_==04&&p_Frame3762Base->dt2_==0x06&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(0x01==p_QueryFreqDivide_F0F51_Up->frequence_division_)
                    {
                        ...
                        else
                        {
                            ...
                            resultFlag=true;
                            p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("路由分频系数测试用例2，测试成功;"));
                        }
                    }
                    else
                    {
...
void Script_FreqManage_FreqDivideTest2::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_FreqManage_FreqDivideTest2::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_FreqManage_FreqDivideTest2::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_FreqManage_FreqDivideTest2::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_FreqManage_FreqDivideTest2::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_FreqManage_FreqDivideTest2::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_HardResetTest_Finish:
...
void Script_FreqManage_FreqDivideTest2::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    if(emScriptRunState==Wait_SetInitFreq_Finish)
    {
