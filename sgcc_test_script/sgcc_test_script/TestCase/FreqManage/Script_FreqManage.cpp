#include "Script_FreqManage.h"

Script_FreqManage::Script_FreqManage(QObject *parent) : QObject(parent)
{
...
Script_FreqManage::~Script_FreqManage()
{
...
void Script_FreqManage::execute()
{
...
void Script_FreqManage::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_FreqManage::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_FreqManage::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_FreqManage::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_FreqManage::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_FreqManage::processMsgFromCCO(DvcType dvcType, int dvcId)
{
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_QueryInitFreq_Finish:
            {
                ...
                else if(p_Frame3762Base->afn_ ==char(0xF0)&&p_Frame3762Base->dt1_==04&&p_Frame3762Base->dt2_==0x06&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    initFreqDivide=p_QueryFreqDivide_F0F51_Up->frequence_division_;
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("初始分频系数为%1").arg(initFreqDivide));
            ...
            case Wait_HardResetTest_Finish:
            {
                ...
                else if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==char(0x80)&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("当前运行频段为%1，符合要求").arg(p_QueryFreq_03F16_Up->carrier_frequence_range_));
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("当前运行频段为%1，不符合要求").arg(p_QueryFreq_03F16_Up->carrier_frequence_range_));
                    }
                }
                else if(p_Frame3762Base->afn_ ==char(0xF0)&&p_Frame3762Base->dt1_==04&&p_Frame3762Base->dt2_==0x06&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("当前分频系数为%1，符合要求").arg(p_QueryFreqDivide_F0F51_Up->frequence_division_));
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("当前分频系数为%1，不符合要求").arg(p_QueryFreqDivide_F0F51_Up->frequence_division_));
                    }
                }
                else
                {
            ...
            case Wait_EraseFlash_Finish:
            {
                ...
                else if(p_Frame3762Base->afn_ ==char(0xF0)&&p_Frame3762Base->dt1_==04&&p_Frame3762Base->dt2_==0x06&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(initFreqDivide==p_QueryFreqDivide_F0F51_Up->frequence_division_)
                    {
                        ...
                        resultFlag=true;
                        p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("路由频段管理测试成功;"));
                    }
                    else
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("当前分频系数为%1，不符合要求").arg(p_QueryFreqDivide_F0F51_Up->frequence_division_));
...
void Script_FreqManage::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_FreqManage::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_FreqManage::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_FreqManage::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_FreqManage::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_FreqManage::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_HardResetTest_Finish:
...
void Script_FreqManage::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    index=0;
