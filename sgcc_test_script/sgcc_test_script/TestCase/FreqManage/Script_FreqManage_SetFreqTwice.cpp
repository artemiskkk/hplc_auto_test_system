#include "Script_FreqManage_SetFreqTwice.h"

Script_FreqManage_SetFreqTwice::Script_FreqManage_SetFreqTwice(QObject *parent) : QObject(parent)
{
...
Script_FreqManage_SetFreqTwice::~Script_FreqManage_SetFreqTwice()
{
...
void Script_FreqManage_SetFreqTwice::execute()
{
...
void Script_FreqManage_SetFreqTwice::stop()
{
...
void Script_FreqManage_SetFreqTwice::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_FreqManage_SetFreqTwice::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_FreqManage_SetFreqTwice::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_FreqManage_SetFreqTwice::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_FreqManage_SetFreqTwice::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_FreqManage_SetFreqTwice::processMsgFromCCO(DvcType dvcType, int dvcId)
{
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_QueryInitFreq_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==char(0x80)&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("发送--设置频段%0（05F16），等待--确认").arg(QString::number(dstFreq1)));
            ...
            case Wait_SetDstFreq1_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("回复确认，再设频段%0").arg(QString::number(dstFreq2)));
            ...
            case Wait_QueryDstFreq_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==char(0x80)&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();
                    shared_ptr<Afn03F16> p_QueryFreq_03F16_Up=dynamic_pointer_cast<Afn03F16>(p_Frame3762Base);
                    if(dstFreq2==p_QueryFreq_03F16_Up->carrier_frequence_range_)
                    {
...
void Script_FreqManage_SetFreqTwice::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_FreqManage_SetFreqTwice::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_FreqManage_SetFreqTwice::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_SetFreq_05F16)
    {
        ...
        if(emScriptRunState==Wait_SetDstFreq1_Finish)
        {
            p_SetFreq_05F16->carrier_frequence_range_=dstFreq1;
        }
        else if(emScriptRunState==Wait_SetDstFreq2_Finish)
        {
            p_SetFreq_05F16->carrier_frequence_range_=dstFreq2;
        }
...
void Script_FreqManage_SetFreqTwice::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_FreqManage_SetFreqTwice::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_FreqManage_SetFreqTwice::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        default:
...
void Script_FreqManage_SetFreqTwice::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
