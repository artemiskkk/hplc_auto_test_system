#include "Script_FreqManage_SetFreq1To2_V2.h"

Script_FreqManage_SetFreq1To2_V2::Script_FreqManage_SetFreq1To2_V2(QObject *parent) : QObject(parent)
{
...
Script_FreqManage_SetFreq1To2_V2::~Script_FreqManage_SetFreq1To2_V2()
{
...
void Script_FreqManage_SetFreq1To2_V2::execute()
{
...
void Script_FreqManage_SetFreq1To2_V2::stop()
{
...
void Script_FreqManage_SetFreq1To2_V2::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_FreqManage_SetFreq1To2_V2::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_FreqManage_SetFreq1To2_V2::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_FreqManage_SetFreq1To2_V2::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_FreqManage_SetFreq1To2_V2::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_FreqManage_SetFreq1To2_V2::processMsgFromCCO(DvcType dvcType, int dvcId)
{
...
//                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                ...
                else if(p_Frame3762Base->afn_ ==char(0xF0)&&p_Frame3762Base->dt1_==04&&p_Frame3762Base->dt2_==0x06&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(p_QueryFreqDivide_F0F51_Up->frequence_division_==1)
                    {
...
//                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                        ...
                        resultFlag=true;
                        p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("路由频段管理(1→2_V2)，测试成功;"));
                    }
                    else
                    {
            ...
            case Wait_EraseFlash_Finish:
            {
                ...
                {
                    p_timer->stop();
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到03F10上报，擦除路由Flash完成！");
                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryFreq_03F16);
                    emScriptRunState=Wait_EraseFlash_Finish;
                    emQueryState=Wait_QueryFreq_03F16;
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--查询当前频段（03F16），等待--回复");
                ...
                else if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==char(0x80)&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("擦除路由Flash后，频段恢复默认%1，符合要求").arg(QString::number(p_QueryFreq_03F16_Up->carrier_frequence_range_)));
...
//                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                        emScriptRunState=ScriptSuccess;
                        resultFlag=true;
                        p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("频段切换测试成功"));
                    }
                    else
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("擦除路由Flash后，频段为%1，未恢复默认2，不符合要求").arg(QString::number(p_QueryFreq_03F16_Up->carrier_frequence_range_)));
                    }
                }
                else if(p_Frame3762Base->afn_ ==char(0xF0)&&p_Frame3762Base->dt1_==04&&p_Frame3762Base->dt2_==0x06&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
...
void Script_FreqManage_SetFreq1To2_V2::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_FreqManage_SetFreq1To2_V2::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_FreqManage_SetFreq1To2_V2::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_FreqManage_SetFreq1To2_V2::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_FreqManage_SetFreq1To2_V2::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_FreqManage_SetFreq1To2_V2::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_HardResetTest_Finish:
...
void Script_FreqManage_SetFreq1To2_V2::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    if(emScriptRunState==Wait_SetInitFreq_Finish)
    {
