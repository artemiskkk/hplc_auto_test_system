#include "Script_FreqManage_FreqDivideTest1.h"

Script_FreqManage_FreqDivideTest1::Script_FreqManage_FreqDivideTest1(QObject *parent) : QObject(parent)
{
    ...
    dst.dstFreq=0x00;//目的频段0
    dst.dstFreqDivide=0x02;//目的分频系数0
    //F1D2
    FreqAndDivideStruct dst1;
    dst1.dstFreq=0x01;//目的频段1
    dst1.dstFreqDivide=0x02;//目的分频系数1
    //F1D1
    FreqAndDivideStruct dst2;
    dst2.dstFreq=0x01;//目的频段2
    dst2.dstFreqDivide=0x01;//目的分频系数2
    freqAndDivideList<<dst<<dst1<<dst2;
}
Script_FreqManage_FreqDivideTest1::~Script_FreqManage_FreqDivideTest1()
{
...
void Script_FreqManage_FreqDivideTest1::execute()
{
...
void Script_FreqManage_FreqDivideTest1::stop()
{
...
void Script_FreqManage_FreqDivideTest1::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_FreqManage_FreqDivideTest1::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_FreqManage_FreqDivideTest1::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_FreqManage_FreqDivideTest1::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_FreqManage_FreqDivideTest1::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_FreqManage_FreqDivideTest1::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_SetDstFreq_Finish:
            {
                ...
                {
                    p_timer->stop();
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("目的运行频段%1设置，回复确认！").arg(index));
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("等待%1s后，设置目的分频系数%2！").arg(waitTime).arg(index));
                    p_delayTimer->start(waitTime*1000);
                }
                else
                {
            ...
            case Wait_SetDstFreqDivide_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("目的分频系数%1设置，回复确认！").arg(index));
            ...
            case Wait_QueryDstFreq_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==char(0x80)&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();
                    shared_ptr<Afn03F16> p_QueryFreq_03F16_Up=dynamic_pointer_cast<Afn03F16>(p_Frame3762Base);
//                    if(freqAndDivideList.at(index).dstFreq==p_QueryFreq_03F16_Up->carrier_frequence_range_)
//                    {
                        ...
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryFreqDivide_F0F51);
                        emScriptRunState=Wait_QueryDstFreqDivide_Finish;
...
//                    }
            ...
            case Wait_QueryDstFreqDivide_Finish:
            {
                if(p_Frame3762Base->afn_ ==char(0xF0)&&p_Frame3762Base->dt1_==04&&p_Frame3762Base->dt2_==0x06&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();
                    shared_ptr<AfnF0F51> p_QueryFreqDivide_F0F51_Up=dynamic_pointer_cast<AfnF0F51>(p_Frame3762Base);
//                    if(freqAndDivideList.at(index).dstFreqDivide==p_QueryFreqDivide_F0F51_Up->frequence_division_)
//                    {
...
//                    }
            ...
            case Wait_EraseFlash_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("收到确认"));
                }
                else if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();
                ...
                else if(p_Frame3762Base->afn_ ==char(0xF0)&&p_Frame3762Base->dt1_==04&&p_Frame3762Base->dt2_==0x06&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(0x01==p_QueryFreqDivide_F0F51_Up->frequence_division_)
                    {
                        ...
                        if(++index<freqAndDivideList.size())
                        {
                            sendMsg(CCO_GW,p_CtrInfoList->at(0)->dvcId,INSIGNIFICANCE,p_AddSlaveNode_11F1);
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("添加1个从节点"));
                            p_timer->start(CCO_CMD_TIMEOUT_TIME*1000);
                        ...
                        else
                        {
                            ...
                            resultFlag=true;
                            p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("路由分频系数测试用例1，测试成功;"));
                        }
                    }
                    else
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("擦除路由Flash后，当前分频系数为%1，不符合要求").arg(QString::number(p_QueryFreqDivide_F0F51_Up->frequence_division_)));
                    }
                }
                else
                {
        ...
        case Wait_AddMeter_Finish:
        {
            ...
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("收到添加从节点确认，等待3s"));
...
void Script_FreqManage_FreqDivideTest1::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_FreqManage_FreqDivideTest1::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_FreqManage_FreqDivideTest1::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_SetFreq_05F16)
    {
        ...
        if(emScriptRunState==Wait_SetInitFreq_Finish)
            ...
            p_SetFreq_05F16->carrier_frequence_range_=freqAndDivideList.at(index).dstFreq;
    ...
    else if(frame==p_SetFreqDivide_F0F50)
    {
        ...
        if(emScriptRunState==Wait_SetInitFreq_Finish)
            ...
            p_SetFreqDivide_F0F50->frequence_division_=freqAndDivideList.at(index).dstFreqDivide;
...
void Script_FreqManage_FreqDivideTest1::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_FreqManage_FreqDivideTest1::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_FreqManage_FreqDivideTest1::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_HardResetTest_Finish:
...
void Script_FreqManage_FreqDivideTest1::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    if(emScriptRunState==Wait_SetInitFreq_Finish)
    {
    ...
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("开始设置目的分频系数%1！").arg(index));
        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_SetFreqDivide_F0F50);
        emScriptRunState=Wait_SetDstFreqDivide_Finish;
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--设置目的分频系数（F0F50），等待--确认");
        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
    }
    ...
    else if(emScriptRunState==Wait_SetDstFreqDivide_Finish)
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("开始查询目的频段！"));
        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryFreq_03F16);
        emScriptRunState=Wait_QueryDstFreq_Finish;
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("发送--查询目的频段（03F16），等待--回复"));
        p_timer->start(CCO_CMD_TIMEOUT_TIME*1000);
    }
    else if(emScriptRunState==Wait_AddMeter_Finish)
    {
        emScriptRunState=Wait_SetDstFreq_Finish;
        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_SetFreq_05F16);
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--设置目的频段（05F16），等待--确认");
