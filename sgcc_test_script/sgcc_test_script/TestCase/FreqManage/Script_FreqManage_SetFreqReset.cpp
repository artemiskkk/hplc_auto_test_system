#include "Script_FreqManage_SetFreqReset.h"

Script_FreqManage_SetFreqReset::Script_FreqManage_SetFreqReset(QObject *parent) : QObject(parent)
{
...
Script_FreqManage_SetFreqReset::~Script_FreqManage_SetFreqReset()
{
...
void Script_FreqManage_SetFreqReset::execute()
{
...
void Script_FreqManage_SetFreqReset::stop()
{
...
void Script_FreqManage_SetFreqReset::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_FreqManage_SetFreqReset::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}

bool Script_FreqManage_SetFreqReset::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_FreqManage_SetFreqReset::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
    ...
    if(emScriptRunState==Wait_BuildNetFinish_Whole)
    {
        ...
        else
        {
            ...
            emScriptRunState=Wait_QueryInitFreq1_Finish;
...
void Script_FreqManage_SetFreqReset::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_FreqManage_SetFreqReset::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        case Wait_QueryInitFreq1_Finish:
        {
            if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==char(0x80)&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("发送--设置频段%0（05F16），等待--确认").arg(QString::number(dstFreq)));
        ...
        case Wait_SetDstFreq_Finish:
        {
            ...
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("回复确认，等待%1s后查询频段").arg(waitTime));
        ...
        case Wait_QueryDstFreq_Finish:
        {
            if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==char(0x80)&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(dstFreq==p_QueryFreq_03F16_Up->carrier_frequence_range_)
                {
                    ...
                    emScriptRunState=Wait_HardReset1_Finish;
        ...
        case Wait_HardReset1_Finish:
        {
            if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("硬件初始化回复确认"));
            }
            else if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryFreq_03F16);
                emScriptRunState=Wait_QueryFreqAfterReset;
                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
            }
            else
            {
        ...
        case Wait_QueryFreqAfterReset:
        {
            if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==char(0x80)&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(dstFreq==p_QueryFreq_03F16_Up->carrier_frequence_range_)
                {
                    ...
                    if(++resetNo>resetNum)
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("发送--设置初始频段%0，等待--确认").arg(QString::number(initFreq)));
                        emScriptRunState=Wait_SetInitFreq_Finish;
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_SetFreq_05F16);
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    }
        ...
        case Wait_SetInitFreq_Finish:
        {
            ...
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("回复确认，开始硬件初始化"));
                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_HardInit_01F1_Down);
                emScriptRunState=Wait_HardReset2_Finish;
                p_timer->start(CCO_CMD_TIMEOUT_TIME*1000);
            }
            else
            {
        ...
        case Wait_HardReset2_Finish:
        {
            ...
            else if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("收到03F10上报，读取当前频段"));
                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryFreq_03F16);
                emScriptRunState=Wait_QueryInitFreq2_Finish;
                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
            }
            else
            {
        ...
        case Wait_QueryInitFreq2_Finish:
        {
            if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==char(0x80)&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_timer->stop();
                ...
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Success,QString("当前运行频段为%1，符合要求").arg(QString::number(p_QueryFreq_03F16_Up->carrier_frequence_range_)));
...
void Script_FreqManage_SetFreqReset::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_FreqManage_SetFreqReset::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_FreqManage_SetFreqReset::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_SetFreq_05F16)
    {
        ...
        if(emScriptRunState==Wait_SetDstFreq_Finish)
        {
            p_SetFreq_05F16->carrier_frequence_range_=dstFreq;
        }
        else if(emScriptRunState==Wait_SetInitFreq_Finish)
        {
...
void Script_FreqManage_SetFreqReset::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_FreqManage_SetFreqReset::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_FreqManage_SetFreqReset::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        default:
...
void Script_FreqManage_SetFreqReset::delayTimer_timeoutProc()
{
    p_delayTimer->stop();

    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "开始查询频段（03F16）");
    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryFreq_03F16);
