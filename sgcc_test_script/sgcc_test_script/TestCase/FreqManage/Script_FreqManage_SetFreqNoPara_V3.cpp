#include "Script_FreqManage_SetFreqNoPara_V3.h"

Script_FreqManage_SetFreqNoPara_V3::Script_FreqManage_SetFreqNoPara_V3(QObject *parent) : QObject(parent)
{
...
Script_FreqManage_SetFreqNoPara_V3::~Script_FreqManage_SetFreqNoPara_V3()
{
...
void Script_FreqManage_SetFreqNoPara_V3::execute()
{
...
void Script_FreqManage_SetFreqNoPara_V3::stop()
{
...
void Script_FreqManage_SetFreqNoPara_V3::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_FreqManage_SetFreqNoPara_V3::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}

bool Script_FreqManage_SetFreqNoPara_V3::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_FreqManage_SetFreqNoPara_V3::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_FreqManage_SetFreqNoPara_V3::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_FreqManage_SetFreqNoPara_V3::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        case Wait_SetFreq_Finish:
        {
            if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                p_delayTimer->start(CCO_CMD_TIMEOUT_TIME*1000);//10s后查询
...
//                p_timer->start(CCO_CMD_TIMEOUT_TIME*1000);
        ...
        case Wait_QueryFreq1_Finish:
        {
            if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==char(0x80)&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("当前运行频段为设置频段%1，符合要求").arg(QString::number(setFreq)));
                ...
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("当前运行频段%1非设置频段%2，不符合要求").arg(QString::number(p_QueryFreq_03F16_Up->carrier_frequence_range_)).arg(QString::number(setFreq)));
                }
            }
            else
            {
...
void Script_FreqManage_SetFreqNoPara_V3::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_FreqManage_SetFreqNoPara_V3::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_FreqManage_SetFreqNoPara_V3::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_FreqManage_SetFreqNoPara_V3::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_FreqManage_SetFreqNoPara_V3::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_FreqManage_SetFreqNoPara_V3::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        default:
...
void Script_FreqManage_SetFreqNoPara_V3::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
