#include "Script_FreqManage_SetFreq0To3.h"

Script_FreqManage_SetFreq0To3::Script_FreqManage_SetFreq0To3(QObject *parent) : QObject(parent)
{
...
Script_FreqManage_SetFreq0To3::~Script_FreqManage_SetFreq0To3()
{
...
void Script_FreqManage_SetFreq0To3::execute()
{
...
void Script_FreqManage_SetFreq0To3::stop()
{
...
void Script_FreqManage_SetFreq0To3::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_FreqManage_SetFreq0To3::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_FreqManage_SetFreq0To3::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_FreqManage_SetFreq0To3::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_FreqManage_SetFreq0To3::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_FreqManage_SetFreq0To3::processMsgFromCCO(DvcType dvcType, int dvcId)
{
            ...
            case Wait_HardResetTest_Finish:
            {
                ...
                else if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==char(0x80)&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(dstFreq==p_QueryFreq_03F16_Up->carrier_frequence_range_)
                    {
                        ...
                        resultFlag=true;
                        p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("路由频段管理(0→3)，测试成功;"));
                    }
                    else
                    {
...
void Script_FreqManage_SetFreq0To3::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_FreqManage_SetFreq0To3::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_FreqManage_SetFreq0To3::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_FreqManage_SetFreq0To3::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_FreqManage_SetFreq0To3::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_FreqManage_SetFreq0To3::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_HardResetTest_Finish:
...
void Script_FreqManage_SetFreq0To3::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    if(emScriptRunState==Wait_SetInitFreq_Finish)
    {
