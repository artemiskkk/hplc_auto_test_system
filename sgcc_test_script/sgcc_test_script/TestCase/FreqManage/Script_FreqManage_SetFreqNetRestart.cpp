#include "Script_FreqManage_SetFreqNetRestart.h"

Script_FreqManage_SetFreqNetRestart::Script_FreqManage_SetFreqNetRestart(QObject *parent) : QObject(parent)
{
...
Script_FreqManage_SetFreqNetRestart::~Script_FreqManage_SetFreqNetRestart()
{
...
void Script_FreqManage_SetFreqNetRestart::execute()
{
...
void Script_FreqManage_SetFreqNetRestart::stop()
{
...
void Script_FreqManage_SetFreqNetRestart::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_FreqManage_SetFreqNetRestart::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}

bool Script_FreqManage_SetFreqNetRestart::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_FreqManage_SetFreqNetRestart::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_FreqManage_SetFreqNetRestart::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_FreqManage_SetFreqNetRestart::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        case Wait_QueryInitFreq_Finish:
        {
            if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==char(0x80)&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("发送--设置频段%0（05F16），等待--确认").arg(QString::number(setFreq)));
        ...
        case Wait_SetFreq_Finish:
        {
            ...
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("回复确认，计时%1s开始").arg(waitTime));
        ...
        case Wait_ParaInit_Finish:
        {
            ...
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("回复确认，添加1个档案"));
        ...
        case Wait_AddMeter_Finish:
        {
            ...
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("添加档案回复确认"));
            }
            else
            {
                QByteArray tmpSendMsg=prcsOther3762Msg(uchar(p_Frame3762Base->afn_),p_Frame3762Base->dt1_,p_Frame3762Base->dt2_,uchar(p_Frame3762Base->info_field_.info_field_up.msg_seq));
        ...
        case Wait_QueryFreq_Finish:
        {
            if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==char(0x80)&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Success,QString("网管启停后当前运行频段为初始频段%1，符合要求").arg(QString::number(initFreq)));
                    emScriptRunState=ScriptSuccess;
                }
                else
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("网管启停后当前运行频段%1非初始频段%2，不符合要求").arg(QString::number(p_QueryFreq_03F16_Up->carrier_frequence_range_)).arg(QString::number(initFreq)));
                }
            }
            else
            {
...
void Script_FreqManage_SetFreqNetRestart::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_FreqManage_SetFreqNetRestart::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_FreqManage_SetFreqNetRestart::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_FreqManage_SetFreqNetRestart::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_FreqManage_SetFreqNetRestart::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_FreqManage_SetFreqNetRestart::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        default:
...
void Script_FreqManage_SetFreqNetRestart::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
