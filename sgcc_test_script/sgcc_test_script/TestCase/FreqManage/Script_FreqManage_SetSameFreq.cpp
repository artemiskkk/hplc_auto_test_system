#include "Script_FreqManage_SetSameFreq.h"

Script_FreqManage_SetSameFreq::Script_FreqManage_SetSameFreq(QObject *parent) : QObject(parent)
{
...
Script_FreqManage_SetSameFreq::~Script_FreqManage_SetSameFreq()
{
...
void Script_FreqManage_SetSameFreq::execute()
{
...
void Script_FreqManage_SetSameFreq::stop()
{
...
void Script_FreqManage_SetSameFreq::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_FreqManage_SetSameFreq::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_FreqManage_SetSameFreq::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_FreqManage_SetSameFreq::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_FreqManage_SetSameFreq::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_FreqManage_SetSameFreq::processMsgFromCCO(DvcType dvcType, int dvcId)
{
...
//            }
            case Wait_QueryInitFreq_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==char(0x80)&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_SetFreq_05F16);
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--设置相同频段（05F16），等待--确认");
                    p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                }
                else
                {
            ...
            case Wait_SetDstFreq_Finish:
            {
                ...
                {
                    p_timer->stop();
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("设置相同频段回复确认"));
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("等待%1s后查询频段").arg(waitTime));
                    p_delayTimer->start(waitTime*1000);
                }
                else
                {
...
//                    if(dstFreqDivide==p_QueryFreqDivide_F0F51_Up->frequence_division_)
//                    {
...
void Script_FreqManage_SetSameFreq::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_FreqManage_SetSameFreq::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_FreqManage_SetSameFreq::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_FreqManage_SetSameFreq::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_FreqManage_SetSameFreq::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_FreqManage_SetSameFreq::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
//        case Wait_HardResetTest_Finish:
...
void Script_FreqManage_SetSameFreq::delayTimer_timeoutProc()
{
    p_delayTimer->stop();

    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "开始查询频段");
    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryFreq_03F16);
