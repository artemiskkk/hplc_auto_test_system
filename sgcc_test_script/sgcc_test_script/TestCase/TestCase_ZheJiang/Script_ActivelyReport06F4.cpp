#include "Script_ActivelyReport06F4.h"

Script_ActivelyReport06F4::Script_ActivelyReport06F4(QObject *parent) : QObject(parent)
{
    ...
    p_ActiveReport_06F4_Up=make_shared<Afn06F4>();
...
Script_ActivelyReport06F4::~Script_ActivelyReport06F4()
{
...
void Script_ActivelyReport06F4::execute()
{
...
void Script_ActivelyReport06F4::stop()
{
...
void Script_ActivelyReport06F4::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_ActivelyReport06F4::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}

bool Script_ActivelyReport06F4::config(const QMap<QString, QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_ActivelyReport06F4::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ActivelyReport06F4::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_ActivelyReport06F4::processMsgFromCCO(DvcType dvcType, int dvcId)
{
...
//                }
                ...
                {
                    p_timer->stop();
                    shared_ptr<Afn06F4> p_ActiveReport_06F4_Up=dynamic_pointer_cast<Afn06F4>(p_Frame3762Base);
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("收到--06F4主动上报从节点信息"));
                    //**统计06F4上报数量
                    nodeNum1++;
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("nodeNum1=%1").arg(nodeNum1));
...
//                }
            ...
            case Wait_HardReset_Building_Finish:
            {
                ...
                else if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "组网成功！");
                ...
                else if(p_Frame3762Base->afn_ == 0x06&&p_Frame3762Base->dt1_ == 0x08&&p_Frame3762Base->dt2_ == 0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    //**统计06F4上报数量
                    nodeNum2++;
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("nodeNum2=%1").arg(nodeNum2));
                    if(nodeNum1==netScale-1 || nodeNum2==netScale-1)
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("06F4主动上报事件数量与入网从节点数量相同"));
                        p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("06F4主动上报脚本测试成功！"));
...
void Script_ActivelyReport06F4::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ActivelyReport06F4::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_ActivelyReport06F4::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_ActivelyReport06F4::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_ActivelyReport06F4::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_ActivelyReport06F4::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        default:
...
void Script_ActivelyReport06F4::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNetScale_10F9);
    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--查询网络规模（10F9），等待--回复");
