#include "Script_CcoOneMinuteNoBeacon.h"

Script_CcoOneMinuteNoBeacon::Script_CcoOneMinuteNoBeacon(QObject *parent) : QObject(parent)
{
...
Script_CcoOneMinuteNoBeacon::~Script_CcoOneMinuteNoBeacon()
{
...
void Script_CcoOneMinuteNoBeacon::execute()
{
...
void Script_CcoOneMinuteNoBeacon::stop()
{
...
void Script_CcoOneMinuteNoBeacon::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_CcoOneMinuteNoBeacon::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}

bool Script_CcoOneMinuteNoBeacon::config(const QMap<QString, QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_CcoOneMinuteNoBeacon::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_CcoOneMinuteNoBeacon::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_CcoOneMinuteNoBeacon::processMsgFromCCO(DvcType dvcType, int dvcId)
{
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_QueryNetScale_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    else
                    {
                        ...
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "有节点入网，符合要求！");
            ...
            case Wait_QueryNetScale_Init_Finish:
            {
                ...
                else if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    {
                        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNetScale_10F9);
                        emScriptRunState=Wait_QueryNetScale_ParameterInit_Finish;
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--查询网络规模（10F9），等待--回复");
                        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                    }
                }
            ...
            case Wait_QueryNetScale_ParameterInit_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();//接收到一条完整报文
...
void Script_CcoOneMinuteNoBeacon::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_CcoOneMinuteNoBeacon::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_CcoOneMinuteNoBeacon::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_CcoOneMinuteNoBeacon::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_CcoOneMinuteNoBeacon::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_CcoOneMinuteNoBeacon::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        default:
...
void Script_CcoOneMinuteNoBeacon::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    ...
    else if(emScriptRunState==Wait_QueryNetScale_Building_Finish)
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "开始查询网络规模！");
        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNetScale_10F9);
        minFlag=true;
