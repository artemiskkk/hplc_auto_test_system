#include "Script_CollectImmediately.h"

Script_CollectImmediately::Script_CollectImmediately(QObject *parent) : QObject(parent)
{
...
Script_CollectImmediately::~Script_CollectImmediately()
{
...
void Script_CollectImmediately::execute()
{
...
void Script_CollectImmediately::stop()
{
...
void Script_CollectImmediately::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_CollectImmediately::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_Zhejiang->setHost(host);
}

bool Script_CollectImmediately::config(const QMap<QString, QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_CollectImmediately::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_CollectImmediately::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_CollectImmediately::processMsgFromCCO(DvcType dvcType, int dvcId)
{
...
void Script_CollectImmediately::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_CollectImmediately::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_CollectImmediately::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_CollectImmediately::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_CollectImmediately::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_CollectImmediately::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        default:
...
void Script_CollectImmediately::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    statisticsNode();
}

void Script_CollectImmediately::statisticsNode()
{
