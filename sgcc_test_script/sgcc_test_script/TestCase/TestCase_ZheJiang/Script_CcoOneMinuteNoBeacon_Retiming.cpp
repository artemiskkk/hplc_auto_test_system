#include "Script_CcoOneMinuteNoBeacon_Retiming.h"

Script_CcoOneMinuteNoBeacon_Retiming::Script_CcoOneMinuteNoBeacon_Retiming(QObject *parent) : QObject(parent)
{
...
Script_CcoOneMinuteNoBeacon_Retiming::~Script_CcoOneMinuteNoBeacon_Retiming()
{
...
void Script_CcoOneMinuteNoBeacon_Retiming::execute()
{
...
void Script_CcoOneMinuteNoBeacon_Retiming::stop()
{
...
void Script_CcoOneMinuteNoBeacon_Retiming::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_CcoOneMinuteNoBeacon_Retiming::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}

bool Script_CcoOneMinuteNoBeacon_Retiming::config(const QMap<QString, QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_CcoOneMinuteNoBeacon_Retiming::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_CcoOneMinuteNoBeacon_Retiming::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_CcoOneMinuteNoBeacon_Retiming::processMsgFromCCO(DvcType dvcType, int dvcId)
{
...
//            }
            case Wait_HardReset_Building_Finish:
            {
                ...
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到03F10主动上报");
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "开始查询网络规模(20s内)!");
                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNetScale_10F9);
                    //minFlag=false;
            ...
            case Wait_QueryNetScale_Building_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(p_QueryNetScale_10F9_Up->network_scale_<=1)
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "路由复位20s内无节点入网，符合要求!!");
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "等待20s后--硬件复位路由（01F1）");
                        p_delayTimer->start(20*1000);
                    }
                    else
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "路由复位1min内有节点入网，不符合要求!测试失败!");
                    }
                }
                else
                {
            ...
            case Wait_HardReset_late_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();//接收到一条完整报文
                ...
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到03F10主动上报");
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "开始查询网络规模(1分钟内)!");
                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNetScale_10F9);
                    //minFlag=false;
                    emScriptRunState=Wait_QueryNetScale_HardRest_Finish;
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--查询网络规模（10F9），等待--回复");
                    p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                }
                else
        ...
        case Wait_QueryNetScale_HardRest_Finish:
        {
            if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                p_timer->stop();//接收到一条完整报文
                ...
                if(minFlag==true)
                {
                    if(p_QueryNetScale_10F9_Up->network_scale_<=1)
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "路由复位1min内无节点入网，符合要求!!");
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "等待60s后开始查询网络规模");
                        emScriptRunState=Wait_QueryNetScale_HardRest_Finish;
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "路由复位1min内有节点入网，不符合要求!!");
                    }
                }
                else
                {
                    ...
                    else
                    {
                        ...
                        p_AbstractScriptHost->updateProgress(ProcessState_Success, "有节点入网，符合要求！脚本测试成功！");
                    }
                }
            }
            else
...
void Script_CcoOneMinuteNoBeacon_Retiming::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_CcoOneMinuteNoBeacon_Retiming::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_CcoOneMinuteNoBeacon_Retiming::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_CcoOneMinuteNoBeacon_Retiming::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_CcoOneMinuteNoBeacon_Retiming::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
//        case Wait_QueryNetScale_HardRest_Finish:
//        {
//            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "开始查询网络规模！");
//            sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNetScale_10F9);
//            minFlag=false;
//            emScriptRunState=Wait_QueryNetScale_HardRest_Finish;
//            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--查询网络规模（10F9），等待--回复");
...
void Script_CcoOneMinuteNoBeacon_Retiming::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        default:
...
void Script_CcoOneMinuteNoBeacon_Retiming::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    if(emScriptRunState==Wait_QueryNetScale_Building_Finish)
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "20s后");
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--硬件初始化（01F1），等待--回复");
        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_HardReset_01F1);
        emScriptRunState=Wait_HardReset_late_Finish;
        p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
    }
    else if(emScriptRunState==Wait_QueryNetScale_HardRest_Finish)
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "开始查询网络规模！");
        sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNetScale_10F9);
        minFlag=false;
