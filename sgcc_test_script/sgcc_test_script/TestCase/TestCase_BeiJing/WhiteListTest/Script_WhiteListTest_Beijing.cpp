#include "Script_WhiteListTest_Beijing.h"

Script_WhiteListTest_Beijing::Script_WhiteListTest_Beijing(QObject *parent) : QObject(parent)
{
...
Script_WhiteListTest_Beijing::~Script_WhiteListTest_Beijing()
{
...
void Script_WhiteListTest_Beijing::execute()
{
...
void Script_WhiteListTest_Beijing::stop()
{
...
void Script_WhiteListTest_Beijing::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_WhiteListTest_Beijing::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_WhiteListTest_Beijing::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_WhiteListTest_Beijing::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_WhiteListTest_Beijing::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_WhiteListTest_Beijing::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_EraseFlash_Finish:
            {
                ...
                {
                    p_timer->stop();
                    emScriptRunState=Wait_QueryWhiteListState_03F131_Finish;
            ...
            case Wait_QueryWhiteListState_03F131_Finish:
            {

                if(p_Frame3762Base->afn_ == 0x03&&p_Frame3762Base->dt1_==0x10&&p_Frame3762Base->dt2_==0x04&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();
                    shared_ptr<Afn03F131_Beijing> p_QueryWhiteListState_03F131_Up=dynamic_pointer_cast<Afn03F131_Beijing>(p_Frame3762Base);
                    //判断是否正确
                    if(p_QueryWhiteListState_03F131_Up->white_list_switch_==defaultState)
                    {
            ...
            case Wait_QueryNetScale_10F9_Finish:
            {
                ...
                else if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing,  QString("网络规模为%1，大于1，符合要求").arg(p_QueryNetScale_10F9_Up->network_scale_));
                        emScriptRunState=ScriptSuccess;
                        p_AbstractScriptHost->updateProgress(ProcessState_Success,  QString("白名单测试成功！"));
                    }
                    else
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing,  QString("网络规模为%1,不大于1").arg(p_QueryNetScale_10F9_Up->network_scale_));
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("等待5s，开始第%1轮查询网络规模！").arg(++index));
                        emScriptRunState=Wait_QueryNetScale_10F9_Finish;
                        p_delayTimer->start(5*1000);
                    }
...
void Script_WhiteListTest_Beijing::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_WhiteListTest_Beijing::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_WhiteListTest_Beijing::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_WhiteListTest_Beijing::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_WhiteListTest_Beijing::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_WhiteListTest_Beijing::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_EraseFlash_Finish:
...
void Script_WhiteListTest_Beijing::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
