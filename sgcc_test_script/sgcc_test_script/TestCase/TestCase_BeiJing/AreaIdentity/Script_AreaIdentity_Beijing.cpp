#include "Script_AreaIdentity_Beijing.h"

Script_AreaIdentity_Beijing::Script_AreaIdentity_Beijing(QObject *parent) : QObject(parent)
{
...
Script_AreaIdentity_Beijing::~Script_AreaIdentity_Beijing()
{
...
void Script_AreaIdentity_Beijing::execute()
{
...
void Script_AreaIdentity_Beijing::stop()
{
...
void Script_AreaIdentity_Beijing::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_AreaIdentity_Beijing::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_AreaIdentity_Beijing::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_AreaIdentity_Beijing::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_AreaIdentity_Beijing::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_AreaIdentity_Beijing::processMsgFromCCO(DvcType dvcType, int dvcId)
{
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_QueryRouterStateDisable_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x08&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(uchar(p_QueryRouterState_10F4_Up->router_operate_state_unit_.work_switch_.area_difference_flag_)==stateDisable)
                    {
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "查询台区区分标志使能，不符合要求");
                    }
                }
                else
                {
            ...
            case Wait_QueryRouterStateEnable_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x08&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(uchar(p_QueryRouterState_10F4_Up->router_operate_state_unit_.work_switch_.area_difference_flag_)==stateEnable)
                    {
            ...
            case Wait_SetAreaIdentityFlagDisable_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    emScriptRunState=Wait_ParameterInit_After_Finish;
                    sendMsg(CCO_GW,dvcId,INSIGNIFICANCE,p_ParameterInit_01F2);
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--参数初始化（01F2），等待--确认");
                    p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                }
            ...
            case Wait_ParameterInit_After_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x06&&p_Frame3762Base->dt1_==0x10&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();//接收到一条完整报文
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "参数初始化，回复确认");
                    emScriptRunState=Wait_HardReset_After_Finish;
                    sendMsg(CCO_GW,dvcId,INSIGNIFICANCE,p_HardReset_01F1);
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--硬件复位（01F1），等待--确认");
                    p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                }
            ...
            case Wait_HardReset_After_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    break;
                ...
                {
                    p_timer->stop();
                    emScriptRunState=Wait_QueryRouterStateDisable_After_Finish;
                    sendMsg(CCO_GW,dvcId,INSIGNIFICANCE,p_QueryRouterState_10F4);
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--查询台区区分标志（10F4），等待--回复");
                    p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                }
            ...
            case Wait_QueryRouterStateDisable_After_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x08&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();//接收到一条完整报文
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "查询台区区分标志禁止，符合要求");
                        p_AbstractScriptHost->updateProgress(ProcessState_Success, "台区识别测试成功");
...
void Script_AreaIdentity_Beijing::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_AreaIdentity_Beijing::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_AreaIdentity_Beijing::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_AddSlaveNode_11F1)
    {
        ...
        if(index+addCntPerTime<meterList.size())
        {
            for(int i=0;i<addCntPerTime;i++)
            {
                NodeParameter nodePara;
                memcpy(nodePara.node_address_.addr,meterList.at(index+i).nodeAddress.addr,6);
                nodePara.protocol_type_=meterList.at(index+i).protocol;
                p_AddSlaveNode_11F1->node_parameter_list_.append(nodePara);
            }
        }
        else
        {
            for(int i=0;i<meterList.size()-index;i++)
            {
                NodeParameter nodePara;
                memcpy(nodePara.node_address_.addr,meterList.at(index+i).nodeAddress.addr,6);
                nodePara.protocol_type_=meterList.at(index+i).protocol;
    ...
    else if(frame==p_SetAreaIdentityFlag_05F130)
    {
        ...
        if(emScriptRunState==Wait_SetAreaIdentityFlagEnable_Finish)
            p_SetAreaIdentityFlag_05F130->area_identity_switch_=stateEnable;
        else if(emScriptRunState==Wait_SetAreaIdentityFlagDisable_Finish)
            p_SetAreaIdentityFlag_05F130->area_identity_switch_=stateDisable;
...
void Script_AreaIdentity_Beijing::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_AreaIdentity_Beijing::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_AreaIdentity_Beijing::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        default:
...
void Script_AreaIdentity_Beijing::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
...
void Script_AreaIdentity_Beijing::getMeterList()
{
    meterList.clear();
