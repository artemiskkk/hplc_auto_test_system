#include "Script_LN_Inverse.h"

Script_LN_Inverse::Script_LN_Inverse(QObject *parent) : QObject(parent)
{
...
Script_LN_Inverse::~Script_LN_Inverse()
{
...
void Script_LN_Inverse::execute()
{
...
void Script_LN_Inverse::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_LN_Inverse::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_LN_Inverse::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_LN_Inverse::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_LN_Inverse::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_LN_Inverse::processMsgFromCCO(DvcType dvcType, int dvcId)
{
...
void Script_LN_Inverse::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_LN_Inverse::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_LN_Inverse::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_LN_Inverse::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_LN_Inverse::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_LN_Inverse::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        default:
...
void Script_LN_Inverse::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
...
void Script_LN_Inverse::statisticsResult()
{
    int count10F31=0;
    int count13F1=0;
    ...
    for(int i=0;i<p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->size();i++)
    {
        ...
        if(findFlag==false)//找不到对应的电表号
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("表号：%1，10F31查询时，在档案内查询不到").arg(QString(QByteArray(Address(p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(i)->mtrAddr).addr,6).toHex())));
            break;
        }
        findFlag=false;
        for(int j=0;j<ReadLNInfoList_13F1.size();j++)
        ...
        if(findFlag==false)//找不到对应的电表号
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("表号：%1，13F1查询时，，在档案内查询不到").arg(QString(QByteArray(Address(p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(i)->mtrAddr).addr,6).toHex())));
            break;
        }
    }
    **/
