#include "Script_LN_Inverse_AddNodeRepeat.h"

Script_LN_Inverse_AddNodeRepeat::Script_LN_Inverse_AddNodeRepeat(QObject *parent) : QObject(parent)
{
...
Script_LN_Inverse_AddNodeRepeat::~Script_LN_Inverse_AddNodeRepeat()
{
...
void Script_LN_Inverse_AddNodeRepeat::execute()
{
...
void Script_LN_Inverse_AddNodeRepeat::stop()
{
...
void Script_LN_Inverse_AddNodeRepeat::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_LN_Inverse_AddNodeRepeat::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_LN_Inverse_AddNodeRepeat::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_LN_Inverse_AddNodeRepeat::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_LN_Inverse_AddNodeRepeat::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_LN_Inverse_AddNodeRepeat::processMsgFromCCO(DvcType dvcType, int dvcId)
{
                    ...
                    if(++index>=times)
                    {
                        ...
                        else if(emCmdType==AddNode)
                        {
                            ...
                            if(errorTimes==0)
                            {
                                ...
                                p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("LN互逆-重复添加档案测试成功！"));
                            }
                            else
                            {
                                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "出现识别错误情况");
                                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("LN互逆-重复添加档案测试失败！"));
                            }
                        }
                    }
                    else
...
void Script_LN_Inverse_AddNodeRepeat::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_LN_Inverse_AddNodeRepeat::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_LN_Inverse_AddNodeRepeat::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_LN_Inverse_AddNodeRepeat::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_LN_Inverse_AddNodeRepeat::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_LN_Inverse_AddNodeRepeat::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        default:
...
void Script_LN_Inverse_AddNodeRepeat::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
...
void Script_LN_Inverse_AddNodeRepeat::statisticsResult()
{
    int count10F31=0;
    int count13F1=0;
