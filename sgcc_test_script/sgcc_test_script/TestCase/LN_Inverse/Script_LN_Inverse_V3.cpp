#include "Script_LN_Inverse_V3.h"

Script_LN_Inverse_V3::Script_LN_Inverse_V3(QObject *parent) : QObject(parent)
{
...
Script_LN_Inverse_V3::~Script_LN_Inverse_V3()
{
...
void Script_LN_Inverse_V3::execute()
{
...
void Script_LN_Inverse_V3::stop()
{
...
void Script_LN_Inverse_V3::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_LN_Inverse_V3::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_LN_Inverse_V3::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_LN_Inverse_V3::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_LN_Inverse_V3::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_LN_Inverse_V3::processMsgFromCCO(DvcType dvcType, int dvcId)
{
            ...
            case Wait_QueryLN_10F31_Finish:
            {
                ...
                else if(p_Frame3762Base->afn_==0x00&&p_Frame3762Base->dt1_==0x02&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(emCmdType==AddNode)
                    {
                        ...
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "添加从节点收到否认，错误状态字为6(表号重复)，符合要求");
                            index+=addCntPerTime;
                            ...
                            else
                            {
                                ...
                                sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryLNInfo_10F31);
                                emScriptRunState=Wait_QueryLN_10F31_Finish;
                                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--查询从节点相线信息（10F31）");
                                p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
                            }
                        }
                        else
                    ...
                    if(++index>=times)
                    {
                        ...
                        else if(emCmdType==HardReset)
                        {
                            ...
                            if(errorTimes==0)
                            {
                                ...
                                p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("LN互逆测试成功！"));
                            }
                            else
                            {
                                ...
                                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("LN互逆测试失败！出现识别错误情况"));
                            }
                        }
...
void Script_LN_Inverse_V3::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_LN_Inverse_V3::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_LN_Inverse_V3::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_LN_Inverse_V3::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_LN_Inverse_V3::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_LN_Inverse_V3::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        default:
...
void Script_LN_Inverse_V3::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
...
void Script_LN_Inverse_V3::statisticsResult()
{
    int count10F31=0;
    int count13F1=0;
    ...
    {
        bool findFlag=false;
        if(memcmp(nodeInfoList.at(j).node_address_.addr,p_CtrInfoList->at(0)->ccoAddr,6)==0)
            findFlag=true;
        for(int i=0;i<p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->size();i++)
        {
            if(memcmp(nodeInfoList.at(j).node_address_.addr,p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(i)->mtrAddr,6)==0)
            {
                ...
                p_AbstractScriptHost->updateProgress(ProcessState_Processing,
                                    QString("电表地址:%1 CCO上报的相位信息字节:%2 档案中的phaseSeq:%3")
                                    .arg(QString(QByteArray(nodeInfoList.at(j).node_address_.addr,6).toHex()))
                                    .arg(QString::number(uchar(nodeInfoList.at(j).node_phase_info_.line_abnormal_flag_), 16))
                                    .arg(QString::number(p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(i)->phaseSeq, 16)));
                ...
                if(p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(i)->slotPosition==SingleSTA || p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(i)->slotPosition==SingleMeter)
                {
                    if((p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(i)->phaseSeq==0&&uchar(nodeInfoList.at(j).node_phase_info_.phase_sequence_)!=0)
                            || (p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->values().at(i)->phaseSeq==1&&uchar(nodeInfoList.at(j).node_phase_info_.phase_sequence_)!=6))
                    {
                        if(wrongFlag==false)
                        {
                            count10F31++;
        ...
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("表号：%1，10F31查询时，在档案内查询不到").arg(QString(QByteArray(nodeInfoList.at(j).node_address_.addr,6).toHex())));
            return;
        }
    }
    ...
    {
        bool findFlag=false;
        if(memcmp(ReadLNInfoList_13F1.at(j).nodeAddress.addr,p_CtrInfoList->at(0)->ccoAddr,6)==0)
            findFlag=true;
        for(int i=0;i<p_CtrInfoList->at(0)->p_MeterInfoForSingleNetList->size();i++)
        {
        ...
        if(findFlag==false)//找不到对应的电表号
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("表号：%1，13F1查询时，在档案内查询不到").arg(QString(QByteArray(ReadLNInfoList_13F1.at(j).nodeAddress.addr,6).toHex())));
            return;
        }
    }
    ...
    if(!identifyError10F31.isEmpty())
    {
        ...
        if(emCmdType==NormalCmd)
        {
            identifyErrorInfo.append(QString("基础测试-10F31查询识别错误电表如下\n%1\n").arg(identifyError10F31));
        }
        else if(emCmdType==AddNode)
        {
            identifyErrorInfo.append(QString("重复添加从节点-10F31查询识别错误电表如下\n%1\n").arg(identifyError10F31));
        }
        else if(emCmdType==HardReset)
        {
            identifyErrorInfo.append(QString("硬件复位-10F31查询识别错误电表如下\n%1\n").arg(identifyError10F31));
        }
    }
    if(!identifyError13F1.isEmpty())
    {
        ...
        if(emCmdType==NormalCmd)
        {
            identifyErrorInfo.append(QString("基础测试-13F1查询识别错误电表如下\n%1\n").arg(identifyError13F1));
        }
        else if(emCmdType==AddNode)
        {
            identifyErrorInfo.append(QString("重复添加从节点-13F1查询识别错误电表如下\n%1\n").arg(identifyError13F1));
        }
        else if(emCmdType==HardReset)
        {
            identifyErrorInfo.append(QString("硬件复位-13F1查询识别错误电表如下\n%1\n").arg(identifyError13F1));
        }
    }
}
