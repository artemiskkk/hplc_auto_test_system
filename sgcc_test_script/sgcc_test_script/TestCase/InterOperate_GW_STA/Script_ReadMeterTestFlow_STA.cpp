...
Script_ReadMeterTestFlow_STA::Script_ReadMeterTestFlow_STA(QObject *parent) : InteroperateBase_GW(parent)
{

}
Script_ReadMeterTestFlow_STA::~Script_ReadMeterTestFlow_STA()
{
    stop();
    powerOffAll12V(p_CtrInfoList,p_AbstractScriptHost);
}
void Script_ReadMeterTestFlow_STA::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_InteroperateInit_GW->setHost(host);
}
void Script_ReadMeterTestFlow_STA::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
bool Script_ReadMeterTestFlow_STA::config(const QMap<QString,QString> *paraDic)
{
    bool result = true;
    if(paraDic!=nullptr)
    {
...
void Script_ReadMeterTestFlow_STA::execute()
{
...
void Script_ReadMeterTestFlow_STA::slotStartFollowTest()
{
    emScriptRunState=Wait_ParameterSynchronousFlowBefore_Finish;
}
void Script_ReadMeterTestFlow_STA::stop()
{
    p_timer->stop();
...
void Script_ReadMeterTestFlow_STA::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_ReadMeterTestFlow_STA::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
...
void Script_ReadMeterTestFlow_STA::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        switch(emScriptRunState)
        {
            case Wait_ParameterSynchronousFlowBefore_Finish:
            {
                ...
                else if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x08&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp&&uchar(p_Frame3762Base->info_field_.info_field_up.msg_seq)==msgSeq)
                {
                    ...
                    if(ptr->router_operate_state_unit_.node_total_num_==1)
                    {
                        ...
                        emScriptRunState=Wait_STAInit_Finish;
                        QList<int> idList;
            ...
            case Wait_IdentityFlow_Finish:
            {
                ...
                else if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp&&uchar(p_Frame3762Base->info_field_.info_field_up.msg_seq)==msgSeq)
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("抄表(从节点):识别流程成功\n\n\n"));
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "抄表(从节点): 档案同步开始");
...
void Script_ReadMeterTestFlow_STA::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
...
void Script_ReadMeterTestFlow_STA::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    QString stringDvcType;
    if(dvcType == SingleSTA)
    {
...
void Script_ReadMeterTestFlow_STA::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_ReadMeterTestFlow_STA::timer_timeout()
{
    switch(emScriptRunState)
    {
        case Wait_STAInit_Finish:
        {
            p_timer->stop();
            p_AbstractScriptHost->updateProgress(ProcessState_Error, QString("timeout Wait_STAInit_Finish!!!"));
            break;
        }
        case Wait_IdentityFlow_Finish:
        {
...
void Script_ReadMeterTestFlow_STA::maxAllowTimer_timeout()
{
    p_maxAllowTimer->stop();
    p_AbstractScriptHost->updateProgress(ProcessState_Error, QString("timeout p_maxAllowTimer!!!"));
}

void Script_ReadMeterTestFlow_STA::delayTimer_timeout()
{
