#ifndef SCRIPT_READMETERTESTFLOW_STA_H
#define SCRIPT_READMETERTESTFLOW_STA_H

#include "../InteroperateBase_GW.h"
enum Script_ReadMeterTestFlow_RunState
{
    ScriptInit,
    Wait_InteroperateInit_Finish,
    Wait_ParameterSynchronousFlowBefore_Finish,
    Wait_STAInit_Finish,
    Wait_IdentityFlow_Finish,
    Wait_ParameterSynchronousFlow_Finish,
    Wait_ReadMeter13F1Flow_Finish,
    Wait_ConcentratorActiveReadMeterFlow_Finish,
    Wait_RouterActiveReadMeterFlow_Finish,
    ScriptFinish
};
class Script_ReadMeterTestFlow_STA  : public InteroperateBase_GW, public AbstractScript,public DynamicCreate<Script_ReadMeterTestFlow_STA>
{
    Q_OBJECT
public:
    explicit Script_ReadMeterTestFlow_STA(QObject *parent = nullptr);
    ~Script_ReadMeterTestFlow_STA();
private:
    Script_ReadMeterTestFlow_RunState emScriptRunState=ScriptInit;
    uchar protocol=0x00;
private:
...
#endif // SCRIPT_READMETERTESTFLOW_STA_H
