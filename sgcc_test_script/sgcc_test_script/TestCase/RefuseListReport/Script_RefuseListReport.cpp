#include "Script_RefuseListReport.h"

Script_RefuseListReport::Script_RefuseListReport(QObject *parent) : QObject(parent)
{
...
Script_RefuseListReport::~Script_RefuseListReport()
{
...
void Script_RefuseListReport::execute()
{
...
void Script_RefuseListReport::stop()
{
...
void Script_RefuseListReport::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_RefuseListReport::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_RefuseListReport::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_RefuseListReport::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_RefuseListReport::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_RefuseListReport::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        case EnableRefuseListReport:
        {
            ...
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Processing, "允许拒绝节点信息上报回复确认，等待上报");

                emScriptRunState=WaitReport_finish;
        ...
        case WaitReport_finish:
        {
            if(p_Frame3762Base->afn_ == 0x06&&p_Frame3762Base->dt1_==0x10&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                if(p_EventReport_06F5_Up->report_event_unit_.node_protocol_type_==0x05)
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("收到06F5拒绝列表事件上报，本次上报数%1，合计上报数%2").arg(reportNum).arg(totalReportNum));
                    if(reportNum > 32)
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "单条报文上报数>32");
                        return;
                    }
                    if(totalReportNum > 100)
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "合计上报数>100");
                        p_AbstractScriptHost->updateProgress(ProcessState_Success, "cco不对上报数量做处理，测试结束");
                        return;
                    }
                    else if(totalReportNum == 100)
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("合计上报数=100，等待5min看是否还上报"));
                        emScriptRunState=Wait5minsAfterReport;
        ...
        case Wait5minsAfterReport:
        {
            if(p_Frame3762Base->afn_ == 0x06&&p_Frame3762Base->dt1_==0x10&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
            {
                ...
                {
                    p_delayTimer->stop();
                    p_AbstractScriptHost->updateProgress(ProcessState_Failed, "总上报数满100后又收到拒绝列表上报，上报数"+QString::number(p_EventReport_06F5_Up->report_event_unit_.frame_content_.at(0)));
                }
                else
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("收到06F5其他事件上报"));
        ...
        case DisableRefuseListReport:
        {
            ...
            {
                p_timer->stop();
                p_AbstractScriptHost->updateProgress(ProcessState_Success, "禁止拒绝列表上报回复确认");
...
void Script_RefuseListReport::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_RefuseListReport::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_RefuseListReport::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_RefuseListReport::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_RefuseListReport::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
    ...
    case WaitReport_finish:
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("已上报拒绝节点%1个，5min内未再上报").arg(totalReportNum));
        break;
    }
    default:
    {
...
void Script_RefuseListReport::maxAllowTimer_timeoutProc()
{
...
void Script_RefuseListReport::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    if(emScriptRunState==WaitReport_finish)
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "允许拒绝列表上报后10min内未收到06F5拒绝列表上报");
    }
    else if(emScriptRunState==Wait5minsAfterReport)
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Processing, "总上报数满100后5min内未再收到拒绝列表上报，符合要求");
