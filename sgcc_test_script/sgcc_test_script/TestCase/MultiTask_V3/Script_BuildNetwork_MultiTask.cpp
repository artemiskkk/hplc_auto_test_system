#include "Script_BuildNetwork_MultiTask.h"

Script_BuildNetwork_MultiTask::Script_BuildNetwork_MultiTask(QObject *parent) : QObject(parent)
{
...
Script_BuildNetwork_MultiTask::~Script_BuildNetwork_MultiTask()
{
...
void Script_BuildNetwork_MultiTask::execute()
{
...
void Script_BuildNetwork_MultiTask::stop()
{
...
void Script_BuildNetwork_MultiTask::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_BuildNetwork_MultiTask::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_BuildNetwork_MultiTask::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_BuildNetwork_MultiTask::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
    ...
    if(emScriptRunState==Wait_BuildNetFinish_Whole)
    {
        ...
        else
        {
           ...
           // p_timer->stop();
            readInfoInit();
            sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_ActiveRegister_11F5);
            emScriptRunState=Wait_MultiTask_Finish;
            p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("发送--11F5启动搜表（%0min），等待--确认").arg(activeTime));
            p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
         //   p_delayTimer->start((activeTime+2)*60*1000);//2min;
        }
    }
...
void Script_BuildNetwork_MultiTask::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_BuildNetwork_MultiTask::processMsgFromCCO(DvcType dvcType, int dvcId)
{
    ...
    while(haveCompleteMsg)
    {
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_MultiTask_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x00&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    p_timer->stop();
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "激活主动注册11F5，回复确认");
                   // activeFlag=true;
                    readNo_13F1=0;
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("13F1抄第%0个表").arg(readNo_13F1+1));
                ...
                else if(p_Frame3762Base->afn_==0x13&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "13F1抄读回复数据内容为空："+p_MonitorSlaveNode_13F1_Up->data_field_up_.frame_content_.toHex());
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "组网未完成13f1回复空帧，发送--查询网络规模（10F9），等待--回复");
...
//                        {
//                            p_AbstractScriptHost->updateProgress(ProcessState_Processing, "13F1抄表完毕，发送--查询网络规模（10F9），等待--回复");
//                            sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_QueryNetScale_10F9);
//                            p_timer->start(CTRL_CMD_TIMEOUT_TIME*1000);
//                        }
//                        else
...
//                    }
                ...
                else if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x01&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(p_QueryNetScale_10F9_Up->network_scale_==readInfoList.size()+1)
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("当前网络规模%0，组网完毕").arg(p_QueryNetScale_10F9_Up->network_scale_));
                        if(searchNum>0)
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Success, QString("组网成功，过程中13F1抄%1个表全部成功，搜表上报%2个").arg(readNum_13F1).arg(searchNum));
                            emScriptRunState=ScriptSuccess;
                        }
                        else
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "组网过程中未搜到表");
                        }
                    }
                    else
                    {
...
//                    nodeInfoList.append(node);
           ...
           //         reportFlag=true;
                ...
                else if(p_Frame3762Base->afn_ == 0x06&&p_Frame3762Base->dt1_==0x04&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(p_ReportRouterState_Up->router_work_task_change_==0x02)
                    {
                       // activeFlag=false;
                        ...
                        if(searchNum==0)
                        {
                            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "未搜到表");
                        }
                    }
                    else
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "06F3路由上报工况变动，变动类型异常："+QString::number(p_ReportRouterState_Up->router_work_task_change_));
                    }
                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_Confirm_00F1);
                }
                ...
                {
                    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "收到06F5事件上报，回复确认");
                    sendMsg(CCO_GW,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_Confirm_00F1);
                }
                else
                {
...
void Script_BuildNetwork_MultiTask::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_BuildNetwork_MultiTask::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_BuildNetwork_MultiTask::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_BuildNetwork_MultiTask::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_BuildNetwork_MultiTask::timer_timeoutProc()
{
    p_timer->stop();
    switch(emScriptRunState)
    {
    ...
    case Wait_MultiTask_Finish:
    {
        p_AbstractScriptHost->updateProgress(ProcessState_Failed, "13F1抄表100s内未回复");
        break;
    }
    default:
    {
...
void Script_BuildNetwork_MultiTask::maxAllowTimer_timeoutProc()
{
    ...
    switch(emScriptRunState)
    {
        default:
        {
            p_AbstractScriptHost->updateProgress(ProcessState_Failed, "p_maxAllowTimer timeout");
            break;
        }
    }
}

void Script_BuildNetwork_MultiTask::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
...
void Script_BuildNetwork_MultiTask::readInfoInit()
{
    readInfoList.clear();
