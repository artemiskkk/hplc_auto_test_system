#include "Script_AreaIdentity_Shaanxi.h"

Script_AreaIdentity_Shaanxi::Script_AreaIdentity_Shaanxi(QObject *parent) : QObject(parent)
{
...
Script_AreaIdentity_Shaanxi::~Script_AreaIdentity_Shaanxi()
{
...
void Script_AreaIdentity_Shaanxi::execute()
{
...
void Script_AreaIdentity_Shaanxi::stop()
{
...
void Script_AreaIdentity_Shaanxi::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_AreaIdentity_Shaanxi::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_AreaIdentity_Shaanxi::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_AreaIdentity_Shaanxi::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_AreaIdentity_Shaanxi::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_AreaIdentity_Shaanxi::processMsgFromCCO(DvcType dvcType, int dvcId)
{
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_QueryRouterStateEnable_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x10&&p_Frame3762Base->dt1_==0x08&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(uchar(p_QueryRouterState_10F4_Up->router_operate_state_unit_.work_switch_.area_difference_flag_)==0x01)
                    {
                        ...
                        //等待多长时间？
                        p_delayTimer->start(waitTimeSpan*60*1000);
                    }
                    else
                    {
            ...
            case Wait_AreaIdentity_Finish:
            {
                if(p_Frame3762Base->afn_ == 0x06&&p_Frame3762Base->dt1_==0x10&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    if(p_ReportNodeEvent_06F5_Up->report_event_unit_.frame_content_.size()==0)
                        continue;
                    else
                    {
                        //回复确认
                    ...
                    QByteArray msgBuf=p_ReportNodeEvent_06F5_Up->report_event_unit_.frame_content_;
                    if(p_ReportNodeEvent_06F5_Up->report_event_unit_.node_protocol_type_==0x02)
                    {
                        ...
                        if(MsgBase_645_ptr==nullptr)
                            continue;
                        if(MsgBase_645_ptr->ctrlCode_!=PLAT_IDENT_OR_BOX_FLAG)
                            continue;
                        if(MsgBase_645_ptr->dataLen_==7)
                        {
                            p_delayTimer->stop();
                            char nodeAddr[6];
                            memset(nodeAddr,char(0xff),6);
                            if(MsgBase_645_ptr->data_field_.at(0)==0x02)
                            {
                                AreaInfoStruct areaInfoST;
                                areaInfoST.thisAreaFlag=true;
                                for(int i=0;i<6;i++)
                                {
                                    areaInfoST.areaAddress.addr[i]=char(MsgBase_645_ptr->data_field_.at(6-i));
                                    areaInfoST.nodeAddress.addr[i]=char(MsgBase_645_ptr->addr_[i]);
                                }
                                if(memcmp(areaInfoST.areaAddress.addr,nodeAddr,6)!=0)
                                {
                                    for(int i=0;i<6;i++)
                                    {
                                        areaInfoST.areaAddress.addr[i]=char(MsgBase_645_ptr->data_field_.at(6-i)-0x33);
                                    }
                                }
                                areaInfoList.append(areaInfoST);
                                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("收到本台区上报电表%1，所属台区主节点地址%2").arg(QString(QByteArray(areaInfoST.nodeAddress.addr,6).toHex())).arg(QString(QByteArray(areaInfoST.areaAddress.addr,6).toHex())));
                            }
                            else if(MsgBase_645_ptr->data_field_.at(0)==0x03)
                            {
                                AreaInfoStruct areaInfoST;
                                for(int i=0;i<6;i++)
                                {
                                ...
                                areaInfoList.append(areaInfoST);
                                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("收到垮台区上报电表%1，所属台区主节点地址%2").arg(QString(QByteArray(areaInfoST.nodeAddress.addr,6).toHex())).arg(QString(QByteArray(areaInfoST.areaAddress.addr,6).toHex())));
...
void Script_AreaIdentity_Shaanxi::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_AreaIdentity_Shaanxi::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_AreaIdentity_Shaanxi::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_AreaIdentity_Shaanxi::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_AreaIdentity_Shaanxi::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_AreaIdentity_Shaanxi::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        default:
...
void Script_AreaIdentity_Shaanxi::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
    p_AbstractScriptHost->updateProgress(ProcessState_Processing,QString("等待%1min未出现06F5上报").arg(waitTimeSpan));
...
void Script_AreaIdentity_Shaanxi::getMeterList()
{
    meterList.clear();
...
void Script_AreaIdentity_Shaanxi::displayResult()
{
    QString rightIdentity;
    QString errorIdentity;
    for(int i=0;i<areaInfoList.size();i++)
    {
        if(areaInfoList.at(i).thisAreaFlag==true)
            rightIdentity.append(QString("表号%1，所属台区地址%2\n").arg(QString(QByteArray(areaInfoList.at(i).nodeAddress.addr,6).toHex())).arg(QString(QByteArray(areaInfoList.at(i).areaAddress.addr,6).toHex())));
        else
            errorIdentity.append(QString("表号%1，所属台区地址%2\n").arg(QString(QByteArray(areaInfoList.at(i).nodeAddress.addr,6).toHex())).arg(QString(QByteArray(areaInfoList.at(i).areaAddress.addr,6).toHex())));
    }
    if(!rightIdentity.isEmpty())
    {
        rightIdentity.prepend("识别为本台区的电表如下：\n");
        p_AbstractScriptHost->updateProgress(ProcessState_Processing,rightIdentity);
    }
    else
    {
        rightIdentity.prepend("识别为本台区的电表为空\n");
        p_AbstractScriptHost->updateProgress(ProcessState_Processing,rightIdentity);
    }
    if(!errorIdentity.isEmpty())
    {
        errorIdentity.prepend("识别为非本台区的电表如下：\n");
        p_AbstractScriptHost->updateProgress(ProcessState_Processing,errorIdentity);
    }
    else
    {
        errorIdentity.prepend("识别为非本台区的电表为空\n");
        p_AbstractScriptHost->updateProgress(ProcessState_Processing,errorIdentity);
    }
