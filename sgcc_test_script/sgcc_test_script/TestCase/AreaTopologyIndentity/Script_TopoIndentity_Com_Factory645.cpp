#include "Script_TopoIndentity_Com_Factory645.h"

Script_TopoIndentity_Com_Factory645::Script_TopoIndentity_Com_Factory645(QObject *parent) : QObject(parent)
{
...
Script_TopoIndentity_Com_Factory645::~Script_TopoIndentity_Com_Factory645()
{
...
void Script_TopoIndentity_Com_Factory645::execute()
{
    ...
    {
        sendMsg(aimDvcType,p_CtrInfoList->at(0)->ctrlID,INSIGNIFICANCE,p_Rqst_WriteData_Current_0x14);
...
void Script_TopoIndentity_Com_Factory645::stop()
{
...
void Script_TopoIndentity_Com_Factory645::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_TopoIndentity_Com_Factory645::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_TopoIndentity_Com_Factory645::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_TopoIndentity_Com_Factory645::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_TopoIndentity_Com_Factory645::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_TopoIndentity_Com_Factory645::processMsgFromCCO(DvcType dvcType, int dvcId)
{
...
void Script_TopoIndentity_Com_Factory645::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_TopoIndentity_Com_Factory645::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_TopoIndentity_Com_Factory645::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
...
void Script_TopoIndentity_Com_Factory645::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_TopoIndentity_Com_Factory645::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_TopoIndentity_Com_Factory645::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_Finish_WriteData:
...
void Script_TopoIndentity_Com_Factory645::delayTimer_timeoutProc()
{
    p_delayTimer->stop();

    sendMsg(aimDvcType,p_CtrInfoList->at(0)->ctrlID,index,p_Rqst_WriteData_Current_0x14);
    p_AbstractScriptHost->updateProgress(ProcessState_Processing, "发送--645写数据，等待--完成");
    p_timer->start(STA_CMD_TIMEOUT_TIME*1000);
}


void Script_TopoIndentity_Com_Factory645::readInfoInit()
{
