#include "Script_TopoIndentity_Timing_OOP.h"

Script_TopoIndentity_Timing_OOP::Script_TopoIndentity_Timing_OOP(QObject *parent) : QObject(parent)
{
...
Script_TopoIndentity_Timing_OOP::~Script_TopoIndentity_Timing_OOP()
{
...
void Script_TopoIndentity_Timing_OOP::execute()
{
    ...
    addrList.clear();
    readInfoInit();
     p_BuildNetwork_GW->flagCJQHighComBaud=true;
...
void Script_TopoIndentity_Timing_OOP::stop()
{
...
void Script_TopoIndentity_Timing_OOP::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_TopoIndentity_Timing_OOP::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_TopoIndentity_Timing_OOP::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_TopoIndentity_Timing_OOP::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_TopoIndentity_Timing_OOP::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_TopoIndentity_Timing_OOP::processMsgFromCCO(DvcType dvcType, int dvcId)
{
        ...
        switch(emScriptRunState)
        {
            ...
            case Wait_Finish_13F1:
            {
                if(p_Frame3762Base->afn_==0x13&&p_Frame3762Base->dt1_==0x01&&p_Frame3762Base->dt2_==0x00&&p_Frame3762Base->ctrl_field_.dir == kDirUp)
                {
                    ...
                    {
                        p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("等待260s，开始下一次发送！"));
                        p_delayTimer->start(260*1000);
                    }
                }
...
void Script_TopoIndentity_Timing_OOP::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_TopoIndentity_Timing_OOP::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_TopoIndentity_Timing_OOP::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_MonitorSlaveNode_13F1)
    {
        ...
        else if(comPrtclType==0x03)
        {
            ...
            if(oiStructList.at(index).oi==0x4E05)
            {
                ...
                QDateTime sendTime=QDateTime::currentDateTime().addSecs(60);
                send_time_ptr->data_.append(getBinTime(sendTime));
            ...
            else if(oiStructList.at(index).oi==0x4E06||oiStructList.at(index).oi==0x4E01)
            {
                ...
                shared_ptr<DataList> para_list_ptr=make_shared<DataList>();
                para_list_ptr->type_=kStructure;

                shared_ptr<DataBasic> bit_width_ptr=make_shared<DataBasic>();
                bit_width_ptr->type_=kLong_unsigned;
                bit_width_ptr->data_.append(char(oiStructList.at(index).currentST.bit_width_>>8));
                bit_width_ptr->data_.append(char(oiStructList.at(index).currentST.bit_width_));
                dynamic_pointer_cast<DataList>(para_list_ptr)->list_data_member_.append(bit_width_ptr);
                shared_ptr<DataBasic> high_pulse_width_ptr=make_shared<DataBasic>();
                high_pulse_width_ptr->type_=kLong_unsigned;
                high_pulse_width_ptr->data_.append(char(oiStructList.at(index).currentST.high_pulse_width_>>8));
                high_pulse_width_ptr->data_.append(char(oiStructList.at(index).currentST.high_pulse_width_));
                dynamic_pointer_cast<DataList>(para_list_ptr)->list_data_member_.append(high_pulse_width_ptr);
                shared_ptr<DataBasic> low_pulse_width_ptr=make_shared<DataBasic>();
                low_pulse_width_ptr->type_=kLong_unsigned;
                low_pulse_width_ptr->data_.append(char(oiStructList.at(index).currentST.low_pulse_width_>>8));
                low_pulse_width_ptr->data_.append(char(oiStructList.at(index).currentST.low_pulse_width_));
                dynamic_pointer_cast<DataList>(para_list_ptr)->list_data_member_.append(low_pulse_width_ptr);
                shared_ptr<DataString> signal_ptr=make_shared<DataString>();
                signal_ptr->type_=kBit_string;
                signal_ptr->data_.append(oiStructList.at(index).currentST.word_info_);
                dynamic_pointer_cast<DataList>(para_list_ptr)->list_data_member_.append(signal_ptr);
                if(oiStructList.at(index).oi==0x4E06)
                {
                    ...
                    dynamic_pointer_cast<DataList>(para_list_ptr)->list_data_member_.append(frequence_ptr);
                    shared_ptr<DataString> extend_ptr=make_shared<DataString>();
                    extend_ptr->type_=kOctet_string;
                    dynamic_pointer_cast<DataList>(para_list_ptr)->list_data_member_.append(extend_ptr);
                }
                dynamic_pointer_cast<DataList>(p_ActionRequest->a_action_.data_ptr)->list_data_member_.append(para_list_ptr);
            ...
            p_MonitorSlaveNode_13F1->ctrl_field_.dir=kDirDown;
            p_MonitorSlaveNode_13F1->ctrl_field_.prm=kActive;
            p_MonitorSlaveNode_13F1->ctrl_field_.comn_type=kCenterRoute;
            ...
            memcpy(tmpCcoAddr,p_CtrInfoList->at(0)->ccoAddr,6);
            memcpy(p_MonitorSlaveNode_13F1->address_field_.src_addr,tmpCcoAddr,6);
            memcpy(p_MonitorSlaveNode_13F1->address_field_.dst_addr,QByteArray(reinterpret_cast<char*>(tmpAddr),6).append(0x63).right(6),6);
...
void Script_TopoIndentity_Timing_OOP::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_TopoIndentity_Timing_OOP::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_TopoIndentity_Timing_OOP::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_Finish_13F1:
...
void Script_TopoIndentity_Timing_OOP::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
...
void Script_TopoIndentity_Timing_OOP::readInfoInit()
{
...
QByteArray Script_TopoIndentity_Timing_OOP::getBinTime(QDateTime time)
{
    QByteArray dateTime;
    dateTime.append(char(time.date().year()>>8));
    dateTime.append(char(time.date().year()));
    dateTime.append(char(time.date().month()));
    dateTime.append(char(time.date().day()));
    dateTime.append(char(time.time().hour()));
    dateTime.append(char(time.time().minute()));
    dateTime.append(char(time.time().second()));
    return dateTime;
}
