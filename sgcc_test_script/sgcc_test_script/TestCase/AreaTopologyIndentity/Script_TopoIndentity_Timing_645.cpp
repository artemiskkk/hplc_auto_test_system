#include "Script_TopoIndentity_Timing_645.h"

Script_TopoIndentity_Timing_645::Script_TopoIndentity_Timing_645(QObject *parent) : QObject(parent)
{
...
Script_TopoIndentity_Timing_645::~Script_TopoIndentity_Timing_645()
{
...
void Script_TopoIndentity_Timing_645::execute()
{
...
void Script_TopoIndentity_Timing_645::stop()
{
...
void Script_TopoIndentity_Timing_645::addAddrsInfo(const QList<ConcentratorInfo> *p_ConcentratorInfoList, const QList<MeterInfo> *p_MeterInfoList, const QList<SchemeCfgInfo> *p_SchemeCfgInfoList, const int freq)
{
    p_CtrInfoList->clear();
...
void Script_TopoIndentity_Timing_645::setHost(AbstractScriptHost *host)
{
    p_AbstractScriptHost=host;
    p_BuildNetwork_GW->setHost(host);
}
bool Script_TopoIndentity_Timing_645::config(const QMap<QString,QString> *paraDic)
{
    bool result = false;
    if(paraDic!=nullptr)
    {
...
void Script_TopoIndentity_Timing_645::processMsg(DvcType dvcType, int id, uchar *data, int datalen)
{
...
void Script_TopoIndentity_Timing_645::processCtrlDvcRes(DvcType dvcType, QList<int> idList, CtrlCmdType ctrlCmdType, bool isSucs, QList<double> params)
{
    if(isSucs==false)
        return;
...
void Script_TopoIndentity_Timing_645::processMsgFromCCO(DvcType dvcType, int dvcId)
{
...
//                            uint sendTime1=sendTime.toTime_t();
//                            if(nowTime<sendTime1)
//                            {
//                                p_AbstractScriptHost->updateProgress(ProcessState_Processing, QString("未按设定时间发送！"));
//                                p_AbstractScriptHost->updateProgress(ProcessState_Failed, QString("未按设定时间发送"));
//                            }
...
void Script_TopoIndentity_Timing_645::processMsgFromMeter645(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_TopoIndentity_Timing_645::processMsgFromMeterOOP(DvcType dvcType, int dvcId, int mtrlID)
{
    bool haveCompleteMsg=true;
    while(haveCompleteMsg)
    {
...
void Script_TopoIndentity_Timing_645::sendMsg(DvcType dvcType, int dvcId, int meterID, shared_ptr<void> frame)
{
    ...
    else if(frame==p_MonitorSlaveNode_13F1)
    {
        ...
        if(comPrtclType==0x02)
        {
            ...
            //特征电流参数
            sendTime=QDateTime::currentDateTime().addSecs(60);
            memcpy(diStructList[index].currentST.send_time_,getBinTime(sendTime),6);
...
void Script_TopoIndentity_Timing_645::sendSrcMsg(DvcType dvcType, int dvcId, QByteArray msg)
{
    if(msg.size()<1)
        return;
...
void Script_TopoIndentity_Timing_645::timer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_BuildNetFinish_Whole:
...
void Script_TopoIndentity_Timing_645::maxAllowTimer_timeoutProc()
{
    switch(emScriptRunState)
    {
        case Wait_Finish_13F1:
...
void Script_TopoIndentity_Timing_645::delayTimer_timeoutProc()
{
    p_delayTimer->stop();
...
void Script_TopoIndentity_Timing_645::readInfoInit()
{
...
QByteArray Script_TopoIndentity_Timing_645::getBinTime(QDateTime time)
{
    QByteArray dateTime;
    dateTime.append(char(time.toString("yy").toInt()));
