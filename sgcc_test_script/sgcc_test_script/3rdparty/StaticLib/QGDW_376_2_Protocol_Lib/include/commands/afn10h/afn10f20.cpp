...
qgdw_3762_protocol::Afn10F20::Afn10F20()
{
    this->afn_=0x10;
    this->dt1_=0x08;
    this->dt2_=0x02;
}
qgdw_3762_protocol::Afn10F20::Afn10F20(const qgdw_3762_protocol::CtrlField ctrl_field, const qgdw_3762_protocol::InfoField info_field, const qgdw_3762_protocol::AddressField address_field)
:Frame3762Base(ctrl_field,info_field,address_field,0x10,0x08,0x02)
{
...
void qgdw_3762_protocol::Afn10F20::DecodeFrameDataField(QByteArray data)
{
    if((this->ctrl_field_.dir&0x01) == qgdw_3762_protocol::kDirUp)
    {
        ...
        {
            DoubleNetworkTypelogyInfo10F21 network_typelogy_info_;
            for(int j=0;j<6;j++)
            {
                network_typelogy_info_.node_address_.addr[j]=data.at(5-j);
            }
            network_typelogy_info_.node_tei_=ushort((short(data.at(7)&0x0F)<<8))+uchar(data.at(6));
            network_typelogy_info_.module_type_=uchar(data.at(7)&0xF0)<<4;
...
QByteArray qgdw_3762_protocol::Afn10F20::EncodeFrameDataField()
{
    QByteArray data;
    if((this->ctrl_field_.dir&0x01) == qgdw_3762_protocol::kDirUp)
    {
        ...
        for(int i=0;i<this->network_typelogy_info_unit_.network_typelogy_info_List_.size();i++)
        {
            ...
            data.append(char(
                            ((this->network_typelogy_info_unit_.network_typelogy_info_List_.at(i).node_tei_>>8)&0x0f)
                            |this->network_typelogy_info_unit_.network_typelogy_info_List_.at(i).module_type_<<4
                             ));
