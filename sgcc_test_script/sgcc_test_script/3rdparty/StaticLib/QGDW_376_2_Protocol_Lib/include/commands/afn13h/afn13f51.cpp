...
qgdw_3762_protocol::Afn13F51::Afn13F51()
{
    this->afn_=0x13;
    this->dt1_=0x04;
    this->dt2_=0x06;
    this->data_field_down_.reserve_=0x00;
    this->data_field_up_.reserve_=0x00;
}

qgdw_3762_protocol::Afn13F51::Afn13F51(const qgdw_3762_protocol::CtrlField ctrl_field, const qgdw_3762_protocol::InfoField info_field, const qgdw_3762_protocol::AddressField address_field)
    :Frame3762Base(ctrl_field,info_field,address_field,0x13,0x04,0x06)
{
...
void qgdw_3762_protocol::Afn13F51::DecodeFrameDataField(QByteArray data)
{
    if((this->ctrl_field_.dir&0x01)==qgdw_3762_protocol::kDirDown) //下行
    {
        ...
        this->data_field_down_.reserve_=data.at(1);

        this->data_field_down_.delay_tag_=data.at(2);
        this->data_field_down_.sub_node_num_=uchar(data.at(3));
        if(data.length()<(5+6*this->data_field_down_.sub_node_num_))
        {
            throw DecodeException(qgdw_3762_protocol::ExceptionCatalogue::kDataLengthError,QString("DataField Length Error"));
        }
        data.remove(0,4);
        ...
        this->data_field_down_.frame_length_=uchar(data.at(0)&0xff)+(uchar(data.at(0))&0xff)*256;
        data.remove(0,2);
        ...
        this->data_field_down_.frame_content_.append(data);
    }
    else//上行
    {
        if(data.length()<4)
        ...
        this->data_field_up_.reserve_=data.at(3);

        this->data_field_up_.frame_length_=uchar(data.at(4)&0xff)+(uchar(data.at(5))&0xff)*256;
        data.remove(0,6);
...
QByteArray qgdw_3762_protocol::Afn13F51::EncodeFrameDataField()
{
    QByteArray data;
    if((this->ctrl_field_.dir&0x01)==qgdw_3762_protocol::kDirDown) //下行
    {
        data.append(this->data_field_down_.protocol_type_);
        data.append(this->data_field_down_.reserve_);
        ...
        data.append((const char*)(&this->data_field_down_.frame_length_),2);
    ...
    else//上行
    {
        ...
        data.append(this->data_field_up_.reserve_);
        data.append((const char*)(&this->data_field_up_.frame_length_),2);
